...
- **ツリーに現存する `//nolint` は次の 11 件（うち G304 が 9 件）で、すべて本番コードの行単位・理由付き・`gosec` 指定である。** いずれも方針上許容する恒久的な抑制で、除去の予定はない。
  ...
  **`_test.go` に実際の抑制指示（有効な `//nolint` ディレクティブ）は 1 つも無い。** `internal/buildconfig` 配下の 2 つのテストには `//nolint` という文字列が現れるが、いずれもこのリポジトリのコードに対する抑制ではない。`internal/buildconfig/golangci_test.go` のものは `nolintlint` が雑な抑制を実際に落とすことを確かめる**検証用フィクスチャの文字列リテラル**であり、`internal/buildconfig/docscheck/docs_nolint_test.go` のものは上表をツリーと突き合わせるガードテスト自身の**doc コメント・検出に使う正規表現・失敗メッセージ**である（後者は上表を見る**ドキュメントの検査**なので `docscheck` 側にある）。どちらも棚卸しの件数には影響しない。突き合わせを行う `countNolintInTree` が `_test.go` を走査対象から除いているためである。
