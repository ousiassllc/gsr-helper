...
**採ったのは Issue #174 が挙げた 3 案のうち「写しを 1 つに減らす」側である。** 説明とテストを突き合わせる弱い検査（設定キーが説明に現れているか等）を組み立てる案は、**照合すべき対象を言い当て続ける必要があり収束しない**——同じ形は行数の散文の検査が 2 周かけて捨てている。写しを 1 つにすれば照合は完全一致でよく、言い回しを列挙する余地が残らない。Issue #164 / #167 / #169 が行数の実測値に対して採ったのと同じ手である。**1 行 1 検査にしたのはこの一致を定義できるようにするためで**、2 つのテストを 1 行にまとめていた頃は、行の説明がどちらの doc コメントに対応するのかが決まらなかった。
