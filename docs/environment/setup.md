...
| [TUI コンポーネント設計](../ui/atomic-design.md#行数の実測値は表だけが持つ)の散文が行数の実測値を数値で持たない（`残り N` と 2 桁以上の `N 行` が現れない。除外は行数表・コードブロック・改訂履歴・過去の値を記録する節の明示の一覧で、`.linterly.yml` の上限と警告帯の境界である 300 / 330 / 2000 / 2200 だけは書いてよい） | `TestLineBudgetProseHasNoMeasuredNumbers` |
