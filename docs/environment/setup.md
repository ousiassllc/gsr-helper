...
| [TUI コンポーネント設計](../ui/atomic-design.md#行数の実測値は表だけが持つ)の散文が行数の実測値を数値で持たない（`残り N` と `N 行` が現れない。見ないのは `1 行` と `2 行` だけ。除外は行数表・コードブロック・改訂履歴・過去の値を記録する節の明示の一覧で、`.linterly.yml` の上限と警告帯の境界である 300 / 330 / 2000 / 2200 だけは書いてよい） | `TestLineBudgetProseHasNoMeasuredNumbers` |
| 同じ散文が行数の合計を `A + B = C` の式で持たない（式に 3 桁以上の数が 1 つでも含まれていれば実測値と見なす。行数の合計は 1 ディレクトリ 2000 行の桁になるので必ず 3 桁以上で、2 桁以下だけで閉じた式は行数ではない算術である。除外は上の行と同じ） | `TestLineBudgetProseHasNoSumExpressions` |
