...
| `guard` は許可リスト形（`push` と同一リポジトリの `pull_request` 以外は失敗する）。判定値は `EVENT_NAME` / `HEAD_REPO` / `BASE_REPO` の env 経由で渡り、ゲートの `run:` に `${{` を直書きしない（head リポジトリ名を通した式インジェクションの余地を残さない）。対象は `.github/workflows/ci.yml` | `TestCIGuardScriptAllowsOnlySameRepositoryEvents` |
...
| `.github/workflows/ci.yml` のアクションはフルコミット SHA でピン留めされている | `TestCIActionsArePinnedToCommitSHA` |
| `.github/workflows/ci.yml` の全ジョブに `timeout-minutes` がある | `TestCIJobsHaveTimeout` |
...
| `.github/workflows/ci.yml` の `actions/setup-go` のキャッシュは無効である（モジュール・ビルドキャッシュはホストに残るため、tar での保存と展開はやり直しの重複でしかない） | `TestCISetupGoDisablesCache` |
...
| 抑制には理由コメントとリンター名が必須で、不要になった抑制も検出される（`require-explanation` / `require-specific` / `allow-unused: false` の 3 つ）。設定でそう定めているだけでなく、その 3 つを破った抑制は実際に落ちる | `TestGolangciEnablesNolintlint` / `TestGolangciLintRejectsSloppyNolint` |
| import は標準ライブラリ / 外部モジュール / 自前パッケージの 3 グループに固定され、混ざったものは実際に落ちる（gofmt はグループ内を並べ替えるだけなので `make fmt-check` では検出できない）。`custom-order: true` も検査する——これが無いと gci は記載順を無視して内蔵の既定順で並べ、順序の取り決めが実効にならない | `TestGolangciEnablesGci` / `TestGolangciLintRejectsMisgroupedImports` |
...
| golangci-lint が同種の指摘を打ち切らない（既定の 50 / 3 のままだと抑制やエラーの棚卸しで件数を数え上げられない） | `TestGolangciDoesNotTruncateIssues` |
| 行数上限は linterly の既定値のまま使う（上限に当たったら数値を上げるのではなく分割する）。あわせて `count_mode: all` と `warning_threshold` の存在も見る（`rules:` が空になると `rules section is required` で exit 2 になる） | `TestLinterlyKeepsDefaultLineLimits` |
| linterly の更新チェックは無効である（既定は有効で、実行のたびに GitHub Releases への外向き HTTP が出る） | `TestLinterlyDisablesUpdateCheck` |
| `lefthook.yml` の構文退行を CI でも機械検知する（`make check` の `test` も次の行の検査を通して同じ `go tool lefthook validate` を走らせており、CI の step はそれと重ねて掛ける二重化である） | `TestCILintJobValidatesLefthookConfig` |
...
| pre-commit は `parallel: false` で同時実行を止め、実行順を `priority` で固定する（`parallel: false` は同時実行を止めるだけで順序を決めず、priority が未指定だと commands のキー名の比較に依存する）。`fmt` → `lint` → `linterly` の順序も検査する（fmt が整形した結果を lint が読む） | `TestLefthookPreCommitOrderIsPinnedByPriority` |
...
| 依存グラフの辺と実装の層をまたぐ import が一致する（欠落・陳腐化の両方向）。図の UI 内部の辺は散文が例外として名指しする `UIApp --> UIParts` の 1 本だけで、`subgraph UI` に UI 層でないノードが混ざらないことも見る | `TestDependencyGraphDrawsEveryCrossLayerImport` / `TestDependencyGraphHasNoStaleEdge` |
...
| **2 文書**（[TUI コンポーネント設計](../ui/atomic-design.md#行数の実測値は表だけが持つ)と[コンポーネント設計](../components/overview.md)）の散文が行数の実測値を数値で持たない（`残り N` と `N 行` が現れない。**免除は左右で非対称である**——`N 行` の側は `0 行` / `1 行` / `2 行`（本書が「表の 1 行」の意味で常用する）に加えて、`.linterly.yml` の上限と警告帯の境界である 300 / 330 / 2000 / 2200 の 4 定数を書いてよい。`残り N` の側は助数詞まで見て `桁` / `件` / `つ` / `本` / `枚` / `箇所` を伴う形（`残りが 1 桁` のような性質の話）を落とすが、**定数の免除は持たない**——上限にも警告帯の境界にも「残り」は無く、残りは常に実測値だからである。除外は行数表・コードブロック・改訂履歴・過去の値を記録する節の明示の一覧で、**一覧は文書ごとに持つ**——除外は文書の構造に結びついており、混ぜると片方にしか無い見出しが消えたことを検査が知らせられなくなる。`overview.md` は除外を 1 つも持たない） | `TestLineBudgetProseHasNoMeasuredNumbers` |
| 同じ 2 文書の散文が行数の合計を式で持たない（**数が演算子で 2 つ以上連なった形**を拾い、`= D` の右辺は任意。演算子は `+` `-` とその全角で、`-` だけは前後の空白を必須にする——空白なしを許すと ISO 形式の日付が式として拾われるため。空白は半角の空白類に加えて全角空白（U+3000）も認める（全角の演算子は全角空白を伴って書かれる）。式に含まれる数が**すべて 2 以下のときだけ**実測値でないと見なす。除外は上の行と**同じもの**で、合計の式にだけ効く除外は持たない——行数と無関係な算術（列幅の見積もり）は、数の間に語を挟んで書けば式の形にならないので、文書側で偽陽性を消している） | `TestLineBudgetProseHasNoSumExpressions` |
| 合計の式の検出が行数の式だけを拾い、行数以外の算術（すべて 2 以下の式・ISO 形式の日付・数の間に語を挟んだ列幅の見積もり）を拾わない（検出の両方向を固定する単体テスト） | `TestProseMeasuredSumMatchesOnlyLineTotals` |
...
| この表が挙げるテストの集合と、`internal/buildconfig` 直下および `internal/buildconfig/docscheck` のテスト関数の集合が一致する（表に無い検査と、実装に無い表の行の両方向。テスト名は**右カラムからだけ**読むので、説明カラムでの言及は「表に載っている」と数えない。`internal/buildconfig` 配下を**深さを問わず**辿り、範囲外のディレクトリがテストを持ったら落ちるので、`buildconfigtest` や子ディレクトリへの再分割を含め検査の置き場が増えたことも黙って見過ごさない） | `TestSetupDocInvariantTableListsEveryTest` |

**この表は網羅である**——`internal/buildconfig` 直下と `internal/buildconfig/docscheck` にあるテスト関数の集合と、この表が挙げるテストの集合は一致する（範囲を定めるのは次の段落で、検査もそれと同じ 2 ディレクトリだけを見る。両者が共有する道具の置き場である `buildconfigtest` は検査を持たないので含めない）。**その範囲そのものも機械的に見る**——`internal/buildconfig` 配下を**深さを問わず**辿り、範囲に載っていないディレクトリが `_test.go` を持っていたら落とす。直書きの範囲だけだと 3 つ目の検査ディレクトリができても検査が黙り、`buildconfigtest` が検査を持ち始めても気付けない（`docscheck` 自体が Issue #161 の分割で生まれており、再分割は現実に起こりうる）。設定やドキュメントに新しい取り決めを入れてテストを足したら、**同じ変更でこの表にも行を足す**こと。それを人の注意に任せず、`TestSetupDocInvariantTableListsEveryTest` が集合の一致を機械的に見る——表に無い検査（足して書き忘れた）と、実装に無い表の行（消した・改名したのに残った）の両方向を報告する。**手で写した一覧は黙って古くなる**というのは Issue #164 / #167 / #169 が行数の実測値の写しに対して繰り返し確かめた事実であり、この表自身が「同じ場所にテストを足す」と定めていながら実在する検査のうち 27 本を取りこぼしていた（Issue #166）のが、その実例である。
...
| 1.30 | 2026-08-25 | Issue #161（`internal/buildconfig` の行数を空ける）を反映。(1) 「設定ファイルの不変条件をテストで守る」節に、**検査が 2 つのディレクトリに分かれ共有ヘルパが 3 つ目にある**こと——設定ファイルの検査は `internal/buildconfig` 直下、ドキュメントの検査は `internal/buildconfig/docscheck`、両方が使う `RepoRoot` だけが `internal/buildconfig/buildconfigtest`——と、新しい検査は対象に合わせて前の 2 つのどちらかを選ぶことを追記した。(2) 不変条件の表の直後の断りを「`internal/buildconfig` とその `docscheck` に置いたものだけ」へ改めた。(3) 多層防御表の「設定の不変条件」の行も同じく `docscheck` を含む記述にした。(4) **PR #163 のレビュー指摘で 4 点を直した**——(a) 「新しい検査を足すときは対象に合わせてどちらかを選ぶこと。」の直後で bold の閉じと次の bold の開きが空白なしで連続しており、CommonMark（goldmark / cmark-gfm）が 4 連のアスタリスクをリテラルとして描画して 2 つの bold が融合していたので、段落を分けた。(b) 「**ディレクトリは 2 つに分かれている**」と書きながら同じ文で 3 つを名指ししていたので、「検査は 2 つのディレクトリに分かれ、共有ヘルパが 3 つ目にある」へそろえた。(c) 「実行時のコードを持たないテスト専用のパッケージ」が 3 つ全部に掛かる形になっていたが、**`buildconfigtest` は `_test.go` ではない通常のパッケージで本番から import できてしまう**ため、検査を置く 2 つと書き分け、`pagetest/import_test.go` の `fixtures` へ登録して `TestNoProductionCodeImportsTestFixtures` の網に入れてあることを添えた。(d) 「抑制の方針」が挙げる `//nolint` の文字列を含む 2 つのテストのうち `nolint_inventory_test.go` を、移動後の実測（`internal/buildconfig/docscheck/docs_nolint_test.go`）へ直した。(5) **2 周目レビューで 2 点を直した**——(e) 分割の判断への参照がアンカー無しのリンクだったので、同じ判断を指す [コンポーネント設計](../components/overview.md#internalbuildconfig) と同じく `../ui/atomic-design.md#行数の予算` へそろえた。(f) 本節冒頭が挙げる設定ファイルの列挙へ `.github/dependabot.yml` を足した（`TestDependabotWatchesGitHubActions` が検証しており、下の逐語のコードブロックにも載っているのに列挙から漏れていた）。 | 1 ディレクトリ 2000 行の上限を空けるためにパッケージを分けた結果、テストの置き場所が本節の記述と食い違うようになった。検査を足す人が最初に読むのがこの節なので、選び方をここに書かないと置き場所が混ざる。(4) のうち (a) は**レンダリング後の表示が壊れる**もので、本書は設定ファイルを逐語で載せる文書であり描画の乱れが実体との差に見える。(b) は同じ文が自分の数え上げと矛盾しており、読み手がどちらを信じてよいか本書から決められない。**(c) は危険側の誤りである**——「テスト専用のパッケージだから本番へ混入しえない」と読ませるが、`buildconfigtest.go` は通常のファイルなので import は Go の側では止まらず、止めているのは `fixtures` への登録と検査のほうである（登録を落とすと網から外れる）。(d) は `TestSetupDocNolintInventoryMatchesTree` を `docscheck` へ移した PR #163 の変更に伴うもので、**存在しないパスを名指しした状態**になっていた。(e) はアンカー無しだと文書の先頭に着地して読み手が節を探す羽目になるもので、同じ判断を指す 2 文書で参照形式が不揃いだった。(f) は**検証されている対象が列挙から漏れている**形で、次に `dependabot.yml` を触る人が「この不変条件は検査されていない」と読む |
