...
| 対象 | 挙動 |
|------|------|
| 設定ファイル | 常に 0600・`SUDO_USER` 所有。一時ファイル + rename で置き換える |
| ディレクトリ（対象ユーザーのホーム配下） | 作成時 0700。既存ディレクトリも 0700 に揃え、`SUDO_USER` 所有にする |
| ディレクトリ（ホームの外。`--config /etc/gsr-helper/…` など） | `MkdirAll` 0700 で作るが、**chmod も chown もしない**。既存ディレクトリのモードは変えない |
| ホームが無い / ディレクトリでない | 所有者を実行ユーザーへ落とす（ホームを持たないサービスアカウントでも書ける）。実行ユーザーのホームも無い場合のみ `Save` がエラーになる |

ホームの外を chmod / chown しないのは、`/etc` のような共有の場所のパーミッションと所有者を書き換えないためである。chmod は `O_NOFOLLOW｜O_DIRECTORY` で開いた fd に対して行うため、末尾がシンボリックリンクの場合は辿らずに失敗する。
