...
行数チェック（`linterly`）の集計は直下のファイルのみを対象とする。**`internal/runner` は余裕が乏しい**ので、サービス制御や追加・削除の Issue が `runner` へ機能を足す場合は、先に切り出し先を決めること。実測値の一次情報は [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の行数表であり、本書はここを重複して持たない。
...
**`internal/disk` は一度警告帯（2000 行超）に入り、Issue #101 で戻した。** 着手時点では 2181 行（エラー境界の 2200 まで 19 行）まで詰まっており、削除パスの検証を `internal/disk/pathguard` へ切り出して当時 1927 行・残り 73 行・pass にした。**その後 Issue #123 の回帰テストの追加でふたたび詰まった**（現在の値は同書の行数表を見よ）。判断と理由は `docs/ui/atomic-design.md` の「UI 層の外のディレクトリ」に記録した（同書の「行数の予算」と同じ規範を `internal/` 側にも適用する）。実測値の一次情報は同書の表であり、本書はここを重複して持たない。
