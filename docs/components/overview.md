...
**この一致は機械的に検査している。** `internal/buildconfig/docscheck/docs_mermaid_test.go` が上のグラフをパースして辺と `subgraph` を読み取り（`dependencyMermaid` / `parseDepGraph`）、`docs_deppkg_test.go` が `go list -json ./...` で実装の側の直接 import（本番ファイルの import だけで、テストの import は含まない）を読んでノードへ畳み、`docs_depgraph_test.go` がその両者を**両方向で**突き合わせる——実装にある層をまたぐ import に辺が無ければ落ち（その辺を生んでいる import を最大 3 件添える）、実装から消えた依存が辺として残っていても落ちる。畳んだノードの内訳（直前の段落の列挙）は `docs_depnodes_test.go` が `internal/ui` 直下の実装と突き合わせるので、パッケージを増やしたときに散文だけが古くなることは無い。**この 4 本はいずれも `internal/buildconfig/docscheck` にある**（Issue #161 が `internal/buildconfig` 直下から移した。判断は [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の「`internal/buildconfig` を 2 つに分けた判断」）。**同種の辺の欠落が改訂 1.22 / 1.27 / 1.29 / 1.32 / 1.33 / 1.40 で 6 度再発したのが検査を置いた理由である**——人手の棚卸しでは次の Issue で再発する（Issue #151）。**ただし検査が及ぶのは畳んだ後の辺までである**——直前の段落が挙げているパッケージ単位の内訳（`ui/tabset` → `runner` / `appconfig` / `exec` など、その辺を実際に生んでいるのがどのパッケージか）はどの検査も見ていない。`ui/hostreq` に `logs` の import を足しても `UIApp --> Logs` の辺が既にあるので検査は 4 本とも緑になる。**内訳の粒度は人手で保つ必要がある。**
...
**本書が描かないのは UI パッケージ同士の依存である**——ただし粒度の要約である 2 ノードの間の 1 本（`UIApp --> UIParts`）だけは例外で、上のグラフに描いてある。`organism` → `keymap` のような UI 層の内部の辺は [TUI コンポーネント設計](../ui/atomic-design.md#依存の方向)の依存グラフが持つ——同じ辺を 2 か所で維持すると片方だけが古くなるため、本書は層と層の間だけを描く。**この除外が効くのは辺の欠落を見る向きだけである**——実装の import に辺を要求する `TestDependencyGraphDrawsEveryCrossLayerImport` は UI 層のノードをグラフの `subgraph UI` から読み取り、両端が UI 層の辺を対象外にする（一覧を検査側にベタ書きすると、図と検査で同じ一覧を 2 か所で維持することになる）——**ただし上の例外の 1 本 `UIApp --> UIParts` は除外しない。** これも除外すると、本書が現在形で宣言する唯一の UI 内部の辺が図から消えても 4 本とも緑になる（一覧ではなく本書が名指しした 1 本なので、この辺だけは検査側にも名前で置いてある）。対になる `TestDependencyGraphHasNoStaleEdge` は除外せず**描いてある辺をすべて**実装の import で裏取りするので、`UIApp --> UIParts` も実装から import が消えれば落ちる。**この 1 本は両方向から見られている**——実装から消えれば陳腐化した辺として、図から消えれば辺の欠落として落ちる。
...
- 同じ `UnitName` を名乗る runner ディレクトリが複数あった場合は、その名前を第一パスの索引から落として第二パス（`WorkingDirectory`）に委ね、重複を警告 1 件として出す（`indexByUnitName`）。先着で決めると外れた側が `Svc == nil` になり、systemd 管理の runner を `run.sh` 直起動として表示する
...
行数チェック（`linterly`）の集計は直下のファイルのみを対象とする。**サービス制御や追加・削除の Issue が `runner` へ機能を足すときは、先に [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の行数表で余裕を確かめ、そのうえで切り出し先を決めること。** 実測値の一次情報は同書の行数表であり、本書はここを重複して持たない。
...
| `internal/setup/name` の `NextIndex(existing []string, prefix string) int` / `RunnerName` / `Names` | 命名の連番決定（**純粋関数**。ホストの状態を一切読まない）。行数上限のため別パッケージへ出した（[TUI コンポーネント設計](../ui/atomic-design.md#internalsetup-の命名を-name-へ出した判断)） |
...
**`internal/disk` は一度警告帯（2000 行超）に入り、Issue #101 で戻した。** エラー境界の 2200 に迫るところまで詰まっていたものを、削除パスの検証を `internal/disk/pathguard` へ切り出して pass に戻した。その後 Issue #123 で回帰テストを足している。**当時の値と判断の理由は `docs/ui/atomic-design.md` の「`internal/disk` から `pathguard` を切り出した判断（Issue #101）」が持つ**（同書の「行数の予算」と同じ規範を `internal/` 側にも適用する）。現在の値は同書の[行数の予算](../ui/atomic-design.md#行数の予算)の行数表が持つ。**本書はどちらも重複して持たない。**
...
行数上限のため 4 つに分ける。依存は **`exec/command` → `exec`・`exec/mask`・`exec/command/limit` の一方向**である。契約（`Executor` / `Result` / `Options`）を最下層に置き、それを実装する側が上に乗る形なので、`exec` はどのサブパッケージも import しない。**内部パッケージを import するのは 4 つのうち `exec/command` だけである**——`exec` / `exec/mask` / `exec/command/limit` はどれも標準ライブラリしか使わず、なかでも `limit` は `unicode/utf8` 1 つで済む。**`limit` も `exec/mask` も上位の型を知らない**——どちらも渡されるのは文字列と付随する値（`limit` はバイト数、`mask` は秘密値の並び）だけで、`Command` / `ExitError` / 監査レコードを 1 つも知らない。だから `limit` の側に「監査に載せるときだけ印を省く」ような迂回を書けない（判断は[TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の「`internal/exec/command` から切り詰めの道具を `limit` へ切り出した判断」）。
...
| `ui/page/pagetest` | page | `page/<tab>` **と親 Model** が共用するテスト用の道具（共有状態（`State`。`Exec` と `ScanProcs` を既定で埋め、テストをホストの外部コマンドと `/proc` から切り離す）・`Spy`・打鍵の組み立て・打鍵の結果の走査（`ScanKey` / `Press1` / `ApplyChrome`）・親 Model 越しの取り出し（`OpenTabOf` / `TakeHostReq` / `IsQuit`）・長寿命の購読を模した `StreamPage`）。**`Cmd` を走らせて束を辿る関数はここには無い**——`Msgs` / `MustMsgs` などは `cmdtest` 側にあり（下記）、ここの道具はその上に載っている（`ScanKey` は `MustMsgs` を呼ぶ）。同名の `Spy.Msgs()` は spy が受け取った `Msg` を返すメソッドで、別物である。**テスト専用で本番からは import しない**（`TestNoProductionCodeImportsTestFixtures` が本番ファイルの import を読んで検査する） |
...
| `buildconfig/buildconfigtest` | 上の 3 つが使う道具（リポジトリルートの解決 `RepoRoot`——テストの実行ディレクトリから遡って `go.mod` を持つリポジトリルートを返す——は 3 つとも使い、表から読んだ数値文字列を `int` にする `Atoi` は `docscheck` と `docscheck/linebudget` の 2 つが使う）。`_test.go` ではなく通常のパッケージなのは **`_test.go` の中のヘルパはパッケージをまたげない**ためで（Issue #161）、**テスト専用で本番からは import しない**（`page/pagetest/import_test.go` の `fixtures` へ登録済み。`TestNoProductionCodeImportsTestFixtures` が検査する）。先例は `ui/page/pagetest` / `ui/organism/table/tabletest` / `setup/setuptest`。**置いてよいのは複数が使う道具だけである**——1 つしか使わないものを寄せると、分けた意味（増え方の違うものを別々に育てる）が消える |
...
| 1.50 | 2026-08-24 | 依存グラフを機械的に検査するようになったことを反映（Issue #151）。「畳んだノード」の段落の直後に、3 つのファイル（新設した検査は 4 本）の責務分担を明記した——`docs_mermaid_test.go` が上のグラフをパースして辺と `subgraph` を読み取り（`dependencyMermaid` / `parseDepGraph`）、`docs_depgraph_test.go` がその辺を `go list -json ./...` の直接 import と両方向（辺の欠落・陳腐化した辺）で突き合わせ、`docs_depnodes_test.go` が畳んだノードの内訳を `internal/ui` 直下の実装と突き合わせる。ノード対応表（`graphNodeRules`）については、最長プレフィックス一致であるために検査が止まる範囲を書き分けた——新しい最上位のツリーだけが `TestEveryPackageIsMappedToGraphNode` で落ち、`internal/ui` 直下に新設した場合だけ `TestCollapsedUINodesMatchDoc` が散文の列挙との一致で拾う。**この欄は当初ここに「既存プレフィックス配下のサブパッケージは意図どおり親ノードへ黙って畳まれ」と書いていたが、それは誤りなので改訂 1.51 が本文から撤回し、改訂 1.52 がこの欄からも取り下げた**——畳み込みが消すのは同一ノード内部の辺だけであり、サブパッケージが層をまたいで import すればその辺は親ノード発の辺として図に要求され、描いていなければ `TestDependencyGraphDrawsEveryCrossLayerImport` が落ちる。あわせて「本書が描かないのは UI パッケージ同士の依存である」の段落に、`UIApp --> UIParts` の 1 本だけは例外として図に描いてあることと、UI 層内部の辺の除外が効くのは `subgraph UI` を読み取る `TestDependencyGraphDrawsEveryCrossLayerImport` の向きだけで、`TestDependencyGraphHasNoStaleEdge` は除外せず描いてある辺をすべて裏取りすることを添えた | 本書は「**「その依存は無い」と読まれる辺の欠落は本書のグラフには無い**」と現在形で宣言しながら、その宣言を裏取りする手段は人手の棚卸しだけだった。**同じ欠落は改訂 1.22 / 1.27 / 1.29 / 1.32 / 1.33 / 1.40 で 6 度、いずれも defect として直っている**——辺の欠落は「その依存は存在しない」と読まれ、正当な import が規則違反と判定されるため、次にその階層へ手を入れる Issue が誤った判断をする。宣言が検査になった以上、読み手がどちらを信じてよいか（散文か実装か）を本書自身が示さないと、検査が在ることに気付かないまま同じ棚卸しをやり直すことになる（Issue #151） |
| 1.51 | 2026-08-24 | PR #153 の 2 周目レビュー（3 観点とも変異注入で実測）を反映。**詳細は下記「改訂 1.51」。** | 下記「改訂 1.51」を見よ |
| 1.52 | 2026-08-24 | PR #153 の最終レビュー（変異注入で実測した major 4 件）を反映。(1) **改訂 1.50 の欄が、改訂 1.51 の撤回した記述を現在形のまま保持していたので取り下げた**——「既存プレフィックス配下のサブパッケージは意図どおり親ノードへ黙って畳まれ」の一節である。本文の側は 1.51 が既に「畳み込みが消すのは同一ノード内部の辺だけである」へ直していた。(2) 「本書が描かないのは UI パッケージ同士の依存である」の段落に、**例外の 1 本 `UIApp --> UIParts` は `TestDependencyGraphDrawsEveryCrossLayerImport` でも除外しない**ことと、この 1 本が両方向から見られていること（実装から消えれば陳腐化した辺として、図から消えれば辺の欠落として落ちる）を明記した。あわせて検査側の穴を 3 つ塞いだ——(a) mermaid フェンスの開始行も `cutAtLineStart` で**行頭に 1 個**と数えるようにした、(b) UI 層内部の辺の除外条件から `UIApp --> UIParts` を外した、(c) `mermaidArrow` の点線の選択肢を「ハイフン直後のドット」へ緩め、テキスト付き点線リンク（`-. text .->` / `-. text .-`）の取りこぼしを塞いだ | (1) は**同一文書内で本文と改訂欄が逆のことを主張している状態**である。改訂履歴を索引に使う次の Issue は 1.50 を読んで「サブパッケージを足してもグラフは触らなくてよい」と判断しうる——1.51 が自ら「1 周目の critical と同型」と呼んだ誤読そのものである。同じ場面で [TUI コンポーネント設計](../ui/atomic-design.md) は**前の欄自体を訂正し、新しい欄にその訂正を記録する**処置を採っており（改訂 1.89 / 1.90）、2 文書で処置が不揃いだった。(a)(b)(c) はいずれも**黙って緑になる**経路で、3 つとも変異注入で実測した。(b) は本書が現在形で宣言する唯一の UI 内部の辺であり、図から 1 行消しても 4 本とも緑だった——**辺の欠落は改訂 1.22 / 1.27 / 1.29 / 1.32 / 1.33 / 1.40 で 6 度再発した、この検査群の存在理由そのものである。**(a) は本書が改訂履歴に本文を逐語引用する慣行を持つため、グラフの改訂前/改訂後が併記されると**レンダリング後に読者が見るほうのグラフが無検査になる**。(c) は正当な mermaid 記法なのに `mermaidEdge` にも矢印ガードにも当たらず、**どの case にも入らずに黙って無視されていた**——落ちた辺が UI 層内部の辺か陳腐化した辺だった場合、両方向の検査ともそれを見ないため完全に緑になる（PR #153 最終レビュー） |
| 1.53 | 2026-08-25 | PR #153 の最終レビュー 2 巡目を反映し、「この一致は機械的に検査している」の段落のファイルの責務を**分割後の 4 ファイル**へ更新した——`go list -json ./...` で実装の側を読んでノードへ畳む部分を `docs_deppkg_test.go` へ切り出したため、`docs_depgraph_test.go` は「読み取った図と実装の両者を突き合わせる」側になった（図を読むのは従来どおり `docs_mermaid_test.go`） | 本書のこの段落は**読み手が検査の実体へ辿る唯一の入口**である。`docs_depgraph_test.go` が `go list` を呼ぶと書いたまま実体が別ファイルへ移ると、検査を直しに来た Issue が該当のコードを見つけられない。ファイル名を挙げる以上、分割のたびに追随しないと 1 周で古くなる（分割の判断と責務の境界は [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の「`internal/buildconfig` を警告帯へ入れた判断」に記録した）（Issue #151） |
| 1.54 | 2026-08-25 | Issue #150 と、その後続（#155〜#157）を反映。**詳細は下記「改訂 1.54」。** | 下記「改訂 1.54」を見よ |
| 1.55 | 2026-08-25 | Issue #161（`internal/buildconfig` の分割）と PR #163 のレビュー指摘を反映。**詳細は下記「改訂 1.55」。** | 下記「改訂 1.55」を見よ |
| 1.56 | 2026-08-25 | Issue #169 を反映し、**現在形の行数の実測値 2 箇所を [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算)の行数表への参照へ寄せた**。(1) [`internal/runner/scope`](#internalrunnerscope) の行数チェックの段落から `internal/runner` 系 4 ディレクトリの使用量と残量を落とし、「集計は直下のファイルのみが対象」と「`internal/runner` は余裕が乏しいので、機能を足す Issue は先に切り出し先を決めること」という指示だけを数値なしで残した。(2) [`internal/disk`](#internaldisk) の段落から Issue #123 後の現在値を落とし、同じく行数表への参照へ寄せた（Issue #101 当時の値は議論の根拠なので、`着手時点では` / `当時` と読める形で残してある）。(3) これに伴い、**本書が `TestLineBudgetProseHasNoMeasuredNumbers` / `TestLineBudgetProseHasNoSumExpressions` の検査対象に入った**（`internal/buildconfig/docscheck/docs_linebudgetdoc_test.go` の `budgetProseDocs`。本書は行数表を持たないので除外の (1) は 0 個で、除外の一覧に載るのは過去の値を記録する「### `internal/disk`」の 1 節だけである） | **同じ写しを改訂 1.19 / 1.39 で 2 度追随している。** 行数は本書が「先に切り出し先を決めること」の判断材料として挙げているので古い値は分割の判断を誤らせるが、人手の追随は 2 度とも**陳腐化してから**行われた。PR #168 が入れた「散文が行数の実測値を持たない」検査は `docs/ui/atomic-design.md` しか読んでいなかったので、**本書の写しは無検査のまま残っていた**。とりわけ `internal/disk` の段落は末尾で「実測値の一次情報は同書の表であり、本書はここを重複して持たない」と**宣言しながら、その直前で現在形の実測値を持っていた**。検査の振る舞い自体の記録は [TUI コンポーネント設計](../ui/atomic-design.md#改訂履歴)の改訂 1.101 が持つ（Issue #169） |
...
| 1.58 | 2026-08-25 | PR #173 の 2 周目レビュー指摘（major 1 件）を反映。`internal/disk` の段落に残っていた「その後 Issue #123 の回帰テストの追加で**ふたたび詰まった**」を落とし、**Issue #123 で回帰テストを足した事実だけを残して現在の値は行数表への参照へ寄せた**。これは数値を伴わない現在形の断定であり、[TUI コンポーネント設計](../ui/atomic-design.md#行数の実測値は表だけが持つ)の規約 (c) が「検査が構造的に見ない範囲」と明記して「現在の状態を語りたくなったら、数値でも語形でも行数表を見よと参照へ寄せること」と定めているものにあたる。**欠陥の質は、同じ改訂 1.57 が隣の `internal/runner` の段落からは同じ理由で「余裕が乏しい」を落としていたのに、この段落だけ残した点にある**——同一コミット内で規約の適用が不揃いだった。 | 数値を伴わない現在形の断定は規約 (c) が「検査が構造的に見ない範囲」と明記したものであり、機械では拾えず規約でしか守れない。同一の改訂が隣の `internal/runner` の段落では同じ理由で落としていたのだから、この段落だけ残す根拠は無い |
...
| 1.64 | 2026-09-04 | レビュー指摘（major 7 件）のうち本書に掛かるぶんを反映。(1) 「分割したパッケージ」の `exec/command/limit` の行へ `NewBuffer(limit int) *Buffer` を足した。(2) 同節の「`exec/command/limit` はこの中で唯一どこも import しない」を `go list` の実測に合う限定（**内部パッケージを import するのは `exec/command` だけ**で、`limit` は上位の型を知らない）へ直した。同じ誤りを持っていた改訂 1.63 の欄も併せて直した。(3) `internal/config` のサブパッケージ表と「テストの配置」表へ `config/edit/validcheck`（Issue #184 で新設したテスト専用パッケージ）の行を足した | (1) `Buffer` の欄（`buf` / `limit`）は非公開で `NewBuffer` が唯一の作成手段なので、表から落ちていると「`Buffer` は複合リテラルで作れる」と読まれる（旧 `limitedBuffer` はそう作れた）。(2) 実測では `internal/exec` も `internal/exec/mask` も internal パッケージを 1 つも import せず、「唯一」は成立しない。同じ 1 文の前半が「`exec` はどのサブパッケージも import しない」と述べており文内で自己矛盾でもあった。(3) 新設パッケージが本書のどこにも無く、対称の事例（`exec/command/limit`）は `internal/exec` の表に行を持つので扱いが非対称だった。**テスト専用パッケージを載せない方針は無い**——`buildconfig/docscheck` と `docscheck/linebudget` はどちらも `doc.go` と `_test.go` だけだが表に行を持つ |
| 1.65 | 2026-09-04 | 3 周目レビューの major 4 件のうち本書に掛かるぶんを反映。「分割したパッケージ」の「**`limit` だけが上位の型を知らない**」を実測に合う限定（**`limit` も `exec/mask` もどちらも上位の型を知らない**——渡されるのは文字列と付随する値だけである）へ直した。あわせて改訂 1.64 の (2) の欄に残っていた同じ「だけ」も落とした | 同じ 1 文の前半が「`exec` / `exec/mask` / `exec/command/limit` はどれも標準ライブラリしか使わず」と述べており、**標準ライブラリしか import しないパッケージは定義上それらを参照できない**ので「だけ」は文内で自己矛盾である。実測でも `internal/exec/mask` の import は `slices` と `strings` の 2 つだけで、`mask.String(s string, secrets []string) string` / `mask.Args(args []string, secrets ...string) []string` は `limit.Tail(s string, n int)` と同じく文字列しか受け取らず `Command` / `ExitError` / 監査レコードを 1 つも知らない。**これは改訂 1.64 の (2) が直した過大主張と同型の再発である**——同じ欄が「`go list` の実測に合う限定へ直した」と宣言しているのに同じ 1 文の後半に等価な「だけ」が残っており、**改訂履歴を信じた読者は解消済みだと誤解する。** だから欄の側の「だけ」も一緒に落とした |
...
PR #153 の 2 周目レビュー（3 観点とも変異注入で実測）を反映。

- (1) **「畳み込みが消すのは同一ノード内部の辺だけである」ことを明記し、直前の改訂 1.50 が入れた「既存プレフィックス配下のサブパッケージは黙って親ノードへ畳まれる——`runner/procs` のようなサブパッケージが層の図に辺を増やさないのはこのため」という記述を撤回した。** サブパッケージでも層をまたいで import すれば親ノード発の辺として図に要求され、描いていなければ `TestDependencyGraphDrawsEveryCrossLayerImport` が落ちる。
  実例として、`Doctor --> Disk` を生んでいる唯一の import 元が `internal/doctor/hostres`、`Config --> GH` が `internal/config/edit`、`Exec --> Audit` が `internal/exec/command` であることを添えた（3 件とも `go list -json` で実測）。
  `runner/procs` が辺を増やしていないのは畳まれるからではなく import が標準ライブラリだけだからである。
  対応表へ規則を足す判断が要るのは独自ノードを与えたいときだけである旨へ書き分けた。
- (2) 「この一致は機械的に検査している」の段落に**検査が及ぶのは畳んだ後の辺までである**ことを添えた（パッケージ単位の内訳は人手。
  `ui/hostreq` に `logs` の import を足しても `UIApp --> Logs` の辺が既にあるため検査は 4 本とも緑になることを実測で確認した）。
- (3) `TestCollapsedUINodesMatchDoc` の参照先が 1 段落ずれていたのを、列挙を持つ「畳んだノード」の段落を指すよう直した。
- (4) 改訂 1.50 の「3 つの検査の責務分担」を「3 つのファイル（新設した検査は 4 本）」へ直した。
  あわせて検査側も塞いだ——矢印の左辺でノードを宣言する記法の取りこぼし、`--x` / `---` / `-.-` / `===` / `~~~` の取りこぼし、`%%` コメント行での誤検知、見出しと段落が 2 つあるときのアンカーのずれ、`subgraph UI` へノードを移したときに除外範囲が黙って広がる経路
...
- (4) は 1 つの文書が同じ対象に 2 つの数を主張する形で、本書が繰り返し是正してきた欠陥である（PR #153 2 周目）

#### 改訂 1.54

**変更内容**

Issue #150 と、その後続（#155〜#157）を反映。

- (1) `ui/page/pagetest/cmdtest` の行の API 列挙が Issue #150 以前のまま（`RunCmd` / `Expand` / `Msgs` / `Pump` / `FindMsg` / `Advance`）だったので、**#150 が中心にした `MustMsgs` / `RunAll` / `ChromeOf` / `HostReqOf` を含む現在の全 API** へ更新し、走らせる側・拾う側・Model を進める側に並べ直した。
  あわせて**この道具立ての核心である 3 つの約束**——`Cmd` を走らせる関数は `Advance` を除いてすべて `timeout` を引数に取ること（#150 / #156）、`ErrNotFound` / `ErrNoCmd` / `ErrCmdTimeout` を必ず分けて返すこと、`MustMsgs` だけは諦めを `panic` にすることを書き足した。**この約束は当初「すべて」と例外なしで書いていたが偽だった**——`Advance(m, cmd, rounds)` は `timeout` を取らず既定値 `CmdTimeout` を `MustMsgs` へ渡す唯一の道具であり、締め切りを渡せる版は `AdvanceQuick` である。
  あわせて並べ直しの区分も実態へそろえた——`Cmds` は束の `Msg` を解くだけで何も走らせないので「走らせて待つ側」から、`Drained` は channel を待つだけで `tea.Model` を進めないので「Model を進める側」から外し、それぞれ独立して書いた（PR #160 のレビュー指摘）。
- (2) `ui/page/pagetest` の行が道具として挙げていた「`Cmd` の展開と走査（`Msgs` / `ScanKey`）」を訂正した——**`pagetest` にパッケージレベルの `Msgs` は無い**（関数は `cmdtest` 側にあり、同名の `Spy.Msgs()` は spy が受け取った `Msg` を返す別物のメソッドである）。
  実在する道具（`State` / `ScanKey` / `Press1` / `ApplyChrome` / `OpenTabOf` / `TakeHostReq` / `IsQuit` / `StreamPage`）へ置き換え、`ScanKey` が `MustMsgs` を呼ぶこと、`State` が `Exec` と `ScanProcs` を既定で埋めてテストをホストから切り離すこと（Issue #155）を添えた

**変更理由**

- (1) の列挙は**読み手がどの道具を使うかを決める一次情報**である。
  `Msgs`（諦めを `error` で返す）と `MustMsgs`（`panic` で止める）の選び分けは Issue #150 の主題そのもので、後者が表に無いと、辿り切れる前提の呼び出し側が `Msgs` を選んで返り値を捨て、**辿れなかった `Msg` を「発行されていない」と読む assertion が静かに緑になる**——#150 が塞いだ欠陥がそのまま再発する。
  締め切りの約束を書かないと、新しい道具が締め切り無しで足されて壊れ方がハングに戻る。
- (2) は**存在しない API を名指ししていた**点で単なる古さより悪い。
  `pagetest.Msgs` を探した読み手は見つけられず、`Spy.Msgs()` を見つけて別物と気付かないまま使う経路もある。
  これは Issue #147 で `cmdtest` を切り出した際の追随漏れで、`internal/buildconfig` の検査は API 名を見ないため CI では捕まらない（検査を足すかは別途判断する。
  Issue #158）
...
Issue #161（`internal/buildconfig` の分割）と PR #163 のレビュー指摘を反映。

- (1) 「この一致は機械的に検査している」の段落が挙げる依存グラフの検査 4 本のパスが分割前のまま（`internal/buildconfig/docs_*_test.go`）だったので、移動後の `internal/buildconfig/docscheck/` へ直し、4 本ともこのサブパッケージにあることと分割の判断の参照先を添えた。
- (2) 「パッケージ責務」に **`internal/buildconfig`** の節と「分割したパッケージ」表を新設し、`buildconfig`（ビルド設定ファイルの不変条件）・`buildconfig/docscheck`（ドキュメントの不変条件）・`buildconfig/buildconfigtest`（両方が共有する `RepoRoot`）の 3 行を載せた。
  `buildconfigtest` の行には、`_test.go` ではなく通常のパッケージである理由・`fixtures` へ登録済みで本番からの import は検査で止まること・置いてよいのは両方が使う道具だけであることを明記した。
- (3) **PR #163 の 2 周目レビューで改訂 1.55 の欄が挙げる前例を実測へ訂正した**（1.34 / 1.47 / 1.48 / 1.49 → **`tabletest` は 1.34・`setuptest` は 1.35・`cmdtest` は 1.49**）。
- (4) **同じレビューで「分割したパッケージ」の散文が挙げる分割前の共有識別子を `RepoRoot` から `repoRoot` へ直した**（分割前は未 export であり、`RepoRoot` は `buildconfigtest` へ出したあとの名前である）。
- (5) **`internal/buildconfig` の対象の列挙へ依存更新設定（`.github/dependabot.yml`）を足した**——`ci_hardening_test.go` の `TestDependabotWatchesGitHubActions` が検証しているのに、本文の 4 分類（`Makefile` / CI ワークフロー / lint 設定 / Git Hooks 設定）のどれにも入っていなかった。
  「分割したパッケージ」表の `buildconfig` 行にも同じ波及を反映した

**変更理由**

- (1) のこの段落は**読み手が検査の実体へ辿る唯一の入口**であり、改訂 1.53 が同じ理由で 1 度追随している。
  ファイル名を挙げる以上、移動のたびに直さないと 1 周で古くなり、検査を直しに来た Issue が該当のコードを見つけられない。
- (2) のサブパッケージ表は実在するパッケージを本書から辿れるようにするためのもので、載せ落とすと次の Issue が同じ道具を書き直す（改訂 1.34 / 1.35 / 1.49 が `tabletest` / `setuptest` / `cmdtest` を同じ理由で追加した前例がある）。**`buildconfigtest` はテスト用の道具でありながら通常のパッケージなので、Go の側では本番からの import が止まらない**——表に載せる以上、その制約と `fixtures` への登録を同じ行に置く必要がある。
- (3) **の前例の挙げ方は 2 周目レビューで訂正した**——初版は 1.34 / 1.47 / 1.48 / 1.49 を挙げていたが、1.47 は Issue #139（`ui/discovery` の行の書き換え）、1.48 は Issue #148（`ui/startup` の行追加で、フィクスチャではなく本番コード）であって前例ではない。**誤った前例は「同じ理由の先例」を辿った次の Issue を無関係な改訂へ連れて行く。**
- (4) の `repoRoot` は**分割前の未 export の名前**で、`RepoRoot` は `buildconfigtest` へ出して export した後の名前である——同じ過去の事実について本書と [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算) が違う識別子名を主張していた（PR #163）
