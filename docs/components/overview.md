...
| 1.54 | 2026-08-25 | Issue #150 と、その後続（#155〜#157）を反映。(1) `ui/page/pagetest/cmdtest` の行の API 列挙が Issue #150 以前のまま（`RunCmd` / `Expand` / `Msgs` / `Pump` / `FindMsg` / `Advance`）だったので、**#150 が中心にした `MustMsgs` / `RunAll` / `ChromeOf` / `HostReqOf` を含む現在の全 API** へ更新し、走らせる側・拾う側・Model を進める側に並べ直した。あわせて**この道具立ての核心である 3 つの約束**——`Cmd` を走らせる関数は `Advance` を除いてすべて `timeout` を引数に取ること（#150 / #156）、`ErrNotFound` / `ErrNoCmd` / `ErrCmdTimeout` を必ず分けて返すこと、`MustMsgs` だけは諦めを `panic` にすることを書き足した。**この約束は当初「すべて」と例外なしで書いていたが偽だった**——`Advance(m, cmd, rounds)` は `timeout` を取らず既定値 `CmdTimeout` を `MustMsgs` へ渡す唯一の道具であり、締め切りを渡せる版は `AdvanceQuick` である。あわせて並べ直しの区分も実態へそろえた——`Cmds` は束の `Msg` を解くだけで何も走らせないので「走らせて待つ側」から、`Drained` は channel を待つだけで `tea.Model` を進めないので「Model を進める側」から外し、それぞれ独立して書いた（PR #160 のレビュー指摘）。(2) `ui/page/pagetest` の行が道具として挙げていた「`Cmd` の展開と走査（`Msgs` / `ScanKey`）」を訂正した——**`pagetest` にパッケージレベルの `Msgs` は無い**（関数は `cmdtest` 側にあり、同名の `Spy.Msgs()` は spy が受け取った `Msg` を返す別物のメソッドである）。実在する道具（`State` / `ScanKey` / `Press1` / `ApplyChrome` / `OpenTabOf` / `TakeHostReq` / `IsQuit` / `StreamPage`）へ置き換え、`ScanKey` が `MustMsgs` を呼ぶこと、`State` が `Exec` と `ScanProcs` を既定で埋めてテストをホストから切り離すこと（Issue #155）を添えた | (1) の列挙は**読み手がどの道具を使うかを決める一次情報**である。`Msgs`（諦めを `error` で返す）と `MustMsgs`（`panic` で止める）の選び分けは Issue #150 の主題そのもので、後者が表に無いと、辿り切れる前提の呼び出し側が `Msgs` を選んで返り値を捨て、**辿れなかった `Msg` を「発行されていない」と読む assertion が静かに緑になる**——#150 が塞いだ欠陥がそのまま再発する。締め切りの約束を書かないと、新しい道具が締め切り無しで足されて壊れ方がハングに戻る。(2) は**存在しない API を名指ししていた**点で単なる古さより悪い。`pagetest.Msgs` を探した読み手は見つけられず、`Spy.Msgs()` を見つけて別物と気付かないまま使う経路もある。これは Issue #147 で `cmdtest` を切り出した際の追随漏れで、`internal/buildconfig` の検査は API 名を見ないため CI では捕まらない（検査を足すかは別途判断する。Issue #158） |
| 1.55 | 2026-08-25 | Issue #161（`internal/buildconfig` の分割）と PR #163 のレビュー指摘を反映。(1) 「この一致は機械的に検査している」の段落が挙げる依存グラフの検査 4 本のパスが分割前のまま（`internal/buildconfig/docs_*_test.go`）だったので、移動後の `internal/buildconfig/docscheck/` へ直し、4 本ともこのサブパッケージにあることと分割の判断の参照先を添えた。(2) 「パッケージ責務」に **`internal/buildconfig`** の節と「分割したパッケージ」表を新設し、`buildconfig`（ビルド設定ファイルの不変条件）・`buildconfig/docscheck`（ドキュメントの不変条件）・`buildconfig/buildconfigtest`（両方が共有する `RepoRoot`）の 3 行を載せた。`buildconfigtest` の行には、`_test.go` ではなく通常のパッケージである理由・`fixtures` へ登録済みで本番からの import は検査で止まること・置いてよいのは両方が使う道具だけであることを明記した。**(3) PR #163 の 2 周目レビューで改訂 1.55 の欄が挙げる前例を実測へ訂正した**（1.34 / 1.47 / 1.48 / 1.49 → **`tabletest` は 1.34・`setuptest` は 1.35・`cmdtest` は 1.49**）。**(4) 同じレビューで「分割したパッケージ」の散文が挙げる分割前の共有識別子を `RepoRoot` から `repoRoot` へ直した**（分割前は未 export であり、`RepoRoot` は `buildconfigtest` へ出したあとの名前である）。**(5) `internal/buildconfig` の対象の列挙へ依存更新設定（`.github/dependabot.yml`）を足した**——`ci_hardening_test.go` の `TestDependabotWatchesGitHubActions` が検証しているのに、本文の 4 分類（`Makefile` / CI ワークフロー / lint 設定 / Git Hooks 設定）のどれにも入っていなかった。「分割したパッケージ」表の `buildconfig` 行にも同じ波及を反映した | (1) のこの段落は**読み手が検査の実体へ辿る唯一の入口**であり、改訂 1.53 が同じ理由で 1 度追随している。ファイル名を挙げる以上、移動のたびに直さないと 1 周で古くなり、検査を直しに来た Issue が該当のコードを見つけられない。(2) のサブパッケージ表は実在するパッケージを本書から辿れるようにするためのもので、載せ落とすと次の Issue が同じ道具を書き直す（改訂 1.34 / 1.35 / 1.49 が `tabletest` / `setuptest` / `cmdtest` を同じ理由で追加した前例がある）。**`buildconfigtest` はテスト用の道具でありながら通常のパッケージなので、Go の側では本番からの import が止まらない**——表に載せる以上、その制約と `fixtures` への登録を同じ行に置く必要がある。**(3) の前例の挙げ方は 2 周目レビューで訂正した**——初版は 1.34 / 1.47 / 1.48 / 1.49 を挙げていたが、1.47 は Issue #139（`ui/discovery` の行の書き換え）、1.48 は Issue #148（`ui/startup` の行追加で、フィクスチャではなく本番コード）であって前例ではない。**誤った前例は「同じ理由の先例」を辿った次の Issue を無関係な改訂へ連れて行く。** (4) の `repoRoot` は**分割前の未 export の名前**で、`RepoRoot` は `buildconfigtest` へ出して export した後の名前である——同じ過去の事実について本書と [TUI コンポーネント設計](../ui/atomic-design.md#行数の予算) が違う識別子名を主張していた（PR #163） |
