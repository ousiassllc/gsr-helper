...
前 2 つは実装上いずれも「取得を終えていない」（`page.ScopeState.Known` が偽）として同じ扱いになる。この 4 つをすり抜けた権限不足は、API を呼んだ時点の 403 として現れる（下記「レート制限とエラー」）。
