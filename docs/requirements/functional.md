...
反映のタイミングは項目ごとに異なる（下表）。**ただし保存の結果として画面に明示するのは `audit_log` を起動時の値から変えたときだけである。** 他の 3 項目は再起動を要さないので、保存しても状態行は「設定を書き込みました」のままにする——再起動が不要な項目にまで毎回案内を添えると読み飛ばされ、本当に再起動が要るときにも伝わらなくなるためで、`audit_log` の案内を起動時の値との比較で出す判断（下記）と理由を共有する。**例外は `--refresh` を付けて起動している間の `refresh_interval` で**、フラグが優先されるため編集しても（再起動しても）間隔は変わらないのに、画面には何も出ない。Config タブはフラグの有無を知る手段を持たないためで、案内を出すには配線の追加が要る。
...
**`audit_log` だけは走行中に反映しない。** 監査ログは起動時に開いたファイルハンドルへ書き続けており、走行中に開き直すことは書き込みの取りこぼしと二重オープンを招く——出力先の変更という頻度の低い操作のために、その危険を抱える理由が無い。**代わりに黙って効かないままにはしない。** 保存時に状態行へ「（監査ログの出力先は再起動後に切り替わります）」を出し、再起動の要否を保存したその場で判断できるようにする。伝えなければ、利用者から見れば「設定に書いたパスが効かない」という体験になり、他の 3 項目が（`--refresh` 起動中の `refresh_interval` を除いて）再起動なしで効くぶん、かえって不可解に映る。
...
| 1.13 | 2026-08-24 | 自身の設定（FR-41〜FR-42）の導入文「反映のタイミングは項目ごとに異なるため、保存の結果として画面に明示する」を実装に合わせて **`audit_log` を起動時の値から変えたときだけ**へ限定し、他の 3 項目は状態行が「設定を書き込みました」のままであること・`--refresh` 起動中の `refresh_interval` は編集しても再起動しても効かないのに画面には何も出ないこと（Config タブがフラグの有無を知る手段を持たないため）を明記した。あわせて同節の「他の 3 項目が再起動なしで効くぶんかえって不可解に映る」の断定に `--refresh` 起動中の例外を添えた。反映タイミングの表 4 行はそのまま残した | 要件が実装より広い約束をしていると、**後続 Issue が「全項目に案内を出す」実装を要件どおりと判断しうる**。実装（`internal/ui/page/config/applyflow.go` の `rememberSelf` / `auditRestartNote`）が案内を添えるのは `m.savedSelf.AuditLog != m.startupAudit` のときだけで、`selfconf_test.go` がその挙動を固定している。また `refresh_interval` の行が「`--refresh` を付けて起動している間は編集しても間隔は変わらない」と書く一方で散文が「他の 3 項目が再起動なしで効く」と断定しており、**表と散文が同じ節の中で矛盾していた**（PR #138 のレビュー指摘） |
