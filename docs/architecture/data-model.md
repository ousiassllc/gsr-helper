...
**代わりに `Result.Warnings` に 1 件出す。** 稼働中の runner が一覧から黙って消えるのは、ディレクトリだけ消えたユニットを孤児として出す [FR-05](../requirements/functional.md#孤児ユニットの検出) と同じ「片側だけ消えた」異常系であり、報告しないと利用者は原因の分からない消失として見ることになる。判定は「導出した `Dir` が実在しないこと」に限る（権限不足などの他の `stat` 失敗は、ディレクトリが消えた根拠にならないため報告しない）。同じディレクトリを指すプロセスが複数あっても警告は 1 件にまとめる。
...
以上より mtime を採る。数秒の差は経過時間の表示単位（分）に出ない。1 回の `os.Stat` で `Started` と `UID` を同時に取れるため、PID が再利用されて両者が別プロセスのものになることも避けられる。
...
**初回起動時の対話ウィザード（[FR-41](../requirements/functional.md)）は未実装である。** 設定が無い場合はこの版では案内を出さずに既定値で起動する。既定値でも全機能が動くため異常ではなく、ウィザードを実装する Issue が案内を足す。
