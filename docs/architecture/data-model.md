...
**`Runner` の値は読み取り専用として扱う。** `Svc` / `Listener` はポインタ、`Workers` はスライスなので、値をコピーしても指す先は共有される。1 周期ぶんの `Result` は全タブへ同時に配られるため（[画面仕様](../ui/screens.md)、`internal/ui` の `page.StateMsg`）、受け取った側が並べ替えや書き換えを行うと他のタブの表示まで変わる。**並べ替え・絞り込みは写しを作ってから行う。**

**周期をまたいだ共有は無い。** `Discover` は毎周期 `.runner` を読み直して新しい実体を組み立てるので、前の周期の `Result` を持ち続けているタブが後の周期の書き換えで変わることはない。これは値が等しいかでは確かめられない（2 周期の `Svc` は同じ内容になる）ため、実体の同一性を見る回帰テストで固定してある（`internal/runner` の `TestDiscoverReturnsFreshInstancesEachCycle`）。
...
**ユニットの実体が無い状態は 2 つあり、まとめて `Exists()` が偽になる。** `Load` が空（上記のプレースホルダ）と `Load` が `not-found`（`svc.sh uninstall` 後の残骸）で、どちらも `Unit` 以外のフィールドは埋まっていない。**`WorkingDir` / `User` を読んでよいのは `Exists()` が真のときだけ**である（空の `User=` を root と読むと全 runner が root 実行として表示される。上記「`RunAsUser` の決定と UID フォールバック」）。値 `not-found` の定義（`systemd.LoadNotFound`）も述語も `internal/runner/systemd` に置く——`LoadState` の語彙は systemctl のものであり、判定する側（紐付け・実行ユーザーの決定・孤児の診断）へ写すと読み手が増えたぶんだけ散らばるためである。**2 つを区別する必要がある側は `Load` を直接見る**（紐付けは `not-found` にだけ警告を出し、プレースホルダには出さない。下記「`Result` の警告」）。
...
| 1.18 | 2026-09-04 | `SvcState` の節に、ユニットの実体が無い 2 つの状態（`Load` が空 = `systemctl show` 失敗のプレースホルダ、`not-found` = `svc.sh uninstall` 後の残骸）をまとめた述語 `Exists()` があることと、値 `not-found` の定義が `internal/runner/systemd` にあることを明記した | 同じ「ユニットの実体があるか」の判定を、紐付け（`attach`）・実行ユーザーの決定（`resolveRunAsUser`）・孤児の診断（`internal/doctor/hostcfg`）の 3 か所がそれぞれの文字列比較で書いており、`"not-found"` の定数も 2 か所に別名で置かれていた。**`LoadState` の語彙は systemctl のものなので、値も述語もその出どころに 1 つだけ置く。** 判定する側へ写すと読み手が増えたぶんだけ散らばり、片方だけを直せば同じユニットに 2 つの解釈が生まれる |
| 1.19 | 2026-09-04 | `Runner` の節に、値のコピーが `Svc` / `Listener` / `Workers` の指す先を共有すること（読み取り専用として扱い、並べ替え・絞り込みは写しを作ってから行う）と、周期をまたいだ共有は無いことを明記した | **1 周期ぶんの `Result` は全タブへ同時に配られるのに、型の側にはそれが読み取り専用である保証が無い。** `internal/ui` の `App.tabs` と `organism.Table` には同じ形の注意書きがあるのに、より広く配られるドメイン層の値にだけ無く、扱いが非対称だった。現状どのタブも書き換えていないが、書き換えても例外もログも出ず、他のタブの表示が黙って変わるだけである。周期をまたいだ共有が無いことは値の比較では確かめられない（2 周期の `Svc` は同じ内容になる）ため、実体の同一性を見る回帰テストを併せて置いた |
