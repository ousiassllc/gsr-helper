...
**`organism/pane/` の分かれ目は「表示専用かどうか」ではない。** `pane.Log` は `bubbles/textinput` とフィルタの入力モードを持つので表示専用ではなく（後述の「organism 一覧」）、それでも `Detail` / `Help` と同じディレクトリに置いてある。分かれ目は **`bubbles/viewport` でスクロールする領域かどうか**である。3 つとも `viewport` を組み立て、既定のキーを本ツールのキーマップへ差し替える同じ関数（`viewportKeyMap`）を共有しており、置き場所を分けても得るものが無い。対して `organism/` 直下に置くのは、項目の並びに対して**カーソルと選択**を持つ部品（`ChoiceList`）である。**入力欄の有無で置き場所を決めない。** 入力欄で分けると、フィルタを足しただけの `Log` が `Detail` / `Help` と別の階層へ移り、スクロールの扱いが 2 箇所に分かれる。
...
| `organism/pane` | `Detail` / `Help` / `Log` / `ProgressList` | スクロールする**表示専用の領域** |
