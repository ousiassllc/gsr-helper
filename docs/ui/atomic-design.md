...
```
internal/ui/
  app.go            親 Model（ページ切替・検出結果・Caps・端末サイズ・背景の明暗）
  keys.go           キーの配送（ctrl+c と、page が差し戻したグローバルキーの解釈）
  chrome.go         親 Model の値を chrome/ の入力へ写す 1 手（Doctor タブの番号キーの解決を含む）
  discover.go       検出の駆動のうち親にしか決められないもの（1 周期の委譲・結果の取り込みと配布・1 周期分の入力の組み立て・走査ルートの合成）
  discovery/        検出の予算・発行・間隔決定・周期の突き合わせと、周期の状態（State と契機 TickMsg）
  startup/          起動後に 1 度だけ取りに行く 3 つの共有状態（前提チェック・_work 集計・保有スコープ）を束ねた State と、その駆動（StartAll）
  hostreq/          起動時のジョブ実行の前提チェック（FR-44）の発行と、その状態（State。1 度きりと件数）
  ...
  page/pagetest/cmdtest/ 発行された tea.Cmd を走らせ、束を辿る道具（RunCmd / Expand / Msgs / MustMsgs / RunAll / Pump / FindMsg / ChromeOf / Advance）
...
| `internal/ui/molecule/listrow/` | 行の組み立て（`RunnerRow` に相当するもの）を 1 つ足す。**依存の規則により `molecule` 以下しかドメイン型を落とした行を描けない**ので、`page/<tab>` の行を組み立てる側から呼ぶ形になる（既定は `page/runners/rowview`。Logs タブは `page/logs/filerow`） | 必須 |
...
**走行中に設定が変わることは、この経路に限って許容する。** 以前は逆の判断だった——動いているアプリ自体は再起動まで起動時の設定で動き続け、親 App の設定を書き換える経路は作らない（自動更新間隔などを走行中に差し替えると、設定を保存しただけでポーリングやしきい値の挙動が変わる）というものである。**反転させたのは、その懸念と Issue #128 が解こうとしている不満が正面から衝突するためである**——「保存しただけで挙動が変わる」ことを避け続ける限り、「設定フォームに書いた数字がその場で効かない」はそのまま残る。走行中に変わって困るのは値が**勝手に**変わる場合だが、この経路の契機は必ず**利用者がフォームで編集し、差分を承認して書き込めたとき**である。明示的に承認した値が黙って効かない方が驚きは大きい。残る懸念は自動更新間隔だが、`refresh_interval` を変えても**走行中の検出は中断されない**——次の周期を予約するのは `discovery.State.OnTick` が呼ぶ `discovery.Tick(in.Every)` であり、その `in` は周期ごとに `discover.go` の `input()` が組み立て直すので、新しい間隔になるのは次の予約からである。実行中の検出には `discovery.State.Start` の二重起動の抑止（実行中なら `nil` を返す）がそのまま働く。
...
**`scan_roots` はこの経路に載せ直した（Issue #132）。** かつては `cfg` ではなく `Options.Roots` として cmd が起動時に `MergeScanRoots(cfg.ScanRoots, o.roots)` を畳んだ値だったため、`cfg` を差し替えても走査の入力は古いままだった。**そこで `Options.Roots` の意味を「`--root` で渡された値だけ」へ狭め、合成は `discover.go` の `scanRoots()` が周期ごとにやり直す。** `--root` は保存後も効き続ける——明示的に渡したルートを設定の保存で消すと、起動コマンドを変えていないのに走査対象が減る。

**`audit_log` だけはこの経路に載せない（Issue #132）。** 親が配るのは `Options.Audit`——**既に開いたファイルハンドル**であり、追従させるには走行中にログを開き直すことになる。差し替えの最中に走っている書き込みの取りこぼしと二重オープンという危険を、出力先の変更という頻度の低い操作のために抱える理由が無い。**代わりに黙って効かないままにはしない**——`page/config` の `rememberSelf` が保存した `AuditLog` を**起動時の値（`startupAudit`）と比べ**、違っていれば結果の行へ `auditRestartNote`（「監査ログの出力先は再起動後に切り替わります」）を添える。**比べる相手が前回の保存値ではないのは、走っているログの出力先を決めるのが起動時の `audit_log` だからである**——前回の保存値と比べると、一度変えてから元へ戻した保存で「再起動後に切り替わります」と出る（再起動しても何も変わらないのに再起動を促す、事実として誤った案内である）。逆に `audit_log` を変えた後で別の項目だけを保存すると、再起動待ちが残っているのに案内が消える。変えていないときに添えないのは、毎回の保存に無関係な案内は読み飛ばされ、本当に再起動が要るときにも伝わらなくなるためである。

**この経路で直らないものは他にもある。** `refresh_interval` は `a.cfg` から読むため**この経路で次の周期から効くが、それは `--refresh` を指定していない場合に限る**——`discover.go` の `input()` が組み立てる `Every: discovery.Interval(a.opts.Refresh, a.cfg.RefreshDuration())` はフラグが正なら無条件にそちらを採るので、`--refresh` を付けて起動している間は編集しても間隔は変わらない。`scan_depth` も `a.cfg` から読むが（同じ `input()` の `Depth: a.cfg.ScanDepth`）、**自身の設定フォームに欄が無いのでこの経路では値そのものが変わらない**——`config/edit` の `SelfValues` が持つのは `scan_roots` / `refresh_interval` / `disk_thresholds` 2 つ / `audit_log` の 5 欄で（FR-41 の編集対象は 4 つ）、`Apply` は `scan_depth` を元の値のまま素通しする。
...
| `organism/table` | `Model[T]`（区画に分かれた一覧） | 区画・複数選択・絞り込みを持つ**一覧の共通実装**。1 部品で 2000 行規模になるため独立させる |
...
**行数チェック（`linterly`）の上限は 2 つある。** 1 つは **1 ディレクトリ 2000 行**（テストを含む。集計は直下のファイルのみを対象とし、サブディレクトリは別枠になる）、もう 1 つは **1 ファイル 300 行**である。`.linterly.yml` はどちらも既定値のまま使っている。**どちらに当たっても `make check` は同じように落ちる**ので、行数を引きに来たときは両方を見ること。ディレクトリの予算は本節が、ファイルの予算は下記「ファイルの行数」が扱う（両方に同時に詰まった実例は「6 周目の空け方」の `internal/ui/app_test.go`）。
...
**空け方の手は 4 つある。** (1) 本番の一部をパッケージ境界で切り出す——**依存の向きを強制できる、あるいは増え方が違うまとまりがあるときだけ**である（`tabset` / `chrome` / `molecule/listrow` / `molecule/chromebar` / `page/runners/rowview` / `page/config/itemview` / `page/logs/filerow` / `ui/startup`）。(2) テストの重複を削る。(3) テスト用の道具・フィクスチャを別ディレクトリの通常パッケージへ出す（`page/pagetest` / `page/pagetest/cmdtest` / `organism/table/tabletest` / `internal/setup/setuptest` / `internal/buildconfig/buildconfigtest`）。**(3) は本番の構造を 1 つも変えずに済む**のが利点で、本番の分割が「内部を export することになる」ため採れない場合（`ui/organism/table`）の唯一の手でもある。代償として本番からも import できてしまうので、足したら `page/pagetest/import_test.go` の `fixtures` へ登録すること（`TestNoProductionCodeImportsTestFixtures` が検査する）。(4) **テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける**——適用条件は (1) と同じく**増え方が違い、互いに依存が無い**ことである（先例は `internal/buildconfig` → `internal/buildconfig/docscheck`）。**(4) が (1) と違うのは、本番コードを 1 行も持たないテスト専用パッケージでも採れる点である**——(1) は「本番の一部を切り出す」手なので、切り出す本番コードが無ければ当たらない。分けると `_test.go` の中のヘルパは共有できなくなるので、共通の道具は (3) と組み合わせて 3 つ目のディレクトリへ出すことになる（`internal/buildconfig/buildconfigtest`。判断は下記「`internal/buildconfig` を 2 つに分けた判断」）。
...
**現在の行数を数値で書いてよいのは上記「行数の予算」の行数表だけである。** 散文は数を書かず、「上記「行数の予算」の行数表を見よ」と参照する。同じ実測値が本書の 3 つの系（2 つの行数表・散文・改訂履歴）へ手で写されていたことが陳腐化の原因であり、**写しを 1 つに減らすのが唯一の確実な手だからである。**

**この規約が掛かるのは本書だけではない。** 散文の検査の対象は `docs/ui/atomic-design.md` と `docs/components/overview.md` の **2 文書**であり（`internal/buildconfig/docscheck/linebudget/docs_linebudgetdoc_test.go` の `budgetProseDocs` が持つ）、**行数の実測値を数で持ってよいのは本書の行数表だけ**である。`overview.md` は行数表を持たないので、行数に触れるときは上記「行数の予算」の行数表を参照する。**対象を広げたのは corpus が 1 ファイルだったからである**——`overview.md` の `internal/runner` 系の使用量は改訂 1.19 / 1.39 で 2 度陳腐化して人手で追随した記録があるのに、PR #168 が入れた検査は本書しか読んでおらず無検査のまま残っていた（Issue #169）。

**表は実測と突き合わせる。** `TestLineBudgetTablesMatchLinterly`（`internal/buildconfig/docscheck/linebudget`）が `go tool linterly check --format json` の実測と、行数・残り・判定・行数の多い順の並びを突き合わせる。ずれたときに直すのは表の側であって、上限でも実測でもない。**手で写した値を書き換えて辻褄を合わせるのではなく、実測を取り直して表を作り直すこと。**

**散文は実測値が無いことだけを見る。** `TestLineBudgetProseHasNoMeasuredNumbers` が対象の 2 文書から下記の除外を落とした散文を走査し、`残り N` と `N 行` が 1 つも現れないことを確かめる。**この検査は言い回しを一切見ない。** 以前は「N 行・残り M 行」「移せるのは N 行まで」のように行数を主張する書き方を並べて表と突き合わせていたが、**言い回しの列挙は収束しなかった**——レビューのたびに未カバーの書き方が見つかり、そのどれもが現に古い数を素通りさせていた。数値の不在だけを見る形なら、新しい言い回しを考えなくてよい。**見ないのは `0 行` / `1 行` / `2 行` の 3 つだけで、`1 行` と `2 行` を外すのは本書が「表の 1 行」「一覧の 1 行」の意味でこの 2 つを常用するからである**（行数の主張ではない）。`0 行` が外れるのは式が 3 以上を拾う帰結だが、**残りがゼロになる状態は実在しうるので、そこは本規約でしか守れない**。**3 以上であれば 1 桁でも検査が拾う**——予算の残りは逼迫したディレクトリほど小さく 1 桁まで下がるので、「移せるのは N 行まで」という指示の N も 1 桁になる。Issue #164 の実害となった原文の N は 2 桁だったが、危険の形は桁で変わらない。`残り N` の側は `残りは` `残りが` `残りの` のように助詞が挟まっても拾うが、`残りが 1 桁` `残り 3 件` のように助数詞を伴う形は実測値ではないので拾わない。

**合計の式も散文には書かない。** `A + B + C = D 行` は 1 文で 4 つの実測値を主張するので、**写しが 1 か所ではなく 4 か所増える**。Issue #167 が挙げた 2 つの選択肢のうち**採ったのは、式を落として行数表への参照へ一本化する側である**——検査を式の評価（各項を行数表と突き合わせ、和が右辺と一致することを確かめる）へ広げる手もあるが、それは表と散文の一致を守るだけで**写しそのものは残る**。`TestLineBudgetProseHasNoSumExpressions`（`internal/buildconfig/docscheck/linebudget`）は式の**不在**を見る——散文に**数が演算子で 2 つ以上連なった形**があり、そこに 3 以上の数が 1 つでも含まれていれば落とす。**`= D` の右辺は任意である**——`=` を要求していた頃は「内訳は A + B + C で、合わせて D になる」という形が素通りしており、これは Issue #167 が挙げた実害（1 文で 4 つの実測値を主張する）そのままだった。演算子は `+` `-` とその全角（`＋` `−`）で、**閉じた集合なので文字クラスに並べても言い回しの列挙にはならない**。ただし `-` だけは**前後の空白を必須**にする——空白なしを許すと ISO 形式の日付（`2026-08-25`）が式として拾われ、改訂履歴の外にある日付までが偽陽性になるためである。**空白は半角の空白類に加えて全角空白（U+3000）も数える**——全角の演算子は全角空白を伴って書かれるほうが自然なので、半角だけを見ていると全角で区切った式が素通りする。空白文字も閉じた集合なので、これも言い回しの列挙にはならない。**床は `N 行` の側とそろえてある**——式に含まれる数が**すべて 2 以下のときだけ**実測値でないと見なす。3 桁で切っていた頃は、ファイル単位や小さいディレクトリの行数が 2 桁になりうるぶんを並べた式がまるごと素通りしていた。**床をここまで下げると行数ではない算術（列幅の見積もり）も式の形に当たるが、除外は足さない**——数の間に語を挟んで書けば（`行頭 6 + 列幅合計 66 + 列間 3 = 75 セル`）式の形を外せる。**偽陽性は検査の外へ節を出すのではなく、文書側の書き方で消すこと**——除外を足すのは検査が見ない範囲を広げることであり、そこは規約でしか守れなくなる。**式を別に拾うのは各項が裸の数だからである**——`N 行` の側が拾えるのは右辺だけで、右辺が `行` を伴わない書き方になった瞬間に全項が素通りする。**過去の値を記録する節（下記 (4) の一覧）では式を書いてよい**が、`当時` と読めるように書くこと——分割の前後で合計がどう動いたかは、その Issue に紐づく事実だからである。

**除外は正規表現ではなく明示の一覧で持つ。** 外すのは次の 4 つだけである。(1) 行数表（実測と突き合わせるのがここである。本書は 2 つ持ち、`overview.md` は 1 つも持たない）。(2) コードブロック。(3) `## 改訂履歴` 以降——**行頭の見出し行でだけ切る**（字下げや文中の引用で切ると、そこから先がまるごと検査から外れる）。(4) 過去の値を記録する節を見出しの文字列で並べた一覧——本書のぶんが `docs_linebudgetprose_test.go` の `budgetProseExcluded` で、「…の空け方」「…判断」の節と「一覧タブを 1 枚足せる余裕」がここに入る。**(4) の一覧は文書ごとに持つ**（`budgetProseDoc.excluded`。**`overview.md` は除外を 1 つも持たない**——唯一あった「### `internal/disk`」は節まるごとを検査から外していながら過去の値だけを持つ節ではなかったので、当時の値を「`internal/disk` から `pathguard` を切り出した判断（Issue #101）」への参照へ寄せて一覧を空にした）。**混ぜてはならない**——除外は文書の構造に結びついており、一覧を 1 つにすると**片方にしか無い見出しが消えたことを検査が知らせられなくなる**（検査は一覧に挙げた見出しが本文に無いと `t.Fatal` するので、混ぜた一覧ではもう一方の文書の見出しを毎回取り逃がす）。**(4) に載せてよいのは過去の値だけを持つ節である。** 現在の値を行数表や `go tool linterly check` の出力への参照へ寄せれば除外は要らなくなるので、そうなった節は一覧から外すこと（「`ui/organism/table` の本体を分割しない判断」は現在のファイル数と行数を落とした時点で外れた）。**一覧は小さいほどよい**——載っている節は検査の外にあり、そこだけは規約で守るしかないからである。**(4) の除外は入れ子では広がらない**——1 つの見出しが外すのは次の見出し（深さは問わない）までの本文だけで、入れ子の小節は自分が一覧に載っているときだけ外れる。**一覧に挙げた見出しの集合が、そのまま除外範囲である。** **この 4 つは 3 つの検査すべてに同じように効く**——合計の式の検査だけが外す節は 1 つも無い。かつては列幅の見積もりを通すために「### 幅」を合計の式の検査からだけ外す 5 つ目の一覧を持っていたが、その節の書き方を数の間に語を挟む形へそろえて偽陽性が消えたので、**一覧ごと落とした**。**除外が現実と食い違ったら検査は声を上げて落ちる**——どちらの文書についても、行数表がその文書に期待した数だけ見つからないとき（本書は 2 つ、`overview.md` は 0）、行頭の `## 改訂履歴` が見つからないとき、その文書の一覧に挙げた見出しが本文から消えたとき、コードブロックの囲みが閉じ忘れられているとき、除外を落とした結果として散文が 1 行も残らないときは `t.Fatal` する。

**300 / 330 / 2000 / 2200 は散文に数で書いてよい。** どれも `.linterly.yml` の上限と警告帯の境界であって実測値ではなく、数で書かれていないと規約そのものが読めない。検査が除くのはこの 4 つだけである。

**検査が構造的に見ない範囲がある。** 拾うのは `N 行` / `残り N` / **数が演算子で連なった式**という**数値**の形だけなので、次の 5 つは検査の外にある——(a) 改訂履歴、(b) 上記 (4) の一覧に挙げた節、(c) **数値を伴わない語形の主張**（`余裕がある` / `潤沢ではない` / `残りはテスト 1 本ぶんしかない`）、(d) **行以外の助数詞で数える値**（`N 個` / `N ファイル` のような個数）、(e) **式にも助数詞にもならない裸の数**（`内訳は A / B / C` のように、スラッシュや読点で並べただけの形）。(e) を拾えないのは**数だけを見て実測値かどうかは判定できない**からである——対象の 2 文書はどちらも Issue 番号も改訂の版番号もセル数も数で書くので、裸の数を一律に落とすと文書が書けなくなる。だから**内訳を語りたくなったら数を並べず、行数表を指すこと**。(a)(b) は除外として意図したものだが、**(c)(d)(e) は意図の外で、一覧の大小に関わらず本規約でしか守れない。** (c) を言い回しの列挙で拾う手は 2 周かけて捨てた方式そのもので、書き方は無限にあり収束しない。だから**現在の状態を語りたくなったら、数値でも語形でも「上記「行数の予算」の行数表を見よ」と参照へ寄せること**。とりわけ「空ける必要は無い」のように**安全側を崩す助言**は、数値を伴わなくても次の Issue を誤らせるので書かないこと——予算の逼迫は書いた後に進む。

**過去の値は実測へ寄せない。** 空け方と判断の記録はその Issue に紐づく事実であり、現在の実測へ書き換えると議論の根拠が消える。だから (3) の改訂履歴と (4) の節は検査の外に置く。**外にあるということは、そこだけは規約で守るしかないということである**——過去の値は `当時` / `着手時点では` / `Issue #NNN の時点では` と読めるように書き、現在の値に触れたくなったら数を書かずに「上記「行数の予算」の行数表を見よ」と参照すること。

#### UI 層の外のディレクトリ

**同じ上限は `internal/` 直下のディレクトリにも掛かる。** 本書は UI 層の設計を記す文書だが、行数チェック（`linterly`）はリポジトリ全体を見る。**空け方の判断はここに集約する**——4 つの手（本節冒頭）は層に依らず同じであり、判断を 2 か所に置くと片方だけが古くなるためである。

**表は UI 層と同じく全件を載せる。** 判断の記録があるディレクトリだけを並べると、記録が無いまま逼迫しているディレクトリ（下の表で残りが 1 桁の行がそれである）が表から消え、**まさに先に空けるべき場所が見えなくなる**。`testdata/` のフィクスチャは対象外なので載せない。
...
| `internal/buildconfig` | 1408 | 592 | pass |
...
| `internal/runner/systemd` | 596 | 1404 | pass |
...
着手時点では 2181 行（残り -181 行）で、**エラー境界の 2200 まで 19 行**しかなかった。ここへ 1 行足すだけで `make check` が落ちる状態である。上限値は緩めず、4 つの手のうち **(1) 本番の一部をパッケージ境界で切り出す**を採った。
...
**切り出した直後は 1927 行（残り 73 行）だった**（Issue #101 当時の値）。**現在の値は上記「行数の予算」の行数表を見よ**（その後の Issue が足しており、残りは実質ゼロである）。**次にこのディレクトリへ手を入れる Issue は 1 行足す前に空けること。**

##### `internal/setup` のフィクスチャを `setuptest` へ出した判断（Issue #103）

着手時点では 2123 行（残り -123 行）だった。4 つの手のうち **(3) テスト用の道具・フィクスチャを別ディレクトリの通常パッケージへ出す**を採った。
...
**(1) は採れなかった。** 本番で切れる境界は `self.go`（自身の設定）と残り（runner の設定）だが、`Validate*` は両方に散っており（`ValidateLine` / `ValidateHook` / `ValidateLabelInput` は `summary.go`、`ValidateRoots` / `ValidateRefresh` / `ValidatePercent` / `ValidateAuditLog` は `self.go`）、検証だけを本番から切り出すと `edit` が自分の入口を持たなくなる。**本番の公開面はそのままにして検査だけを分ける**のが (4) の利点で、ここではそれがそのまま当たる。
...
**上限の値は出さずに `command` へ残した。** どこに何バイトの上限を置くかは呼び出し側の方針であって切り詰めの仕組みではないので、3 つの定数（監査レコード・抜粋・取り込み）は `command/limits.go` が持つ。**外へ出したのは収める仕組みであって、収める義務ではない**——`recordedError` が `limit.Tail` を、`ExitError.Stderr` が `limit.Head` を必ず通る構造は変えていない。export したのは `Tail` / `Head` / `Prefix` / `Suffix` / `Buffer` / `NewBuffer` の 6 つで、断片を落とす 2 つ（`dropPartialRuneAtEnd` / `dropPartialRuneAtStart`）は新しいパッケージの中で非公開のままである。**`NewBuffer` を落とせない**——`Buffer` の欄（`buf` / `limit`）は非公開なので、これが唯一の作成手段であり、外した時点で呼び出し側（`command.go` の取り込み）が `Buffer` を作れなくなる。
...
**同じチェックが 1 ファイル 300 行にも掛かる。** ディレクトリと同じく `warning_threshold: 10` が効くので、301〜330 行が **WARN**、330 行を超えると **ERROR** である（設定の詳細は[環境構築](../environment/setup.md#linterly)）。**ファイル単位の実測は本書のどの表にも載せない**——対象がディレクトリと違って数百あり、写した時点で古くなるからである。どのファイルが 300 行に近いかは `go tool linterly check` の出力を見ること。

**300 行に張り付いたファイルは、その都度この節が前例として記録してきた。** `internal/ui/page/runners/runners_test.go` は Issue #159 が行の描画の検証を `page/runners/rowview` へ移して外れた（同じ Issue で `internal/ui/page/overlay_test.go` も下がっている）。`internal/buildconfig/docscheck/docs_depgraph_test.go` は改訂 1.91 が責務の境界で 2 つに分けて外れた（分けた当時は分割前の `internal/buildconfig` 直下にあり、Issue #161 が `docscheck` へ移した。行数は移動では動かない）。`internal/ui` 直下の `app_test.go` は 6 周目に 300 行へ張り付き、Issue #139 が周期の状態を `discovery.State` へ移して下がった（下記「9 周目の空け方」）。`cmdtest/run.go` を分けた経緯は下記「前例」にある。

**採る手は 1 つだけである——同じディレクトリの中で責務の境界に沿ってファイルを分ける。** ディレクトリの 4 つの手（本節冒頭）と違い、ファイルの超過はパッケージ境界の問題ではないので、切り出し先を別ディレクトリにする理由が無い。上限値を緩めるのも、行を詰めて 300 行に収めるのも採らない——**前者は次の Issue が読む予算を壊し、後者は 1 行あたりの情報量を増やして読みにくくするだけで、責務は 1 つも減らない。**
...
**前例。** Issue #77 は `internal/ui/app.go` が 300 行をわずかに超えたのを `Init` の `tea.Batch` を 1 行に畳んで戻したが、これは責務を動かしていない（数行の超過なので境界を切るまでもなかった）。Issue #108〜#114 は 300 行を超えていた 7 ファイルを責務の境界で分けている（`page/config` の回帰テストを自身の設定と runner 設定へ、`runner/discover.go` の紐付けを `attach.go` へ、`appconfig` の書き込みを `save.go` / `save_test.go` へ、`exec/command` の秘匿値検証を `auditsecret_test.go` へ、`keymap` のヘルプ組み立てを `help.go` へ、`hostcaps` の時間の検証を `timeout_test.go` へ）。**Issue #150 も同じ手を採った**——`cmdtest/run.go` が `ChromeOf` へ締め切りを通した時点で 300 行を超えたので、「走らせた結果から何を拾うか」を `find.go` へ出した（`ChromeOf` / `HostReqOf` / `FindMsg` と `ErrNotFound`）。`run.go` に残したのは「Cmd をどう走らせ、どう待つか」（`RunCmd` / `bundle` / `Pump` / `Drained`）で、辿るキューは両方から使う。テストも同じ境界で `run_test.go` / `cmd_test.go` / `find_test.go` に分かれ（`find_test.go` は PR #154 のレビュー指摘で `ChromeOf` / `HostReqOf` の未検証の腕を足した際に `run_test.go` から分けた）、Issue #157 が `run.go` の `Pump` / `Drained` の未検証の分岐を集めた `pump_test.go` を足して 4 つになった。ディレクトリの予算は当時十分に空いていたので、上記「ただしディレクトリの予算を先に見ること」には掛からなかった。**分けた先の名前が責務を言い当てられないなら、それは境界を間違えている。**

#### `ui/page/config` を `page/configmodal` へ分けた判断（Issue #12 / 実施は Issue #104）

Config タブは項目の一覧・フォーム 6 種・差分の承認・反映方法の選択・初回設定ウィザードを持ち、素直に書くと 2100 行を超えた（**この段落の数はいずれも Issue #104 当時の値である**）。**そこで tea に依らない部分を `internal/config/edit`（設定項目ごとの変更の組み立て・差分・書き込み・ラベルの API）へ出し、1985 行に収めた。**
...
**その後の研磨で警告帯に入った（当時 2059 行・残り -59 行。ピークは当時 2117 行・残り -117 行で、Issue #104 / #108 を経て当時 1737 行まで下がり、Issue #128 が +95 行し、Issue #132 が +148 行し、Issue #140 が +9 行し、Issue #141 が +66 行、Issue #142 が +43 行して当時 2098 行・残り -98 行の **warn** になった。Issue #147 が `page/config/itemview` を切り出して警告帯を出た。**現在の値は上記「行数の予算」の行数表を見よ**）。** 押し上げたのは critical 7 件の回帰テストである（二重承認でバックアップが壊れる、実行中の対象切り替えで別の runner へ書く、処理中に新しい編集を始められる、など）。**安全側の検証を行数の都合で落とさない方を採った。** 同じ判断を `ui/page/disk`（当時 2101 行。Issue #102 で分割済み）と `ui/page/runners`（当時 2164 行。Issue #107 が削減し、Issue #155 の時点では残り 0 行まで詰まっていたのを Issue #159 が空け直した）も記録している。**どちらも現在の値は上記「行数の予算」の行数表を見よ。**

**Issue #104 で当時 2117 行から 1719 行へ戻した**（その後 Issue #108 が `regress_test.go` を 2 ファイルに分けたぶんを含めて 1737 行になり、Issue #128 が自身の設定を親へ返す経路とその回帰テストで +95 行し、Issue #132 が監査ログの案内とその回帰テストで +148 行し、Issue #140 が +9 行（回帰テストと道具は `page/pagetest` へ出したため増分が小さい）、Issue #141 / #142 が見出しの回帰テスト（`title_test.go`、92 行）で +109 行して当時 2098 行・残り -98 行になった。Issue #147 が一覧の行の組み立てを `page/config/itemview` へ出して警告帯を出た。**現在の値は上記「行数の予算」の行数表を見よ**）**。** ただし本節がそれまで挙げていた候補——「残った tea 非依存の部分（`items.go` の要約、`form.go` の検証）を `config/edit` へ出す」——は**もう使えなかった。**
...
**次にこのタブへ手を入れる Issue の予算は、上記「行数の予算」の行数表の `ui/page/config` の行が持つ**（Issue #147 が当時 2098 行の警告帯から 1990 行へ戻し、Issue #150 の `cmdtest` への追従で 2 行増えた。下記「Issue #147 の空け方」）。**警告帯は出たが余裕があるわけではない**ので、行数表の残りを超える追加が要るなら足す前に空けること。**採れる手として本節が挙げていた `selfconf.go` の切り出しは、Issue #147 が検討して採らなかった**——5 つとも `Model` のメソッドで、出せば親の非公開を export することになる（`ui/organism/table` を分割しない判断と同じ理由）。次に空けるなら、残る候補は `config.go` / `flow.go` / `results.go` の状態遷移ではなく、テストの重複削減である。

##### `ui/page` を警告帯へ入れた判断（Issue #155）

着手時点では 1998 行（残り 2 行）だった。**足したのは `page.StateMsg` のフィールド 1 つ（`ScanProcs`）と doc 4 行で、当時 2003 行（残り -3 行・warn）になった。**
...
1. **4 つの手のうち (1) は本 Issue の丈に合わない。** `ui/page` に残っているのは `StateMsg` と `Overlay` のようにタブ全体が参照する土台だけである。**`StateMsg` は循環しない**——`StateMsg` とそれが持つ型（`DiskState` / `WorkUsage` / `ScopeState` / `SetupDeps` / `ConfigDeps`）が参照するのは `appconfig` / `audit` / `exec` / `gh` / `runner` / `keymap` / `token` / `atom` / `setup/job` / `setup/tarball` だけで、`page` 自身の識別子は 1 つも要らない。切り出せない理由は**波及の広さ**である——`page.StateMsg` は全タブ・`tabset`・`pagetest` から数十箇所で名指しされており、`state.Msg` への改名はリポジトリの端まで届く。バグ 1 件の修正に混ぜる変更ではない。**`Overlay` の側は循環しうる**——`overlay.go` / `overlaystate.go` と `modal.go` の `handlesBack` に跨がり、`Do` / `TabMsg` / `StateMsg` を使うため、`page` に `Overlay` の利用者を残したまま出すと戻りの辺が生じる（`helpmodal.go` を道連れにすれば避けられる）。
2. **(2) テストの重複削減は本 Issue の範囲外である。** `ui/page` 直下のテストは 6 ファイル 824 行で、削るなら `overlay_test.go`（296 行）の重なりの検証であり、**本 Issue が触れていない部分を触ることになる**（**この 2 つは Issue #155 当時の値である**。切り出した Issue #159 がこの候補をそのまま採り、ファイル数も行数も下がっている。**現在のファイル単位の実測は `go tool linterly check` の出力を見よ**）。
3. **足す側は削った。** doc は当初 13 行だったものを 4 行に詰め、詳しい理由は、まだ余裕のある `pagetest.ScanOf`（`ui/page/pagetest`。**現在の値は上記「行数の予算」の行数表を見よ**。Issue #159 の時点では 1932 行・残り 68 行で、Issue #161 が `fixtures` へ 1 件登録して 3 行増えた）側へ置いた。**同じ Issue で `ui/page/runners` は Issue #155 の時点では 2000 行（残り 0 行・pass）に収めてあった**（現在は Issue #159 が空けた後であり、経緯は下記「Issue #159 の空け方」が、値は上記「行数の予算」の行数表が持つ）——回帰の検証は既存の `TestDrainStopsAfterWaiting` に足し、走査の既定は `pagetest.State` へ寄せた。
4. **当時、ERROR 境界（2200 行）まで 197 行あった。** `make check` は通っていた。
5. **1 ファイル 300 行も守っている**——`page.go` は当時 284 行（残り 16）で、警告帯 301〜330 行の外だった。**現在のファイル単位の実測は `go tool linterly check` の出力を見よ。**

**切り出した Issue #159 が空けた。** 残りマイナスの状態は解消しており（**現在の値は上記「行数の予算」の行数表を見よ**）、上記 2. で候補に挙げたテストの重複削減がそのまま採られた。本 Issue は「判断を残して警告帯へ入る」逃げ道を 1 度使い、次の Issue がそれを畳んだ形である。詳細は下記「Issue #159 の空け方」。

##### Issue #159 の空け方——`ui/page` を警告帯から出し、`ui/page/runners` に余裕を作った

Issue #155 が残した 2 つ——当時の `ui/page` 2003 行（残り -3 行・warn）と `ui/page/runners` 2000 行（残り 0 行・pass。次に 1 行でも足せば warn）——を、**本番コードを 1 行も変えずに**空けた。4 つの手のうち使ったのは (2) テストの重複削減だけである。

**`ui/page` は当時 2003 → 1955 行（残り 45 行・pass）になった。** Issue #155 が「候補は上記 2. のテストの重複削減」と名指ししたとおりに進めた。

1. **`TestOverlayRegisteredModalFollowsStackRules` を削除した。** この検証が主張していた「種類を登録しても重なりの規則はそのまま効く」は**別の経路を通っていない**——共有の `newOverlay()` 自身が同じ `Register` で `kindFirst` / `kindSecond` を登録しているためである。個々のアサーションは `overlay_test.go` が持つ（キーが最上位だけに届く＝`TestOverlayDeliversToTopOnly`、フッタのヒントが最上位のものになる＝`TestOverlayHintsAndTitleFollowTop`、esc が 1 枚だけ閉じて背後が描かれる＝`TestOverlayCloseOneByOne`）。**ただし当初 1 件だけ移し損ねた**——「2 枚重ねた状態で `Overlay.View()` が描くのは最上位である」は上の 3 つのどれも覆っていなかった。`TestOverlayCloseOneByOne` が `View()` を見るのは esc を 1 度送った後であり、そのときスタックは 1 枚しか無い（最上位と背後が同じものになる）。`TestOverlayHintsAndTitleFollowTop` が見るのは `Hints()` で、別の関数である。**本 PR のレビュー指摘で `TestOverlayCloseOneByOne` の 1 度目の esc の直前へ足し直したので、現在この検証を持つのは `TestOverlayCloseOneByOne` である。** 削除の判断自体は変わらないが、**「すべて別の経路が持つ」と書く前に 1 件ずつ実際に落ちることを確かめること**（`overlay.go` の `View()` を `topModal()` から `o.s.modals[o.s.stack[0]]` へ書き換え、`internal/ui/page` が落ちるかを見る）。
2. **残った `TestOverlayHelpScopeIsPerPage` を `helpmodal_test.go` へ移し、`modalstack_test.go` を削除した。** 重なりの規則を持たなくなった時点で `modalstack` という名前は責務を言い当てておらず（上記「ファイルの行数」の**分けた先の名前が責務を言い当てられないなら、それは境界を間違えている**）、ヘルプの検証は `helpmodal_test.go` に集まる。移動後も当時 114 行で 300 行の上限内だった。
3. **共有状態が欠けずに配られたことを見るアサーションを `wantFullState` 1 本に寄せた。** `TestOverlayReplaysStateOnRegister` と `TestNewOverlayReplaysFullStateOnRegister` に同じ検査が重複していた。**両テストとも存続する**——前者は遅延登録への直前の `SetState` のリプレイ、後者は `NewOverlay` が持つ初期状態と、見ている経路が違う。アサーションは両者の和集合にしてあり、片側にしか無かった `Keys` の検査と枠の引き算の検査は両方へ広げた。

**`ui/page/runners` は当時 2000 → 1923 行（残り 77 行・pass）になった。** Issue が挙げた 2 案（`rowview` の先例どおりのネスト / テストの重複削減）のうち、**新しいパッケージは作らなかった。**

1. **行の注意記号と SVC 列の不明表示を見る 2 つのテストを `page/runners/rowview` へ移した**（`TestUnavailableManagedRowIsWarned` / `TestUnknownServiceStateIsDistinguished`。`runners_test.go` の該当ハンクで **−47 行**）。**これは行数のためだけの移動ではない。** 判定を持つのは `rowview` の `warned` / `svcUnknown` なのに、検証はタブ全体を描いて記号の文字列を探しており、しかも `rowview_test.go` はこの 2 つを 1 つも覆っていなかった（覆っていたのは `_WORK` 列だけである）。移動先では `RunnerView.Warn` / `RunnerView.SvcUnknown` を直接見るので**検証はむしろ強くなる**。`rowview` への export の追加は 0 件で、`rowview.go` は無変更である（上記「`ui/organism/table` の本体を分割しない判断」が退けた「内部を export することになる」形には当たらない）。
2. **塞がれた操作の 2 つのテストを表にまとめた**（直起動・非 root）。本体が同じで、違うのはフィクスチャ・キーの集合・理由の定数だけだった。直起動側にしか無かった「確認ダイアログが開かない」の検査は両ケースへ広げてある。
3. **`pagetest` の再実装を 2 つ消した**。`logs_test.go` の `openTabMsg` は `pagetest.OpenTabOf` の再実装で、しかも `ErrNotFound` と `ErrCmdTimeout` を bool 1 つに潰しており「配線が無い」と「Cmd が戻らない」を区別できなかった。`opsflow_test.go` の `standaloneRunner` は `pagetest.StandaloneRunner` と同じもので、唯一の呼び出しが渡す名前は `SampleRunner` の既定と一致していた。**ただし置き換えた当初、孤児ユニットの行の 1 箇所だけが `err == nil` しか見ておらず、区別を取り戻せていなかった**——`ErrCmdTimeout` を「開こうとしていない」と読む形で、`pagetest.OpenTabOf` の doc が名指しで禁じているものである。本 PR のレビュー指摘で `errors.Is(err, cmdtest.ErrNotFound)` へ直した。**道具を共有へ寄せるだけでは区別は戻らない。呼び出し側が返り値を分けて読むところまでが 1 組である。**
4. **`newModel` / `newOpsModel` / `mustModel` の 3 つを 1 本にした**。どれも `runners.New(0, st).Update(st)` に還元できた。`newModel` が `testState` を 2 回呼んで別のスナップショットを 2 つ配っていた不具合もここで直している。

**内訳は手ごとではなくファイルごとの差し引きで示す。** 2.〜4. は削った行と足した行（表の骨組み、共有ヘルパの doc、両ケースへ広げたアサーション）が同じファイルに混ざるため、手ごとの数は差し引きにならない。**`origin/feat/#1` との実測（`wc -l`）は次のとおりで、合計 −77 行が当時の 2000 → 1923 行に一致する**——`runners_test.go` −54（うち 1. のハンクが −47、残りは 4. で消えた `mustModel`）、`logs_test.go` −10（3.）、`ops_test.go` −9（4.）、`opsflow_test.go` −9（2. と 3.）、`helper_test.go` +5（4. の doc）、`chromeorder_test.go` / `opsdrain_test.go` ±0（4. の呼び出し側の置き換えだけで行数は動かない）。**1. の移動先は別ディレクトリなので `page/runners` の予算には効かない**——`page/runners/rowview` は当時 269 → 308 行になった（**現在の値は上記「行数の予算」の行数表を見よ**）。

**採らなかった手と理由。**

- **(1) ネストしたパッケージの新設は要らなかった。** `keys.go` の Setup タブへの引き渡し（`n` / `D` / `u`）は親の非公開に触れずに `page/runners/setupreq` として出せる形をしており、候補としては成立する（入力はすべて他パッケージの公開型で、出力は `tea.Cmd` と通知の文字列である）。**ただし上記のテスト側の手だけで当時の残り 77 行に届いた**ので、パッケージを 1 つ増やす代償に見合わない。**次に `page/runners` の本番コードが増えて空ける必要が出たときの第一候補はこれである。**
- **フィクスチャ（`sampleRunner` / `unmanagedRunner` / `orphanUnits`）を `pagetest` へ出す手（3）は採らなかった。** `page/runners` から 48 行減る代わりに `ui/page/pagetest`（当時 1932 行・残り 68 行。**現在の値は上記「行数の予算」の行数表を見よ**）が十数行増える。**行数を空ける Issue が別のディレクトリを逼迫させるのは本末転倒である**（上記「9 周目の空け方」と同じ判断）。
- **`TestDrainWaiterSwallowsGlobalKeys` の削除は見送った。** `TestConfirmModalSwallowsGlobalKeys` がより広いキーの集合で同じ分岐（`keys.go` の `m.overlay.Active()`）を通るため重複ではあるが、この検証は Issue #155 がドレイン停止の待機に入れた守りに隣接しており、当時 17 行だったこの 1 本を落とす価値が無い。

**副産物として 1 ファイル 300 行の側も緩んだ。** `runners_test.go` が当時 298 行（残り 2）から 244 行になり、上記「ファイルの行数」の首位から外れた（同じ周で `overlay_test.go` は 296 → 284 行）。**ディレクトリの予算を先に見ること**という同節の指示に、今回は逆向き——ディレクトリを空けた結果としてファイルの逼迫も解けた形で当たっている。

**次に `ui/page` 直下・`ui/page/runners` へ足す Issue へ。** 残りは上記「行数の予算」の行数表を見よ。**警告帯は出たが潤沢ではない**ので、行数表の残りを超える追加が要るなら足す前に空けること。`ui/page` の候補は依然として (1) の切り出し（`StateMsg` の改名の波及と `Overlay` の戻りの辺は上記「`ui/page` を警告帯へ入れた判断」の 1. が評価済み）、`page/runners` の候補は上記の `setupreq` である。

##### `internal/buildconfig` を警告帯へ入れた判断（PR #153 の 2 周目レビュー）

着手時点では 1969 行（残り 31 行）だった。2 周目レビューが 3 観点とも**変異注入で実測して**出した指摘——`UIApp[ui] --> Logs` のように矢印の左辺でノードを宣言すると辺が黙って捨てられる、`--x` / `---` / `-.-` / `===` / `~~~` を取りこぼす、`%%` コメント行で誤検知する、同じ段落が 2 つあると古いほうが素通りする、`subgraph UI` にノードを 1 つ移すだけで除外範囲が黙って広がる——を塞ぐと当時 **2025 行（残り -25 行・warn）**になった。**切り出さずに警告帯へ入る判断を採った。**

**最終レビューの 1 巡目で +16 行して、当時 2041 行（残り -41 行・warn）になった。** 塞いだのは 3 つで、いずれも**変異注入で実測した**「黙って緑になる」経路である——mermaid フェンスに重複ガードが無く「## 依存関係」節に 2 つ目のグラフを置くと**読者が見るほうが無検査**になる、散文が唯一の例外として名指しする `UIApp --> UIParts` が除外条件に飲まれて**図から消しても緑**、`mermaidArrow` がテキスト付き点線リンク `-. text .->` を取りこぼして**どの case にも入らず黙って無視**。下記 1.（正しさを行数より優先する）と同じ理由で、行数を根拠に省かなかった。

**最終レビューの 2 巡目でさらに +81 行して、当時 2122 行（残り -122 行・warn）になった。** 塞いだのは 3 つで、いずれも**変異注入で実測した**「黙って緑になる」経路である——(a) 1 巡目で入れたフェンスの重複ガードが CommonMark の許す形（3 スペースまでのインデント、4 個以上のバッククォート）を数え落とし、**GitHub は描画するのに検査は 1 つ目しか見ない**状態が残っていた（とりわけ 4 個以上のバッククォートは**フェンスを逐語引用する**標準的な書き方で、本書の「改訂履歴に本文を逐語引用する」慣行で最も起きやすい形である）。(b) 例外として名指しした 1 本 `UIApp --> UIParts` の定数に生存確認が無く、図と `graphNodeRules` の両方でノードを改名すると**1 巡目で塞いだ経路が改名 1 手で復活した**。(c) 散文が「例外は 1 本だけ」と現在形で宣言しているのに、UI 内部の 2 本目を図に足しても通った。**あわせてフェンスを数える範囲を「## 依存関係」節の中に限った**（後方の無関係な節に別種の図を足すと「1 つに統合すること」という誤った助言で落ちていた）。**+81 行のうち約 40 行はこの 3 つのガードと分割に伴う doc コメントで、残りは分割で増えた package 宣言と import である。**

1. **正しさを行数より優先した。** 塞いだのはいずれも「**描いてあるのに検査されない**」「**食い違っているのに緑になる**」経路であり、この検査群の存在意義そのものである。行数を理由に 1 件でも省くと、検査が在ることを根拠に人手の棚卸しをやめた本書の宣言が裏付けを失う。
2. **(1) は本番コードが無いため当たらない——ただし当時の判断であり、後に (4) が当たった。** `internal/buildconfig` は [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)が明記するとおり**実行時のコードを持たないテスト専用パッケージ**（`doc.go` のほかはすべて `_test.go`）で、「本番の一部をパッケージ境界で切り出す」対象が存在しない。**ここまでが PR #153 の 2 周目レビュー時点の判断である**——当時の本節冒頭は 3 つの手しか挙げておらず（改訂 1.48）、本番コードを持たない以上どれも当たらないという行き止まりだった。**その後 Issue #161 が (4) テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける手で解いた**（下記「`internal/buildconfig` を 2 つに分けた判断」。この手はそのとき第 4 の手として本節冒頭へ足したものである）。
3. **(2) テストの重複削減は実施済みである。** 見出しの重複ガード（`dependencyMermaid`）と段落の重複ガード（`collapsedNodeProse`）で同じ「行頭で数えて 1 個でなければ落とす」処理を 2 度書いていたのを `cutAtLineStart` 1 本へ寄せ、**+77 行を +56 行まで削った**。これ以上はコメントを削ることになるが、本リポジトリは**なぜ危険かを書き残す**ことを作法としているので削らない。
4. **当時、ERROR 境界（2200 行）まで 78 行あった。** `make check` は通っていた（**PR #153 の 2 周目レビュー時点、2122 行の値である**。現在は Issue #161 が 3 つのディレクトリへ分けており、`internal/buildconfig` は警告帯の外にある。下記「`internal/buildconfig` を 2 つに分けた判断」と上記「行数の予算」の行数表が現在の値を持つ）。
5. **1 ファイル 300 行は全ファイルが守っている**（**以下の括弧内は PR #153 の 2 周目レビュー時点では次の値だった**——ディレクトリ全体の最大は `golangci_test.go` **247 行（残り 53）**、本 PR が新設した 4 ファイルの最大は `docs_deppkg_test.go` **196 行（残り 104）**で、いずれも警告帯 301〜330 行の外。`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行。**現在は 4 ファイルとも `internal/buildconfig/docscheck` にあり、`golangci_test.go` だけがビルド設定側に残っている。ファイル単位の実測は `go tool linterly check` の出力を見よ**）。**最終レビューの 2 巡目で `docs_depgraph_test.go` を責務の境界で 2 つに分けた**——296 行で残りが 4 行しかなく、例外定数の生存確認と「例外は 1 本だけ」の表明を足すと確実に超えたためである。境界は「ファイルの行数」の方針どおり行数だけでなく責務でも引いてある——`docs_mermaid_test.go` は「図をどう読むか」、`docs_deppkg_test.go` は「実装の側を `go list` でどう読むか」、`docs_depgraph_test.go` は「読んだ両者をどう突き合わせるか」を持つ。**当時ここに置いた「次にここへ足す Issue へ」の指示——新設分のファイル側には 104 行の余裕があるが、ディレクトリ側は既に残りマイナスである——は Issue #161 が解消した。** ディレクトリの逼迫はもう無いので、次に足す Issue が見るべき予算は下記「`internal/buildconfig` を 2 つに分けた判断」と上記「行数の予算」の行数表が持つ（足す前に残りを確認すること）。

**次に `internal/buildconfig` へテストを足す Issue へ。**（**以下は当時の指示であり、Issue #161 が実施して解消した**。現在の予算は下記「2 つに分けた判断」が持つ）残りはマイナスである。**空けること自体を独立した Issue に立てること**（7 周目・8 周目が使った「判断を残して警告帯へ入る」逃げ道を 3 度続けて使わない）。候補は**検査そのものをパッケージ境界で別ディレクトリへ分ける**手で、ビルド設定の検査（`Makefile` / CI / `.golangci.yml` / `lefthook.yml` / `.linterly.yml` / depguard / nolint 棚卸し）とドキュメントの検査（`docs_*_test.go`）は**増え方が違い、互いに依存も無い**。ただし `repoRoot` などの共通ヘルパの置き場所を先に決める必要がある。**この指示は Issue #161 として立て、実施した**（挙げた候補をそのまま採っている。下記「`internal/buildconfig` を 2 つに分けた判断」）。**ただしこの候補が「ビルド設定の検査」側に並べた `nolint 棚卸し` は、実際にはドキュメントの検査である**——落ちたときに直すのは `docs/environment/setup.md` の表なので、`docscheck` へ置いた。

##### `internal/buildconfig` を 2 つに分けた判断（Issue #161）

着手時点では 2122 行（残り -122 行・warn）だった。上限値は緩めず、4 つの手のうち **(4) テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける**を採り、ビルド設定の検査（`internal/buildconfig` **当時 1216 行・残り 784 行**）とドキュメントの検査（`internal/buildconfig/docscheck` **当時 936 行・残り 1064 行**）に分けた。上記「警告帯へ入れた判断」が名指しした候補そのものである。**(4) は本 Issue が本節冒頭へ第 4 の手として足したものである**——PR #153 当時の 3 つの手にはこれが無く、同節の 2.「(1) は本番コードが無いため当たらない」で行き止まりになっていた。**検査は 1 件も削っていない**——分割の前後で同じテストがすべて走る（同節 1.「正しさを行数より優先した」を覆さないため、削る手は初めから検討していない）。

**境界は候補が挙げたとおりに引けた。** 実測で裏付けが取れたのは 2 点である。

1. **増え方が違う。** ビルド設定側は `Makefile` / CI / `.golangci.yml` / `lefthook.yml` / `.linterly.yml` / depguard を触るたびに増え、ドキュメント側は文書の取り決めを増やすたびに増える。**nolint の棚卸しはドキュメント側である**——落ちたときに直すのは setup.md の表だからである（下記「同じ文書を見る検査は同じディレクトリに寄せた」）。
2. **互いに依存が無い。** 2 つのファイル群が共有していた識別子は **`repoRoot` 1 つだけ**だった。図の検査が持つ型（`graphEdge` / `depGraph` / `graphNodeRule`）や `soleLine` / `cutAtLineStart` はドキュメント側で閉じており、ビルド設定側に残った **9 つの `_test.go`** からは 1 つも参照されていない。逆向きも `writeFiles` / `newModule` / `runMake` / `goEnv` がビルド設定側で閉じている。

**共通ヘルパは 3 つ目のディレクトリへ出した（本節冒頭の (3)）。** `_test.go` の中のヘルパはパッケージをまたげないので、選べるのは (a) `repoRoot` を両方に写す、(b) 通常のパッケージへ出す、の 2 つである。**(a) は採らなかった**——本書が繰り返し戒めている「写しの片方だけが古くなる」形そのもので、当時 19 行だった `repoRoot` のために作る写しではない。(b) の先例は `page/pagetest` / `organism/table/tabletest` / `setup/setuptest` で、いずれも「`_test.go` に置けない事情で通常のパッケージになった道具」である。**代償も先例と同じ——本番からも import できてしまう**ので、`page/pagetest/import_test.go` の `fixtures` へ `internal/buildconfig/buildconfigtest` を登録し、`TestNoProductionCodeImportsTestFixtures` の網に入れた（「足すときはここへ登録すること」と同ファイルが指示している。登録しないと検査の網から外れる）。

**分けても合計は増える。** 分けた当時は 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・**新設した 2 つの doc コメント**（`docscheck/doc.go` と `buildconfigtest/buildconfigtest.go` の doc 部分。`internal/buildconfig/doc.go` は分割前からあるもので行数が動いておらず、+74 には効いていない）と、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。**現在の 4 つの行数は上記「行数の予算」の行数表を見よ**（Issue #164 が行数表を実測と突き合わせる検査と、散文から実測値を無くす検査を足したぶん、`docscheck` が伸びており、合計は分けた当時より増えたままである）。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である（その後 Issue #171 が `docscheck` をさらに分けたので現在は 4 つである）——4 つとも警告帯の外にある。

**同じ文書を見る検査は同じディレクトリに寄せた。** `docs/environment/setup.md` の nolint 棚卸しをツリーと突き合わせる `TestSetupDocNolintInventoryMatchesTree` は、対象が設定ファイルではなくドキュメントなので `docscheck` 側である（PR #163 のレビュー指摘で `nolint_inventory_test.go` → `docscheck/docs_nolint_test.go` へ移した。同じ setup.md を読む `TestSetupDocEmbedsConfigFilesVerbatim` が `docs_sync_test.go` にあり、**同一文書の検査が 2 パッケージへ割れていた**）。**分ける境界は「検査に使う道具」ではなく「検査の対象」である**——nolint 棚卸しは `.golangci.yml` の `nolintlint` 設定と関係が深いが、落ちたときに直すのは setup.md の表であって設定ファイルではない。

**1 ファイル 300 行の側はほとんど動いていない。** 分割の前後で最大は変わらず、ビルド設定側が `golangci_test.go`、ドキュメント側が `docs_deppkg_test.go` で、どちらも数行の増加にとどまり警告帯 301〜330 行の外である（その後 Issue #164 が足した検査がドキュメント側の最大を一度は更新したが、その 2 ファイルは Issue #171 で `docscheck/linebudget` へ移っており、ここで言う `docscheck` の最大はもうそれではない。**ファイル単位の現在の実測は `go tool linterly check` の出力を見よ**）。移した `docs_nolint_test.go` は分割当時 **183 行**で、どちらの側でも首位にならなかった。

**次に `internal/buildconfig` へテストを足す Issue へ。**（**振り分けは Issue #171 が `docscheck` をさらに分けたので更新してある**。最新の振り分けは下記「`docscheck` から行数の予算の検査を分けた判断」が持つ）残りは上記「行数の予算」の行数表を見よ。**行数表の残りを超える追加が要るなら足す前に空けること。** **足す先は対象で選ぶこと**——設定ファイルの不変条件は `buildconfig`、行数の予算の不変条件は `docscheck/linebudget`、それ以外のドキュメントの不変条件は `docscheck` である。`buildconfigtest` へ置いてよいのは**複数が使う道具だけ**で、1 つしか使わないものをここへ寄せると分けた意味（増え方の違うものを別々に育てる）が消え、もう 1 つの逼迫する置き場になる。

##### `docscheck` から行数の予算の検査を分けた判断（Issue #171）

着手時点では 2158 行（残り -158 行・warn）だった。上限値は緩めず、4 つの手のうち **(4) テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける**を採り、行数の予算の検査を `internal/buildconfig/docscheck/linebudget` へ出した（**当時 728 行・残り 1272 行**）。残った `docscheck`（**当時 1441 行・残り 559 行**）が持つのは依存グラフ・`//nolint` 棚卸し・改訂履歴・設定ファイルの埋め込み・不変条件テスト一覧表の 5 種類である。
...
**分けたのが行数の予算の側なのは、そこが最も速く伸びているからである。** Issue #164 / #167 / #169 が続けて検査を足しており、この 4 ファイルだけで分ける前の 3 分の 1 を占めていた。依存グラフの 4 ファイルも同じくらいの大きさで自己完結しているが、そちらは Issue #151 以降ほとんど動いていない。**空きは伸びる側へ渡すほうが利く**——どちらを出しても残る側の残りはほぼ同じだが、出した側に渡る 1272 行を使うのは行数の予算の検査だからである。

**分けても合計は増える。** 着手時点の 2158 行が 1441 + 728 = **2169 行**（+11）になった。増えたのは新設した `linebudget/doc.go` の doc コメントと package 宣言・import で、`buildconfigtest` へ出したヘルパもあちらで doc コメントを得たぶん数行増えている。**本節の数はすべて分けた当時（コミット `4898973`）の実測であり、その後の増減は反映しない**——PR #176 のレビュー修正が doc コメントへ足したぶんのような後日の変化は改訂履歴が持ち、**現在の行数は上記「行数の予算」の行数表を見よ**。本節は `budgetProseExcluded` にあって検査が見ないので、**当時の値として凍結し続けることは規約でしか守れない**（上記「行数の実測値は表だけが持つ」の「過去の値は実測へ寄せない」）。
...
**警告帯へ入れたのは 2 つの Issue で、入れたディレクトリが違う。** 同じ PR に載る 3 件のコミット単位の実測は次のとおりである。

| 時点 | `ui/page/config` | `ui/page/pagetest` |
|------|-----------------|-------------------|
| 分岐元 | 1980（pass） | 1936（pass） |
| Issue #140 | 1989（pass。**+9**） | 2121（**warn**。+185） |
| Issue #141 | 2055（**warn**。+66） | 2121（変化なし） |
| Issue #142 | 2098（warn。+43） | 2121（変化なし） |

つまり `ui/page/pagetest` を警告帯へ入れたのは Issue #140、`ui/page/config` を入れたのは Issue #141 である。**Issue #140 は `ui/page/config` をほとんど増やしていない**（+9 行）——回帰テストと道具を一度は置いたものの、下記のとおり `page/pagetest` へ出したためである。

**本節冒頭の (3) は実際に採った。** 束を辿る道具は初め `ui/page/config` の `helper_test.go` に `findMsg` として置いたが、引数が `tea.Cmd` と述語だけで config 固有の型に依存しないため、**回帰テストごと `page/pagetest` へ出して `FindMsg` にした**（`doneMsg` に依存するのは述語を渡す `doneOf` の側だけである）。これで `ui/page/config` は 2119 行から 1989 行へ下がり、Issue #140 の時点では警告帯に入らずに済んだ（その後 Issue #141 / #142 が見出しの回帰テストで +109 行して 2098 行・残り -98 行になり、Issue #147 が `page/config/itemview` を切り出して当時 1990 行・残り 10 行へ戻した）。**受け皿の `page/pagetest` は代わりに 2121 行へ上がり、こちらが警告帯の深いほうになった**——合計は減っていないので、2 つとも警告帯を出るには別ディレクトリの通常パッケージを新設するしかなかった。**Issue #147 がそれを実施した**（`page/pagetest/cmdtest` と `page/config/itemview` の新設。2 つとも警告帯を出ている）。

それでも最終的に 2 つとも警告帯に残したのは、次の 2 つによる。
...
着手時点で警告帯にいたのは 3 つである。`ui/page/pagetest` 2197 行（残り -197。**ERROR 境界 2200 まで 3 行**）、`ui/page/config` 2098 行（残り -98）、`ui/page/logs` 2025 行（残り -19）。**`logs` を入れたのは同じ PR の Issue #145 である**（`Expand` の締め切りの追従と回帰テスト）。上限値は緩めず、4 つの手のうち **(1) を 2 回・(3) を 1 回**打って 3 つとも出した。

1. **`Cmd` を走らせて束を辿る道具を `page/pagetest/cmdtest` へ出した**（手 (3)）。`run.go` / `cmd.go` / `advance.go` とその test を移し、`page/pagetest` は当時 **2197 → 1564 行（残り 436 行）**、`cmdtest` は 687 行になった。境界は責務に沿わせてある——`page/pagetest` 本体が持つのは「タブと親 Model を組み立てて動かす」道具（共有状態・フィクスチャ・`Spy`）、`cmdtest` が持つのは「発行された Cmd をどう走らせ、どう辿るか」だけである。依存は `cmdtest` → `page` の一方向で、`pagetest` からは import するが逆は無い。
2. **Config タブの一覧の行を `page/config/itemview` へ出した**（手 (1)）。`ui/page/config` は当時 **2098 → 1990 行（残り 10 行）**。**Issue #140 の節が挙げていた候補（`selfconf.go`）は採っていない**——5 つとも `Model` のメソッドで、出せば親の非公開を export することになる（`ui/organism/table` を分割しない判断と同じ理由）。採った `items.go` は `edit.Summary` から表示用の値を作る純粋関数であり、`page/runners/rowview` / `page/disk/cleanview` と同じ形である。**これまで間接的にしか駆動されていなかった**ので、移送のついでに専用テストを足した。
3. **Logs タブのファイル一覧の行を `page/logs/filerow` へ出した**（手 (1)）。`ui/page/logs` は当時 **2025 → 1927 行（残り 73 行）**。`ui/page/logs` の節が指示していた「まず道具を `page/pagetest` へ出す」は**採れる状態に戻った**（手 1 で受け皿が空いた）が、残っている道具は Logs 固有（`track` / `_diag` を持つ共有状態）で、出すと他のタブが使わない道具が共有の置き場に溜まる——同節が自ら但し書きしていたとおりである。そこで (1) を採った。こちらも専用テストを足した。
...
**テストも境界に沿って分けた。** 到着順に依らず確定する（進捗の出し切りと終了通知の両方がそろうまで結果を確定しない）ことは `Job` を直に駆動する `page/diskclean` のテストが見る。タブ側に残したのは後始末——報告を状態行へ出し、選択を解き、実行中の状態を捨てること——の検証である。結果は当時 2128 行から 1943 行（残り 57 行）になった。

**その後 Issue #127 の回帰テストで当時 1996 行（残り 4 行）まで詰まった**（Issue #147 の `cmdtest` への追従でさらに 1 行増えた。**現在の値は上記「行数の予算」の行数表を見よ**）**。** 押し上げたのは inode だけが逼迫した行で `⚠ 警告閾値超過` が出ることを固定する検証で、doctor との判定の食い違いを閉じるためのものである（[画面仕様](screens.md#disk-タブ)）。**次にこのディレクトリへ 1 行でも足す Issue は、足す前に空けること。** 採れる手は本節冒頭の (3)——タブ側のテストの道具を `page/pagetest` へ出すこと——である。**受け皿の空きも潤沢ではない**（Issue #140 で当時 2121 行の警告帯に入っていたが、Issue #147 が当時 1564 行まで戻し、その後の追従で戻ってきている。**現在の値は上記「行数の予算」の行数表を見よ**）。移せるのは行数表が示す `ui/page/pagetest` の残りまでなので、それを超えるなら `page/pagetest` 側を先に空けること。まだ余裕のある `page/disk/cleanview`（残りは同じ行数表を見よ）へ寄せられる純粋関数が無いかを先に見る手も引き続き採れる。

**`ui/page/logs` は Issue #147 で当時 1927 行（残り 73 行）になった。**（以下は Issue #145 で 2025 行の警告帯へ入るまでの記録である。）**当時の残りは 6 行しかなかった。** Logs タブ（Issue #9）は本文の組み立てと購読の 2 つを 1 つのタブに持つため、`page/<tab>` のなかで最も大きい。**次にこのディレクトリへ足す Issue は、まず道具を `page/pagetest` へ出すこと。** Issue #9 の 2 周目で回帰テストを 2 本足したときもそうして 120 行あまりを空けた（`Cmd` を回す道具 `RunCmd` / `ChromeOf` / `Pump` / `Drained` と、`_diag` のフィクスチャ `DiagRunner` / `WriteDiagLog`。`page/pagetest` が当時 824 行から 1003 行へ増えたのはこの移動ぶんである）。**ただしその受け皿は当時もう空いていなかった**——`page/pagetest` は Issue #140 で警告帯に入り 2121 行・残り -121 行だったので、何行の移動であってもそのままでは入らず、`page/pagetest` 側を先に空けるか別の受け皿を選ぶ必要があった。**Issue #147 が受け皿ごと空けた**（`page/pagetest` は当時 1564 行・残り 436 行）が、その後の追従で戻ってきている（**現在の値は上記「行数の予算」の行数表を見よ**）。次にこの手を採る Issue が移せるのは行数表が示す残りまでで、それを超えるなら `page/pagetest` 側を先に空けること。ただし `ui/page/logs` 自身は本節冒頭の (1) で空けた（`page/logs/filerow`）。
...
Setup タブは追加・削除・バージョン更新の 3 操作と、フォーム・確認・進捗・結果の 4 つのモーダルを持つ（進捗と結果報告は Issue #75 で `page/progressmodal` へ切り出し済みで、当時このディレクトリに残っていたのはフォームと確認の 2 種だった）。**当時は 2081 行で警告帯（2000〜2200）に入っており、上限 2000 を 81 行超過していた**（その 2 種も Issue #105 で `page/setupmodal` へ出して警告帯から出た。**現在の値は上記「行数の予算」の行数表を見よ**。下記「4 周目」）。
...
**この見積りは実績で確かめられた。** 最後の一覧タブである Doctor（Issue #11）が消費したのは `molecule/listrow` で 142 行（`doctor_row` 本文 50 行 + 検査 92 行）、`ui/token` で 34 行（列定義と落とし方）であり、見積り（1 枚ぶん最大 300 行程度）に収まった。**現在の `ui/molecule/listrow` と `ui/token` の残りは上記「行数の予算」の行数表を見よ。**
...
**Disk / Logs の 2 枚ぶんは既に消費済みである。** 残りは 1392 行から `disk_row`（122 + 170 = 292 行）と `log_row`（62 + 79 = 141 行）を引いて 959 行になった——**この 959 行は Issue #35 当時の値であって、現在の余裕ではない**（現在の `ui/molecule/listrow` の残りは上記「行数の予算」の行数表を見よ）。**足す必要のある一覧タブも 2 枚減り、見出しどおり Doctor の 1 枚だけになった**ので、余裕の判定は変わらない。むしろ 2 枚ぶんの実測値（292 行 / 141 行）が得られたことで、見積り 272 行が概ね妥当であること（Disk のように選択不可の理由まで持つ行は上振れすること）が確かめられた。
...
**この Issue も本節の指示どおり、足す前に道具を出した。** 出したのは 3 つである。(1) Cmd の束から `ChromeMsg` を拾う走査を `pagetest.ChromeMsgs` へ（`applyChrome` は 5 行になった）。**`ChromeMsgs` は現存しない**——`ApplyChrome` が入れ子の束まで辿る形になって呼び出し側が無くなり、Issue #145 で削除した（`ApplyChrome` が呼ぶのは Issue #150 以降 `cmdtest.MustMsgs` である）。(2) 診断項目の差し替えを `pagetest.StubCheck` へ。(3) `sampleRunner` の写しを捨てて `pagetest.SampleRunner` を呼ぶだけにした。さらに**発行そのものを `internal/ui/hostreq` へ切り出した**（`hostreq.Start`。親の非公開な状態に触れないため出せる）。番号キーの検索も `tabset.KeyOf` へ寄せた（タブの番号を知るのは `tabset` だけ、という分担そのものである）。それでも 2191 行で、エラー境界の 2200 まで **9 行**しかなかった。
...
**5 周目の空け方（Issue #77）。** Issue #8 が `route_test.go` に足した通しの検証で当時 2199 行（残り -199 行）まで伸び、警告帯に入っていた。上限値は緩めず、本節が命じてきた 2 つの手を順に実行して当時 1988 行（残り 12 行）へ戻した。**本番コードに入れた手は 1 つだけである**——`Init` の `tea.Batch` を 1 行に畳んで `app.go` を 302 行から 299 行にした（**1 ディレクトリではなく 1 ファイル 300 行の警告**を解くためで、ディレクトリの 12 行はこれが無くても収まる）。
...
**逼迫しているのはディレクトリだけではない。** 本 PR で `internal/ui/app_test.go` は **当時 300 行（1 ファイル 300 行の上限に張り付き・残り 0）**になり、`internal/ui/page/` も **当時 1998 行（残り 2 行）**になった。**前者のその後はこうである**——9 周目（Issue #139）が周期の状態を `discovery.State` へ移した際に `app_test.go` は当時 274 行まで下がり、`ui` 直下で最大のファイルは `app.go` に替わった（上記「ファイルの行数」。**現在のファイル単位の実測は `go tool linterly check` の出力を見よ**）。**後者の 1998 行・残り 2 行も当時の値である**——Issue #155 が `page.StateMsg` にフィールド 1 つと doc を足し、`ui/page` は当時 **2003 行・残り -3 行・warn** になった（上記「`ui/page` を警告帯へ入れた判断（Issue #155）」）。**残りがマイナスの状態も当時のものである**——切り出した Issue #159 が `ui/page` 直下のテストの重複を削り、現在は pass に戻っている（上記「Issue #159 の空け方」。**値は上記「行数の予算」の行数表を見よ**）。**残りは同じ行数表を見よ。`page` 直下へ共通の `Msg` を 1 つ足すなら、行数表の残りを超える追加が要らないことを足す前に確かめること。**タブをまたぐ移動や本例外と同じく「双方から見える場所がここしか無い」ことを確かめるのは、空けたうえでの話である。

**当時の結論は「残り 0 行は文字どおりゼロである」だった。** 上の (1)〜(3)（本番の切り出し・テストの重複削減・道具を `page/pagetest` へ）はいずれも使い切っており、次に `ui` 直下へ 1 行でも足す Issue は、まず親 Model から切り出せるまとまりを見つけるところから始めること、という指示である。あわせて**警告帯（2000〜2200 行）へ入ること自体は CI を落とさないので、切り出しが今回のスコープに混ざるくらいなら、判断を本節へ残したうえで警告帯へ入る方を採ってよい**（`ui/page/runners` / `ui/page/disk` と同じ判断である）とも記した。**この逃げ道を実際に使ったのが 7 周目であり、8 周目も同じ手を採った**——そして 9 周目がその借りを返した。**現在の予算と次の Issue への指示は下記「9 周目の空け方」が持つ。**

**残りは Issue #9 で 125 行から 27 行へ減り、Issue #8 でついに超過した。** タブをまたぐ移動（`page.OpenTabMsg`）は親でしか実現できず、`keys.go` の `openTab` とその検証（`route_test.go` の 3 本）が加わったためである。検証に使う道具のうち App の非公開な状態に触れないもの（受け取った `Msg` を型で数える `Delivered`）は `page/pagetest` へ出してある。**当時（Issue #8 の 2199 行）の結論は「次に `ui` 直下へ足す Issue は、まず既存のテストで `page/pagetest` へ出せるものを探すこと。超過した以上、テストを足す前に道具を出すこと」だった。** これは当時の記録であり、**現在の予算と次の Issue への指示は下記「9 周目の空け方」が持つ**（その手は 6 周目までに使い切っている）。

**この周（Issue #31）は、余裕を「重複削減」ではなく「道具を `page/pagetest` へ出す」で作った。** Issue #31 でキーの配送を検証する道具を足したとき `ui` 直下は当時 1967 行（残り 33 行）まで詰まったが、走査の道具（`ScanKey`）とその形の網羅テストを `page/pagetest` へ移して当時 1875 行（残り 125 行）に戻した。`page/pagetest` の**現在の値は上記「行数の予算」の行数表を見よ**（Issue #140 で当時 2121 行の警告帯へ入り、Issue #147 が `page/pagetest/cmdtest` を切り出して当時 1564 行へ戻し、Issue #150 以降の追従で増えた）。**そこは元々「タブと親で共用する検証の道具」の置き場である**（`helper_test.go` 冒頭の方針）が、次に道具を出すときは `page/pagetest` 側も同時に空けなければ入らない。

出せるものと出せないものの境目は「`App` の非公開な状態に触れるか」である。`ScanKey` は `page.ChromeMsg` / `page.GlobalKeyMsg` と `cmdtest.MustMsgs`（Issue #147 までは `pagetest.Msgs`、Issue #150 までは `cmdtest.Msgs`）しか使わないので `App` の非公開な状態を 1 つも export せずに出せた。一方 `gate_test.go`（`a.chrome` / `next.active` / `a.disc`。当時は `after.inflight` と非公開の `tickMsg` を直に見ていた。9 周目で前者は `disc.Busy()` / `disc.Seq()` に、後者は `discovery.TickMsg` になった）と `app_keys_test.go`（`a.tabs` / `a.active` / `a.chrome`）の**テスト本体**は非公開に触れる内部テストなので出せない。**これらを出そうとして export を増やすのは採らない**（`ui/organism/table` の本体を分割しない判断と、非公開の export を避ける点で理由を共有する）。**ただしテストが使う道具は別である**——Issue #77 は `press1` / `isQuit` / `blocked` を `page/pagetest` へ出した（どれも `tea.Model` の口と `page` の Msg しか使わない）。**この「まず道具を `page/pagetest` へ出せないかを見る」という指示も Issue #77 当時のものである**——6 周目までに出せる道具は尽きており、次の Issue への指示は下記「9 周目の空け方」に一本化してある。

##### 7 周目の判断（Issue #132）——切り出さずに警告帯へ入った

`scan_roots` の追従（上記「`scan_roots` はこの経路に載せ直した」）は `ui` 直下へ本番 19 行（`discover.go` の `scanRoots()` とその doc が +17、`app.go` の `Options.Roots` の doc が +2）と回帰テスト 44 行（`discover_test.go`）を持ち込み、`ui` 直下は当時 2000 行から **2063 行（残り -63 行・WARN）** になった。

**先に分割の是非を検討したうえで、切り出さずに警告帯へ入る方を採った。** 6 周目が残した逃げ道（「切り出しが今回のスコープに混ざるくらいなら、判断を本節へ残したうえで警告帯へ入ってよい」）をそのまま使っている。根拠は 3 つある。

1. **切り出せるまとまりが見つからなかった。** 4 つの手のうち (2)（テストの重複削減）と (3)（道具を `page/pagetest` へ）は 6 周目までに使い切っている。**(4)（検査そのものをパッケージ境界で別ディレクトリへ分ける）は当たらない**——`ui` 直下のテストは同じディレクトリの本番コード（`App` の非公開な状態）を見る内部テストなので、パッケージ境界で切り離せない（下記「残るのは非公開に触れる内部テストだけである」）。残る (1)（本番の切り出し）は「依存の向きを強制できる、あるいは増え方が違うまとまり」があるときだけの手だが、`ui` 直下に残るのは親 Model そのもの（`app.go` / `keys.go` / `discover.go` / `background.go` / `chrome.go` / `hostreq.go`）であり、どれも `App` の非公開な状態を読み書きする。**`hostreq.go` は既に本体を `ui/hostreq` へ出した後の、当時 26 行のアダプタ**（`startHostReq` が `hostreq.StartOnce` を呼ぶだけ）で、これ以上出せるものが無いことの実例である。切り出せば `App` の内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。
2. **本 Issue はバグ修正であり、切り出しはスコープ外である。** 走査ルートが再起動まで反映されないという不具合の修正に親 Model の再構成を混ぜると、レビューでどちらの変更がどのテストを動かしたのかを切り分けられない（Issue #132 が #128 から切り出された理由と同じである）。
3. **CI は落ちなかった。** ERROR の境界は 2200 行で、**当時は 137 行の余裕があった**（`ui` が 2063 行だった時点の値である。8 周目が Issue #136 の回帰テスト 42 行を足して 95 行に減り、9 周目（Issue #139）が 2 つの状態を切り出して **247 行**まで戻した。**現在の値は上記「行数の予算」の表と下記「9 周目の空け方」が持つ**）。

**当時の結論は「警告帯に入った以上、次は『足すついでに空ける』ではなく空けること自体を独立した Issue として立てること」だった。** 切り出しの候補として挙げたのは、親 Model の背景処理（`background.go` の `startBackground` と `discover.go` の `tick` / `onTick` / `discover` / `scanRoots` / `refresh`）を `internal/ui/discovery` 側へ寄せる案である。これらは `a.inflight` / `a.seq` / `a.applied` を進めるため、状態ごと移さなければ意味が無い。**「増え方が違うまとまり」であることは言えるので、(1) の条件は満たしうる**——ただし移す状態の設計から始まるので、行数を空ける目的だけで片手間に着手しないこと。**この指示はここで完了している**——8 周目が独立した Issue #139（`ui` 直下の分割）として実際に立て、9 周目がそれを実施した。挙げた候補（背景処理を `discovery` 側へ状態ごと寄せる）はそのまま採られている。**現在の予算と次の Issue への指示は下記「9 周目の空け方」が持つ。**

##### 8 周目の判断（Issue #136）——警告帯のまま回帰テストを足した

**`ui`（警告帯）へ Issue #136 の回帰テスト 42 行を足した。分割はしない。** 足したのは `ui/chrome_test.go` の 1 テストで、状態行が枠（`template.Frame`）を通った後も幅に収まることを見るものである。**この検証は `ui` 直下にしか置けない**——`App` を組み立てて `View()` を描く経路そのものが対象であり、`ui/chrome` へ移すと `chrome.Status` の戻り値を見るだけになって、枠が中略記号なしで断ち切る問題（Issue #136 が実際に踏んだ形）を素通りする。4 つの手（本節冒頭）のうち (3) テスト用の道具の切り出しは、この 1 テストが `pagetest` の道具を使うだけで新しい道具を足していないため空きを生まない。`ui` 直下は当時 2063 行から **2105 行（残り -105 行・WARN）** になった。

**当時の予算は「ERROR 境界 2200 まで 95 行」だった**（7 周目の 137 行ではない）。7 周目が「空けること自体を独立した Issue として立てること」と定めた指示はこの周で果たしており、`ui` 直下の分割そのものは **Issue #139** として切り出した。**その Issue を実施したのが下記「9 周目の空け方」であり、現在の予算と次の Issue への指示はそちらが持つ。**

##### 9 周目の空け方（Issue #139）——警告帯から出た

着手時点では **2105 行（残り -105 行・WARN）** で、8 周目が残した予算は ERROR 境界（2200 行）まで 95 行だった。7 周目・8 周目が 2 周続けて使った逃げ道（判断を残して警告帯へ入る）を返す周であり、**行数を空けること自体が唯一のスコープである**。上限値は緩めず、4 つの手のうち **(1) 本番の一部をパッケージ境界で切り出す**を 2 回打って当時 **1953 行（残り 47 行・pass）** へ戻した。7 周目が挙げた候補（背景処理を `discovery` 側へ**状態ごと**寄せる）をそのまま採っている。

1. **検出周期の状態を `discovery.State` へ寄せた。** 親が持っていた `inflight` / `seq` / `applied` と検出の結果・エラーを 1 つの `discovery.State` にまとめ、`Result()` / `Err()` / `Busy()` / `Seq()` / `FirstTick()` / `Tick()` / `OnTick()` / `Start()` / `Apply()` を生やした。`(*State).Start` は実行中なら nil を返す（`workscan.State.Start` と同形）。親から `tickMsg` / `tick` / `firstTick` / `discover` / `refresh` が消え、`discover.go` に残ったのは 4 つである——`discovery.State` へ 1 周期を委ねる `onTick()`（1 行）、結果を取り込む `applyDiscovered()`（本体 23 行・doc コメント込みで 29 行。`Apply` の返す `Outcome` を見て `Stale` なら捨て、そうでなければ `distribute()` を呼び、`StartHostReq` なら `startBackground()` を束ねる。**この関数は現存しない**——10 周目（Issue #148）が `startup.State.StartAll` へ移した）、1 周期分の入力を組み立てる `input()`、走査ルートを合成する `scanRoots()` である。**周期の管理（実行中は重ねない・追い抜かれた周期を捨てる・次の Tick を予約する）は残らず `discovery.State` 側にあるが、検出まわりで親が保持する責務が消えたわけではない**——`Outcome` を受けて共有状態を配り背景処理を起こす束ね（`applyDiscovered`）は、`a.distribute()` と当時の `a.startBackground()`（現在は `a.bg.StartAll`）という親の非公開に触れるため親に残っている。**「実行中は重ねない」「古い周期を捨てる」は検出の側の不変条件なので、判断ごと移さなければ意味が無い**——状態だけを移して判断を親に残すと、片方だけを更新する経路ができる。
2. **起動時の前提チェックの状態を `hostreq.State` へ寄せた。** 親の `hostReq` / `hostReqDone` / `hostChecks` の 3 フィールドが `hr hostreq.State` の 1 つになり、自由関数だった `hostreq.StartOnce` は `(*State).StartOnce` へ畳んだ。**これで `disc` / `work` / `scopes` / `hr` の 4 つが同じ形にそろった**——従来 `hostreq` だけが「1 度きり」の真偽と件数を親に持たせる非対称だった。26 行のアダプタ `internal/ui/hostreq.go` は消え、`startHostReq()` は `background.go` の `startBackground()` へ、`doctorKey()` は `chrome.go` へ移した（7 周目が「これ以上出せるものが無いことの実例」として挙げていたファイルである）。

**`tickMsg` は公開型 `discovery.TickMsg` へ反転した。** 親の非公開型だったのは「外から自動更新を駆動できないようにする」ためだったが、周期の管理をサブパッケージへ移すにあたって公開する側を採った。理由は 2 つある。(a) **非対称だった**——同じく親が受け取る `hostreq.Msg` / `workscan.Msg` / `ghscope.Msg` はいずれも公開されており、契機の型だけを隠しても揃わない。(b) **実効性が限定的である**——`State.Start` は実行中なら nil を返すので、外から余計な `TickMsg` を撒いても起きるのは高々 1 周期ぶんの前倒しであり、多重起動を防ぐ判定（`Start` の doc の 40 プロセス）は破れない。**代替として import 検査を足す案（`page/pagetest/import_test.go` に `TestOnlyParentDrivesDiscovery` を足し、`discovery.TickMsg` を作る本番ファイルを親だけに限る）も検討したが採らなかった**——`page/pagetest` は当時 1936 行で残り 64 行しかなく、**`ui` 直下の行数を空ける Issue が別のディレクトリを逼迫させるのは本末転倒である**（その後 Issue #140 で警告帯へ入り、Issue #147 が当時 1564 行・残り 436 行まで戻した。**現在は警告帯の外で、検査 1 本ぶんの余地はまだある——値は上記「行数の予算」の行数表を見よ**）。**この判断は散文での記録に留めてある**（機械的な検査は無い）。次に `page/pagetest` へ余裕ができたときに足すのが筋である。

**`internal/ui/refresh_test.go`（80 行）は削除したが、表明は 1 つも落ちていない。** 内訳は 3 本である。

- `TestTickSkipsDiscoverWhileOneIsRunning` と `TestStaleDiscoverResultDoesNotOverwrite` は `discovery/state_test.go` へ移送した（`TestOnTickDoesNotOverlapRuns` / `TestApplyDropsStaleCycle`）。**移送のついでに強化してある**——`OnTick` が検出を始めなかった周期でも `TickMsg` を返すこと（自動更新がそこで止まらないこと）まで見るようにした。
- `TestRefreshKeySkipsDiscoverWhileOneIsRunning` は削除した。**`gate_test.go` の `TestRefreshDuringDiscoveryShowsNotice` が同一経路・同一表明の真上位集合だからである**——どちらも `r` の打鍵で通し番号が進まないことを見ており、包含側はそれに加えて案内の文言と、次の打鍵で案内が消えることまで見る。**包含側にはその旨の注記を足した**（消した表明がどこで守られているかを、消した側からではなく残った側から辿れるようにするため）。

あわせて `discovery/reconcile_test.go` を新設した。`reconcile.go` は 4 周目（Issue #63）で切り出して以来、親のテスト越しにしか駆動されていなかった（初の専用テストである）。

**9 周目が次の Issue へ残した予算は、当時 残り 47 行・ERROR 境界 2200 まで 247 行である。** 8 周目の 95 行ではない。**当時の指示は「次に `ui` 直下へ足す Issue は、この 47 行を予算として読むこと」だった**——当時の 47 行は回帰テスト 1 本ぶんであり、それを超えるなら足す前に空ける、という趣旨である。**採れる手はもう (1) しか残っていない**（(2) テストの重複削減と (3) 道具を `page/pagetest` へ出すは 6 周目までに使い切っており、受け皿の `page/pagetest` 自身も当時 残り 64 行しかなかった）。**そして (1) の候補も 7 周目が挙げた 2 つを本周で使い切った。** 残る `app.go` / `background.go` / `chrome.go` / `discover.go` / `keys.go` の 5 ファイル（**当時**の `ui` 直下の本番ファイルはこれで全部である。`background.go` は 10 周目が `ui/startup` へ出して消えた）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**当時はこれを「次に 47 行を超える追加が要る Issue は、まず切り出せるまとまりの設計から始めること」と結んでいた**——行数を空ける目的だけで片手間に着手しないこと、という 7 周目の但し書きも引き継いでいた。

**この予算は 10 周目（Issue #148）が更新した。現在の予算と次の Issue への指示は下記「10 周目の空け方」が持つ。** 本周が「(1) の候補を使い切った」としたのは**親 `App` の非公開な状態に触れるものに限った判断**であり、10 周目は**触れないまとまり**（起動後に 1 度だけ取りに行く 3 つの State の駆動）がまだ 1 つ残っていたことを示している。

**次の周への手順——突き合わせは行数だけでなくコード記号と列挙まで洗うこと。** 改訂 1.80 の全件突き合わせは**行数の主張だけ**を対象にしていたため、散文の側に 4 箇所の取り残しが出た（改訂 1.81 で是正）。**この 4 件は同一の欠陥ではなく、2 つの型に分かれる。**

- **(a) 削除済みの識別子を現在形で名指ししていた**（2 件）。例外 3 の 2 箇所で、本周が消した `tick` / `discover()` / `refresh()` を現在の担い手であるかのように書いていた。
- **(b) 実在するものの列挙が足りていなかった**（2 件）。本節の手 1 が `discover.go` に残った関数を「`input()` と `scanRoots()` だけ」としていたもの（名指しした 2 つはどちらも実在し、落ちていたのは `onTick()` と `applyDiscovered()` である）と、直上の段落が `ui` 直下の本番ファイルの列挙から `background.go` を落としていたものである。**型 (b) は「名指しした識別子を実在確認する」手順では検出できない**——書かれている名前はすべて実在しており、書かれていない名前のほうが問題だからである。

型 (a) には識別子の抽出を、型 (b) には列挙そのものの突き合わせを当てる。**本書へ手を入れる周は両方を通すこと。**

**型 (a)——関数名・型名・ファイル名を現在形で名指ししている散文を機械的に洗い出し、実在を確認する。** 消えていれば現在の経路名へ直すか、当時の値である旨を添えた過去形の記録へ改める。抽出には次を使う。

```sh
grep -oE '`[A-Za-z_][A-Za-z0-9_.]*(\(\))?`' docs/ui/atomic-design.md | tr -d '`' | sort -u
```

末尾の `()` を任意にし `.` を語に含めているのが要点で、`input()` のような関数名だけでなく、`()` の付かない識別子（`tick` / `tickMsg` / `firstTick` / `inflight`）・ファイル名（`hostreq.go`）・パッケージ名を接頭辞に持つ型やメソッド（`hostreq.StartOnce` / `discovery.State.Start`）も 1 本で拾える。**本書全体を対象にすると一意な識別子が大量に挙がるので全件は当てない**——**現在形の散文が名指ししているものに絞り**、それを `grep -rn` で本番コードに当てること。

**型 (b)——ファイルやシンボルを列挙している散文は、列挙そのものを実測と突き合わせる。** 「残ったのは … だけである」「… の 5 ファイル（これで全部である）」のように網羅を主張する文が対象で、**実測の側を先に採ってから文と突き合わせる**こと（文を読んでから確かめると、書かれていない名前に気づけない）。

```sh
ls internal/ui/*.go | grep -v _test      # ui 直下の本番ファイル（実測 4 個）
grep -n '^func' internal/ui/discover.go  # discover.go の関数（実測 4 個）
```

##### 10 周目の空け方（Issue #148）——`background.go` を `ui/startup` へ出した

着手時点では **1968 行（残り 32 行・pass）** だった（9 周目が残した 1953 行に、同じ PR の Issue #145 が `Expand` の締め切りの追従と回帰テストで 15 行足した後の値である）。**警告帯に入る前に空ける周**であり、9 周目と違って行数の逼迫が動機ではない——3 つの State が同じ性質を持つことが 9 周目の整列（`disc` / `hr` / `work` / `scopes`）で見えるようになったのが動機で、行数はその副産物である。

**手 (1) を 1 回打った。** `background.go` の `startBackground()` と、親が並べて持っていた `hr` / `work` / `scopes` の 3 フィールドを `internal/ui/startup` へ出し、親は `bg startup.State` の 1 つを持つ形にした。`ui` 直下は当時 **1925 行（残り 75 行・pass）**、新設した `ui/startup` は当時 176 行（本番 83 行 + テスト 93 行）だった。**現在の値は上記「行数の予算」の行数表を見よ。**
...
**次の Issue への予算は上記「行数の予算」の行数表の `ui` の行が持つ。** 9 周目が残した当時の 47 行ではない。**採れる手は依然 (1) だけである**（(2)(3) は 6 周目までに使い切っている。受け皿の `page/pagetest` は Issue #147 が当時 1564 行まで空けたので、道具を出す先としては使えるようになった。**その残りも同じ行数表を見よ**）。残る `app.go` / `chrome.go` / `discover.go` / `keys.go` の 4 ファイル（`ui` 直下の本番ファイルはこれで全部である）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**次に行数表の残りを超える追加が要る Issue は、足す前に空けること——まず切り出せるまとまりの設計から始めること。**
...
- `ui/page` 直下には置けない。`page` は 7 タブすべてが import する共通の土台であり、runner 固有の制御をそこへ混ぜると Disk / Logs / Doctor まで引きずる。**この理由は行数に依らない**——当時の `ui/page` は 2003 行・残り -3 行の警告帯だったが、Issue #159 が警告帯から出した現在も置き場所の判断は変わらない。`page/runnerdetail` を分けたのと同じ判断である。
...
`ui/page/runners` は 1187 → 2091 行になり、**警告帯に入った**。Setup タブ（Issue #8）が `n` / `D` / `u` の引き渡し（`keys.go` の `openSetup` / `setupBlocked`）を足したことで 2164 行（残り -164 行）まで伸びた（**当時の値である。現在の値は上記「行数の予算」の行数表を見よ**）。増分の大半はサービス制御の検証（発行コマンド列・確認の経路・一括操作・可否の再判定・詳細画面からの起点）で、Logs タブ（Issue #9）が足した `l` の経路もここに乗る。**分割せず警告帯に入ることを選んだのは `ui/page/disk` と同じ判断である**（上記）。サービス制御の配線と Logs タブのマージを 1 つの差分で行っており、そこへパッケージ移動を混ぜるとレビューで「どちらが壊したか」を切り分けられなくなる。エラー境界（2200 行）までは 36 行しかなく、`page/runners/rowview` を切り出して 2018 行へ戻した。**その後 Issue #107 が本節の指示どおりの削減を実施し、当時 2018 行から 1983 行（残り 17 行）まで空けた**（Issue #147 の `cmdtest` への追従で 6 行増え、Issue #150 の追従でさらに 2 行増え、Issue #155 が走査の既定を `pagetest.State` へ寄せた追従で 9 行増えて当時 2000 行・残り 0 行になり、Issue #159 がテストの重複を削って空けた。**現在の値は上記「行数の予算」の行数表を見よ**）**。** `runners/helper_test.go` が持っていた `findChrome`（束を辿る再帰）と `collect`（Msg の平坦化）はどちらも `pagetest.ChromeOf` / `pagetest.Msgs` の写しだったので捨てた。**そのとき `pagetest.ChromeOf` の側にも欠陥が見つかった**——「1 段だけ展開すればよい（ChromeMsg が入れ子の奥から出てくることは無い）」という前提が成り立たず、絞り込みを始める `/` では page 自身の `ChromeMsg` と部品の返した束がもう 1 段深い形になる。写しの側だけが正しく辿っていたので、共有の道具を再帰へ直した（見つかった時点で打ち切るので、点滅の `Cmd` を踏まないという性質は変えていない）。

**残りは上記「行数の予算」の行数表を見よ**。この一文が「残り 0 行——上限まで 1 行も無い」と書いていたのは Issue #155 当時の状態で、Issue #159 が空けた。**警告帯は出たが潤沢ではない**ので、行数表の残りを超える追加が要るなら足す前に空けること。道具の重複は Issue #107 の周で、テストの重複は Issue #159 の周で使い切っている（`ops_test.go` / `opsdrain_test.go` / `opsflow_test.go` はサービス制御の経路を 3 ファイルに分けて見ているが、Issue #159 が塞がれた操作の 2 本を表にまとめた後である）。**次の第一候補は `keys.go` の Setup タブへの引き渡しを `page/runners/setupreq` へ出すことである**（上記「Issue #159 の空け方」の採らなかった手）。
...
- **`ui/tabset`（切り出し当時 344 行。現在の値は上記「行数の予算」の行数表を見よ）** — タブのメタ情報と並び、およびそのテスト。タブを足す Issue が触るのはここであって親 Model ではない
- **`ui/chrome`（切り出し当時 283 行。現在の値は上記「行数の予算」の行数表を見よ）** — ヘッダ・タブ行・状態行・フッタの組み立て。親 Model の型も bubbletea もドメインの型も知らない純粋関数にしたので、`App` も `page` も組み立てずに表示用の値だけで検証できる。`ui` 側に残るのは `App` の値を `chrome.View` へ写す `chromeView` の 1 メソッドだけである
...
**(2) や (3) を採らなかった理由（以下の数はいずれも Issue #106 当時の値である）。** 必要なのは 34 行だが、`molecule` のテストは内部テスト（`package molecule`）であり、道具（`plainStyles` / `columnIDs` / `containsID`）は 28 行しかない。仮に出しても残りは 6 行で、次の 1 部品で再び超える。増え方の違いを残したまま数十行を削るより、**同じ軸でもう一度分けて 487 行の余裕を作る**方を採った。


#### `ui/organism/table` の本体を分割しない判断

**本体（`Model[T]` の実装）は分割しない。** 実体は `api.go` / `keys.go` / `rows.go` / `section.go` / `state.go` / `table.go` である（**構成の実測は `ls internal/ui/organism/table/*.go`、ファイル単位の行数は `go tool linterly check` の出力を見よ**）。中身は `Model[T]` という 1 つの型に対する区画・行・列・キー・状態の内訳であり、切り出せる単位はいずれも `Model[T]` の非公開な状態に触れる。サブパッケージへ出すには内部を export して `table` から切り出し先への参照を作ることになり、**「一覧の共通実装は 1 つ」（`Table` を増やさない）という規則を構造で守れなくなる**。分割の目的は行数上限の分散であって部品同士の依存を増やすことではない、という本書の方針とも衝突する。
...
Issue #31 で `table_test.go` の空振りしていたテスト（`View() != ""` しか見ていなかったもの）に検証を足したため 2061 → 2157 行（エラー境界まで 43 行）になり、警告帯に入っていた。**上限値は緩めず、テスト側のフィクスチャを `organism/table/tabletest` へ出して当時 1969 行（残り 31 行）へ戻した。**
...
**残りはテスト 1 本ぶんしかない（値は上記「行数の予算」の行数表を見よ）。次にこのディレクトリへ足す Issue は、まず `tabletest` へ出せるものを探すこと**（`tabletest` 側にはまだ十分な余裕がある。残りは同じ行数表を見よ）。出せないのは `fitcells_test.go` だけである——詰め（足りないセルを空文字で埋める）が `View()` からは観測できず、`fitCells` を白箱で見る内部テスト（`package table`）でしか固定できないためで、外へ出すには `fitCells` を export することになる。
...
| 1.6 | 2026-08-22 | `Detail.SetContent` を「受け取ったスライスは写しを取る」約束の対象に追加。**詳細は下記「改訂 1.6」。** | 下記「改訂 1.6」を見よ |
...
| 1.32 | 2026-08-23 | Logs タブ（Issue #9）のレビューで見つかった本書の誤りを 6 点直した。**詳細は下記「改訂 1.32」。** | 下記「改訂 1.32」を見よ |
...
| 1.49 | 2026-08-24 | 「`page/` は「1 ディレクトリ 1 タブ」ではない」の段落が 2 つ並んでいたのを 1 つに統合し、共有部品の列挙を `page/pagetest/import_test.go` の `shared` と一致させたうえで、その一致を守る検査 `TestSharedPackagesMatchDoc` を足した。あわせて PR #115 のレビュー指摘を反映。**詳細は下記「改訂 1.49」。** | 下記「改訂 1.49」を見よ |
| 1.50 | 2026-08-24 | 「ディレクトリの行数」に「UI 層の外のディレクトリ」を新設し、`internal/setup` / `internal/disk` / `internal/disk/pathguard` の実測値と、`internal/disk` から削除パスの検証を `pathguard` へ切り出した判断（Issue #101）を記録 | 行数チェックはリポジトリ全体を見るのに、空け方の判断は本書にしか無かった。UI 層の外で超過したディレクトリの判断を別の場所に書くと、同じ手の使い分けが 2 か所に分かれて片方だけが古くなる |
...
| 1.67 | 2026-08-24 | 設定の再読み込み（Issue #128）を反映。「タブを 1 つ追加するときに触る箇所」に 3 つ目の例外「親が配っている値をタブが書き換えたとき」を追加し、`page.ConfigSavedMsg` の対象/変更内容の表・成功時のみ発行する理由・`page.Do` で包まない理由・この経路では直らない設定値（`scan_roots` / `audit_log`）を明記。ディレクトリの行数表を当時の実測へ更新し（`ui` 2000 行 / `ui/page` 1995 行 / `ui/page/config` 1830 行。**この 3 つは 1.68 以降の実測で更新済みなので、予算は本項ではなく「行数の予算」の表を見ること**）、「6 周目の空け方」を新設して `App.current`（添字の範囲しか見ていなかった 6 行）を `tabset.Live` へ寄せて捨てた削減と、残り 0 行になった事実・次の Issue への指示を記録 | 設定ファイルには書けているのに親 Model の `cfg` が `New` の 1 度きりしか代入されず、`disk_thresholds` を編集しても Disk タブの警告と doctor のリソース診断が再起動まで古い閾値で判定していた。設定の真実を持つのは親なので、書き換えたタブが親へ返す以外に直す手が無く、**本書が「1 行も触らない」と宣言している `internal/ui` 直下へ `case` が 1 つ増える**。既存の 2 つの例外と同じ形で明文化しないと、次に同種の問題（親が配る値をタブが書き換える）を踏んだ Issue が「契約違反だから直せない」と読む。行数表は次の Issue が予算を引く唯一の材料であり、残り 0 行を記さないと `ui` 直下へテストを足す Issue が境界に当たってから気付くことになる（Issue #128） |
| 1.68 | 2026-08-24 | PR #133 のレビュー指摘を反映。**詳細は下記「改訂 1.68」。** | 下記「改訂 1.68」を見よ |
| 1.69 | 2026-08-24 | PR #133 の 2 周目レビュー指摘（すべて記述の不整合）を反映。(1) Config タブの節の散文 3 箇所が現在形で主張していた 1737 行・残り 263 行を実測（1832 行・残り 168 行）へ直した。(2) 「6 周目の空け方」の直後に残っていた Issue #8 / #77 当時の段落 2 つを当時の記録（過去形・当時の値である旨）へ改め、**次の Issue への現在形の指示を 6 周目の節に一本化**した。(3) 「行数の予算」の節の冒頭に **1 ファイル 300 行**（警告帯 301〜330 行）を併記。(4) 「状態の所有」表に gsr-helper 自身の設定（`appconfig.Config`）の行を追加し、親が持つ一方で Config タブが `confSet` で写しを持つ二重所有を明記。(5) `refresh_interval` が効くのは `--refresh` を指定していない場合に限ること（`discovery.Interval` はフラグが正なら無条件に優先する）と、`scan_depth` は `edit.SelfValues` に欄が無く `Apply` が素通しするためこの経路では変わらないことへ訂正。(6) 改訂 1.67 の行数を当時の値と明示し、`App.current` の「写し」という説明を 1.68 の訂正（添字の範囲しか見ていなかった）に合わせた。(7) 行数表と散文を実測へ更新（`ui/page` 1997 → 1998・残り 2） | 行数表だけを実測へ直して散文を取り残す欠陥が本書で繰り返し再発しており、Config タブでは**予算を 95 行過大に表示していた**（危険側の誤り）。同じ節に「まず `page/pagetest` へ道具を出せ」と「その手は使い切った」という**正面から矛盾する現在形の指示**が並ぶと、次の Issue がどちらを予算・方針とみなすかが読み手に委ねられる。`refresh_interval` は `--refresh` 起動中に編集しても効かないのに「効く」と読める記述で、同じ段落が `scan_roots` を「合成済みだから直らない」と切り分けている以上、非対称なまま残すと利用者向けの説明を誤らせる。1 ファイル 300 行が節の冒頭に無いと、上限を引きに来た読み手がファイル上限は無いと誤読する（PR #133 2 周目） |
| 1.70 | 2026-08-24 | PR #133 の 3 周目（`feat/#1` 合流後）のレビュー指摘を反映。(1) 合流の解消で巻き戻っていた `page/pagetest` の散文を実測へ戻した（「現在 1744 行・残り 256 行」→「現在 1937 行・残り 63 行」）。(2) 「ファイルの行数」節の「最大は `app.go` の 299 行」を実測へ改め、上限ちょうどで張り付いているのが `app_test.go`（300 行・残り 0）であることと次点（`runners_test.go` 297 行）を明記。(3) 1 ファイルの警告帯の記述を 301〜330 行へ統一した（節冒頭が 300〜330、詳細節が 301〜330 と 1 行ずれていた）。(4) `ui/page/disk` の散文の「現在 1943 行」を実測（1996 行・残り 4 行）へ。(5) 「残りが 1 桁のディレクトリ」の UI 側の列挙を実測の全件（`ui` 0 / `ui/page` 2 / `ui/page/disk` 4 / `ui/organism/dialog` 6 / `ui/page/logs` 6）へ。(6) 合流で採番し直した #128 の 3 行（1.67〜1.69）を指す本文中の相互参照と、改称前の節名「ディレクトリの行数」への参照を現行へそろえた | `feat/#1` に PR #134 が入って衝突し、その解消で **合流先が直したばかりの行数（`page/pagetest` 1744 → 1937）を巻き戻していた**。行数は本書が「次の Issue が予算を引く唯一の材料」と位置づけるものなので、193 行ぶん過大な予算を提示したまま合流すると、次の Issue が空けずに足して境界を越える。同じ「表だけを実測へ直して散文を取り残す」欠陥が 4 周続けて再発しているため、今回は**現在形で行数を主張する箇所を機械的に全件抽出して実測と突き合わせた**（過去形の記録は対象外）。改訂の採番し直しに本文の相互参照が追随しておらず、#128 の 3 行を指すはずの参照がすべて合流先の別の改訂を指していたのも同じ周で直した（Issue #128 / PR #133） |
| 1.71 | 2026-08-24 | PR #135（Issue #119 / import のグループ分けを `gci` で固定）のレビュー指摘に伴い、「行数の予算」の 2 つの表と散文を実測へ更新した（`internal/buildconfig` 1394 → 1475・残り 525、`ui/page/pagetest` 1937 → 1936・残り 64、`internal/doctor/hostres` 1384 → 1383・残り 617）。降順の並びが崩れていないことも確認した（3 件とも元の位置のままで並びは保たれている） | 表は「実測値」と明記してあり、**この節が次の Issue の読む行数予算の規範である**。PR #135 は `internal/buildconfig` に `gci` の設定とその回帰テストを足しており、行数が 81 行増えたのに表が据え置きだと、次に `buildconfig` へテストを足す Issue が 81 行ぶん過大な予算を引く。`ui/page/pagetest` と `internal/doctor/hostres` の 1 行ずれは、本 PR が該当ファイルの import を並べ替えた（空行の増減）ことによる。散文だけ取り残す欠陥が 1.69 / 1.70 で連続して再発しているため、今回も現在形で行数を主張する箇所を全件突き合わせた |
| 1.72 | 2026-08-24 | 設定の保存が `scan_roots` と `audit_log` へ届かない件（Issue #132）を反映。**詳細は下記「改訂 1.72」。** | 下記「改訂 1.72」を見よ |
| 1.73 | 2026-08-24 | PR #138（Issue #132）のレビュー指摘を反映。(1) 例外 3 の `audit_log` の段落で `rememberSelf` が比べる相手を「保存前後の `AuditLog`」から**起動時の値（`startupAudit`）**へ訂正し、前回の保存値と比べてはならない理由（一度変えて元へ戻した保存で誤って再起動を促す／変えた後に別項目だけを保存すると案内が消える）を明記した。(2) 「行数の予算」の非 UI の表を実測へ更新（`cmd/gsr-helper` 991 → 1077 行・残り 923。降順の並びは `internal/doctor/hostcfg` 1184 と `internal/doctor/authz` 822 の間のままで崩れていない）。(3) Config タブの節の散文が現在形で与えていた予算「次にこのタブへ手を入れる Issue は 168 行を使ってよい」を実測（残り 20 行）へ直し、実質ゼロである旨と先に空ける指示を添えた | (1) は**本書の 1 行だけが実装と正面から矛盾していた**。実装（`page/config/applyflow.go` の `rememberSelf`）は `m.savedSelf.AuditLog != m.startupAudit` であり、改訂 1.72・`rememberSelf` の doc・`startupAudit` の doc・回帰テスト「起動時の値へ戻したら伝えない」はすべて起動時比較を前提にしている。本書だけが**この PR が明示的に却下した設計（前回の保存値との比較）**を記述しており、読み手が実装をバグと読み違えるか、本書に合わせて実装を壊しうる。(3) は**表だけを直して散文を取り残す欠陥の 4 周目**である。1.72 は Config タブの散文 2 箇所を直したと記しながら、**最も危険な 1 箇所——次の Issue に与える予算の数値そのもの——を148 行過大なまま残していた**。残り 20 行のディレクトリに 168 行の予算を提示すると、次の Issue はそれを信じて足し、警告帯を越えてから気付くことになる。なお 1.73 は 1.70 が定めた手順（現在形で行数を主張する箇所を機械的に全件抽出して実測と突き合わせる）を**通していない**——実際に突き合わせたのは指摘のあった `cmd/gsr-helper` の 1 件と Config タブの 1 件だけで、`internal/disk` の節末尾が主張する 1927 行（残り 73 行）を 53 行過大なまま取り落としていた（改訂 1.74 で訂正）（PR #138 のレビュー指摘） |
| 1.74 | 2026-08-24 | PR #138（Issue #132）の 2 周目レビュー指摘を反映。(1) `internal/disk` から `pathguard` を切り出した判断（Issue #101）の節末尾が現在形で主張していた 1927 行（残り 73 行）を、切り出し直後の当時の値である旨へ改め、**現在値 1980 行・残り 20 行**を併記した。残り 20 行は実質ゼロなので、`internal/setup` の節末尾と同じ「1 行足す前に空けること」の指示も添えた。(2) 「一覧タブを 1 枚足せる余裕」の 959 行が **Issue #35 当時の値**であることを明示し、現在の残りは「行数の予算」の行数表を見よ、という参照を添えた（数値そのものは変えていない）。(3) 改訂 1.73 の理由欄から「1.70 の手順（現在形で行数を主張する箇所を機械的に全件抽出）を通した」という記述を外し、実際に突き合わせたのが `cmd/gsr-helper` と Config タブの 2 件だけで、`internal/disk` の 1 件を取り落としていた旨へ訂正した。(4) 今回は 1.70 の手順を実際に通した——現在形で行数を主張する箇所を全件抽出して実測（`go tool linterly check --format json`）と突き合わせ、**2 つの表は 38 行 / 37 行とも全数値・網羅性・降順の並びのいずれも実測と一致**しており、散文側の食い違いは (1) の 1 件だけであることを確かめた（他 21 箇所は一致） | (1) は**表だけが正しく散文が取り残される欠陥の 5 周目**である。同じ文書の非 UI の表は 1980 行と正しく書いており、**表と散文が同じ節で矛盾していた**。しかも姉妹文（`internal/setup` の「結果は 1970 行（残り 30 行）である」）が現在値として維持されているため、この文も現在値として読まれ、**残り 20 行しかないディレクトリへ 53 行過大な予算**を提示する危険側の誤りだった——次の Issue はそれを信じて足し、警告帯を越えてから気付くことになる。(2) は改訂 1.74 当時の正しい値が数行上にあるため、959 を現在の余裕と誤読しうる（改訂 1.69 が 6 周目の段落に施したのと同型の処置）。(3) は**実施していない検証を実施したと書くこと自体が指摘対象**だからである。1.73 は全件抽出を通したと書きながら通っておらず、その主張を残すと次の周が「もう全件見た」と信じて同じ取り落としを繰り返す（PR #138 のレビュー指摘） |
| 1.75 | 2026-08-24 | PR #138 の 3 周目レビュー指摘を反映。(1) **改訂 1.72 の理由欄を、実際にやったことへ書き直した**——「1.70 が定めた手順（現在形で行数を主張する箇所を機械的に全件抽出して実測と突き合わせる）を改めて通した」と記していたが通しておらず、突き合わせたのは 2 つの表と Config タブの散文 2 箇所だけで、Config タブの 3 箇所目（148 行過大）と `internal/disk` の節末尾（53 行過大）を取り落としていた。あわせて同欄が過大量を「73 行」としていたのを実際の **148 行**へ直した（`internal/disk` の「残り 73 行」からの混入で、隣接する改訂 1.73 の「148 行過大」と矛盾していた）。(2) `ui/page/runners` の節の「2164 行（残り -164 行）まで**伸びている**」を過去形へ改め、当時の値である旨と現在値（1983 行・残り 17 行。同じ段落の末尾が既に記している）への参照を添えた。**改訂 1.49 がこの是正を実施したと記していながら本文へ適用されていなかった**もので、本項の全件突き合わせで検出した。(3) 本項では実際に全件突き合わせを行った——2 つの表（UI 38 行・UI 層外 37 行）は全数値・残り・判定・網羅性・降順の並びが `go tool linterly check` の実測と一致、散文側は現在形で行数を主張する 20 箇所を抽出して照合し、食い違いは (2) の 1 件のみだった | **実施していない検証を実施したと書くこと自体が欠陥である。** 本書は「表だけを直して散文を取り残す」再発を 1.69 / 1.70 / 1.71 / 1.72 と 4 周にわたり記録してきたが、その是正を宣言する欄が偽だと、次の周は「もう全件見た」と信じて同じ箇所を読み飛ばす。現に 1.72 の偽の宣言は、取り落とし 2 件の訂正に 1.73 / 1.74 の 2 周を要させ、(2) の `ui/page/runners` に至っては 1.49 以来 26 改訂ぶん残存していた。隣接する改訂行が同一の出来事に矛盾した数値（73 行 / 148 行）を持つのも、どちらを事実とみなすかを読み手に委ねる点で同じ害である。**本項が (3) で検証の範囲と結果を具体的に書いたのは、次の周がこの欄を検算できるようにするためである**——「通した」とだけ書く欄は検算できない（PR #138 3 周目） |
| 1.76 | 2026-08-24 | 行数表の `ui` を 2063 → 2105、`ui/chrome` を 347 → 490、`ui/atom` を 1262 → 1264、`ui/molecule` を 1513 → 1515 へ実測に合わせて更新し、`ui/chrome` を降順の正しい位置へ移した。切り出しの記録（`ui/chrome`）の散文にある「現在 347 行」も直した。`ui/atom` と `ui/molecule` は doc コメントの加筆分である。状態行を幅に収める検証（114 行）は、初めから `internal/ui/chrome/statuswidth_test.go` に置いた——`chrome_test.go`（206 行）へ足すと 320 行となり 1 ファイル 300 行の警告帯（301〜330 行）に入るためである。**`chrome_test.go` 自体には手を入れていない**（同ファイルは 206 行のまま）。境界は行数だけで引いたのではなく、「ファイルの行数」の方針どおり責務でも切ってある（`chrome_test.go` は「状態行に何が出るか」、`statuswidth_test.go` は「幅に収まらないときに何を削るか」）。`ui/chrome` は残り 1500 行超なのでディレクトリの予算は問題にならない。あわせて、警告帯の `ui` へ Issue #136 の回帰テスト 42 行を足した判断と、その検証を `ui` 直下から動かせない理由を「行数の予算」節に残した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、`ui` は既に警告帯だった。**表の値を据え置いたまま行を足すと、次に予算を引きに来た Issue が実際より多い残量を前提に判断する**。改訂 1.58 / 1.60 が「行数表は後続 Issue が読む予算の規範」「1 つの文書が 2 つの値を主張する欠陥」として 2 度是正した箇所そのものである（PR のレビュー指摘） |
| 1.77 | 2026-08-24 | PR #144（Issue #136）のレビュー指摘（MAJOR 4 件）を反映。**詳細は下記「改訂 1.77」。** | 下記「改訂 1.77」を見よ |
| 1.78 | 2026-08-24 | `make check` の並列負荷で config の自己設定テストが散発的に落ちる件（Issue #140）を反映。**詳細は下記「改訂 1.78」。** | 下記「改訂 1.78」を見よ |
| 1.79 | 2026-08-24 | 同じ PR の Issue #141（自身の設定フォームの見出しを `firstRunUnsaved` で決める）/ #142（対象選択の見出しを runner に限定しない文言へ）を行数表と散文へ反映した——`ui/page/config` **1989 → 2098 行・残り -98 行・warn**（#141 が +66 行、#142 が +43 行）。**詳細は下記「改訂 1.79」。** | 下記「改訂 1.79」を見よ |
| 1.80 | 2026-08-24 | Issue #139（`ui` 直下の分割）を反映。**詳細は下記「改訂 1.80」。** | 下記「改訂 1.80」を見よ |
| 1.81 | 2026-08-24 | PR #146（Issue #139）のレビュー指摘（MAJOR 4 件）を反映。**詳細は下記「改訂 1.81」。** | 下記「改訂 1.81」を見よ |
| 1.82 | 2026-08-24 | PR #146（Issue #139）2 周目のレビュー指摘（MAJOR 3 件）を反映。**詳細は下記「改訂 1.82」。** | 下記「改訂 1.82」を見よ |
...
| 1.85 | 2026-08-24 | Issue #147（`ui/page/config` と `ui/page/pagetest` を行数の警告帯から出し、束を辿る道具を 1 つに寄せる）を反映。**詳細は下記「改訂 1.85」。** | 下記「改訂 1.85」を見よ |
| 1.86 | 2026-08-24 | Issue #150（`RunAll` / `Msgs` / `ChromeOf` / `IsQuit` が締め切り無しで `Cmd` を走らせる）を反映。**詳細は下記「改訂 1.86」。** | 下記「改訂 1.86」を見よ |
| 1.87 | 2026-08-24 | PR #154 の 2 巡目のレビュー指摘（CRITICAL 1 件・MAJOR 4 件・MINOR 4 件）を反映。**詳細は下記「改訂 1.87」。** | 下記「改訂 1.87」を見よ |
| 1.88 | 2026-08-24 | Issue #151（依存グラフの辺を `go list` の import と突き合わせる検査の新設）を反映し、「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（1475 → **1969 行・残り 31 行**・pass）。**詳細は下記「改訂 1.88」。** | 下記「改訂 1.88」を見よ |
| 1.89 | 2026-08-24 | PR #153 の 2 周目レビューを反映。(1) 「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（1969 → **2025 行・残り -25 行・warn**）。降順の先頭（`internal/setup/tarball` 1999 の上）へ移した。(2) 表の直後の「**警告帯に入っているディレクトリは 1 つも無い**」が偽になったので、`internal/buildconfig` の 1 つである旨と判断への参照へ直した。(3) **`##### internal/buildconfig を警告帯へ入れた判断（PR #153 の 2 周目レビュー）`** を新設し、切り出さずに警告帯へ入れた 5 つの根拠（正しさを行数より優先した／手 (1) は本番の切り出しなのでテスト専用パッケージには当たらない／手 (2) の重複削減は `cutAtLineStart` への集約で実施済みで +77 行を +56 行まで削った／ERROR 境界まで 175 行／1 ファイル 300 行は最大 286 行で全件が守っている。**この 5 つは当時の記録であり、現在値は改訂 1.90 が更新した**）と、次の Issue への指示（空けること自体を独立した Issue に立てる。候補はビルド設定の検査とドキュメントの検査をパッケージ境界で分ける手）を記録した。(4) 改訂 1.88 の 2 つの誤りを直した——**手 (1)「本番の一部をパッケージ境界で切り出す」が採れると書いていたが、`internal/buildconfig` は実行時のコードを持たないテスト専用パッケージ**（`doc.go` 12 行のほかはすべて `_test.go`）なので噛み合わない（挙げていた理由「増え方が違う」自体は妥当なので、手の呼び方と対象の書き方を実態に合わせた）。また現在形で与えていた予算「残りは 31 行しかない」を当時の記録へ改め、現在値（2025 行・残り -25 行・warn）と出所への参照を添えた（**この数は 1.89 当時の値である。現在値は上記「行数の予算」の行数表を見よ。改訂 1.94**） | 表は「実測値」と明記してあり、**この節が次の Issue の読む行数予算の規範である**。56 行を足して**警告帯へ入った**のに表と「警告帯に入っているディレクトリは 1 つも無い」を据え置くと、次に `buildconfig` へ検査を足す Issue が**残量をプラスだと信じて足し、ERROR 境界（2200 行）へ近づいてから気付く**。本節は「**警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと**」と自ら定めており、記録の無いまま入ると次の周が同じ逃げ道を根拠なく使う（7 周目・8 周目が 2 周続けて使った前例がある）。(4) は**存在しない選択肢を次の Issue へ提示する**もので、本番コードを持たないパッケージから「本番の一部」を切り出そうとして手が止まる。表だけを直して散文や過去の欄の現在形を取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 で繰り返し再発しているため、今回も降順の位置・散文・改訂 1.88 の現在形を同時に直した（PR #153 2 周目） |
| 1.90 | 2026-08-24 | PR #153 の最終レビュー（変異注入で実測した major 4 件）を反映。**詳細は下記「改訂 1.90」。** | 下記「改訂 1.90」を見よ |
| 1.91 | 2026-08-25 | PR #153 の最終レビュー 2 巡目（3 観点が**変異注入で実測**した MAJOR 3 件）を反映。**詳細は下記「改訂 1.91」。** | 下記「改訂 1.91」を見よ |
| 1.92 | 2026-08-25 | Issue #155〜#157 を反映。**詳細は下記「改訂 1.92」。** | 下記「改訂 1.92」を見よ |
| 1.93 | 2026-08-25 | Issue #159 を反映。**詳細は下記「改訂 1.93」。** | 下記「改訂 1.93」を見よ |
| 1.94 | 2026-08-25 | Issue #161 を反映。**詳細は下記「改訂 1.94」。** | 下記「改訂 1.94」を見よ |
| 1.95 | 2026-08-25 | Issue #164 を反映。(1) **書かれた行数を実測と突き合わせる検査を 2 本新設した**（`internal/buildconfig/docscheck`）——`TestLineBudgetTablesMatchLinterly`（`docs_linebudget_test.go`）は `go tool linterly check --format json` の実測と 2 つの行数表（UI 層・UI 層の外）を、行数・残り・判定・降順の並びまで突き合わせる。`TestLineBudgetProseMatchesTables`（`docs_linebudgetprose_test.go`）は本書から 2 つの行数表と改訂履歴を除いた散文を文に切り、「N 行・残り M 行」「残り M 行」という主張が表の行として実在するかを見る。(2) 「行数の予算」に小節「**行数表と散文は実測に合わせる**」を新設し、規約を明文化した——表は手で書き換えるのではなく実測を取り直して合わせる、散文が現在形で主張する数も表と突き合わせる、**当時の値を書くときは同じ文に `当時` か `時点では` を入れる（これが唯一の除外）**、ファイル側の残りは `行` を付けず `（残り 4）` の形で書く、改訂履歴の過去欄には当時の値だけを残す。(3) 行数表の `internal/buildconfig/docscheck` を実測へ更新し、降順を保つため `internal/svc` の下へ移した。(4) 新しい検査が挙げた散文を全件見直した——当時の値でありながら印の無かった 36 箇所へ `当時` / `時点では` を入れ、印が無いまま現在形で古かった 3 箇所（「2 つに分けた判断」の `docscheck` の実測、3 ディレクトリの合計、移した `docs_nolint_test.go` の行数）を実測へ直した。(5) ディレクトリの予算とファイルの予算を同じ書き方で混ぜていた 5 箇所を、ファイル側の `（残り N）` の形へそろえた | 同じ実測値が本書の 3 つの系（2 つの行数表・散文・改訂履歴）へ手で写されているのに、`docscheck` の検査は依存グラフの図とパッケージ名の一覧しか見ておらず、**書かれた数を実測と突き合わせるテストはどこにも無かった**。表も散文も `make check` が緑のまま静かに古くなる。直前の Issue #165 が `docscheck` を育てたときも、表の値は当時のまま残って何も落ちていない。実害は改訂 1.94 の (9) が記録している——散文が次の Issue へ「移せるのは 68 行まで」と、受け皿の実容量より 3 行過大な予算を指示していた。**表と散文を機械が突き合わせれば、写し間違いも取り残しも次の `make check` で落ちる。** 過去の値まで実測へ寄せると Issue に紐づく記録が失われるため、`当時` / `時点では` を唯一の除外として規約に固定し、印の無い数は現在の主張として扱う。散文を検査対象に入れた以上、印の付け直しは本改訂で全件済ませてある |
| 1.96 | 2026-08-25 | PR #168 のレビュー指摘（major 8 件）を反映。**詳細は下記「改訂 1.96」。** | 下記「改訂 1.96」を見よ |
| 1.97 | 2026-08-25 | Issue #164 の再設計を反映。**詳細は下記「改訂 1.97」。** | 下記「改訂 1.97」を見よ |
| 1.98 | 2026-08-25 | PR #168 のレビュー指摘（major 7 件）を反映。**詳細は下記「改訂 1.98」。** | 下記「改訂 1.98」を見よ |
| 1.99 | 2026-08-25 | PR #168 のレビュー指摘（major 6 件）を反映。**詳細は下記「改訂 1.99」。** | 下記「改訂 1.99」を見よ |
| 1.100 | 2026-08-25 | Issue #167（散文の行数の「合計の式」が行数の検査の網から外れている）を反映。**詳細は下記「改訂 1.100」。** | 下記「改訂 1.100」を見よ |
| 1.101 | 2026-08-25 | Issue #169（`components/overview.md` の行数の実測値が PR #168 の検査の対象外で無検査のまま残っている）を反映。**詳細は下記「改訂 1.101」。** | 下記「改訂 1.101」を見よ |
| 1.102 | 2026-08-25 | Issue #166 で `internal/buildconfig/docscheck` へ検査を 1 本足した（`docs_invarianttable_test.go`——[環境構築](../environment/setup.md#設定ファイルの不変条件をテストで守る)の不変条件テスト一覧表が挙げるテストの集合と、`internal/buildconfig` 直下および `docscheck` のテスト関数の集合の一致を両方向で見る）ぶん、「UI 層の外のディレクトリ」の表の当該行を `go tool linterly check --format json` の実測へ更新した。行数が `internal/exec/command` を上回ったので、**行数の多い順の並びを保つため行を 1 つ上へ移した**（`internal/config/edit` の下） | **行数表は実測しか書かない。** 表の数値は `TestLineBudgetTablesMatchLinterly` が実測と突き合わせており、行数・残り・判定に加えて**行数の多い順の並び**も見ているため、値だけ直して行の位置を据え置くと落ちる。判断の材料は「どこが逼迫しているか」であり、並びが崩れた表は先に空けるべき場所を指せない |
| 1.103 | 2026-08-25 | PR #173 のレビュー指摘（major 6 件）を反映。**詳細は下記「改訂 1.103」。** | 下記「改訂 1.103」を見よ |
| 1.104 | 2026-08-25 | PR #173 の 2 周目レビュー指摘（major 3 件と併せて直す minor）を反映。**偽陽性は除外ではなく文書側の書き方で消せたので、除外の仕組みごと落とせた**——「### 幅」の列幅の式を、同じ節の隣の行が既に使っていた「数の間に語を挟む」形（`行頭 6 + 列幅合計 66 + 列間 3 = 75 セル`）へそろえたところ、合計の式の検査の唯一の偽陽性が消えた。そこで (1) 「除外は次の 5 つ」を **4 つ**へ戻し、(5) の「合計の式にだけ効く見出しの一覧」（`budgetProseSumExcluded`）と、それが外していた「### 幅」の節ぶんの除外を落とした。**除外を足すことは検査が見ない範囲を広げることであり、そこは規約でしか守れなくなる**ので、偽陽性は書き方で消すのが先である、と床の説明に明記した。(2) 同じ床の説明の括弧「（`internal/buildconfig/buildconfigtest` は 2 桁である）」を落とした——**本節が (c) として自ら禁じた「数値を伴わない現在形の語形の断定」**であり、床の根拠は現在値に依存しないためである。(3) 合計の式の空白の文字クラスへ**全角空白（U+3000）**を足した。Go の `\s` は半角の空白類しか含まず、全角空白で区切った式（とくに全角マイナスは全角空白を伴って書かれる）が素通りしていた。**空白文字も演算子と同じく閉じた集合なので、文字クラスに並べても言い回しの列挙にはならない。**（半角ハイフンの空白なしの減算を拾わないのは、ISO 形式の日付の偽陽性を避けるための意図的な割り切りのままである。）(4) 改訂 1.103 の (4) の参照方向「**下記**「`internal/disk` から `pathguard` を切り出した判断（Issue #101）」」を**上記**へ直した（当該節は改訂履歴より上にある）。(5) 行数表を実測へ更新した（`internal/buildconfig/docscheck` が 1992 → **1964 行**。除外の仕組みを落としたぶんで、行数の多い順の並びも 2 つ下へ移した）。 | (1) は検査が見ない範囲を広げないための判断である——除外は規約でしか守れない領域を増やすので、文書側の書き方で消せる偽陽性は書き方で消す。(3) の全角空白は演算子と同じく閉じた集合であり、言い回しの列挙を増やさずに死角を塞げる。(2) と (4) はいずれも本節が自ら定めた規約に対する違反である——規約を書いた文書がその規約を守れていないと、規約の側が信用を失う |
...
| 1.109 | 2026-08-25 | PR #176 のレビュー指摘（major 8 件）を反映。**詳細は下記「改訂 1.109」。** | 下記「改訂 1.109」を見よ |
...
| 1.119 | 2026-09-03 | Issue #183 を反映。`internal/exec/command` から切り詰めの道具（`truncateTail` / `truncateHead` / 断片落としの 2 つ / `limitedBuffer`）を `internal/exec/command/limit` へ切り出し（`Tail` / `Head` / `Prefix` / `Suffix` / `Buffer` / `NewBuffer` の 6 つを export、上限の 3 定数は `command/limits.go` に残す）、4 つの手のうち (1) を採った判断と (2)(3)(4) を採れなかった理由を「`internal/exec/command` から切り詰めの道具を `limit` へ切り出した判断（Issue #183）」として記録した。「行数の予算」の非 UI の表を実測へ更新した（`internal/exec/command` 1989 → **1888 行**・残り 112・pass、`internal/exec/command/limit` を **285 行**・残り 1715・pass として追加。並びは `internal/exec/command` が `internal/config/edit` と `internal/runner` の間へ下がり、`limit` が `internal/disk/pathguard` と `internal/setup/setuptest` の間） | `dropPartialRuneAtStart` が断片を落とす行は 1 度も実行されていなかった（素材の `stderrflood` が ASCII しか吐かないため、末尾 4 KiB をどこで切っても多バイト文字の途中に当たらない）。回帰テストを足すには先に行数を空ける必要があり、内部テストしか無いこのディレクトリでは (3)(4) がどちらも循環か非公開の大量 export になるため、本番の境界で切るしかなかった |
| 1.120 | 2026-09-03 | Issue #187 を反映。`internal/ui/page/setupmodal` に回帰テストを 3 ファイル（`helper_test.go` / `confirm_test.go` / `form_test.go`）足した。見るのは包み方——確認ダイアログは決定を**自分の種類で**差し戻すこと（実行前プレビューと入力の破棄を同時に登録した状態で `y` を打って縛る）、`esc` を Overlay ではなくダイアログが先に受けること、追加フォームは完了・中断・破棄の 3 つをどれも `FormKind` で差し戻すこと、**組み立て関数へ渡す** huh のテーマを開くときに渡された共有状態から組むこと（**内側の `dialog.Form` が実際に着る配色までは見ていない**。改訂 1.124 の (2)）、見出しとフッタが他のモーダルの Model で落ちないこと——である。「行数の予算」の UI の表の `ui/page/setupmodal` を実測へ更新した（253 → **675 行**・残り 1325・pass。並びは `ui/page/configmodal` と `ui/template` の間へ上がった） | テストを 1 本も持たない本番の UI パッケージだった。判断を持つのは**決定の差し戻し先**で、取り違えると「破棄するつもりで押した `y` で実行が始まる」形の事故になる（`ConfirmKind` の doc が挙げている危険そのもの）。打鍵から決定までを 2 段（ダイアログが返す Cmd → `page.WrapModal` の包み → 配り直し）で辿るのは、`dialog.DecidedMsg` を直に流すと包みのタブ番号と種類が壊れていても緑になるためである |
...
**上限を超えた版の欄はここへ出す。** 表のセルは改行を持てないので、欄が長くなると 1 文字の修正でもその行がまるごと変更行として差分に出る（Issue #170）。**移したのは格納の形だけで、本文は 1 文字も落としていない**——列挙の `(1)` `(a)` はマーカーごと箇条書きへ移してあるので、欄の中の相互参照（「(5) は…」）もそのまま辿れる。上限と方針は[開発環境セットアップ](../environment/setup.md#改訂履歴の欄は要約と参照に留める)にある。

#### 改訂 1.6

**変更内容**

`Detail.SetContent` を「受け取ったスライスは写しを取る」約束の対象に追加。
背景の明暗を「起動後に届き、切り替わることもあるため届くたびに解決し直す」入力として定義。
page と organism の約束に配色の配り直しを organism 側（取り込んだ配色を後から配り直せる形にする）と page 側（`StateMsg` ごとに渡し直す）の 2 行として追加し、配り直しの 2 方式（`Restyle` を生やす / 作り直して必要な状態だけ引き継ぐ）と選び分けの基準を「配色とキー定義の配り直し」に定義。
配り直しが要る部位を「`SetStyles` で `bubbles/table` へ渡す見出し／行へ焼き込むセル・カーソル記号・チェックボックス／`bubbles/textinput` が既定を持つ絞り込みの入力欄」と書き分け、区切り線と確定後の絞り込みの行は organism が毎回描くため差し替えだけで追随する側だと明記。
作り直し方式には「保つべき状態を作り直した後に、大きさが決まってから戻す」義務があること、`pane.Help` / `helpmodal.go` は現状その順序を満たせずスクロール位置を引き継げていないことを追記。
配り直しの義務を負うタブの列挙を Disk / Logs / Doctor に揃えた

**変更理由**

`bubbles/viewport` の `SetContentLines` は渡されたスライスを書き戻すため、写しを取らないと page 側の行が organism に書き換えられていた。
文書は背景の明暗を起動時に 1 度解決するものとして書いていたが、応答は起動後に届き切り替わりもするため、解決済みの `token.Styles` を配り渡す記述だけでは配り直しの義務が読み取れなかった。
一覧は配色を行へ焼き込む以上フィールドの差し替えでは追随せず、未実装の Disk / Logs / Doctor タブをこの文書から実装すると配り直しが漏れて同じ欠陥が再発する。
配り直しの方式は実装 3 者で分かれており（`table.Model` は差し替え + 行の組み立て直し + 入力欄の渡し直し、`ChoiceList` は差し替えのみ、`pane.Help` は `helpmodal.go` が作り直す）、`Restyle` を一律の義務として書くと `pane.Help` が約束違反に読めた。
配り直しが要る部位の列挙も実態とずれており、選択行には ANSI の入れ子を避けるためあえて装飾を付けないのに列挙に含み、区切り線は organism 自身が `token.Styles` から毎回描く（`bubbles/table` は区切り線に相当するスタイルを持たない）のに `bubbles/table` の描画物として挙げ、実際に配り直しが要る絞り込みの入力欄が漏れていた。**この列挙漏れは実装の欠陥をそのまま追認していた**——`table.Model.Restyle` は入力欄を配り直しておらず、背景色を切り替えても `bubbles/textinput` の既定色が残り、色を無効にした設定でも色が付いていた。
`pane.Help` の引き継ぎも「位置以外に失うものが無い」と断言していたが、`Overlay.SetState` が `StateMsg` の後に `SizeMsg` を送るため `SetOffset` は高さ 0 で丸められ、実際には位置が 0 に落ちていた

#### 改訂 1.32

**変更内容**

Logs タブ（Issue #9）のレビューで見つかった本書の誤りを 6 点直した。

- (1) ディレクトリ構成の一覧の `molecule/listrow/` に `LogRow` を追加。
- (2) 同じ一覧の `organism/pane/` を「スクロールする表示専用の領域（Detail / Help）」から「スクロールする領域（Detail / Help / Log）。
  Log は入力欄を持ち表示専用ではない」へ訂正し、`pane` に置くかどうかの分かれ目が「表示専用か」ではなく「`bubbles/viewport` でスクロールする領域か」であることと、入力欄の有無で置き場所を決めない理由を本文に追加。
- (3)「キー入力の配送」の `?` の段落を訂正。
  「Context と同じ範囲で組む」を「Context と同じ除外（`tab`）を適用する」に限定し、ヘルプ（`LogsHelp` は `logsGlobal` + `Log.Bindings()` + `List.Bindings()` **全体** + `List.FilterBindings()`）と Logs 用 Context（`List` は `Up` / `Down` / `Top` / `Bottom` / `PageDown` / `PageUp` / `Filter` / `Enter` の 8 つだけ）で範囲が違うこと、その目的の違い（`?` は押せるキーを人に見せる / Context は同時に有効なキーの重複を機械が検査する）を表と本文で追加。
  ヘルプに残る `space` / `ctrl+a` の穴も明記。
- (4)「幅」の表に `LogColumnRules()`（Drop: `UPDATED` → `RUNNER` / Keep: `LOG` / `SIZE`）と `SizeColumnWidth`（7）の行を追加し、列の集合の列挙に `LogColumns()` を追加。
  落とす順の根拠と `SizeColumnWidth` を定数にする理由（`atom.Bytes` の最長表記を `TestBytesFitsColumnWidth` が突き合わせる）を本文に追加。
- (5)「行頭に 6 セル」を、選択できる一覧は 6 セル・選択できない一覧（Logs のファイル一覧）は 2 セルという表へ訂正。
  あわせて `molecule.Columns` の判定だけは選択の可否によらず常に 6 セルで見積もることと、列を足すときに確認するのは実測ではなく判定側の見積もりであることを追加。
- (6) ディレクトリの行数表を今回の修正を終えたあとの実測値へ更新し（`ui/page/logs` 1666→1984、`ui/keymap` 1140→1233、`ui/organism/pane` 1108→1124、`ui/page/pagetest` 824→1003）、崩れていた降順の並びを直した。
  あわせて `ui/page/logs` の残りが 16 行しかないことと、次にこのディレクトリへ足す Issue が採るべき順序（まず道具を `page/pagetest` へ出す → 次にテストの重複削減 → ファイル分割は合計を減らさないので採らない）を本文に追加

**変更理由**

- (1) 同じ文書の他の 5 箇所（molecule の表・実装済み部品の列挙・画面別の部品表・実装状況の表・`LogRow` の説明）が `LogRow` を数えているのに、部品の置き場所を引くときに最初に読まれるこの一覧だけが漏れていた。
- (2) `pane.Log` は `textinput` と入力モード（`filtering`）を持ち**表示専用ではない**。
  同文書の organism 一覧と [コンポーネント設計](../components/overview.md)は 1.30 / 1.21 で訂正済みで、**この一覧だけが偽の記述として残っていた**（**「この一覧だけが」は誤りである。
  同じ本書の「organism の分割方針」の表が `organism/pane` を「表示専用の領域」と定義したまま残っており、しかも本項が新設した分かれ目（`bubbles/viewport` を使うか）そのものが偽だった。
  ともに 1.33 で訂正した**）。
  「表示専用でないものは `pane` に置けない」と読まれると、次のスクロール領域が `organism` 直下へ散る。
- (3) 実装を読むと範囲は同じではない。
  `LogsHelp()` は `List.Bindings()` を丸ごと（`Toggle` / `SelectAll` を含む）渡し、`FilterBindings()` も別グループで出すが、Logs 用 Context は `List` の 8 つしか列挙しない。
  しかも**同じ文書の 3 段落上にある Context の表は「`List` の一部」と正しく書いており、1 つの文書が同じ事実について 2 つのことを主張していた**。
  「同じ範囲で組む」を真に受けて Context を `List` 全体へ広げると、入力中にしか効かない `enter`（確定）/ `esc`（取消）が通常モードの `enter` / `esc` と重複して検査が落ちる——直すべきでない側を直す誘導になっていた。
  なお `tab` の除外を `Set.logsGlobal` 1 箇所に集約している点は正しかったので残した。
- (4) 同節は「別の列を持つタブ（Disk / Logs / Doctor）を足すたびに区画の定義が落とす順を宣言する」と明記しているのに、Logs タブがまさにそれを行った差分で表が `RunnerColumnRules()` の 1 行のままであり、**宣言の実例を 1 つも読めない**状態だった。
  列の集合の列挙も同じく `LogColumns()` を欠いていた。
- (5) Logs のファイル一覧は `Selectable: false` でチェックボックスのガターを持たず、行頭は 2 セルである（`token/width.go` の `LogColumns` の doc が明記している）。
  無条件に 6 セルと書いてあると、この節が指示する余裕確認の手順が選択できない一覧で誤った桁数を出す。
  一方で判定側の `molecule.Columns` は選択の可否によらず 6 セルで見積もるため、実測の 2 セルへ書き換えるだけでは逆向きに誤る。
  両方を書き分けないと確認手順として使えない。
- (6) 表には「実測値。
  2026-08-23 時点」と明記してあるのに 4 行が古く、**この節が次の Issue の読む行数予算の規範である**以上、残り 334 行と読んだ Issue が実際には 104 行しか無いディレクトリへ部品を足すことになっていた。
  降順の並びも崩れており、どこが逼迫しているかを表から読み取れなくなっていた（Issue #9 のレビュー指摘）

#### 改訂 1.49

**変更内容**

「`page/` は「1 ディレクトリ 1 タブ」ではない」の段落が 2 つ並んでいたのを 1 つに統合し、共有部品の列挙を `page/pagetest/import_test.go` の `shared` と一致させた（`page/action` / `page/runnerdetail` / `page/runnerop` / `page/progressmodal` / `page/pagetest`）。
列挙の正が `shared` の側であることを本文に明記し、食い違いを止める検査 `TestSharedPackagesMatchDoc` を `import_test.go` に追加。
あわせて

- (1)「`page` 階層も同じ理由で分ける」の段落から 2 つ目の列挙を落とし、分割の軸（タブの数に比例して増えるか / 依存の向きを一方向に閉じられるか）と、共有部品同士の参照を許す一方向依存の記述へ改めた（どれが共有部品かの正は `shared` であることは前段落への参照に委ね、ここでは列挙し直さない）。
- (2)「ディレクトリの行数」の表の `ui/page/pagetest` を実測へ更新（1674 → 1722 行・残り 278）し、表が宣言する行数の多い順に合わせて `ui/page/jobs` との並びも直した。
  散文側の同じ数値も同時に更新。
- (3) Disk / Setup の将来の切り出し手順を、リポジトリが実際に採ったネスト（`page/disk/cleanview` / `page/disk/confirmmodal`）に合わせて `page/disk/clean` / `page/setup/modal` へ改め、`shared` への登録と列挙の段落への追記は**フラットな名前（`page/diskclean` / `page/setupmodal`）を採る場合にだけ**要る手順であることを明記した（`tabName` が候補にするのは `page/` 直下の 1 階層だけなので、ネストしたパッケージはタブ判定の対象にならない）。
  あわせて Disk 節の切り出し対象を実在するファイル（`clean.go` / `progress.go` とそのテスト・計 728 行）へ訂正し、`Model` の状態ごと移す必要があることを但し書きにした。
- (4) 理由欄の版番号 `1.32` を `1.44`（Issue #75）へ訂正。
- (5) 2 周目のレビュー指摘を反映し、「`page` 階層も同じ理由で分ける」の段落から「`page` 直下に置くのは…だけ」の断定を外して、タブをまたぐ移動の `Msg` と移動先の名前（`OpenTabMsg` / `TabLogs` / `ShowLogMsg` / `SetupRequestMsg` など）が例外として `page` 直下に同居することを但し書きに戻し、`page/action` を import しない旨の括弧の係り先を `page/action` について明示した。
- (6) Setup 節の行数を実測へ訂正（2190 → 2081 行・エラー境界まで 119 行）し、将来の切り出し対象を実在するモーダル 2 種（`formmodal.go` / `confirm.go`）へ直した。
  Disk 節の 2101 行を実測の 2128 行（境界まで 72 行）へ、「一覧タブを 1 枚足せる余裕」の `ui/molecule/listrow` の残りを 817 行から実測の 477 行へ更新した。
- (7) 3 周目のレビューで残っていた散文の行数を実測へそろえた——Config 節の「その後の研磨で 2059 行になり、警告帯に入った（残り -59 行）」を「当時 2059 行・現在 2117 行・残り -117 行」を併記する形へ改め、同節の「エラー境界の 2200 まで 141 行」を **83 行**へ訂正。
  `page/runnerop` 節の「`ui/page` 直下（残り 392 行）」を実測の**残り 30 行**へ訂正。
  `ui/page/runners` 節の「2164 行（残り -164 行）まで伸びている」を過去形（伸びた）へ改め、同じ段落が末尾で述べる現在値 2018 行と現在形で衝突しないようにした。
  `ui` 直下の「4 周目の空け方」が結果を「2191 行で、境界まで 9 行のまま据え置きである」と現在形で締めていたのも過去形へ改めた（直後の 5 周目が 1988 行・残り 12 行へ戻しており、現在値としては読ませない）

**変更理由**

同じ書き出しの段落が 2 つあり、読み手は同じ説明を 2 回読まされたうえに列挙が食い違っていた。
古い方は `runnerop` を欠き、**新しい方も `progressmodal`（1.44 / Issue #75 で `shared` に追加済み）を欠いていた**ため、どちらを写しても `shared` とずれる。
本書は「どれをタブとみなすかは `shared` が決める」という判断をこの列挙に依存させているのに、その一致は誰も検査していなかった。
1.21 / 1.24 / 1.25 / 1.37 / 1.40 が繰り返し是正してきた「1 つの節が 2 つのことを主張する」欠陥の再発である（PR #93 のレビュー指摘 / Issue #96）。
あわせて PR #115 のレビューで次の 3 点が出た。

- (1) 統合した段落とは別に「`page` 階層も同じ理由で分ける」の段落が**2 つ目の列挙**を持ち、実在する `page/runnerop` / `page/progressmodal` を欠いたまま依存も「`page/action` → `page` の一方向」で閉じており、同じ本書が後述する `page/runnerop` → `page` / `page/action` / `page/runnerdetail` / `organism/dialog` / `svc` と食い違っていた（Issue #96 が消したのと同じ欠陥の作り直しである）。
- (2) **本 PR 自身が `import_test.go` に 48 行足したのに行数表を更新しておらず**、表が宣言する「行数の多い順」の並びも崩れていた。
  本表は後続 Issue が読む予算の規範である。
- (3) 新設した `TestSharedPackagesMatchDoc` により、Disk / Setup の将来手順どおりに `shared` だけ足すと**列挙の段落と食い違って CI が必ず赤になる**状態だった。
  版番号の誤り（1.32）は本書の 1.32 が Logs タブ（Issue #9）の改訂であり `progressmodal` に触れていないうえ、`docs/components/overview.md` の版番号と取り違えたまま自書の改訂履歴に裸で書くと自書の版として読まれる。
  2 周目のレビューで、1 周目の書き換えが本書 669-674 行の「例外はタブをまたぐ移動である」（`page` 直下に `OpenTabMsg` / `TabLogs` / `ShowLogMsg` を置き、理由を「移動元と移動先の双方から見える場所がここしか無い」とする）と正面から食い違う断定を作り込んでいた（実装の `page/opentab.go` も `ShowLogMsg` / `SetupRequestMsg` を持つ）。
  括弧も係り先の変更で `page` パッケージ全体の中身の主張として読める形に壊れていた。
  Setup / Disk / listrow の散文は Issue #75 の `progressmodal` 切り出し（setup が約 109 行減った）以降の実測に追随しておらず、切り出し対象に挙げた `progress.go` は既に実在しない。
  本節は次の Issue が読む予算の規範なので、実測とずれていると境界に当たってから気付くことになる。
  3 周目のレビューでも同じ欠陥が残っていた——2 周目が Setup / Disk / listrow を直した一方で、Config 節（2059 行・エラー境界まで 141 行）と `page/runnerop` 節（`ui/page` 直下の残り 392 行）は実測（2117 行・83 行 / 1970 行・残り 30 行）に取り残されており、同じ文書の行数表と正面から食い違っていた。
  とりわけ `ui/page` 直下の残りは 30 行しか無いのに 392 行と読め、余裕を 13 倍に過大表示していた。
  `ui/page/runners` 節の 2164 行も現在形で書かれていたため、同じ段落の末尾が記す 2018 行と衝突していた。
  4 周目のレビューで、2 周目が Setup 側だけを直した非対称が残っていた——Disk 節は行数（2101 → 2128）だけ直され、切り出し対象は Issue #75 で削除済みの `page/disk/confirm.go` を指したまま、切り出し先も本タブが実際に採ったネストではなくフラットな `page/diskclean` のままだった。
  ネストなら不要な `shared` への登録を必須の手順として書いていたため、そのとおりに実行するとタブ判定の例外が理由なく 1 つ増える
...
- (1) 例外 3（`ConfigSavedMsg`）のブロックを「名前は文字列で突き合わせる」の**後ろ**へ移し、タブをまたぐ移動の段落（`OpenTabMsg` の表 → `page.Do` で包まない → 名前の突き合わせ）が連続するようにした。
- (2) 例外 3 に、**旧判断（走行中に親の設定を差し替えない）を反転させた理由**を 1 段落追記した（避けたかった「保存しただけで挙動が変わる」と Issue #128 の「書いた数字がその場で効かない」は正面から衝突する / 契機は必ず利用者の明示的な承認と書き込みである / `refresh_interval` を変えても走行中の検出は中断されない——`tick` は次の周期を予約する時点で `refresh()` を読む）。
- (3) 反映の時点が Disk（次の `StateMsg` から即時）と doctor（次回の診断実行から。
  `setState` は診断を始めない）で非対称であることを追記。
- (4) 6 周目の `App.current` の根拠を訂正——`tabset.Live` の「写し」ではなく**添字の範囲しか見ていなかった**（`Enabled` は呼び出し元の `View` が`ok && t.Model != nil` で補っていた）。
  挙動が変わらない根拠は `tabset.New` が `Model` と `Enabled` を同一の分岐でしか設定しないこと（`Enabled ⟺ Model != nil`）である。
- (5) 行数表を実測へ更新（`ui/page` 1997・残り 3、`ui/page/config` 1832・残り 168。
  `ui/page/doctor` の 1580 → 1600 は合流先の 1.64 が同じ周に直しており、記録はそちらが持つ）し、`page/runnerop` 節の散文「`ui/page` 直下（残り 30 行）」も残り 3 行へそろえた。
- (6) 5 周目の次の Issue への指示を当時の記録（過去形・当時の値である旨）へ改め、現在の予算は 6 周目の節が持つと明記。
- (7) 6 周目に**ファイル単位の逼迫**（`internal/ui/app_test.go` が 300 行で 1 ファイルの上限に張り付き）と`ui/page` 直下の残りを記録

**変更理由**

例外 3 を段落の途中へ差し込んだため、「タブをまたぐ移動」への注意書き（タブ名を変えると移動だけが静かに効かなくなる）が、名前で突き合わせない `ConfigSavedMsg` への注意書きとして読める並びになっていた。
反転した設計判断は旧コメントが理由まで明示していたのに、PR #133 はそれを削って結果（次の周期から効く）だけを残し、**なぜ今その懸念を許容してよいのか**がコードにも本書にも残っていなかった。
本書は判断とその理由を残す場所であり、理由の無い反転は次に同じ懸念を持った Issue が元へ戻す根拠になる。
`App.current` の根拠が事実と違うと、「写しだから捨ててよい」という誤った一般化（実際は判定が厳しくなる置き換えで、成立の根拠は`tabset.New` の不変条件にある）を次の削減へ持ち越す。
行数表と散文は次の Issue が予算を引く唯一の材料であり、5 周目と 6 周目が別々の残り行数を現在形で指示していると、どちらを予算とみなすかが読み手に委ねられる。
ファイル単位の上限（300 行）は周回ごとの叙述の中にしか無く（5 周目の「`app.go` を 302 行から 299 行にした」）、上限を引きに来た読み手が辿る場所——「ディレクトリの行数」の節の冒頭——には無かったため、`app_test.go` へ 1 行足す Issue が境界に当たってから気付くことになる（Issue #128 / PR #133）

#### 改訂 1.72

**変更内容**

設定の保存が `scan_roots` と `audit_log` へ届かない件（Issue #132）を反映。

- (1) 例外 3（`ConfigSavedMsg`）の「親が持たない設定値はこの経路では直らない」の段落を 3 つに割り、`scan_roots` を**この経路へ載せ直した**こと（`Options.Roots` の意味を `--root` の値だけへ狭め、合成は `discover.go` の `scanRoots()` が周期ごとにやり直す。
  `--root` は保存後も効き続ける）と、`audit_log` は**載せないと決めた**こと（親が配るのは開いたファイルハンドルであり、走行中の開き直しは取りこぼしと二重オープンを招く。
  代わりに `rememberSelf` が起動時の値と比べて `auditRestartNote` を結果へ添える）を明記した。
- (2) 「行数の予算」の 2 つの表を実測へ更新し（`ui` 2000 → **2063 行・残り -63 行・warn**、`ui/page/config` 1832 → 1980 行・残り 20 行、`internal/appconfig` 1343 → 1381 行、`cmd/gsr-helper` 989 → 991 行）、降順の並びを直した。**表と同時に散文も直した**——Config タブの節の 2 箇所（1832 行・残り 168 行 → 1980 行・残り 20 行）と、「残りが 1 桁のディレクトリ」の `ui` の括弧書き。
- (3) 「7 周目の判断（Issue #132）——切り出さずに警告帯へ入った」を新設し、切り出さなかった 3 つの根拠（切り出せるまとまりが無い／バグ修正にスコープ外の再構成を混ぜない／ERROR まで 137 行）と、次の Issue への指示（空けること自体を独立した Issue に立てる／候補は背景処理を `ui/discovery` へ寄せる案）を記録した。
  `ui` 直下の全件列挙には `hostreq.go` を含め、既に本体を出した後のアダプタであることを添えた。
- (4) 6 周目の現在形の指示を当時の記録へ改め、**現在の予算と指示は 7 周目が持つ**と参照を付け替えた（本文 3 箇所）。
- (5) 「ファイルの行数」の次点を実測へ（`app.go` 298 行・`runners_test.go` 297 行）

**変更理由**

Issue #128 が親の `cfg` を差し替えるようにしても、設定ファイルの 4 つの編集可能項目のうち `scan_roots` と `audit_log` は `cfg` を経由しておらず、**設定ファイルには書けているのに再起動まで反映されない**ままだった。
利用者から見れば #128 と同じ「書いた値が効かない」体験である。
本書は 1.67 でこの 2 つを「この経路では直らない」と記していたので、片方を載せ替え片方を明示的に見送った判断を書き換えないと、記述が実装と正面から食い違う。
行数表は次の Issue が予算を引く唯一の材料であり、**警告帯に入った事実と入ってよいと判断した理由**を残さないと、次の Issue が同じ逃げ道を根拠なく使う。**表だけを直して散文を取り残す欠陥は 1.69 / 1.70 / 1.71 で 3 周続けて再発しており、本項もそれを踏んだ。** 1.70 が定めた手順（現在形で行数を主張する箇所を機械的に全件抽出して実測と突き合わせる）は、**本項の時点では通していない**——実際に突き合わせたのは 2 つの表と Config タブの散文 2 箇所だけで、同じ Config タブの節の 3 箇所目（「次にこのタブへ手を入れる Issue が使えるのは 168 行」。
実測の残りは 20 行で **148 行過大**）と、`internal/disk` の節末尾（「結果は 1927 行（残り 73 行）である」。
実測 1980 行・残り 20 行で **53 行過大**）を取り落としていた。
前者は改訂 1.73 が、後者は改訂 1.74 が訂正している。**実施していない検証を実施したと書くと、次の周が「もう全件見た」と信じて同じ取り落としを繰り返す**——本欄が当初その主張を持っていたこと自体が、取り落としの訂正に 1.73 / 1.74 の 2 周を要した原因である（訂正は改訂 1.75。
Issue #132）
...
PR #144（Issue #136）のレビュー指摘（MAJOR 4 件）を反映。

- (1) UI 層の行数表で `ui/chrome`（490 行）が `ui/page/configmodal`（453 行）の直後に置かれ降順が崩れていたのを、`ui/page/disk/cleanview`（491 行）と `ui/page/configmodal`（453 行）の間へ移した（表の直前の散文が「行数の多い順に並べる」と定めている）。
- (2) 改訂 1.76 の「回帰テスト 40 行」を実測の **42 行**へ直した（同じ文書の「行数の予算」節は 42 行と書いており、1 つの文書が 2 つの値を主張していた。
  実測は 2063 → 2105 の +42 行）。
- (3) **「`chrome_test.go` が 1 ファイル 300 行を超えたので分けた」という存在しない事実を、改訂 1.76 の欄と `internal/ui/chrome/statuswidth_test.go` の doc コメントの 2 箇所とも書き直した**——同ファイルは 206 行で本 PR は 1 行も変えていない。
  実際は「この検証 114 行を `chrome_test.go` へ足すと 320 行となり警告帯（301〜330 行）に入るため、初めから別ファイルに置いた」である（コメントは行数を変えないよう同じ 5 行に収めた）。
- (4) 7 周目（Issue #132）の根拠 3 の現在形「まだ 137 行の余裕がある」を、`ui` が 2063 行だった当時の値である旨と現在値（**95 行**）およびその出所への参照を添えた過去形へ改めた（改訂 1.75(2) が `ui/page/runners` の 2164 行に施したのと同型の処置）。
  あわせて構成を是正し、系列の外（UI 層の行数表の直後）にあった Issue #136 の判断の段落を **`##### 8 周目の判断（Issue #136）`** として 7 周目の節の直後へ移した（6 周目 → 7 周目 の連番の系列に載せた）。
  本書の作法（改訂 1.72(4)）に従い、7 周目の現在形の指示「次に `ui` 直下へ足す Issue へ。
  …」を当時の記録へ改め、予算の引き継ぎを指す 4 箇所の相互参照（「残りが 1 桁のディレクトリ」の段落・6 周目の末尾・Issue #8 当時の記録・Issue #77 当時の記録）を「7 周目の判断」から「8 周目の判断」へ付け替えた（1 つ目は節より前にあるため「上記」を「下記」へ直した）。
  同段落が現在形で主張していた `ui` の 2063 行・残り -63 行も 2105 行・残り -105 行へ直した。**全件突き合わせを実際に通した結果は次のとおり。** `go tool linterly check --format json` の実測（82 ディレクトリ・622 ファイル）と、UI 層の表 38 行・UI 層の外の表 37 行のすべての行数・残り・判定・網羅性・降順を機械的に突き合わせ、**食い違いは上記 (1) の降順 1 件のみ**で数値・判定・網羅性は全件一致した。
  現在形で行数を主張する散文は 14 箇所を抽出して照合し、**食い違いは上記 (4) の 137 行と 2063 行・残り -63 行の 2 箇所のみ**——`internal/disk` 1980 行・残り 20 行、`internal/setup` 1970 行・残り 30 行、`ui/page/config` 残り 20 行、`ui/page/disk` 1996 行・残り 4 行、`ui/page/logs` 残り 6 行、`page/pagetest` 1936 行・残り 64 行、`page/disk/cleanview` 491 行、`ui/page/setup` 1883 行・残り 117 行、`molecule/listrow` 残り 477 行、`ui/token` 残り 783 行、`ui/tabset` 現在 625 行、`ui/chrome` 現在 490 行は実測と一致していた。
  「ファイルの行数」節の 3 つの主張（`internal/ui/app_test.go` 300 行・`internal/ui/app.go` 298 行・`internal/ui/page/runners/runners_test.go` 297 行、WARN 以上のファイルは無い）も実測と一致。
  `docs/ui/screens.md` は行数の主張を持たず、状態行の中略に関する記述と改訂 1.33 は実装（`internal/ui/chrome` の `Status`）と一致していたため変更なし。

**変更理由**

4 件はいずれも**危険側**の誤りである。
(4) は残り 95 行しかないところへ 137 行の予算を提示しており、次に `ui` 直下へ足す Issue が 42 行ぶん多い余裕を前提に判断して ERROR 境界（2200 行）を踏む。
(2) は同じ文書が同じ増分に 40 と 42 の 2 つの値を主張する形で、改訂 1.21 / 1.40 / 1.58 / 1.60 が 4 度是正した欠陥の再発である。
(3) は**実施していない事実を実施したと書く**もので、本書が 1.74 / 1.75 で 2 度是正した欠陥そのもの——「300 行を超えたら分ける」という前例として読まれると、次の Issue は超えるまで分けないことになり、実際の判断（超える前に別ファイルへ置く）とは逆の運用を導く。
(1) は表の直前の散文が自ら定めた並びを表が破っているため、どこが逼迫しているかを表から読み取れない（改訂 1.57 / 1.60 が同じ理由で 2 度直している）。
構成の是正は、#136 の判断だけが 6 周目 → 7 周目 の系列の外にあったことが 7 周目の「137 行」を取り残す一因だったためで、系列に載せないと次の周も同じく前周の現在形を取り残す（PR #144 のレビュー指摘）

#### 改訂 1.78

**変更内容**

`make check` の並列負荷で config の自己設定テストが散発的に落ちる件（Issue #140）を反映。

- (1) 行数表を実測へ更新した——`ui/page/pagetest` 1936 → **2121 行・残り -121 行・warn**（`ui` の上へ移した）。**`ui/page/config` を警告帯へ入れたのは本 Issue ではない**——#140 の 2 コミット後は 1989 行・残り 11 行の pass で、警告帯へ入れたのは同じ PR の Issue #141 である（改訂 1.79）。
- (2) **表と同時に散文も直した**——現在形で行数を主張する箇所を全件抽出して照合し、この 2 つのディレクトリに触れる 7 箇所すべてを更新した（「残りが 1 桁のディレクトリ」の段落、Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「使えるのは 20 行しかない」）、Disk タブの節の受け皿の余裕（「そちらも残り 64 行」）、Logs タブの節の受け皿の余裕（「1936 行・残り 64 行」）、`ui` 直下 5 周目の記録（「現在 1936 行で、残りは 64 行しかない」））。
  他のディレクトリの数値は本 PR が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した。
- (3) 「Issue #140 の判断——警告帯へ入れて回帰テストを足した」を Config タブの節へ新設し、**本節冒頭の (3)（テストの道具を別ディレクトリの通常パッケージへ出す）を実際に採ったこと**——束を辿る道具を `ui/page/config` の `findMsg` から `page/pagetest` の `FindMsg` へ回帰テストごと移し、`ui/page/config` を 2119 行から 1989 行（残り 11 行・pass）へ下げた——と、それでも `page/pagetest` が警告帯に残る理由（合計は減らないので別パッケージの新設が要るが、それはバグ修正の範囲外）、および空けること自体を Issue #147 へ切り出したことを記録した

**変更理由**

本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、**`ui/page/pagetest` を警告帯へ入れた**のは本 Issue である（`ui/page/config` を入れたのは同じ PR の Issue #141。
改訂 1.79）。
表を据え置くと、次に `ui/page/pagetest` へ手を入れる Issue は残り 64 行のつもりで 185 行ぶん過大な予算を引く。
`page/pagetest` の逼迫はとりわけ危うい——Logs / Disk の両節が「詰まったら道具をそこへ出せ」と受け皿として名指ししており、その受け皿が本 PR の移動で**最も逼迫したディレクトリになった**事実を書かないと、次の Issue はもう存在しない逃げ道を当てにして計画を立てる。
表だけを直して散文を取り残す欠陥は 1.69〜1.72 で 4 周続けて再発しているため、本項でも全件突き合わせを実際に通し、その範囲と結果を上記 (2) に具体的に書いた。**なお本項は当初「`findMsg` は `doneMsg` に依存するので `page/pagetest` へ出せない」と書いていたが、これは事実に反していた**——`findMsg` の引数は `tea.Cmd` と述語だけで、`doneMsg` に依存するのは述語を渡す `doneOf` の側である。
レビューでこの誤りが判明したため、記述を直すのではなく**実際に移した**（Issue #140）

#### 改訂 1.79

**変更内容**

同じ PR の Issue #141（自身の設定フォームの見出しを `firstRunUnsaved` で決める）/ #142（対象選択の見出しを runner に限定しない文言へ）を行数表と散文へ反映した——`ui/page/config` **1989 → 2098 行・残り -98 行・warn**（#141 が +66 行、#142 が +43 行）。**`ui/page/config` を警告帯へ入れたのはこの 2 件のうち #141 である**（Issue #140 の 2 コミット後は 1989 行・残り 11 行の pass だった）。
降順の位置は `ui` の下のまま変わらない。
散文は Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「ERROR の境界まで 145 行」→ **102 行**）と「残りが 1 桁のディレクトリ」の段落を直し、「Issue #140 / #141 / #142 の判断」の節へコミット単位の実測表を置いて、どの Issue がどちらのディレクトリを警告帯へ入れたかを分けて書いた。
`title_test.go`（**92 行**。
#141 が 57 行、#142 が 35 行）を新設したのは、**#141 の検証だけでも `selfconf_test.go` が 307 行となり 1 ファイル 300 行の警告帯（301〜330 行）に入るため**である。
#142 の検証（35 行）も足した現在、1 ファイルに置いていれば 342 行となり ERROR 境界（330 行）を超えていた（実際は `selfconf_test.go` 266 行 + `title_test.go` 92 行に分かれており、どちらも警告帯に入っていない）。
境界は行数だけでなく責務でも切ってある（`selfconf_test.go` は「保存の経路が何を書くか」、`title_test.go` は「画面が何と名乗るか」）。
`ui/page/pagetest`（2121 行）と `ui`（2105 行）は #141 / #142 が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した

**変更理由**

本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と定めている。**同じ PR の中で行数が 3 度動いたのに、当初の記述は増分を Issue ごとに正しく割り振れていなかった**——#140 に +75 行、#141 / #142 に +43 行と書いていたが、実測は #140 +9 行・#141 +66 行・#142 +43 行である。
#140 は道具と回帰テストを `page/pagetest` へ出したので `ui/page/config` をほとんど増やしておらず、**警告帯へ入れた Issue を取り違えたまま残すと、次に予算を引く Issue が「バグ修正が config を埋めた」という誤った前例を読む**。
`title_test.go` の「43 行」も同様の取り違えで、43 は #142 単独のディレクトリ増分であってファイルの行数ではない。
ファイルの行数を誤ると、分割の前例として引かれたときに判断の根拠が崩れる——改訂 1.77(3) が「300 行を超えたので分けた」という存在しない事実を是正したのと同型の欠陥である（レビュー指摘。
Issue #141 / #142）
...
Issue #139（`ui` 直下の分割）を反映。

- (1) 行数表を実測へ更新（`ui` **2105 → 1953 行・残り 47 行・pass**、`ui/discovery` 290 → **617**、`ui/hostreq` 283 → **393**）。
  3 行とも降順の位置を移した（`ui` は `ui/organism/table` 1969 と `ui/page/setup` 1883 の間、`ui/discovery` は `ui/tabset` 625 と `ui/molecule/chromebar` 543 の間、`ui/hostreq` は `ui/page/configmodal` 453 と `ui/page/diskclean` 372 の間）。
- (2) **`##### 9 周目の空け方（Issue #139）——警告帯から出た`** を新設し、打った 2 手（検出周期の状態を `discovery.State` へ／起動時の前提チェックを `hostreq.State` へ）、`tickMsg` を公開型 `discovery.TickMsg` へ反転させた設計判断とその 2 つの理由、代替の import 検査（`TestOnlyParentDrivesDiscovery`）を**採らずに散文の記録に留めた**理由（`page/pagetest` が残り 64 行しかなく、行数を空ける Issue が別のディレクトリを逼迫させる）、`refresh_test.go`（80 行）を消しても表明が落ちていない根拠（2 本は `discovery/state_test.go` へ移送し `OnTick` が busy でも `TickMsg` を返すことまで強化、1 本は `gate_test.go` の `TestRefreshDuringDiscoveryShowsNotice` が真上位集合として包含し包含側に注記を追加）、次の Issue への予算（残り 47 行・ERROR 境界まで 247 行）と、(1) 以外の手が尽きている旨を記録した。
- (3) **8 周目の現在形の指示（「ERROR 境界 2200 まで 95 行である」「次に足す Issue はこの 95 行を予算として読むこと」）を当時の記録へ改め**、予算の引き継ぎを指す 5 箇所の相互参照（「残りが 1 桁のディレクトリ」の段落・6 周目の末尾・Issue #8 当時の記録・Issue #77 当時の記録・7 周目の根拠 3 と末尾）を「8 周目の判断」から「9 周目の空け方」へ付け替えた。
- (4) 「ファイルの行数」の散文を実測へ——`internal/ui/app_test.go` の「上限ちょうどの 300 行（残り 0）で張り付いている」は 274 行になったので落とし、最大が `internal/ui/page/runners/runners_test.go` の 297 行、`ui` 直下の最大が `app.go` の 286 行であることへ書き換えた。
  6 周目の「本 PR で `app_test.go` は 300 行になった」にも当時の値である旨を添えた（`page/` の 1998 行・残り 2 行は現在も一致するのでそのまま）。
- (5) ディレクトリ構成のツリーを実ファイルへそろえた——`app.go` の説明から「Tick」を落とし（`discovery` へ移った）、`discover.go` を「検出の駆動のうち親にしか決められないもの」へ、`discovery/` を「周期の状態（State と契機 TickMsg）」を持つ形へ、`hostreq/` を「その状態（State）」を持つ形へ書き換え、`chrome.go` に `doctorKey()` の移動を反映した（`hostreq.go` は元々ツリーに無いため削除に伴う行の除去は不要だった）。
- (6) 実装状況の「実装済み」の行で `discovery` / `workscan` / `ghscope` の括弧書きを、`hostreq` を含む 4 つが `State` を持つ形にそろったことへ更新。
- (7) Issue #77 当時の記録が挙げていた `gate_test.go` の非公開参照（`after.inflight` / `tickMsg`）に、現在の姿（`disc.Busy()` / `disc.Seq()` / `discovery.TickMsg`）を添えた。**全件突き合わせを実際に通した結果は次のとおり。** `go tool linterly check --format json` の実測（82 ディレクトリ・625 ファイル）と、UI 層の表 38 行・UI 層の外の表 37 行のすべての行数・残り・判定・網羅性・降順を機械的に突き合わせ、**食い違いは上記 (1) の 3 行のみ**で、残る 72 行は数値・判定とも一致し、両表とも降順は崩れておらず、`internal/` と `cmd/` の行を持つディレクトリで表に載っていないものは無かった。
  行数を主張する散文（過去形の記録を含む）は 39 箇所を抽出して照合し、**実測と食い違っていたのは 5 箇所（数値としては 7 個）だけである**——「残りが 1 桁のディレクトリ」の段落の `ui` 2105 行・残り -105 行、「ファイルの行数」の `app_test.go` 300 行と `app.go` 298 行、6 周目の `app_test.go` 300 行、7 周目の「現在の余裕は 95 行」、8 周目の「2200 まで 95 行」である。
  残る 34 箇所——`internal/setup/tarball` 残り 1 行、`internal/logs` 残り 2 行、`internal/gh` 残り 4 行、`internal/disk` 1980 行・残り 20 行、`internal/setup` 1970 行・残り 30 行、`ui/page` 1998 行・残り 2 行、`ui/page/disk` 1996 行・残り 4 行、`ui/organism/dialog` / `ui/page/logs` 各 残り 6 行、`ui/page/runners` 1983 行・残り 17 行、`ui/page/config` 1980 行・残り 20 行、`ui/organism/table` 1969 行・残り 31 行（本体 6 ファイル 1053 行）、`page/pagetest` 1936 行・残り 64 行、`ui/page/setup` 1883 行・残り 117 行、`molecule/listrow` 残り 477 行、`ui/token` 残り 783 行、`ui/tabset` 625 行、`ui/chrome` 490 行、`page/disk/cleanview` 491 行、`organism/table/tabletest` 243 行はいずれも実測と一致していた。**依存グラフの裏取りも通した**——`go list` の直接 import で `ui/discovery` → `runner` / `exec` のみ（`appconfig` の辺は増えていない。
  `discovery.Input` はプリミティブだけを持つ）、`ui/hostreq` → `doctor` のみであることを確認し、[コンポーネント設計](../components/overview.md#依存の方向)のグラフに追記の要る辺は無かった。

**変更理由**

**`ui` 直下は 7 周目・8 周目と 2 周続けて「判断を残して警告帯へ入る」逃げ道を使っており、Issue #139 はその借りを返す周である。** 行数表は次の Issue が予算を引く唯一の材料であり、実測とずれていると境界に当たってから気付くことになる——**今回は危険側ではなく安全側のずれ（実際には 247 行あるのに 95 行と読ませる）だが、それでも誤りである**。
予算を過小に見せると、47 行で足りる追加までもが「まず空けてから」という不要な前置きを負い、逆に 7 周目・8 周目が実際にそうしたように「どうせ警告帯だから」と分割の検討を省く根拠にもなる。
あわせて **Issue #139 は #77 の再発である**（#77 も `ui` 直下が超過して空けた周で、当時の結論は「次に手を入れる Issue は 1 行足す前に必ず空けること」だった）。
同じディレクトリが 5 周（#77 → #128 → #132 → #136 → #139）にわたって上限へ張り付き続けているのは、**現在形の予算がどの周にあるかを読み手が辿れないと、前周の指示が古いまま参照される**ためであり、改訂 1.69 が定めた「次の Issue への現在形の指示は最新の周に一本化する」規範を本項でも守った（上記 (3)）。
散文を取り残す欠陥は 1.69〜1.75 で 4 周繰り返しているため、1.70 が定めた全件抽出の手順を実際に通し、その結果を上欄に列挙した（Issue #139）

#### 改訂 1.81

**変更内容**

PR #146（Issue #139）のレビュー指摘（MAJOR 4 件）を反映。**4 件は同一の欠陥ではなく 2 つの型に分かれる**——

- (1)
- (2) は**本 PR が削除した識別子を散文が現在形で名指ししていた**もの、
- (3)
- (4) は**実在するものの列挙が足りていなかった**もの（名指しされた名前はいずれも実在し、落ちていた名前のほうが問題である）で、後者は識別子の実在確認では検出できない（改訂 1.82 で手順を補った）。
  (1) 例外 3 の「この経路で直らないものは他にもある」の段落で、間隔の解決を `discover.go` の `refresh()` が呼ぶ形としていたのを、現在の担い手である `input()` の `Every: discovery.Interval(a.opts.Refresh, a.cfg.RefreshDuration())` へ、`scan_depth` の `discover()` の `depth` を同じ `input()` の `Depth: a.cfg.ScanDepth` へ直した（主張の中身——`--refresh` 指定時はフラグ優先／`scan_depth` は自身の設定フォームに欄が無いので値そのものが変わらない——は実装と一致しているので経路の名前だけを現在の姿へ改めた）。
  (2) 同節の Issue #128 の根拠「走行中の検出は中断されない」で、次の周期を予約するのが `discover.go` の `tick` で間隔を `refresh()` から読むとしていたのを、`discovery.State.OnTick` が `discovery.Tick(in.Every)` で予約し、その `in` を `discover.go` の `input()` が周期ごとに組み立て直す形へ辿れるように直した。
  抑止の主体も `onTick` から `discovery.State.Start`（実行中なら `nil` を返す）へ改めた（結論は維持）。
  (3) 「9 周目の空け方」の手 1 の「残ったのは `input()` と `scanRoots()` だけである」を実測の 4 関数（`onTick()` / `applyDiscovered()` / `input()` / `scanRoots()`）へ書き直し、`applyDiscovered()` が本体 23 行（doc コメント込みで 29 行）で `Outcome` の判定・`distribute()`・`startBackground()` の束ねを持つこと、それが `App` の非公開に触れるため親に残っていることを明記した。
  (4) 同節の「次の Issue への予算は残り 47 行」の段落が挙げる `ui` 直下の本番ファイルに `background.go` が落ちていたのを、実測の 5 ファイル（`app.go` / `background.go` / `chrome.go` / `discover.go` / `keys.go`）へ直した（7 周目の同種の列挙は `background.go` を含む 6 つを挙げており、`hostreq.go` の削除に伴って 1 つ減らす際に取り落としたものである。
  結論——いずれも `App` の非公開な状態を読み書きするので切り出せない——は `background.go` にも成立する）。
  あわせて**再発防止として「9 周目の空け方」の末尾に次の周への手順を追記した**——行数の主張だけでなく、関数名・型名・ファイル名を現在形で名指ししている散文も機械的に洗い出して実在を確認すること（抽出コマンドの例を添えた）。
  検証として削除済みの識別子（`tickMsg` / `tick` / `firstTick` / `discover()` / `refresh()` / `hostreq.go` / `hostReq` / `hostReqDone` / `hostChecks` / `inflight` / `applied` / `seq` / `hostreq.StartOnce`）で本書を機械的に grep し、残るヒットが「N 周目の空け方」「当時の記録」など過去の周の記録に限られることを確認した。

**変更理由**

本書は**行数表だけを実測へ直して散文を取り残す**欠陥を改訂 1.69〜1.75 で 4 周繰り返しており、今回はその**コード記号版**である——1.80 が実際に通した全件突き合わせは**行数の主張だけ**を対象にしていたため、同じ PR が削除した関数名を名指しする散文は抽出の網に掛からなかった。
(3)(4) に至っては 1.80 自身が書いた行で生じており、手順が対象を行数に限っている限り毎周同じ形で再発する。
害は読み手が経路を辿れないことに留まらない——(3) は次の Issue が「本番の切り出し候補」を探すために読む箇所で、親が保持する検出まわりの責務（`applyDiscovered` の本体 23 行）を実際より小さく見せ、(4) は「次に `ui` 直下へ足す Issue はこの 47 行を予算として読むこと」と明示された唯一の引き継ぎ材料でありながら切り出せないファイルの列挙を 1 つ欠いていた。
どちらも次の周の判断材料を欠けたまま渡す形である（PR #146 のレビュー指摘）

#### 改訂 1.82

**変更内容**

PR #146（Issue #139）2 周目のレビュー指摘（MAJOR 3 件）を反映。**3 件とも改訂 1.81 自身が持ち込んだものである。**

- (1) 「9 周目の空け方」の手 1 と改訂 1.81 の欄が `applyDiscovered()` を実測にない **35 行**という概数で書いていたのを実測へ直した（`internal/ui/discover.go` の 24〜46 行=**本体 23 行**、doc コメント（18 行目から）込みで **29 行**。
  約 52% の過大表示だった）。
  数え方が読み取れるよう「本体 23 行・doc コメント込みで 29 行」と併記してある。
- (2) 改訂 1.81 が再発防止として添えた抽出コマンド ``grep -oE '`[a-zA-Z_][a-zA-Z0-9_]*\(\)`' …`` は**末尾が `()` の語しか拾えず**、防ごうとしていた当の識別子（`tick` / `tickMsg` / `firstTick` / `inflight`）もファイル名（`hostreq.go`）もパッケージ名を接頭辞に持つ型・メソッド（`hostreq.StartOnce` / `discovery.State.Start`）も 1 件も拾えなかった（散文は「同じ形で拾える」と述べていた）。
  末尾の `()` を任意にし `.` を語に含める形（``grep -oE '`[A-Za-z_][A-Za-z0-9_.]*(\(\))?`' docs/ui/atomic-design.md | tr -d '`' | sort -u``）へ差し替え、9 件すべてを拾えることを実測で確認した。
  本書全体では一意な識別子が **875 個**挙がるので、**現在形の散文が名指ししているものに絞って `grep -rn` で実在を確認する**手順を添えた。
- (3) 1 周目の指摘 4 件を「いずれも削除済み識別子の名指しという同一の欠陥」と括っていたのを、実測の内訳どおり **2 つの型**（
  - (a) 削除済み識別子の名指し=2 件／
  - (b) 列挙の不足・脱落=2 件）へ分けて書き直し（本節末尾と改訂 1.81 の欄の 2 箇所）、**型 (b) を検出する手順を追記した**——ファイルやシンボルを**列挙**している散文は列挙そのものを実測と突き合わせ、実測の側を先に採ること（`ls internal/ui/*.go | grep -v _test` で 5 ファイル、`grep -n '^func' internal/ui/discover.go` で 4 関数）。
    あわせて型 (b) の未修正の実例だったディレクトリ構成のツリーの `discover.go` の説明（「1 周期分の入力の組み立てと走査ルートの合成」だけを挙げ、実在する `onTick()` と `applyDiscovered()` を落としていた）を実測 4 関数と整合する形へ直した

**変更理由**

- (1) は本書が改訂 1.70 / 1.80 で自ら定めた「行数の主張は実測で管理する」規範を破っており、しかも 1.81 は旧記述を「親の責務を実際より小さく見せる」と批判したうえで**今度は逆方向へ 12 行外している**——次の Issue が切り出し候補を評価する際の見積りが狂う。
- (2) は**再発防止の手順が防ごうとしている対象を 1 件も拾えない**もので、そのまま次の周が実行すると「洗い出した」という誤った安心だけが残る（1.81 の欄が名指しした `tick` すら網に掛からない）。
- (3) は原因の認識そのものが事実と食い違っており、1 周目の指摘の半分（`discover.go` の関数の列挙不足・`background.go` の脱落）は「名指しした識別子を実在確認する」手順を通しても検出できない——**行数の主張と識別子の名指しに加えて列挙の突き合わせまで手順に含めない限り、同じ形で再発する**。
  ツリーの 1 件は単独なら軽微だが、手 1 を実測 4 関数へ直した結果**同一文書内で `discover.go` の中身について 2 つの説明が並立**しており、(3) が「新しい手順でも検出できない型の実例」として名指しした箇所でもあるため、直さないと (3) の是正が主張だけになる（PR #146 のレビュー指摘）

#### 改訂 1.85

**変更内容**

Issue #147（`ui/page/config` と `ui/page/pagetest` を行数の警告帯から出し、束を辿る道具を 1 つに寄せる）を反映。

- (1) **`##### Issue #147 の空け方——警告帯の 3 つを全部出した`** を新設し、打った 3 手（`page/pagetest/cmdtest` / `page/config/itemview` / `page/logs/filerow`）と実測、Issue が挙げていた候補（`selfconf.go`）を**採らなかった理由**（5 つとも `Model` のメソッドで、出せば親の非公開を export することになる）、`FindMsg` と `Pump` を**辿り方だけ**寄せた理由、**`FindMsg` に打ち切りのガードを付けなかった理由**（キューが伸びる経路が無い）、フィクスチャ同士の import を検査の対象外にした理由を記録した。
- (2) 行数表を実測へ更新した（`ui/page/pagetest` 2197 → **1564**、`ui/page/config` 2098 → **1990**、`ui/page/logs` 2025 → **1927**、`ui` → **1925**。
  新規に `ui/page/pagetest/cmdtest` 687 / `ui/page/config/itemview` 256 / `ui/page/logs/filerow` 233 / `ui/startup` 176 を追加）。
- (3) 「残りが 1 桁のディレクトリ」の段落を実測へ書き直した（**警告帯のディレクトリは 0 件**、残り 1 桁は 6 つ）。
- (4) Issue #140 / #141 / #142 の節の「空けること自体は Issue #147 に立ててある」を実施済みの記録へ改め、候補と実際に採った手の食い違い（`selfconf.go` → `items.go`）を明記した。
- (5) `ui/page/logs` の節の現在形（「残りは 6 行しかない」「受け皿はもう空いていない」）を当時の記録へ改め、現在値と受け皿が空いたことを添えた。
- (6) ディレクトリ構成のツリー・実装状況の「実装済み」の行・空け方の手 (1)(3) の実例に、新設した 3 パッケージと `ui/startup` を加えた。
- (7) 10 周目の実測を最終値へ直した（`ui` 1922 → **1925**、残り 78 → **75**。
  Issue #147 の `cmdtest` の import が `ui` 直下のテスト 3 ファイルに 1 行ずつ増えたぶんである）

**変更理由**

本節は「次にこの階層へ手を入れる Issue が置き場所と予算を決める唯一の材料」であり、**3 つのディレクトリが警告帯から出た周でこそ更新を落とせない**——古い表を読んだ次の Issue は、実際には 436 行空いている `page/pagetest` を「受け皿はもう空いていない」と読んで別の受け皿を探す。
(4)(5) は本書が改訂 1.81 で型 (a)（削除済み識別子の名指し）と型 (b)（列挙の不足）として分類した欠陥のうち、**現在形の指示が古いまま残る**型そのもので、1.69 が定めた「次の Issue への現在形の指示は最新の周に一本化する」規範に従って処置した。
(7) は 10 周目の欄が同じ PR の後続コミットで動いた値を据え置いていたもので、予算を 3 行多く見せる危険側のずれだった（Issue #147）

#### 改訂 1.86

**変更内容**

Issue #150（`RunAll` / `Msgs` / `ChromeOf` / `IsQuit` が締め切り無しで `Cmd` を走らせる）を反映。

- (1) ディレクトリ構成の `page/pagetest/cmdtest/` の道具の列挙へ、本 PR が中心に据えた `MustMsgs` / `RunAll` / `ChromeOf` を追加した。
- (2) 「行数の予算」の表を実測へ更新し、降順の不変条件が崩れていた 2 行を並べ直した（`ui` **1925 → 1943**・残り 57 で `ui/page/logs` の上へ、`page/pagetest` **1564 → 1677**・残り 323 で `ui/page/doctor` の上へ。
  ほかに `page/config` 1990 → **1992**・残り 8、`page/runners` 1989 → **1991**・残り 9、`page/setup` 1886 → **1890**・残り 110、`cmdtest` 687 → **992**）。
- (3) 「残りが 1 桁のディレクトリ」を **6 つから 8 つ**へ改め、本 PR が落とした `page/config`（8）と `page/runners`（9）を列挙へ加えた。
- (4) 「ファイルの行数」の最大ファイルの一覧から `cmdtest/run.go`（292 行）を外し、`run.go` 193 行 / `find.go` 158 行という現在値へ差し替えた。
- (5) 「前例」へ本 PR の `run.go` → `find.go` の分割を追記した。
- (6) 現在形で古い値を主張していた散文を実測へそろえた（`page/config` の残り 10 → 8、`page/setup` の 1886 → 1890、`page/pagetest` の 1564・残り 436 → 1677・残り 323 の 2 箇所、`ui` 直下の残り 75・ERROR まで 275 → 57・257、`page/runners` の 1989・残り 11 → 1991・残り 9 の 3 箇所）。
- (7) `ApplyChrome` と `ScanKey` が `Msgs` を使うという記述を **`MustMsgs`** へ訂正した

**変更理由**

本 PR は 158 行の新規ファイル（`find.go`）と `cmdtest` へ 413 行を足しながら、配置と予算の判断の唯一の拠り所である本書を更新していなかった。
行数表は 6 ディレクトリが実測とずれ、うち `page/config` と `page/runners` は**本 PR 自身が残り 1 桁の帯へ落としたのに**「この N 個に 1 行でも足す Issue は足す前に空けること」の列挙から漏れており、次の Issue が予算を読み違える状態だった。
表が自ら宣言する降順の並びも 2 行で破れていた。
`find.go:16` は分割の根拠として本書の「ファイルの行数」を指しているのに、その節の一覧は分割前の `run.go` 292 行を現在値として載せ、「前例」には本 PR の分割が無く、**コードが存在しない記録を指す**状態だった。
`ApplyChrome` / `ScanKey` の 2 箇所は、本 PR が `Msgs` から `MustMsgs`（待ち時間切れで panic）へ替えたことで、諦めた分が黙って落ちるか否かという**壊れ方の違いを取り違えさせる**記述になっていた（PR #154 のレビュー指摘）

#### 改訂 1.87

**変更内容**

PR #154 の 2 巡目のレビュー指摘（CRITICAL 1 件・MAJOR 4 件・MINOR 4 件）を反映。

- (1) **10 周目の予算段落の自己矛盾を直した**——見出しの「残り 57 行」に対し、同じ段落の**指示の側**が「次に 75 行を超える追加が要る Issue は…」と改訂 1.86 以前の値のままだった。
  予算ではなく指示を読む次の Issue が 75 行を足すと `ui` は 2018 行の警告帯に入る——節が防ごうとしているものそのものである。
  9 周目の同型の文（予算と指示がどちらも 47 行）に合わせて **57** へそろえた。
- (2) 改訂 1.86 が見落とした現在形の古い値 4 箇所を実測へそろえた（`page/config` の 1990 行・残り 10 行 → **1992 行・残り 8 行** が 2 箇所、`page/disk` の 1996 行・残り 4 行 → **1997 行・残り 3 行**、`page/runners` の 1983 行 → **1991 行**）。
  `page/disk` は同じ文書の別の箇所（「残り 3 行」）と食い違っており、**1 つの文書が 2 つの値を主張していた**。
  過去形の記録（Issue #147 当時の 1990 行）は当時の値として残した。
- (3) `ui` 直下の最大ファイルの現在値を **286 → 284 行**へ直した（改訂 1.86 が「ファイルの行数」節では 284 へ直しながら、9 周目の節の同じ主張を残していた）。
  同じ文の `app_test.go` 274 行は **当時の値として残した**——文が「前者は当時の値である」と明示しており、`git show 78a4d1c` で Issue #139 当時に実測 274 行であることを確認した（現在値は上記「ファイルの行数」を見よ）。
  現在値へ書き換えると「ファイルの行数」節の同じ記録と新たに食い違う。
- (4) 改訂 1.86 の変更理由の 2 つの数を実測へ直した（`find.go` **157 → 158 行**——同じ行の項目 (4) と実測はどちらも 158 で、1 行の中で 2 つの値を主張していた。
  `cmdtest` の増分は「約 253 行」の概数をやめ、origin/feat/#1 の 687 行からの純増 **406 行**とした）。
- (5) 「ファイルの行数」の `cmdtest` のテストの内訳を **`run_test.go` 161 行 / `cmd_test.go` 151 行 / `find_test.go` 141 行**へ更新した（本 PR が `find.go` の検証を `find_test.go` へ分けた）。
- (6) 行数表を実測へ更新した（`page/pagetest` 1677 → **1763**・残り 237 で `ui/page/jobs` の上へ移し降順を保った。
  `cmdtest` 992 → **1100**・残り 900）。
  あわせて 10 周目の節の `page/pagetest` の現在値も 1677・残り 323 → **1763・残り 237** へそろえた

**変更理由**

予算段落の

- (1) は**指示として読まれる文が予算と食い違う**という点で、単なる古い数値より害が大きい——次の Issue は見出しの残り行数ではなく「次に N 行を超える追加が要るなら」の N を自分の上限として読むためである。
- (2) の 3 件は改訂 1.86 が「現在形で行数を主張する散文を全件照合した」と宣言しながら取り残したもので、`page/disk` に至っては同一文書内で 2 つの値が並立していた。
- (3) は同じ型の取り残しで、**同じ commit が片方だけ直して他方を残した**ため、文書が自分自身と食い違う状態が新たに生まれていた。
- (4) の `find.go` 157/158 も 1 行の中での食い違いである。
  本 PR のコード側の修正（`assertNested` が締め切り無しで Cmd を走らせていた点、`IsQuit` の doc がコードと逆を述べていた点、`OpenTabOf` の未検証の error の腕）は本書の記述と直接には関わらないが、
- (5)
- (6) はその修正がファイルとディレクトリの行数を動かしたことへの追従である（PR #154 のレビュー指摘）

#### 改訂 1.88

**変更内容**

Issue #151（依存グラフの辺を `go list` の import と突き合わせる検査の新設）を反映し、「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（1475 → **1969 行・残り 31 行**・pass）。
降順の並びが変わるため、`internal/doctor/jobreq`（1495）の直前から `internal/setup`（1970）と `internal/config/edit`（1922）の間へ移した。
新設したのは 3 ファイル（`docs_depgraph_test.go` 259 行・`docs_depnodes_test.go` 125 行・`docs_mermaid_test.go` 110 行）で、いずれも 1 ファイル 300 行の警告帯（301〜330 行）に入らない。**これも当時の値である。
3 ファイルはその後 Issue #161 が 3 つとも `internal/buildconfig/docscheck/` へ移しており、改訂 1.90 が現在値として書いた 296 行 / 123 行 / 147 行はいずれも古い**（現在値は上記「ファイルの行数」を見よ。
出所は下記「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」。
この訂正は改訂 1.94）。
警告帯に入らないことは現在も変わらない（余裕がどれだけ残っているかも上記「ファイルの行数」が持つ）。**パーサを別ファイルへ分けたのは行数のためだけではなく責務の境界でもある**——`docs_mermaid_test.go` は「mermaid をどう読むか」、`docs_depgraph_test.go` は「読んだ辺を実装とどう突き合わせるか」を持つ（「ファイルの行数」の方針どおり）。
ディレクトリは 2000 行の警告帯には入っておらず残りは 31 行だった。**これは当時の値であり、改訂 1.89 の時点では 2025 行・残り -25 行、改訂 1.91 の時点では 2122 行・残り -122 行の warn だった。**現在は警告帯の外の pass である**（Issue #161 が `docscheck` を切り出して警告帯を出た。
現在値は上記「行数の予算」の行数表を見よ。
出所は下記「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」。
この訂正は改訂 1.94）**（PR #153 の 2 周目が +56 行。
予算と次の Issue への指示は下記「`internal/buildconfig` を警告帯へ入れた判断」が持つ）。
次に `internal/buildconfig` へテストを足す Issue は先に空け方を検討すること（採れるのは**検査そのものをパッケージ境界で別ディレクトリへ分ける**手である——ビルド設定の検査とドキュメントの検査は増え方が違う。
手 (1) は「**本番の一部を**パッケージ境界で切り出す」であり、`internal/buildconfig` は [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)が明記するとおり**実行時のコードを持たないテスト専用パッケージ**（`doc.go` 12 行のほかはすべて `_test.go`）なのでそのままでは当たらない。
理由——増え方が違う——は同じである。
この訂正は改訂 1.89）

**変更理由**

表は「実測値」と明記してあり、**この節が次の Issue の読む行数予算の規範である**。
494 行を足したのに表を据え置くと、次に `buildconfig` へ検査を足す Issue が 494 行ぶん過大な予算を引き、警告帯を越えてから気付くことになる。
表だけを直して散文や並びを取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 で繰り返し再発しているため、今回は降順の位置も同時に直し、`internal/buildconfig` の行数を現在形で主張する散文が他に無いことも確認した（改訂 1.71 の記述は当時の値の記録なので変更しない）（Issue #151）

#### 改訂 1.90

**変更内容**

PR #153 の最終レビュー（変異注入で実測した major 4 件）を反映。

- (1) **改訂 1.88 が現在形で与えていた 3 ファイルの行数（`docs_depgraph_test.go` 259 行・`docs_depnodes_test.go` 125 行・`docs_mermaid_test.go` 110 行）を当時の記録へ改め、現在値（296 行・123 行・147 行。**この 3 つは 1.90 当時の値である。
  改訂 1.91 の分割後は `docs_deppkg_test.go` 196 行・`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行**）と出所への参照を添えた**——改訂 1.89 がディレクトリ値に施したのと同じ処置であり、同じ欄のファイル値だけが取り残されていた。
- (2) 「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（2025 → **2041 行・残り -41 行・warn**）。
  降順の先頭という位置は変わらない。
  表の直後の散文の数値も揃えた。
- (3) 「`internal/buildconfig` を警告帯へ入れた判断」に最終レビューの **+16 行**とその内訳（塞いだ 3 つの「黙って緑になる」経路）を追記し、項 4 の ERROR 境界までの残り（175 → **159 行**）と項 5 のファイル行数（最大 286 → **296 行・残り 4 行**）を実測へ更新した。
  あわせて**`docs_depgraph_test.go` の残り 4 行がディレクトリの残量より先に尽きる**こと、5 行以上足すなら先にこのファイルを責務の境界で分けることを指示として書いた。
- (4) 「1 ファイル 300 行」の次点の一覧に `docs_depgraph_test.go` 296 行を加えた。
- (5) 改訂 1.89 が自らの記録として並べた 5 つの根拠のうち 2 つ（ERROR 境界まで 175 行／最大 286 行）が現在形のままだったので、当時の記録である旨と現在値の出所（改訂 1.90）を添えた。
- (6) 改訂 1.89 の文末が指す改訂番号が `1.86` のままだったのを `1.88` へ直した（ベース取り込みで PR #154 が 1.86 / 1.87 を使ったため #151 側を採番し直した際の取り残し）

**変更理由**

**1 つの文書が同じファイルに 2 つの行数を与えている状態だった**——改訂 1.88 は `docs_depgraph_test.go` を 259 行と現在形で主張し、同じ文書の「`internal/buildconfig` を警告帯へ入れた判断」の項 5 は 286 行と主張していた。
[コンポーネント設計](../components/overview.md)の改訂 1.51 が「1 つの文書が同じ対象に 2 つの数を主張する形で、本書が繰り返し是正してきた欠陥である」と呼ぶ形そのものである。**しかも危険側のずれである**——1.88 を読んだ次の Issue は `docs_depgraph_test.go` の 300 行までの余裕を **41 行**と見積もるが、**実際は 4 行しかない**（改訂 1.85 の (7) が「予算を 3 行多く見せる危険側のずれ」として直したのと同型で、ずれ幅はより大きい）。
改訂 1.89 がディレクトリ値には但し書きを足しながら**同じ欄のファイル値には足していない**ことから、意図的な当時記録ではなく取り残しであることが読み取れる。
(2)(3) は表と判断の節が**次の Issue の読む行数予算の規範である**ため、16 行を足して据え置くと次の周が残量を過大に見積もり、ERROR 境界（2200 行）へ近づいてから気付く。
(5) を放置すると、改訂履歴を索引に使う読み手が存在しない訂正関係を辿る（PR #153 最終レビュー）

#### 改訂 1.91

**変更内容**

PR #153 の最終レビュー 2 巡目（3 観点が**変異注入で実測**した MAJOR 3 件）を反映。

- (1) 「行数の予算」の非 UI の表と「残りが 1 桁のディレクトリ」の段落の `internal/buildconfig` を実測へ更新した（2041 → **2122 行・残り -122 行・warn**。
  表の位置は最上段のままで降順は保たれている）。
- (2) 「`internal/buildconfig` を警告帯へ入れた判断」に **`**最終レビューの 2 巡目**` の段落を新設**し、塞いだ 3 経路（フェンスの重複ガードが CommonMark の許すインデント付き・4 個以上のバッククォートを数え落としていた／例外として名指しした `UIApp --> UIParts` の定数に生存確認が無く改名 1 手で 1 巡目の穴が復活した／散文が「例外は 1 本だけ」と宣言しながら 2 本目を描いても通った）と、フェンスを数える範囲を「## 依存関係」節の中に限ったことを記録した。
  既存の段落は「1 巡目で +16 行して」と当時の記録へ改めた。
- (3) 項 4 の ERROR 境界までの残りを 159 → **78 行**へ、項 5 を**分割後の実態**へ更新した（ディレクトリ全体の最大は `golangci_test.go` 247 行、本 PR の新設分の最大は `docs_deppkg_test.go` **196 行・残り 104 行**。
  `docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行）。**`docs_depgraph_test.go` を責務の境界で 2 つに分けた判断**（296 行・残り 4 行で、例外定数の生存確認と「1 本だけ」の表明を足すと確実に超えた）と、境界を責務でも引いた根拠（図をどう読むか／実装の側を `go list` でどう読むか／読んだ両者をどう突き合わせるか）を同項に残した。
  あわせて項 5 が主張していた「残り 4 行はディレクトリの残量より先に尽きる」という**同じ節の中で矛盾する記述**（同節と表はディレクトリを -41 行、すなわち既に尽きていると書いていた）を、比較対象が ERROR 境界までの余裕であることを明示する形へ直した。
- (4) 改訂 **1.88** のディレクトリ値の停滞を訂正した——同欄は「現在は 2025 行・残り -25 行の warn である」と現在形で主張しており、同じ文書の表と判断の節（2122 行）と食い違っていた。
  あわせて **1.89 (4)** の「現在値（2025 行・残り -25 行）」と **1.90** のファイル行数（296/123/147）にも当時の記録である旨と現在値を添えた。
- (5) 判断の節の「上記 1.」が実際には 2 行下を指していたのを直した

**変更理由**

(4) は**危険側のずれ**である。
改訂履歴を索引に使う次の Issue は残量を -25 と読み、実際より 97 行多い予算を引いて ERROR 境界（2200 行）へ近づいてから気付く。**これは改訂 1.90 が同じ欄の「ファイル値」に対して直したのと同型の欠陥の鏡像**で、1.90 は「1.89 がディレクトリ値には但し書きを足しながらファイル値には足していないことから、意図的な当時記録ではなく取り残しであることが読み取れる」と自ら述べていた——今回はファイル値を更新しながらディレクトリ値の但し書きだけが取り残されていた。**表だけを直して散文や改訂欄を取り残す欠陥は 1.69 / 1.70 / 1.71 / 1.72 / 1.73 に続く再発**であり、本項では現在形で行数を主張する箇所を機械的に全件抽出して実測（`go tool linterly check --format json`）と突き合わせた。

- (1)〜(3) は表が「実測値」と明記しており**この節が次の Issue の読む行数予算の規範である**ため、81 行を足したまま据え置くとその分だけ過大な予算を引かせる。
- (2) の 3 経路はいずれも**この検査群の存在意義（黙って緑にしない）そのもの**に反する穴で、判断の記録に残さないと「1 巡目で塞いだ」という記述だけが残り、次の周が同じ変異を試さない。
- (3) の矛盾は本書が繰り返し是正してきた「1 つの文書が同じ対象に 2 つの数を主張する」形である（Issue #151）
...
- (1) 行数表を実測へ更新した（`ui/page` 1998 → **2003 行・残り -3・warn**、`ui/page/runners` 1991 → **2000 行・残り 0**、`ui/page/pagetest` 1763 → **1932 行・残り 68**、`ui/page/pagetest/cmdtest` 1100 → **1341 行・残り 659**、`ui/page/runnerop` 1400 → **1404 行・残り 596**、`ui` 1943 → **1944 行・残り 56**）。
  降順を保つため `ui/page/runners` を `ui/page/disk` の上へ、`ui/page/pagetest` を `ui/page/logs`（1927 行）の上へ移した。
- (2) 「残りが 1 桁のディレクトリ」の段落を 8 つ → **7 つ**へ改め、警告帯のディレクトリを `internal/buildconfig` の 1 つ → **`internal/buildconfig` と `ui/page` の 2 つ**へそろえた。
- (3) 新しい節「`ui/page` を警告帯へ入れた判断（Issue #155）」を足した——ドレイン停止の走査の継ぎ目（`page.StateMsg.ScanProcs`）は共有状態にしか置けず、着手時点の残りが 2 行だったため doc 付きのフィールド 1 つで警告帯に入る。
  空ける作業は Issue #159 へ切り出した。
- (4) 「ファイルの行数」の `cmdtest` の内訳を実測へ更新した（`run.go` **193 行** / `find.go` 158 → **163 行**、テストは `run_test.go` 161 → **182 行** / `pump_test.go` **204 行**（Issue #157 の新設） / `cmd_test.go` **151 行** / `find_test.go` 141 → **149 行**）。**当初この項は `find.go` / `find_test.go` の 2 つだけを直して「実測へ更新した」と書いていたが、実際には `run_test.go` が 161 行のまま残り、テストファイルの本数も Issue #157 が `pump_test.go` を足して 4 つになったのに「3 つに分かれている」のままだった**（本項の記述をその実態に合わせて書き直し、本文の数値と本数も直した）。
- (5) 同節の「次点は 296 行が 3 つ」から `internal/buildconfig/docs_depgraph_test.go` を外した——改訂 1.91 が責務の境界で 2 つに分けて **136 行**になっており、296 行のままだと**そのファイルに残り 4 行しか無いと読ませる**。
  次点は `runners.go` / `overlay_test.go` の 2 つである。
- (6) 「6 周目の空け方」の「後者の 1998 行・残り 2 行は現在も同じ」を当時の記録へ改め、現在値（**2003 行・残り -3 行・warn**）と本改訂 (3) の節への参照を添えた。
- (7) 「10 周目の空け方」の現在形の予算を実測へそろえた（`ui` の残り 57 → **56 行**、ERROR 境界 2200 までの余裕 257 → **256 行**、受け皿の `page/pagetest` 1763 行・残り 237 行 → **1932 行・残り 68 行**）。
- (8) 「残りが 1 桁のディレクトリ」の段落末の `ui` 直下の現在値を表とそろえた（1943 行・残り 57 行 → **1944 行・残り 56 行**）。
- (9) 本改訂 (3) の節の項 3 が `ui/page/pagetest` の残りを **217 行**と書いていたのを実測（**1932 行・残り 68 行**）へ直した——同じコミットの表が別の数を書いており、1 つの文書が同じ対象に 2 つの数を主張していた。
- (10) **本 PR の残る現在形の行数を全件測り直してそろえた**（`go tool linterly check` と `wc -l`）。
  `page/pagetest` の現在値を **1932 行・残り 68 行**へ統一した 4 箇所（`ui/page/disk` の節の「受け皿は空いた」、`ui/page/logs` の節の「受け皿ごと空けた…使える」、「Issue #31 の周」、「10 周目の空け方」）、`ui/page/runners` の現在値を **2000 行・残り 0 行**へ直した 3 箇所（Config タブの節の「現在 1991 行」、サービス制御の節の 2 箇所）、`ui/page` 直下を **2003 行・残り -3 行の警告帯**へ直した 1 箇所（サービス制御の節の「残り 2 行」）、`ui/tabset` の現在値 635 → **633 行**、`runners` のサービス制御のテスト 3 ファイルの合計 589 → **602 行**、`organism` 一覧の `organism/table` の規模 1700 → **2000 行**、本節の型 (a) の手順が挙げる本書の一意な識別子の総数 875 → **948 個**（同じ抽出コマンドの実測）。
- (11) **助言として読まれる文を残量に合わせて書き直した**——`ui/page/disk` と `ui/page/logs` の 2 節は `page/pagetest` を「空いた／使える」とだけ述べており、68 行という上限を示していなかった。
  `ui/page/runners` の「**残り 9 行は実質ゼロである**」は残量が 0 になったので「上限まで 1 行も無い」へ改めた

**変更理由**

- (1)(4) 行数表は次の Issue が予算を引く唯一の材料であり、実測とずれていると境界に当たってから気付くことになる（改訂 1.47 / 1.86 / 1.87 が同じ理由で更新している）。
- (2) は本節が自ら「この N 個に 1 行でも足す Issue は、足す前に空けること」という**指示として読まれる文**を持つため、対象の集合が実態とずれると次の Issue が空けるべき場所を取り違える（改訂 1.87 の (1) と同型）。
- (3) は本節が定める「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」に従ったものである。**当初この節は「切り出せば循環になる」と書いていたが、これは誤りだった**——`StateMsg` が参照するのは下位パッケージだけで `page` の識別子を 1 つも使わないため、`page/state` への切り出しに循環は生じない。**コンパイラが禁じている**と書くと、実際には開いている選択肢を次の Issue が検討せずに捨てる。
  理由を波及の広さ（数十箇所の改名）へ改め、循環の主張は実際に跨がりのある `Overlay` に限って条件付きで残した。
  (5)〜(9) は**行数を現在形で主張する散文が、同じコミットで直した表と食い違って残っていた**箇所である。
  表だけを直して散文を取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 で繰り返し再発しており、本項では `wc -l` と `go tool linterly check` で全件を測り直した。
  とりわけ (6) と (9) は**危険側のずれ**である——(6) は `ui/page` の予算を +2 行と読ませるが実際は -3 行の警告帯であり、(9) は `page/pagetest` の余裕を実際の 2 倍以上に見せる。
- (4)
- (5) は「実測へ更新した」という改訂欄の記述自体が**実際にやったことより広く**書かれていた例で、改訂履歴を索引に使う読み手が「もう直っている」と信じて元の数を読み続ける。
  (10)(11) は本項の (5)〜(9) と**同じ周で同じ欠陥が残っていた**もので、表と一部の散文だけを直して残りを取り残す形（改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 に続く再発）である。
  とりわけ `ui/page/runners` の「残り 9 行」と `ui/page` 直下の「残り 2 行」は**危険側のずれ**であり、前者は上限まで 0 行しか無いところを 9 行あると読ませ、後者は既に警告帯（-3 行）にあるところを 2 行の余地があると読ませる。
  本項 (1)(4)(7)(9) の数も本 PR のその後のコミット（Issue #157 の `pump_test.go` の追加検証と `pagetest.ScanOf` の doc）で動いたため、改訂欄自身を最終値へそろえた——改訂欄が表と食い違うと、索引として読む次の Issue が古い予算を引く（PR のレビュー指摘 / PR #160 のレビュー指摘）
...
- (1) 行数表を実測へ更新した（`ui/page` 2003 → **1955 行・残り 45・pass**、`ui/page/runners` 2000 → **1923 行・残り 77**、`ui/page/runners/rowview` 269 → **308 行・残り 1692**）。
  降順を保つため `ui/page` を `ui` の上へ、`ui/page/runners` を `ui/page/setup` の上へ、`ui/page/runners/rowview` を `ui/ghscope` の上へ移した。
- (2) 「残りが 1 桁のディレクトリ」を 7 つから **6 つ**へ改め、`ui/page/runners` を一覧から外した。
  警告帯のディレクトリを「`internal/buildconfig` と `ui/page` の 2 つ」から**`internal/buildconfig` の 1 つだけ**へ改めた。
- (3) 「ファイルの行数」の最大を `runners_test.go` 298 行から **`runners.go` 296 行（残り 4）**へ改めた。
- (4) 「`ui/page` を警告帯へ入れた判断（Issue #155）」の末尾を、切り出した Issue #159 が空けた旨へ書き換え、新節「**Issue #159 の空け方**」を追加した。
- (5) **表を直しながら現在形のまま取り残していた散文を全件測り直してそろえた**——`ui/page/runners` を **1923 行・残り 77 行**へ直した 4 箇所（Config タブの節の「現在 2000 行・残り 0 行」、「`ui/page` を警告帯へ入れた判断」の項 3、サービス制御の節の段落中と段落末の 2 箇所）とその節末の指示（「残りは 0 行——上限まで 1 行も無い」→ 残り 77 行と次の第一候補 `setupreq`。
  同節のテスト 3 ファイルの合計 602 → **584 行**）、`ui/page` 直下を **1955 行・残り 45 行・pass** へ直した 2 箇所（「6 周目の空け方」の「残りはもうプラスではない」、サービス制御の節の置き場所の理由）、「`ui/page` を警告帯へ入れた判断」の項 2 のテストの本数と行数（6 ファイル 824 行・`overlay_test.go` 296 行を Issue #155 当時の値と明示し、現在値 **5 ファイル 776 行**・**284 行**を併記）、「ファイルの行数」の `overlay_test.go` を 277 → **284 行**。
- (6) 本改訂 (4) の新節の 2 つの誤りを直した——「削除した `TestOverlayRegisteredModalFollowsStackRules` の個々のアサーションは**すべて**別のテストが持つ」は偽で、「2 枚重ねた状態で `View()` が最上位を描く」だけがどこにも残っていなかった（`TestOverlayCloseOneByOne` へ足し直し、その経緯を節に記した）。
  手ごとの内訳（−45 / −21 / −19 / −14）は削除行だけを数えており節自身の −81 行と合わず、ファイルごとの差し引き（合計 **−77 行**）へ置き換えた。
- (7) 同節の項 3 に、`pagetest.OpenTabOf` への置き換えが孤児ユニットの行の 1 箇所だけ `err == nil` のままで `ErrNotFound` と `ErrCmdTimeout` を再び潰していたこと（本 PR のレビュー指摘で `errors.Is` へ修正）を書き足した

**変更理由**

Issue #155 が `ui/page` を警告帯（残り -3 行）に、`ui/page/runners` を残り 0 行に残しており、どちらも次の 1 行で warn に入る状態だった。
本番コードを 1 行も変えず、手 (2) のテストの重複削減だけで両方に余裕を作ったため、実測値と「次に足す Issue への予算」の記述が古くなっていた。
ネストしたパッケージの新設（`setupreq`）とフィクスチャの `pagetest` への移動を**採らなかった**理由も、次の Issue が同じ検討を繰り返さないように書き残した。
(5) は**表だけを直して散文を取り残す**欠陥で、改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 / 1.92 で繰り返し再発しており、1.92 が全件の測り直しを規範として書いたにもかかわらず本改訂の初版で再び残っていた。
とりわけ `ui/page/runners` の「残り 0 行」は**危険側のずれ**であり、77 行の余裕があるところを「1 行も無い」と読ませて、次の Issue に不要な分割を検討させる。
(6) の「すべて別の経路が持つ」は**テストを削る判断の根拠そのものが偽だった**もので、`View()` を `stack[0]` へ書き換える変異で実際に覆いが 1 つ落ちていたことを確かめた。
内訳の数は節自身が主張する −81 行と足し合わせが合わず、読み手が どちらかを誤りと判断できない（本 PR のレビュー指摘）。
行数は本 PR のレビュー修正（`overlay_test.go` +7 行 / `logs_test.go` +4 行）で動いたため、表・散文・本欄をすべて最終の実測へそろえた
...
- (1) 行数表を実測へ更新した（`internal/buildconfig` 2122 → **1216 行・残り 784・pass**、`ui/page/pagetest` 1932 → **1935 行・残り 65**）。
  降順を保つため `internal/buildconfig` を最上段から `internal/svc` の下へ移し、新設した `internal/buildconfig/docscheck`（**936 行・残り 1064**）を `cmd/gsr-helper` の下へ、`internal/buildconfig/buildconfigtest`（**44 行・残り 1956**）を表の末尾へ足した。
- (2) 「残りが 1 桁のディレクトリ」の段落を、**警告帯に入っているディレクトリが 1 つも無い**状態へ書き換えた。
- (3) 「`internal/buildconfig` を警告帯へ入れた判断」の末尾に、その節が出した「空けること自体を独立した Issue に立てること」という指示が Issue #161 として実施されたことを追記し、実施の判断を「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」の節に記録した（境界を引けた根拠の実測——共有していた識別子は `repoRoot` 1 つだけ——、共通ヘルパを写さず `buildconfigtest` へ出して `pagetest/import_test.go` の `fixtures` へ登録した判断、分けても合計は **+74 行**になること、ファイル側の実測）。
- (4) 「Issue #159 の空け方」の `ui/page/pagetest` の実測 2 箇所を、表と同じ 1935 行・残り 65 行へそろえた。
- (5) **PR #163 のレビュー指摘で `nolint` 棚卸しの検査を `docscheck` へ移し、本欄を含む実測値を全件測り直した**——`TestSetupDocNolintInventoryMatchesTree` は `docs/environment/setup.md` の表を見るドキュメントの検査でありながらビルド設定側に残っており、同じ setup.md を読む `TestSetupDocEmbedsConfigFilesVerbatim`（`docs_sync_test.go`）と 2 パッケージへ割れていた（`nolint_inventory_test.go` → `docscheck/docs_nolint_test.go`）。
  移動で動いた値は本欄の (1)(3) のほか、「残りが 1 桁のディレクトリ」の段落、「2 つに分けた判断」の節（合計 **2196 行**・ビルド設定側に残るのは **9 つの `_test.go`**・ファイルの最大は `golangci_test.go` **249 行**とドキュメント側の `docs_deppkg_test.go` **202 行**）である。
- (6) 同じレビューで「`internal/buildconfig` を警告帯へ入れた判断」の項 4・項 5 を**当時の値と明示したうえで現在値を併記する**形へ直し、項 5 末尾の次 Issue 向け指示（「ディレクトリ側は既に残りマイナスである」）を Issue #161 が解消した旨へ書き換えた。
- (7) 「空け方の手は 4 つある」（当時は 3 つ）の (3) が挙げるフィクスチャ用の通常パッケージに `internal/buildconfig/buildconfigtest` を、「ファイルの行数」の `docs_depgraph_test.go` の参照を移動後のパス（`internal/buildconfig/docscheck/`）へ足した。
- (8) **PR #163 の 2 周目レビュー指摘を反映した。**
  - (a) **本節冒頭の列挙へ第 4 の手を足した**——「テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける」で、適用条件は (1) と同じく**増え方が違い、互いに依存が無い**こと、先例は `internal/buildconfig` → `internal/buildconfig/docscheck` である。
    あわせて第 4 の手で古くなる言い回しを全文で洗い、「3 つの手のうち」8 箇所と「3 つの手（本節冒頭）」3 箇所を「4 つの手」へ、6 周目の「上の 3 つの手」を「上の (1)〜(3)」へそろえた（**改訂 1.48 / 1.50 の欄は当時の記録なので数はそのままにし**、1.50 の理由欄からは本文と食い違う「3 つ」だけを外した）。
  - (b) 「警告帯へ入れた判断」の項 2 を**当時の判断と明示したうえで、Issue #161 が (4) で解いた旨を添える**形へ直した。
  - (c) 「2 つに分けた判断」の節に採った手の番号 **(4)** を他節と同じ形式で明記し、共通ヘルパの段落にも **(3)** を添えた。
  - (d) 「増えたのは package 宣言・import・**3 つ**の doc コメント」を**新設した 2 つ**（`docscheck/doc.go` 14 行・`buildconfigtest/buildconfigtest.go`）へ直した——`internal/buildconfig/doc.go` は分割前からあり **12 行のまま**で +74 には効いていない（合計 +74 の結論は変わらない）。
  - (e) 「次に `internal/buildconfig` へテストを足す Issue へ」の一文で**句点だけが bold で囲まれていた編集残り**を直した。
  - (f) `internal/buildconfig/doc.go` の列挙へ**依存更新設定（`.github/dependabot.yml`）**を、`internal/buildconfig/docscheck/doc.go` の列挙へ **`//nolint` 抑制の棚卸し**を足した（`TestDependabotWatchesGitHubActions` と `TestSetupDocNolintInventoryMatchesTree` が列挙から漏れていた）。**どちらも行を折り返して行数（12 行 / 14 行）を保った**ので、上の行数表と散文の実測値は 1 つも動かない。
- (9) **PR #163 の 3 周目レビュー指摘を反映し、(5) の「移動で動く値を全件測り直してそろえた」という本欄の宣言を実態に合わせた。**
  - (a) `ui/page/pagetest` を **1932 行・残り 68 行**と現在形で主張していた散文 5 箇所（「Disk タブの空け方」の 2 箇所・「`ui` 直下の空け方」の 6 周目と `tickMsg` の項と 10 周目）を実測（**1935 行・残り 65 行**）へそろえた。
    うち 2 箇所は次の Issue へ「**移せるのは 68 行まで**」と**受け皿の実容量より 3 行過大な予算**を指示していたので、予算そのものも **65 行**へ直した（68 行移すと 2003 行で警告帯に入る）。**当時の値であることが明記された箇所（Issue #159 の時点・`tickMsg` の項の「当時 1936 行」・改訂 1.92 の欄）は据え置いた。**
  - (b) 改訂 1.88 の欄が現在形で主張していた `internal/buildconfig` **2122 行・残り -122 行の warn** を実測（**1216 行・残り 784・pass**）へ、`docs_depnodes_test.go` **123 行**・`docs_depgraph_test.go` **296 行**・`docs_mermaid_test.go` **147 行** を実測（**129 行 / 136 行 / 194 行**。
    移動後のパスは `internal/buildconfig/docscheck/`）へ直した。
    改訂 1.89 の欄が同じく現在形で持っていた 2122 行・残り -122 行も同様に直した。
  - (c) 「次に `internal/buildconfig` へテストを足す Issue へ」の一文にあった**全角句点の直後の半角空白**（文書中この 1 箇所だけ）を除いた

**変更理由**

`internal/buildconfig` が警告帯（2122 行・残り -122 行）にあり、同節自身が空けることを独立した Issue に立てるよう指示していた。
ドキュメントの検査を `docscheck` へ分けて警告帯を出したため、行数表・段落・判断の節が実装と食い違う状態になった。
(5)〜(7) は PR #163 のレビュー指摘である。**`nolint` 棚卸しの置き場所は「検査に使う道具」ではなく「検査の対象」で決まる**——落ちたときに直すのは setup.md の表であって `.golangci.yml` ではないので、ビルド設定側に残すと `internal/buildconfig/doc.go` の宣言（ビルド設定ファイルの不変条件だけを置く）と食い違い、同一文書の検査を探す読み手が 2 つのパッケージを見る羽目になる。**項 4・項 5 は現在形のまま偽になっていた**もので、とりわけ項 5 末尾の「ディレクトリ側は既に残りマイナスである」は同じ文書の後続節「3 つとも余裕があるので、足す前に空ける必要は無い」と正面から矛盾し、次の Issue に不要な分割を検討させる**危険側のずれ**である。**表だけを直して散文を取り残す**欠陥は改訂 1.69〜1.73 / 1.90〜1.93 で繰り返し再発しており、本改訂の初版でも再発したため、移動で動く値を全件測り直してそろえた。**(8) は 2 周目レビューの MAJOR である。** 本節は「空け方の判断はここに集約する」と宣言し、他のすべての判断節が採った手を番号で名指しする形式を守っている（「4 つの手のうち (1)」「(3) を採った」「(1) は当たらない」）のに、**Issue #161 が実際に採った手——検査そのものをパッケージ境界で分ける——は 3 つのどれにも当たらず、936 行の移動が列挙から漏れていた**。
(3) の例示へ `buildconfigtest` を足したのは共有ヘルパ 44 行分の説明にしかならない。
列挙に無い手は次の Issue が同じ場面で辿れず、とりわけ項 2 の「(1) は当たらない」が現在形のままだと**同じ対象について「切り出す対象は無い」と「切り出した」が並んで見える**。**手を 1 つ足す以上、文書内の数え方の食い違いを残さないことが同時に要る**——数の食い違いは本 PR が 1 周目に指摘された欠陥そのものである。**(9) は 3 周目レビューの MAJOR である。** (5) が「移動で動く値を全件測り直してそろえた」と宣言しながら、実際に直したのは行数表と散文 2 箇所だけで、**同じ文書のなかに表（1935 行）と散文（1932 行）が正面から矛盾したまま 5 箇所残っていた**。
とりわけ「移せるのは 68 行まで」は次の Issue へ**受け皿の実容量より 3 行多い予算**を渡すもので、そのとおりに移すと `ui/page/pagetest` が 2003 行の警告帯に入る**危険側のずれ**である。**宣言と実態が食い違う欄は、次の周が「ここは測り直し済み」と信じて読み飛ばす**ため、表だけを直して散文を取り残す欠陥（改訂 1.69〜1.73 / 1.90〜1.93）をそのまま再生産する。
改訂 1.88 の欄を直すのは、同じ欄を改訂 1.89 / 1.91 が現在値へ訂正してきた前例に従うものである（当時の記録として明示された数ではなく、**現在形で主張している数**だから直す）。

#### 改訂 1.96

**変更内容**

PR #168 のレビュー指摘（major 8 件）を反映。

- (1) 「行数表と散文は実測に合わせる」の除外を**文どまりから句どまりへ**狭めた——`TestLineBudgetProseMatchesTables` は `当時` / `時点では` を `、` で区切った句の中でだけ効かせる。
  規約の文言も同じくそろえた。
- (2) **文が backtick で名指ししたディレクトリの行とだけ突き合わせる**ようにした（新設した `docs_linebudgetdir_test.go` の `budgetRowIndex` が、UI 層の表の `internal/` 抜きの綴りと散文の `internal/…/` 付き・末尾スラッシュ付きの綴りを同じ行へ向ける。
  名指しの無い文だけが表全体との照合に落ちる）。
  規約にも明記した。
- (3) 次の Issue へ予算を渡す 2 つの指示（「この N 行を予算として読む」「移せるのは N 行まで」）も残りの主張として拾うようにし、`## 改訂履歴` の切り出しを**行頭の見出し行だけに効く**形へ直した。
- (4) 新しい検査が挙げた散文 6 箇所へ `当時` を付け直した（`ui/page/config` が警告帯へ入った経緯、`internal/ui/app_test.go` と `internal/ui/page/` の逼迫、`ui` 直下を 1875 行へ戻した記録、9 周目が次の Issue へ渡した予算の指示）。
- (5) **本 PR が新設した規約に本 PR 自身が違反していた箇所を直した**——「`internal/buildconfig` を 2 つに分けた判断」の `docscheck` を分割当時の値（当時 936 行・残り 1064 行）へ戻し、`internal/ui/page/` の「1998 行（残り 2 行）」へ `当時` を付け、改訂履歴の過去欄が現在形のまま行数を主張していた 4 箇所を当時の値か「上記「行数の予算」の行数表を見よ」への参照へ書き換えた。
- (6) 規約の小節に、`TestLineBudgetProseMatchesTables` は `## 改訂履歴` 以降を検査対象から外しているので**過去欄は規約で守るしかない**ことを明記した。
- (7) 行数表の `internal/buildconfig/docscheck` を実測へ更新し（本 PR が検査を足したぶん 1334 → **当時 1463 行・残り 537・pass**）、降順を保つため `internal/svc` の下から `internal/doctor/jobreq`（当時 1495 行）の直下へ移した。
  あわせてその値を現在形で参照していた散文——「2 つに分けた判断」の 3 つの合計——を **当時 2723 行**（1216 + 1463 + 44）へ直し、分割時点の値であることが読み取れなかった `docs_nolint_test.go` の 183 行に `分割当時` を付けた

**変更理由**

新設した検査と規約に、**それ自身では塞げない抜け道と、本 PR 自身の違反**が残っていた。
(a) 除外が文全体に掛かっていたため、過去と現在を 1 文で対比する本書の文体では、`当時` を 1 つ書いた時点で**同じ文の現在形の主張までまるごと検査から外れる**——この検査が最も守るべき数がちょうど抜ける。
(b) **ディレクトリ名と数値が結び付いていなかった**ので、表のどこかに同じ組があれば緑になった。
現に `internal/ui/page/` の「1998 行（残り 2 行）」は真値が 1955 行・残り 45 行でありながら、`internal/logs` がちょうど 1998 行・残り 2 行だったため**別ディレクトリの値と偶然一致して素通りしていた**——検査を入れた当の PR が、その検査で守ったはずの数を古いまま通していた。
(c) 「N 行を予算として読む」形式は未カバーで、9 周目が次の Issue へ渡した予算が現に陳腐化していた。
これは Issue #164 が実害として挙げた「移せるのは 68 行まで（実際は 65）」と**まったく同型**であり、片方だけ塞いでも次の周で同じことが起きる。
(d) `## 改訂履歴` を文字列として探していたため、散文がこの見出しをコード引用として書くだけで**それ以降がまるごと検査から外れる**。
(e) 「改訂履歴・過去の記録には当時の値だけを残す」は本 PR が新設した規約であり、**その規約を作った PR 自身が現在形の行数を過去欄と判断の記録に書いていた**。
規約と前例が食い違えば、次の周は規約ではなく前例のほうを真似る。
(f) 表を実測へ合わせるのは本節が定める手順そのもので、本 PR が検査を足して `docscheck` を伸ばした以上、表と、その値を現在形で参照する散文を同じ変更で合わせなければ、新設した検査が最初の `make check` で落ちる

#### 改訂 1.97

**変更内容**

Issue #164 の再設計を反映。**散文の陳腐化を「言い回しの検出」で防ぐのをやめ、散文から実測値そのものを無くした。**

- (1) 小節「行数表と散文は実測に合わせる」を「**行数の実測値は表だけが持つ**」へ改め、規約を「現在の行数を数値で書いてよいのは行数表だけで、散文は表を参照する」という形に置き換えた。
- (2) `TestLineBudgetProseMatchesTables` と、名指しされたディレクトリの行を引く `docs_linebudgetdir_test.go`（`budgetRowIndex`）を削除し、`TestLineBudgetProseHasNoMeasuredNumbers` を新設した——除外を落とした散文に `残り N` と 3 桁以上の `N 行` が 1 つも現れないことだけを見る。**言い回しは一切見ない。**
- (3) 除外を正規表現ではなく明示の一覧にした——2 つの行数表 / コードブロック / 行頭の `## 改訂履歴` 以降 / 過去の値を記録する節の見出しを並べた `budgetProseExcluded`（「…の空け方」「…判断」と「一覧タブを 1 枚足せる余裕」）。**行数表が 2 つ見つからない・一覧に挙げた見出しが消えた・除外を落とすと散文が空になる、のいずれでも `t.Fatal` する。**
- (4) 散文に数で書いてよい定数を 300 / 330 / 2000 / 2200 の 4 つに限った（`.linterly.yml` の上限と警告帯の境界であって実測値ではない）。
- (5) 散文から実測値を落とした——「残りが 1 桁のディレクトリが 6 つある」の段落、「UI 層の外のディレクトリ」の導入、「ファイルの行数」のファイル別実測の一覧と前例、「一覧タブを 1 枚足せる余裕」「`internal/disk` から `pathguard` を切り出した判断」「`internal/setup` のフィクスチャを `setuptest` へ出した判断」の現在値、`ui/organism/table` の節の見出しの括弧——はいずれも行数表への参照に書き換え、過去の値は当時の記録として残した。
- (6) 改訂履歴の過去欄が現在値を指していた 2 箇所（改訂 1.74 の `ui/molecule/listrow`、改訂 1.96 の `docscheck` の実測と 3 つの合計）を、行数表への参照と `当時` 付きの値へ直した。
- (7) 改訂 1.95 が定めた「ファイル側の残りは `行` を付けず `（残り 4）` と書く」書き分けは廃した——`残り N` は `行` の有無に関わらず実測値として拾うためで、書き分けで検査を避ける道を残さない

**変更理由**

**言い回しを並べて陳腐化を検出する形は収束しなかった。** 改訂 1.95 が 2 形式、1.96 がさらに 2 形式を足してなお、PR #168 のレビューは変異注入で未カバーの書き方を挙げ続けた（1 周目 8 件・2 周目 9 件）。**日本語で行数を主張する書き方に上限は無い**ので、検出側を足す限り次の周でも同じ指摘が出る。
検出をやめて**散文から数値を無くす**ほうが、規約が 1 行で言えて（現在の行数を数で持ってよいのは表だけ）、検査も「数値が不在であること」しか見ないので**言い回しの追加では破れない**。
除外を正規表現ではなく見出しの一覧にしたのも同じ理由で、読み手が**何が検査されていないか**をその場で数えられる。
過去の値を検査の外に置き続けるのは、空け方と判断の記録がその Issue に紐づく事実であり、現在の実測へ寄せると議論の根拠そのものが消えるからである。**外にあるということは規約で守るしかないということ**なので、その旨を規約の小節に明記した

#### 改訂 1.98

**変更内容**

PR #168 のレビュー指摘（major 7 件）を反映。**改訂 1.97 が定めた方式（現在の行数を数で持ってよいのは行数表だけ）はそのままに、適用の穴を塞いだ。**

- (1) **除外の一覧へ逃がしてあった節から、現在形の実測値を全部落とした**——ディレクトリの値は「行数の予算」の行数表への参照へ、ファイル単位の値は `go tool linterly check` の出力への参照へ寄せ、その Issue に紐づく過去の値だけを `当時` の印を付けて残した。
- (2) 後続の Issue へ与える予算の指示から数値を落とし、「行数表の `残り` 列を見よ。
  足す前に空けること」の形へ揃えた（Issue #164 が実害として名指しした「移せるのは N 行まで」がまさにこの形の指示である）。
- (3) 現在形の実測値が無くなった節（「`ui/organism/table` の本体を分割しない判断」）を `budgetProseExcluded` の一覧から外し、**除外に残してよいのは過去の値だけを含む節に限る**と規約へ明記した。
- (4) **除外が入れ子で黙って広がっていた**のを直した——打ち切りが「同じ深さの次の見出しまで」だったため、一覧に挙げた見出しに対して実効の除外はその 1.5 倍ほどへ膨らみ、一覧に無い小節（`internal/buildconfig` の 2 つの判断が `ui/page/config` の節の配下にあった）まで検査の外にあった。
  打ち切りを**任意の深さの次の見出しまで**へ変えて**一覧の見出しの集合がそのまま除外範囲**になるようにし、それまで実効的に外れていた入れ子の小節は一覧へ明示した。
- (5) 検査の床を 3 桁から 2 桁へ下げた（`N 行` は `N` が 2 桁以上なら拾う）。
  あわせて `残り` の直後の助詞（`は` / `が` / `も`）を吸収し、逆に助数詞を伴う形（`残りが 1 桁` / `残り 3 件` など）は実測値ではないので拾わないようにした。
- (6) 本 PR が入れた数値が同じ PR の中で既に偽になっていた 2 か所（3 つのディレクトリの合計を書いた式と、ドキュメント側の最大ファイル）を参照へ寄せた。
- (7) **コードブロックのフェンスを閉じ忘れると、そこから本書の末尾までが黙って検査対象から消えていた**のを `t.Fatal` するようにし、本書の「落ちる条件」の列挙を実装と一致させた。
  あわせて 2 件——規約の説明にあった「小さい残りは `残り N` の側が桁に関わらず拾う」という**嘘**（実際には助詞が挟まると拾わなかった）を実装に合わせて直し、`internal/buildconfig/docscheck` のパッケージ doc が旧方式の言い方（「表と散文が linterly の実測とずれても」）のままだったのを新方式（「表が実測とずれても、散文が実測値を数で持っても」）へ直した

**変更理由**

- (1) が本 PR の主眼である。**方式は正しかったが適用が不完全だった**——改訂 1.97 は規約を 1 行に縮める一方で、実測値の残る節を片端から除外の一覧へ逃がしており、除外領域は本書の本文の約 3 割を占め、そこに現在形の実測値がそのまま残った。
  つまり**散文は古い予算を現在形で語り続けているのに検査は緑**という、Issue #164 が問題にした状態が除外の中でそっくり生き延びていた。
  しかも Issue #164 の実害（後続の Issue が古い残量を信じて超過を指示する）は、まさに空け方と判断を記録するこれらの節で起きる。
  除外を「逃がし先」に使う限り、規約は書かれただけで効かない。
- (2) は #164 の実害そのものの形なので、言い回しを変えるのではなく数値ごと落とすほかにない。
- (3) を明記しないと、次に実測値の残る節が出たとき再び一覧へ足して (1) の状態へ戻る。
- (4) は**一覧が嘘になる**ものである。
  除外を正規表現ではなく見出しの一覧にしたのは、読み手が**何が検査されていないか**をその場で数えられるようにするため（改訂 1.97）だったのに、その数え上げは実効の除外範囲の 3 分の 2 しか捉えていなかった。
  加えて、入れ子の小節を 1 つ足すだけで除外が黙って広がるので、**除外は誰も意図しないまま増え続ける。**
- (5) **3 桁の床は実害の帯をまるごと見逃していた**——予算の残りは逼迫したディレクトリほど小さく、危険な場所ほどほぼ常に 2 桁以下である。
  Issue #164 が名指しした原文の数も 2 桁だった。
  3 桁以上しか見ない検査は、**最も危険な行だけを素通りさせる**形になっていた。
  助詞を吸収するのも同じ理由で、`残りは` と一字挟むだけで検査から逃げられる状態だった（これは言い回しの列挙ではない——拾う形は「残り」と数の並びのままで、間に挟まる閉じた集合の助詞を吸収しているだけである）。
  助数詞を落とすのは逆向きで、本書が「残りが 1 桁のディレクトリ」のように性質を語る形まで実測値として拾うと、偽陽性を避けるために規約のほうを曲げることになる。
- (6) 同じ PR の中で偽になる数を新たに入れるのは、規約の趣旨（写しを 1 つに減らす）に真っ向から反する——**規約を入れた PR 自身が規約を破っていた。**
- (7) 他の 4 つの保証（行数表が 2 つある / 行頭の `## 改訂履歴` がある / 一覧の見出しが本書にある / 除外を落として散文が残る）はどれも食い違えば `t.Fatal` するのに、フェンスの閉じ忘れだけが保証の外だった。**閉じ忘れは検査対象を静かに縮めるので、緑になったことが検査されたことを意味しなくなる。** 残る 2 件のうち前者は規約の説明が実装より広い保証を約束していたもので、読んだ人が拾われると思って書いた形が実際は素通りする。
  後者はパッケージ doc が検査の目的を旧方式で語り続けるもので、次に検査を触る人が最初に読む場所である

#### 改訂 1.99

**変更内容**

PR #168 のレビュー指摘（major 6 件）を反映。**改訂 1.97 / 1.98 が定めた方式（現在の行数を数で持ってよいのは行数表だけ）はそのままに、検査の床と保証の書き方を実態へそろえた。**

- (1) **`TestLineBudgetProseHasNoMeasuredNumbers` の床を 2 桁から 1 桁へ広げた**——`N 行` は `N` が 3 以上なら拾い、**見ないのは `1 行` と `2 行` だけ**である（本書がこの 2 つを「表の 1 行」「一覧の 1 行」の意味で常用するため。
  3 観点のレビューが独立に、検査対象の散文で行の意味に使う 1 桁はこの 2 つだけだと実測している）。
  あわせて `残り` の直後に吸収する助詞へ `の` を足した（`残りの N 行` が助詞の集合から漏れて第 1 式に一致していなかった）。
  規約小節「行数の実測値は表だけが持つ」と [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)の一覧表の説明も同じ床へそろえた（setup.md 改訂 1.36）。
- (2) 「9 周目の空け方」の中で**本書の一意な識別子の総数を数で書いていた**のを落とした（改訂 1.92 が 948 個と測った値。
  本改訂の時点で同じ抽出コマンドを実行すると 1021 個で、既に食い違っていた）。
- (3) 「`ui/organism/table` の本体を分割しない判断」が実体を「**6 ファイルである**」と現在形で断定していたのを、ファイル名の列挙と実測コマンド（`ls internal/ui/organism/table/*.go` と `go tool linterly check`）への参照へ寄せた。
- (4) 「`internal/buildconfig` を 2 つに分けた判断」末尾の「**3 つとも余裕があるので、足す前に空ける必要は無い**」を、他節と同じ安全側の書き方（行数表を見よ／行数表の残りを超える追加が要るなら足す前に空けること）へそろえた。
  同じ助言が「警告帯へ入れた判断」の項 5 末尾にもあったので同時に直した。
- (5) 改訂 1.98 の理由欄の「除外領域に現在形の実測値が**約 20 か所**そのまま残った」から**件数の断定を落とした**。
- (6) `budgetProseExcluded` の doc コメントの保証を**実際に検証した範囲へ狭めた**——「**この検査が拾う形（`N 行` と `残り N`）が残っていないことだけを、一覧から 1 件ずつ外して確かめた**」とし、規約小節にも**検査が構造的に見ない範囲**（改訂履歴／除外一覧の節／数値を伴わない語形の主張／行以外の助数詞で数える個数）を明記した。**その範囲にある語形の主張（`余裕がある` / `潤沢ではない` / `残りはテスト 1 本ぶんしかない`）は本改訂では書き換えていない**——判断の記録に多数あり、言い回しを並べて拾う手は 2 周かけて捨てた方式そのもので収束しないので、**規約に範囲を明記することをもって対応とした**（(4) だけは数値を伴わないが安全側を崩す助言なので例外として直した）。
- (7) (1) で `docs_linebudgetprose_test.go` が伸びたので、「行数の予算」の UI 層の外の表の `internal/buildconfig/docscheck` を実測（**1505 行・残り 495・pass**）へ更新した（降順の並びは変わらない）

**変更理由**

- (1) **2 桁の床は今日いちばん逼迫している帯をまるごと素通りさせていた。** 予算の残りは逼迫したディレクトリほど小さく、本改訂の時点で残りが最小の 3 つはいずれも 1 桁である。
  そこへ「移せるのは N 行まで」「あと N 行しか無い」と書けば、Issue #164 が実害として名指しした形が**最も危険な帯でだけ無検査**という状態になる。
  助詞 `の` の追加は同じ穴で、`残りの` と一字挟むだけで第 1 式から逃げられた（これは言い回しの列挙ではなく、閉じた集合である助詞の吸収である）。
- (2)
- (3) は**除外の前提そのものを崩していた**——(2) の節は当時の値だけを記録するという前提で除外の一覧に載っており、(3) の節は「現在のファイル数と行数を落とした」ことを根拠に改訂 1.98 が一覧から外したのに、どちらも現在形の実測値を残していた。**個数は検査が構造的に見ない**ので、規約でしか止まらない。
- (4) は他のすべての節が「行数表の残りを超える追加が要るなら足す前に空けること」と安全側に揃えているなかで、**ここだけが現在の残予算を根拠に「空けなくてよい」と指示していた**。
  `internal/buildconfig` 系が埋まった時点で次の Issue を誤らせる（改訂 1.94 の理由欄が同型の矛盾を**危険側のずれ**として指摘している）。
- (5) は**自分で数えていない件数を書く**もので、本 PR は 1 周目にも「約 20 箇所と書いてあるが実際は 42 箇所」と同型の指摘を受けている。
  数えた数だけを書くか、件数を書かないかのどちらかにする。
- (6) は**保証が循環していた**——「現在形で実測値を語る節は 1 つも残っていない」は、その確かめ方（一覧から 1 件ずつ外して検査を回す）では裏付けられない。
  手順が示せるのは「この検査が拾う形が残っていないこと」だけで、検査が構造的に見ない形（個数・語形の主張・床より下の桁）は最初から検出対象外である。
  実際 (2)(3) がその反例だった。**検査が何を見ないかを書かない限り、読み手は緑を「全部見た」と読む。**

#### 改訂 1.100

**変更内容**

Issue #167（散文の行数の「合計の式」が行数の検査の網から外れている）を反映。

- (1) **合計の式の不在を見る検査を新設した**——`TestLineBudgetProseHasNoSumExpressions`（`internal/buildconfig/docscheck/docs_linebudgetsum_test.go`）が、除外を落とした散文に `A + B = C` の形があり、そこに 3 桁以上の数が 1 つでも含まれていれば落とす。
  既存の `docs_linebudgetprose_test.go` が 1 ファイル 300 行の上限に近かったのでファイルを分けた（除外を落とす `budgetProseLines` はそのまま共有する）。
  あわせて検出の両方向を固定する `TestProseMeasuredSumMatchesOnlyLineTotals` を置いた——行数の合計の式（右辺が bold で囲まれる形と、右辺が助数詞を持たない形を含む）は拾い、列幅の見積もり（`6 + 66 + 3 = 75 セル`）は拾わない。
- (2) 規約の小節「行数の実測値は表だけが持つ」に方針を明記した——**式を落として行数表への参照へ一本化する**（検査を式の評価へ広げる手もあるが、それは表と散文の一致を守るだけで写しそのものは残る）、**過去の値を記録する節では `当時` と読めるように書けば式を書いてよい**、**桁で切るのは行数の合計が 1 ディレクトリ 2000 行の桁で数えるものだから**（2 桁以下だけで閉じた式は行数ではない算術である）。
- (3) 「検査が構造的に見ない範囲」を 4 つから 5 つへ広げ、(e) **式にも助数詞にもならない裸の数**（`内訳は A / B / C` のように並べただけの形）を足した——数だけを見て実測値かどうかは判定できないので、ここも規約でしか守れない。
  あわせて拾う形の列挙を `N 行` / `残り N` / `A + B = C` の 3 つへそろえた。
- (4) 「`internal/buildconfig` を 2 つに分けた判断」末尾の括弧が、散文側の検査を「行数表と散文を実測と突き合わせる」という**旧方式のまま**説明していたのを、「行数表を実測と突き合わせる検査と、散文から実測値を無くす検査」へ直した。
- (5) (1) で `docscheck` が伸びたので、UI 層の外の行数表の `internal/buildconfig/docscheck` を実測（**1620 行・残り 380・pass**）へ更新した（降順の並びは変わらない）

**変更理由**

**合計の式は各項が裸の数なので、既存の `N 行` の検査では右辺しか拾えない。** `A + B + C = D 行` は 1 文で 4 つの実測値を主張するのに、検査に見えているのは右辺の 1 つだけで、左辺の 3 項は助数詞を持たないため最初から検査の外にある。
しかも**右辺が `行` を伴わない書き方になった瞬間に 4 項ともまるごと素通りする**——写しを 1 つに減らすという本規約の目的に照らせば、ここは 1 か所ではなく 4 か所ぶんの写しが無検査で増える場所だった。
日本語の言い回しを並べて拾う方式は 2 周かけて捨てた（改訂 1.97）ので、拾うのは「数値の形」だけに保ち、除外は桁という**閉じた条件**で切った——行数の合計は 1 ディレクトリ 2000 行の桁で数える以上どうしても 3 桁以上になり、2 桁以下だけで閉じた式は行数ではない算術（本書が書く列幅の見積もり）に限られるので、**規則を足し続けることにはならない**。
単体テストで両方向を固定するのは、拾えなくなれば Issue #167 の実害がそのまま戻り、逆に拾いすぎれば正当な算術を通すために除外の一覧を膨らませることになる（本書の規約は**一覧は小さいほどよい**と定めている）ためである。
(3) を足したのは、式を塞いだ結果として**次に安いのが数を裸で並べる形**になるからで、そこは検査が構造的に見ない——見ない範囲を書かない限り、読み手は緑を「全部見た」と読む（改訂 1.99 の (6) と同じ理由である）

#### 改訂 1.101

**変更内容**

Issue #169（`components/overview.md` の行数の実測値が PR #168 の検査の対象外で無検査のまま残っている）を反映。

- (1) **散文の検査の対象を 1 文書から 2 文書へ広げた**——`TestLineBudgetProseHasNoMeasuredNumbers` と `TestLineBudgetProseHasNoSumExpressions` が `docs/ui/atomic-design.md` に加えて `docs/components/overview.md` も走査する。
  登録は新設した `docs_linebudgetdoc_test.go` の `budgetProseDocs`（既存の `docs_linebudgetprose_test.go` が 1 ファイル 300 行の上限に近かったのでファイルを分けた）が持ち、パス・エラーメッセージに出す名前・**その文書が持つ行数表の数**・**その文書用の除外の一覧**を 1 件ずつ書く。
  既存の 5 つの `t.Fatal` の保証（行数表が期待した数だけある / 行頭の `## 改訂履歴` がある / 一覧の見出しが本文にある / コードブロックが閉じている / 除外を落として散文が残る）は 1 つも弱めず、どれも文書名を添えて落ちる形にした。
- (2) **除外の一覧は文書ごとに持つ**ことにし、規約の (4) にその旨と理由を明記した——`overview.md` の一覧（`overviewProseExcluded`）に載るのは「### `internal/disk`」の 1 つだけである。
- (3) `overview.md` から現在形の実測値 2 箇所を落とした（`internal/runner` 系 4 ディレクトリの使用量と残り / `internal/disk` の現在値）——どちらも本節の行数表への参照へ寄せ、Issue #101 当時の値は `当時` と読める形で残した。
- (4) 規約の冒頭に、この規約が掛かるのは本書だけではないこと（対象は 2 文書で、実測値を数で持ってよいのは本書の行数表だけであること）を明記した。
- (5) (1) で `docscheck` が伸びたので、UI 層の外の行数表の `internal/buildconfig/docscheck` を実測（**1708 行・残り 292・pass**）へ更新し、`internal/runner`（1704 行）を追い越したので降順を保つため 1 行上へ移した

**変更理由**

**検査の corpus が 1 ファイルだったので、同じ形の写しが隣の文書で無検査のまま残っていた。** `overview.md` の `internal/runner` 系の使用量は改訂 1.19 / 1.39 で **2 度陳腐化して人手で追随した記録がある**——つまり Issue #164 が実害として名指しした「散文が古い予算を現在形で語り続ける」状態が、検査の外で現に 2 度起きていた場所である。
しかも `internal/disk` の段落は末尾で「実測値の一次情報は同書の表であり、本書はここを重複して持たない」と**宣言しながら、その直前で現在形の実測値を持っていた**。
規約を本書の中だけで守っても、判断材料を引く側の文書が古い数を語れば次の Issue は誤る。
除外の一覧を文書ごとに分けたのは、**除外が文書の構造に結びついている**からである——本書は行数表を 2 つ持ち記録の節を 20 以上抱えるが、`overview.md` は表を持たず記録の節は 1 つだけで、混ぜると片方にしか無い見出しが消えたことを検査が知らせられなくなる（一覧に挙げた見出しが本文に無いと `t.Fatal` する仕組みが、混ぜた瞬間にもう一方の文書の見出しを毎回取り逃がす）。
同じ理由で表の数も文書ごとに持つ——数が違っても「見つからなければ落ちる」保証は残したいからである

#### 改訂 1.103

**変更内容**

PR #173 のレビュー指摘（major 6 件）を反映。**規約はそのままに、合計の式の検査を素通り経路が塞がる形へ直した。**

- (1) 拾う式の形を広げた——**必須なのは「2 つ以上の数が演算子で連なっていること」だけで `= D` の右辺は任意**、演算子は `+` `-` とその全角、ただし `-` は**前後の空白を必須**にする（空白なしを許すと ISO 形式の日付が式として拾われ、改訂履歴の外にある日付まで偽陽性になる）。
- (2) **床を `N 行` の側とそろえた**——式に含まれる数が**すべて 2 以下のときだけ**実測値でないと見なす。
- (3) (2) の結果として行数ではない算術（列幅の見積もり）が偽陽性になるので、**合計の式の検査にだけ効く見出しの一覧**（`budgetProseSumExcluded`）を新設し、規約の除外を 4 つから 5 つへ広げた。
  載せたのは「幅」の 1 つだけで、実装後に全文を走査して偽陽性を全件列挙した結果それだけだった。
- (4) **`overview.md` の除外を空にした**——唯一あった「### `internal/disk`」は節まるごと（API 責務表と全段落）を外していながら「載せてよいのは過去の値だけを持つ節」を満たしておらず、当時の値は上記「`internal/disk` から `pathguard` を切り出した判断（Issue #101）」が既に持っている。
- (5) 「見ないのは `1 行` と `2 行` だけ」を**実装どおり `0 行` / `1 行` / `2 行` の 3 つ**へ直した（式が `[3-9]` を要求する帰結で、残りがゼロになる状態は実在しうるので本規約でしか守れない）。
- (6) 「**本節の**行数表」という不揃いな指し方を、他の参照と同じ「上記「行数の予算」の行数表」へそろえた（2 箇所）。
- (7) (1)〜(3) で `docscheck` が伸びたので、UI 層の外の行数表の当該行を実測（**1992 行・残り 8・pass**）へ更新し、降順を保つため `internal/gh` の直下へ移した

**変更理由**

**塞いだ 3 形はいずれも実測で素通りを再現している。** (a) `内訳は 44 + 30 = 74 である。`——3 桁の床が同じスイートの `N 行` 側の床（3 以上）と食い違っており、ファイル単位や小さいディレクトリの 2 桁の行数を並べた式がまるごと通っていた。
(b) `内訳は 1216 + 1334 + 44 で、合わせて 2594 になる。`——`=` を要求していたので `+` の連なりだけの形が通っており、**これは Issue #167 が実害として挙げた「1 文で 4 つの実測値を主張する」形そのままである**。
(c) 全角の演算子。**検査が「数値・記号の形の不在」を見る方式である以上、形の集合に穴があれば規約の側は何も守らない**——日本語の言い回しを並べる方式を 2 周かけて捨てた（改訂 1.97）代わりに引き受けたのがこの責任である。
(3) で正規表現を足さず見出しの一覧にしたのは同じ理由で、式の形で除き分けようとした瞬間に捨てたはずの方式へ戻る。
(4) は**除外の一覧が自分で定めた条件を満たしていなかった**もので、24 行ぶんの節が「過去の値を持つ」という理由だけで丸ごと検査の外にあった——一覧は小さいほどよいという規約に照らせば、参照へ寄せて空にできるものを載せ続けてはならない

#### 改訂 1.109

**変更内容**

PR #176 のレビュー指摘（major 8 件）のうち本書に掛かる 4 点を反映した。

- (1) 「行数の実測値は表だけが持つ」の**現在形の指示が Issue #171 で移したファイルとパッケージを指したままだった**のを直した——`budgetProseDocs` の置き場を `internal/buildconfig/docscheck/linebudget/docs_linebudgetdoc_test.go` へ、`TestLineBudgetTablesMatchLinterly` と `TestLineBudgetProseHasNoSumExpressions` の所属を `internal/buildconfig/docscheck/linebudget` へそろえた（改訂履歴の中の言及は当時の記録なので触っていない）。
- (2) 「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」の**締めの指示が Issue #171 の後で誤った振り分けを教える状態**になっていたのを直した——「足す先は対象で選ぶこと」に行数の予算の不変条件は `docscheck/linebudget` である旨を足し、`buildconfigtest` の条件を「両方が使う道具だけ」から「**複数が使う道具だけ**」へ改め、最新の振り分けは下記「`docscheck` から行数の予算の検査を分けた判断」が持つと明記した。同節の「現在の 3 つ」「3 つとも警告帯の外にある」も 4 つへ直した（当時の実測と `1216 + 936 + 44` の式は過去の値なのでそのままである）。
- (3) 「`docscheck` から行数の予算の検査を分けた判断（Issue #171）」の**行数が実測と 1 行ずれており、そのうえで合計の式を組み立てていた**のを直した。分けた側・残った側・`buildconfigtest` の 3 つを実測へ取り直し、合計の式も取り直した値で組み直した（**本節は `budgetProseExcluded` にあり検査が見ないので、ここの算術は手で確かめるほかない**）。
- (4) 改訂 1.49 の欄が**語の途中で切れており**、`（` が閉じないまま**バッククォートが奇数個**になってコードスパンと直後の太字が壊れて描画されていたのを、他の 26 行と同じ「要約 1 文＋参照」の形へ書き直した。

**変更理由**

**(1)(2) はどちらも現在形の規範が移動に追随していない形で、次の Issue を誤った置き場へ導く。** 本書は「新しい検査は対象で置き場を選べ」と定めておきながら、その選択肢に Issue #171 が作ったばかりの `docscheck/linebudget` を欠いていた——**規範が古いと、規範に従った人が間違える**。

**(3) は本書が繰り返し是正してきた「散文が持つ実測値の写しが古くなる」欠陥の再発で、しかも今回は検査の外側で起きた。** 過去の値を記録する節は `budgetProseExcluded` にあり、Issue #164 / #167 / #169 が積み上げた検査はどれもここを見ない。**外にあるということは、そこだけは規約で守るしかないということである**——本書自身がそう書いている節が、まさにその理由で 1 行ずれた値と合わない式を持っていた。

**(4) は Issue #170 の移設が残した描画の欠陥である。** 欄を「改訂の詳細」へ出すときに要約が語の途中で切れており、閉じない `（` と奇数個のバッククォートが後続の記法をまるごと巻き込んでいた。移したのは 32 小節（本書 27・[コンポーネント設計](../components/overview.md#改訂履歴) 3・[開発環境セットアップ](../environment/setup.md#改訂履歴) 2）で、本書で壊れていたのはここだけである——本書の残る 26 小節は点検して同じ形が無いことを確かめた（[コンポーネント設計](../components/overview.md#改訂履歴)の側でも改訂 1.51 が同じ形で壊れており同じ周で直したが、同書の改訂 1.55 に残っていた 1 件は 2 周目に見つかった）。
