...
**この 33 行は次のテスト追加でほぼ尽きる。** Issue #31 でキーの配送を検証する道具（`scanKey` とその形の網羅テスト）を足したぶん、1939 行から 1967 行へ増えた。次に `ui` 直下へテストを足す Issue は、**先に分割の是非を検討すること**（本節冒頭の規則）。分割するなら候補はキーの配送の検証（`gate_test.go` / `app_keys_test.go` / `scan_test.go` と `helper_test.go` の走査の道具）で、親 Model の非公開な状態には触らず `App` の公開の振る舞いだけを使っているため、`page/pagetest` と同じ性質の「テスト用の道具のパッケージ」として切り出せる。
