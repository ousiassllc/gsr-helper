...
**次にこのタブへ手を入れる Issue は、機能を足すかどうかに関わらず先に分割すること。** 境界は明確で、クリーンアップ（`clean.go` / `confirm.go` とそのテスト）は集計・表示と独立しており `page/diskclean` として切り出せる。切り出すときは **`page/pagetest/import_test.go` の `shared` マップに `"diskclean"` を足すこと**（足さないとタブとして扱われ `TestOnlyTabsetImportsTabs` が落ちる）。**あわせて前述の「`page/` は 1 ディレクトリ 1 タブではない」の段落の列挙にも `page/diskclean` を足すこと**（`shared` だけ足して段落を直さないと `TestSharedPackagesMatchDoc` が「atomic-design.md の列挙に page/diskclean が無い」で落ちる。列挙の正は `shared` の側なので、直す向きは常に `shared` → 段落である）。切り出し先が `page/<tab>` の命名規約（1 ディレクトリ 1 タブ）から外れるため、`shared` への登録がその例外を明示する役割を持つ。
...
**次にこのディレクトリへ手を入れる Issue は、1 行足す前に行数を空けること。** エラー境界までは 119 行あるが、上限 2000 を 81 行超過した警告帯のままである。**ただし上の 2 つの手はもう使い切っている。** 残る道具は `newModel` だけで、それは上記の理由で出せない。テストの重複もこの 3 周目で畳んだので、次に取れるのは本文（`flow.go` / `form.go` / `setup.go`）ではなく**このディレクトリに残るモーダル 2 種（`formmodal.go` / `confirm.go`）を `page/setupmodal` として切り出すこと**である。ファイルを分けるだけでは 1 ディレクトリの合計は 1 行も減らない（`ui/organism/table` の本体を分割しない判断と同じ理由）ので、別ディレクトリへ出すこと自体が要件である。切り出すときは `page/pagetest/import_test.go` の `shared` マップに `"setupmodal"` を足すこと（足さないとタブとして扱われ `TestOnlyTabsetImportsTabs` が落ちる）。**あわせて前述の「`page/` は 1 ディレクトリ 1 タブではない」の段落の列挙にも `page/setupmodal` を足すこと**（`shared` だけ足して段落を直さないと `TestSharedPackagesMatchDoc` が落ちる）。
