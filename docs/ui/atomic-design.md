...
##### Issue #140 の判断——警告帯へ入れて回帰テストを足した

Issue #140（`make check` の並列負荷で自己設定テストが散発的に落ちる）は、`ui/page/config` を 1980 行から 2055 行へ、`ui/page/pagetest` を 1936 行から 2121 行へ押し上げ、**2 つとも警告帯に入れた**。

**本節冒頭の (3) は実際に採った。** 束を辿る道具は初め `ui/page/config` の `helper_test.go` に `findMsg` として置いたが、引数が `tea.Cmd` と述語だけで config 固有の型に依存しないため、**回帰テストごと `page/pagetest` へ出して `FindMsg` にした**（`doneMsg` に依存するのは述語を渡す `doneOf` の側だけである）。これで `ui/page/config` は 2184 行から 2055 行へ下がった（同じ PR の Issue #141 / #142 が見出しの回帰テストで +43 行するので、最終的な値は 2098 行・残り -98 行である）。**受け皿の `page/pagetest` は代わりに 2121 行へ上がり、こちらが警告帯の深いほうになった**——合計は減っていないので、2 つとも警告帯を出るには別ディレクトリの通常パッケージを新設するしかない（Issue #147 に切り出した）。

それでも 2 つとも警告帯に残したのは、次の 2 つによる。

- **バグ修正にスコープ外の再構成を混ぜない。** 新しいパッケージの新設まで踏み込むのは、7 周目（Issue #132）と 8 周目（Issue #136）が `ui` 直下で見送ったのと同じ範囲外の作業である。
- **落とせる検証が無い。** 足した 3 本は、待ち時間切れと「Cmd が本当に出なかった」の取り違えを縛るものである。取り違えたままだと、失敗メッセージが原因と違う場所（差分の判定）を指し、次に同じ散発的な失敗を見た人がまた同じ回り道をする——それが Issue #140 で実際に起きたことである。
...
| 1.78 | 2026-08-24 | `make check` の並列負荷で config の自己設定テストが散発的に落ちる件（Issue #140）を反映。(1) 行数表を実測へ更新した——`ui/page/config` 1980 → **2055 行・残り -55 行・warn**、`ui/page/pagetest` 1936 → **2121 行・残り -121 行・warn**。どちらも新たに警告帯へ入ったため降順の位置も動かした（`ui/page/pagetest` が `ui` の上、`ui/page/config` が `ui` の下）。(2) **表と同時に散文も直した**——現在形で行数を主張する箇所を全件抽出して照合し、この 2 つのディレクトリに触れる 7 箇所すべてを更新した（「残りが 1 桁のディレクトリ」の段落、Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「使えるのは 20 行しかない」）、Disk タブの節の受け皿の余裕（「そちらも残り 64 行」）、Logs タブの節の受け皿の余裕（「1936 行・残り 64 行」）、`ui` 直下 5 周目の記録（「現在 1936 行で、残りは 64 行しかない」））。他のディレクトリの数値は本 PR が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した。(3) 「Issue #140 の判断——警告帯へ入れて回帰テストを足した」を Config タブの節へ新設し、**本節冒頭の (3)（テストの道具を別ディレクトリの通常パッケージへ出す）を実際に採ったこと**——束を辿る道具を `ui/page/config` の `findMsg` から `page/pagetest` の `FindMsg` へ回帰テストごと移し、`ui/page/config` を 2184 行から 2055 行へ下げた——と、それでも 2 つとも警告帯に残る理由（合計は減らないので別パッケージの新設が要るが、それはバグ修正の範囲外）、および空けること自体を Issue #147 へ切り出したことを記録した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、**2 つのディレクトリを同時に警告帯へ入れた**のは本 PR である。表を据え置くと、次に `ui/page/config` へ手を入れる Issue は残り 20 行のつもりで 75 行ぶん過大な予算を引く。`page/pagetest` はさらに危うい——Logs / Disk の両節が「詰まったら道具をそこへ出せ」と受け皿として名指ししており、その受け皿が本 PR の移動で**最も逼迫したディレクトリになった**事実を書かないと、次の Issue はもう存在しない逃げ道を当てにして計画を立てる。表だけを直して散文を取り残す欠陥は 1.69〜1.72 で 4 周続けて再発しているため、本項でも全件突き合わせを実際に通し、その範囲と結果を上記 (2) に具体的に書いた。**なお本項は当初「`findMsg` は `doneMsg` に依存するので `page/pagetest` へ出せない」と書いていたが、これは事実に反していた**——`findMsg` の引数は `tea.Cmd` と述語だけで、`doneMsg` に依存するのは述語を渡す `doneOf` の側である。レビューでこの誤りが判明したため、記述を直すのではなく**実際に移した**（Issue #140） |
| 1.79 | 2026-08-24 | 同じ PR の Issue #141（自身の設定フォームの見出しを `firstRunUnsaved` で決める）/ #142（対象選択の見出しを runner に限定しない文言へ）が足した見出しの回帰テスト（`internal/ui/page/config/title_test.go`、43 行）を行数表と散文へ反映した——`ui/page/config` 2055 → **2098 行・残り -98 行・warn**（降順の位置は変わらない）。散文は Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「ERROR の境界まで 145 行」→ **102 行**）と「残りが 1 桁のディレクトリ」の段落を直し、改訂 1.78 の (3) が記録した「2184 行から 2055 行へ下がり 145 行を確保した」にも、同じ PR の後続分を含めた最終値（2098 行）への断りを添えた。`title_test.go` を新設したのは、見出しの検証を `selfconf_test.go` へ足すと 307 行となり 1 ファイル 300 行の警告帯（301〜330 行）に入るためだが、境界は行数だけでなく責務でも切ってある（`selfconf_test.go` は「保存の経路が何を書くか」、`title_test.go` は「画面が何と名乗るか」）。`ui/page/pagetest`（2121 行）と `ui`（2105 行）は #141 / #142 が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と定めており、`ui/page/config` は改訂 1.78 の時点で既に警告帯だった。**同じ PR の中で 2 度行数が動いたのに表が 1 度目の値のままだと、次に予算を引く Issue が 43 行ぶん過大な余裕を前提にする**——ERROR 境界（2200 行）まで実際には 102 行しかない。改訂 1.76 が「警告帯の `ui` へ Issue #136 の回帰テスト 42 行を足した判断」を同じ理由で残したのと同型の処置である。ファイル側の警告帯を踏まずに新設した判断を書いたのは、改訂 1.77(3) が「300 行を超えたので分けた」という**存在しない事実**を書いた誤りを是正した際に、正しい運用（超える前に別ファイルへ置く）を記録として残すと定めたためである（Issue #141 / #142） |
