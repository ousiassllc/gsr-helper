...
**逼迫しているのはディレクトリだけではない。** 本 PR で `internal/ui/app_test.go` は **300 行（1 ファイル 300 行の上限に張り付き・残り 0）**になり、`internal/ui/page/` も **1998 行（残り 2 行）**になった。**前者は当時の値である**——9 周目（Issue #139）が周期の状態を `discovery.State` へ移した際に `app_test.go` は 274 行まで下がっており、いま `ui` 直下で最大のファイルは `app.go` の 284 行である（上記「ファイルの行数」）。後者の 1998 行・残り 2 行は現在も同じで、`page` 直下へ共通の `Msg` を 1 つ足すなら**2 行以内**であり、それを超えるならタブをまたぐ移動や本例外と同じく「双方から見える場所がここしか無い」ことを確かめたうえで、先に何かを出すこと。
...
**次の Issue への予算は残り 57 行、ERROR 境界 2200 まで 257 行である。** 9 周目の 47 行ではない。**採れる手は依然 (1) だけである**（(2)(3) は 6 周目までに使い切っている。受け皿の `page/pagetest` は Issue #147 が 1564 行まで空けたので、道具を出す先としては使えるようになった。現在は 1763 行・残り 237 行である）。残る `app.go` / `chrome.go` / `discover.go` / `keys.go` の 4 ファイル（`ui` 直下の本番ファイルはこれで全部である）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**次に 57 行を超える追加が要る Issue は、まず切り出せるまとまりの設計から始めること。**
