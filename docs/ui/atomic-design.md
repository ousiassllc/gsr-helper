...
| `internal/buildconfig/docscheck` | 936 | 1064 | pass |
...
**残りが 1 桁のディレクトリが 6 つある**（`internal/setup/tarball` 1 行・`internal/logs` 2 行・`internal/gh` 4 行。UI 側では `ui/page/disk` が残り 3 行、`ui/organism/dialog` が残り 6 行、`ui/page/config` が残り 8 行である）。**この 6 つに 1 行でも足す Issue は、足す前に空けること。** **`ui/page/runners` はこの一覧から外れた**——Issue #155 が残り 0 行まで詰めた（その前に Issue #150 が `cmdtest` の締め切りへの追従で `page/config` を 1990 → **1992 行**、`page/runners` を 1989 → **1991 行**にしていた）のを、Issue #159 が **1923 行・残り 77 行**へ戻したためである（下記「Issue #159 の空け方」）。残る 6 つはどれも警告帯には入っていないので `make check` は通るが、通ることと余裕があることは違う。**警告帯に入っているディレクトリは 1 つも無い**——最後まで残っていた `internal/buildconfig` を Issue #161 が出した（2122 行・残り -122 行・warn → **1216 行・残り 784 行・pass**）。ドキュメントの検査を `internal/buildconfig/docscheck` へ分けたもので、判断は下記「`internal/buildconfig` を 2 つに分けた判断」にある。**`ui/page` は Issue #159 が警告帯から出した**——Issue #155 が 2003 行・残り -3 行・warn まで押し上げたのを **1955 行・残り 45 行・pass** へ戻している。その前に入っていた 3 つは Issue #147 が出した——`ui/page/pagetest` 2197 → **1564**、`ui/page/config` 2098 → **1990**、`ui/page/logs` 2025 → **1927**（判断は下記「Issue #147 の空け方」の節）。**`ui` 直下も警告帯の外にある**（1944 行・残り 56 行・pass。Issue #139 が 2105 行から戻し、Issue #148 がさらに下げた。予算と次の Issue への指示は下記「10 周目の空け方」）。

##### `internal/disk` から `pathguard` を切り出した判断（Issue #101）

着手時点で 2181 行（残り -181 行）で、**エラー境界の 2200 まで 19 行**しかなかった。ここへ 1 行足すだけで `make check` が落ちる状態である。上限値は緩めず、4 つの手のうち **(1) 本番の一部をパッケージ境界で切り出す**を採った。
...
着手時点で 2123 行（残り -123 行）だった。4 つの手のうち **(3) テスト用の道具・フィクスチャを別ディレクトリの通常パッケージへ出す**を採った。
...
Issue #155 が残した 2 つ——`ui/page` 2003 行（残り -3 行・warn）と `ui/page/runners` 2000 行（残り 0 行・pass。次に 1 行でも足せば warn）——を、**本番コードを 1 行も変えずに**空けた。4 つの手のうち使ったのは (2) テストの重複削減だけである。
...
**副産物として 1 ファイル 300 行の側も緩んだ。** `runners_test.go` が 298 行（残り 2 行）から 244 行になり、上記「ファイルの行数」の首位から外れた（`overlay_test.go` は 296 → 284 行）。**ディレクトリの予算を先に見ること**という同節の指示に、今回は逆向き——ディレクトリを空けた結果としてファイルの逼迫も解けた形で当たっている。
...
着手時点で 1969 行（残り 31 行）だった。2 周目レビューが 3 観点とも**変異注入で実測して**出した指摘——`UIApp[ui] --> Logs` のように矢印の左辺でノードを宣言すると辺が黙って捨てられる、`--x` / `---` / `-.-` / `===` / `~~~` を取りこぼす、`%%` コメント行で誤検知する、同じ段落が 2 つあると古いほうが素通りする、`subgraph UI` にノードを 1 つ移すだけで除外範囲が黙って広がる——を塞ぐと **2025 行（残り -25 行・warn）**になった。**切り出さずに警告帯へ入る判断を採った。**

**最終レビューの 1 巡目で +16 行して 2041 行（残り -41 行・warn）になった。** 塞いだのは 3 つで、いずれも**変異注入で実測した**「黙って緑になる」経路である——mermaid フェンスに重複ガードが無く「## 依存関係」節に 2 つ目のグラフを置くと**読者が見るほうが無検査**になる、散文が唯一の例外として名指しする `UIApp --> UIParts` が除外条件に飲まれて**図から消しても緑**、`mermaidArrow` がテキスト付き点線リンク `-. text .->` を取りこぼして**どの case にも入らず黙って無視**。下記 1.（正しさを行数より優先する）と同じ理由で、行数を根拠に省かなかった。

**最終レビューの 2 巡目でさらに +81 行して 2122 行（残り -122 行・warn）になった。** 塞いだのは 3 つで、いずれも**変異注入で実測した**「黙って緑になる」経路である——(a) 1 巡目で入れたフェンスの重複ガードが CommonMark の許す形（3 スペースまでのインデント、4 個以上のバッククォート）を数え落とし、**GitHub は描画するのに検査は 1 つ目しか見ない**状態が残っていた（とりわけ 4 個以上のバッククォートは**フェンスを逐語引用する**標準的な書き方で、本書の「改訂履歴に本文を逐語引用する」慣行で最も起きやすい形である）。(b) 例外として名指しした 1 本 `UIApp --> UIParts` の定数に生存確認が無く、図と `graphNodeRules` の両方でノードを改名すると**1 巡目で塞いだ経路が改名 1 手で復活した**。(c) 散文が「例外は 1 本だけ」と現在形で宣言しているのに、UI 内部の 2 本目を図に足しても通った。**あわせてフェンスを数える範囲を「## 依存関係」節の中に限った**（後方の無関係な節に別種の図を足すと「1 つに統合すること」という誤った助言で落ちていた）。**+81 行のうち約 40 行はこの 3 つのガードと分割に伴う doc コメントで、残りは分割で増えた package 宣言と import である。**
...
5. **1 ファイル 300 行は全ファイルが守っている**（**以下の括弧内は PR #153 の 2 周目レビュー時点の値である**——ディレクトリ全体の最大は `golangci_test.go` **247 行・残り 53 行**、本 PR が新設した 4 ファイルの最大は `docs_deppkg_test.go` **196 行・残り 104 行**で、いずれも警告帯 301〜330 行の外。`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行。**現在は 4 ファイルとも `internal/buildconfig/docscheck` にあり、実測は `docs_deppkg_test.go` 202 行・`docs_mermaid_test.go` 194 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 129 行、`golangci_test.go` は 249 行である**）。**最終レビューの 2 巡目で `docs_depgraph_test.go` を責務の境界で 2 つに分けた**——296 行で残り 4 行しかなく、例外定数の生存確認と「例外は 1 本だけ」の表明を足すと確実に超えたためである。境界は「ファイルの行数」の方針どおり行数だけでなく責務でも引いてある——`docs_mermaid_test.go` は「図をどう読むか」、`docs_deppkg_test.go` は「実装の側を `go list` でどう読むか」、`docs_depgraph_test.go` は「読んだ両者をどう突き合わせるか」を持つ。**当時ここに置いた「次にここへ足す Issue へ」の指示——新設分のファイル側には 104 行の余裕があるが、ディレクトリ側は既に残りマイナスである——は Issue #161 が解消した。** ディレクトリの逼迫はもう無いので、次に足す Issue が見るべき予算は下記「`internal/buildconfig` を 2 つに分けた判断」が持つ（3 つとも余裕があり、先に空ける必要は無い）。
...
着手時点で 2122 行（残り -122 行・warn）だった。上限値は緩めず、4 つの手のうち **(4) テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける**を採り、ビルド設定の検査（`internal/buildconfig` **1216 行・残り 784 行**）とドキュメントの検査（`internal/buildconfig/docscheck` **936 行・残り 1064 行**）に分けた。上記「警告帯へ入れた判断」が名指しした候補そのものである。**(4) は本 Issue が本節冒頭へ第 4 の手として足したものである**——PR #153 当時の 3 つの手にはこれが無く、同節の 2.「(1) は本番コードが無いため当たらない」で行き止まりになっていた。**検査は 1 件も削っていない**——分割の前後で同じテストがすべて走る（同節 1.「正しさを行数より優先した」を覆さないため、削る手は初めから検討していない）。
...
**分けても合計は増える。** 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・**新設した 2 つの doc コメント**（`docscheck/doc.go` 14 行と `buildconfigtest/buildconfigtest.go` の doc 部分。`internal/buildconfig/doc.go` は分割前からあるもので 12 行のまま、+74 には効いていない）と、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。
...
**1 ファイル 300 行の側はほとんど動いていない。** ビルド設定側の最大は `golangci_test.go` の 247 → **249 行**（残り 51 行）、ドキュメント側の最大は `docs_deppkg_test.go` の 196 → **202 行**（残り 98 行）である。移した `docs_nolint_test.go` は 130 行で、どちらの側でも首位にならない。
...
`scan_roots` の追従（上記「`scan_roots` はこの経路に載せ直した」）は `ui` 直下へ本番 19 行（`discover.go` の `scanRoots()` とその doc が +17、`app.go` の `Options.Roots` の doc が +2）と回帰テスト 44 行（`discover_test.go`）を持ち込み、`ui` 直下は 2000 行から **2063 行（残り -63 行・WARN）** になった。
...
着手時点で **2105 行（残り -105 行・WARN）** で、8 周目が残した予算は ERROR 境界（2200 行）まで 95 行だった。7 周目・8 周目が 2 周続けて使った逃げ道（判断を残して警告帯へ入る）を返す周であり、**行数を空けること自体が唯一のスコープである**。上限値は緩めず、4 つの手のうち **(1) 本番の一部をパッケージ境界で切り出す**を 2 回打って **1953 行（残り 47 行・pass）** へ戻した。7 周目が挙げた候補（背景処理を `discovery` 側へ**状態ごと**寄せる）をそのまま採っている。
...
**次の Issue への予算は残り 47 行、ERROR 境界 2200 まで 247 行である。** 8 周目の 95 行ではない。**次に `ui` 直下へ足す Issue は、この 47 行を予算として読むこと**——47 行は回帰テスト 1 本ぶんであり、それを超えるなら足す前に空けること。**採れる手はもう (1) しか残っていない**（(2) テストの重複削減と (3) 道具を `page/pagetest` へ出すは 6 周目までに使い切っており、受け皿の `page/pagetest` 自身も残り 64 行しかない）。**そして (1) の候補も 7 周目が挙げた 2 つを本周で使い切った。** 残る `app.go` / `background.go` / `chrome.go` / `discover.go` / `keys.go` の 5 ファイル（**当時**の `ui` 直下の本番ファイルはこれで全部である。`background.go` は 10 周目が `ui/startup` へ出して消えた）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**次に 47 行を超える追加が要る Issue は、まず切り出せるまとまりの設計から始めること。** 行数を空ける目的だけで片手間に着手しないこと（7 周目の但し書きはそのまま生きている）。
