...
| `internal/buildconfig` | 1346 | 654 | pass |
...
| `internal/buildconfig/docscheck` | 796 | 1204 | pass |
...
4. **ERROR 境界（2200 行）まで 78 行ある。** `make check` は通る。
5. **1 ファイル 300 行は全ファイルが守っている**（ディレクトリ全体の最大は `golangci_test.go` **247 行・残り 53 行**、本 PR が新設した 4 ファイルの最大は `docs_deppkg_test.go` **196 行・残り 104 行**で、いずれも警告帯 301〜330 行の外。`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行）。**最終レビューの 2 巡目で `docs_depgraph_test.go` を責務の境界で 2 つに分けた**——296 行で残り 4 行しかなく、例外定数の生存確認と「例外は 1 本だけ」の表明を足すと確実に超えたためである。境界は「ファイルの行数」の方針どおり行数だけでなく責務でも引いてある——`docs_mermaid_test.go` は「図をどう読むか」、`docs_deppkg_test.go` は「実装の側を `go list` でどう読むか」、`docs_depgraph_test.go` は「読んだ両者をどう突き合わせるか」を持つ。**次にここへ足す Issue へ**——新設分のファイル側には 104 行の余裕があるが、**ディレクトリ側は既に残りマイナスである**（先に尽きているのはディレクトリの方で、比べられるのは上記 4. の ERROR 境界までの 78 行だけである）。

**次に `internal/buildconfig` へテストを足す Issue へ。** 残りはマイナスである。**空けること自体を独立した Issue に立てること**（7 周目・8 周目が使った「判断を残して警告帯へ入る」逃げ道を 3 度続けて使わない）。候補は**検査そのものをパッケージ境界で別ディレクトリへ分ける**手で、ビルド設定の検査（`Makefile` / CI / `.golangci.yml` / `lefthook.yml` / `.linterly.yml` / depguard / nolint 棚卸し）とドキュメントの検査（`docs_*_test.go`）は**増え方が違い、互いに依存も無い**。ただし `repoRoot` などの共通ヘルパの置き場所を先に決める必要がある。 **この指示は Issue #161 として立て、実施した**（挙げた候補をそのまま採っている。下記「`internal/buildconfig` を 2 つに分けた判断」）。

##### `internal/buildconfig` を 2 つに分けた判断（Issue #161）

着手時点で 2122 行（残り -122 行・warn）だった。上記「警告帯へ入れた判断」が名指しした候補——**検査そのものをパッケージ境界で別ディレクトリへ分ける**——をそのまま採り、ビルド設定の検査（`internal/buildconfig` **1346 行・残り 654 行**）とドキュメントの検査（`internal/buildconfig/docscheck` **796 行・残り 1204 行**）に分けた。**検査は 1 件も削っていない**——分割の前後で同じテストがすべて走る（同節 1.「正しさを行数より優先した」を覆さないため、削る手は初めから検討していない）。

**境界は候補が挙げたとおりに引けた。** 実測で裏付けが取れたのは 2 点である。

1. **増え方が違う。** ビルド設定側は `Makefile` / CI / `.golangci.yml` / `lefthook.yml` / `.linterly.yml` / depguard / nolint 棚卸しを触るたびに増え、ドキュメント側は文書の取り決めを増やすたびに増える。
2. **互いに依存が無い。** 2 つのファイル群が共有していた識別子は **`repoRoot` 1 つだけ**だった。図の検査が持つ型（`graphEdge` / `depGraph` / `graphNodeRule`）や `soleLine` / `cutAtLineStart` はドキュメント側で閉じており、ビルド設定側の 9 ファイルからは 1 つも参照されていない。逆向きも `writeFiles` / `newModule` / `runMake` / `goEnv` がビルド設定側で閉じている。
...
**分けても合計は増える。** 2122 行が 1346 + 796 + 44 = **2186 行**（+64）になった。増えたのは package 宣言・import・3 つの doc コメントで、**検査の中身は 1 行も足していない**。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。

**1 ファイル 300 行の側はほとんど動いていない。** 最大は `golangci_test.go` の 247 → **249 行**（残り 51 行）、ドキュメント側の最大は `docs_deppkg_test.go` の 196 → **198 行**（残り 102 行）で、増分はどちらも import 1 行と空行 1 行である。
...
| 1.94 | 2026-08-25 | Issue #161 を反映。(1) 行数表を実測へ更新した（`internal/buildconfig` 2122 → **1346 行・残り 654・pass**、`ui/page/pagetest` 1932 → **1935 行・残り 65**）。降順を保つため `internal/buildconfig` を最上段から `internal/svc` の下へ移し、新設した `internal/buildconfig/docscheck`（**796 行・残り 1204**）を `internal/doctor/check` の下へ、`internal/buildconfig/buildconfigtest`（**44 行・残り 1956**）を表の末尾へ足した。(2) 「残りが 1 桁のディレクトリ」の段落を、**警告帯に入っているディレクトリが 1 つも無い**状態へ書き換えた。(3) 「`internal/buildconfig` を警告帯へ入れた判断」の末尾に、その節が出した「空けること自体を独立した Issue に立てること」という指示が Issue #161 として実施されたことを追記し、実施の判断を「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」の節に記録した（境界を引けた根拠の実測——共有していた識別子は `repoRoot` 1 つだけ——、共通ヘルパを写さず `buildconfigtest` へ出して `pagetest/import_test.go` の `fixtures` へ登録した判断、分けても合計は +64 行になること、ファイル側の実測）。(4) 「Issue #159 の空け方」の `ui/page/pagetest` の実測 2 箇所を、表と同じ 1935 行・残り 65 行へそろえた | `internal/buildconfig` が警告帯（2122 行・残り -122 行）にあり、同節自身が空けることを独立した Issue に立てるよう指示していた。ドキュメントの検査を `docscheck` へ分けて警告帯を出したため、行数表・段落・判断の節が実装と食い違う状態になった |
