...
**`audit_log` だけはこの経路に載せない（Issue #132）。** 親が配るのは `Options.Audit`——**既に開いたファイルハンドル**であり、追従させるには走行中にログを開き直すことになる。差し替えの最中に走っている書き込みの取りこぼしと二重オープンという危険を、出力先の変更という頻度の低い操作のために抱える理由が無い。**代わりに黙って効かないままにはしない**——`page/config` の `rememberSelf` が保存前後の `AuditLog` を比べ、変わっていれば結果の行へ `auditRestartNote`（「監査ログの出力先は再起動後に切り替わります」）を添える。変えていないときに添えないのは、毎回の保存に無関係な案内は読み飛ばされ、本当に再起動が要るときにも伝わらなくなるためである。
