...
**散文は実測値が無いことだけを見る。** `TestLineBudgetProseHasNoMeasuredNumbers` が本書から下記の除外を落とした散文を走査し、`残り N` と 3 桁以上の `N 行` が 1 つも現れないことを確かめる。**この検査は言い回しを一切見ない。** 以前は「N 行・残り M 行」「移せるのは N 行まで」のように行数を主張する書き方を並べて表と突き合わせていたが、**言い回しの列挙は収束しなかった**——レビューのたびに未カバーの書き方が見つかり、そのどれもが現に古い数を素通りさせていた。数値の不在だけを見る形なら、新しい言い回しを考えなくてよい。2 桁以下の `N 行` を見ないのは、本書が「表の 1 行」「一覧の 1 行」の意味で `1 行` を常用するからである（小さい残りは `残り N` の側が桁に関わらず拾う）。

**除外は正規表現ではなく明示の一覧で持つ。** 外すのは次の 4 つだけである。(1) 2 つの行数表（実測と突き合わせるのがここである）。(2) コードブロック。(3) `## 改訂履歴` 以降——**行頭の見出し行でだけ切る**（字下げや文中の引用で切ると、そこから先がまるごと検査から外れる）。(4) 過去の値を記録する節を見出しの文字列で並べた一覧（`docs_linebudgetprose_test.go` の `budgetProseExcluded`）——「…の空け方」「…判断」の各節と「一覧タブを 1 枚足せる余裕」である。**除外が現実と食い違ったら検査は声を上げて落ちる**——行数表が 2 つ見つからないとき、一覧に挙げた見出しが本書から消えたとき、除外を落とした結果として散文が 1 行も残らないときは `t.Fatal` する。
...
**その後の研磨で警告帯に入った（当時 2059 行・残り -59 行。ピークは当時 2117 行・残り -117 行で、Issue #104 / #108 を経て 1737 行まで下がり、Issue #128 が +95 行し、Issue #132 が +148 行し、Issue #140 が +9 行し、Issue #141 が +66 行、Issue #142 が +43 行して当時 2098 行・残り -98 行の **warn** になった。Issue #147 が `page/config/itemview` を切り出した現在は **1992 行・残り 8 行・pass** である）。** 押し上げたのは critical 7 件の回帰テストである（二重承認でバックアップが壊れる、実行中の対象切り替えで別の runner へ書く、処理中に新しい編集を始められる、など）。**安全側の検証を行数の都合で落とさない方を採った。** 同じ判断を `ui/page/disk`（当時 2101 行、現在 1997 行・残り 3 行。Issue #102 で分割済み）と `ui/page/runners`（当時 2164 行、現在 1923 行・残り 77 行。Issue #107 が削減し、Issue #155 の時点では残り 0 行まで詰まっていたのを Issue #159 が空け直した）も記録している。

**Issue #104 で 2117 行から 1719 行へ戻した**（その後 Issue #108 が `regress_test.go` を 2 ファイルに分けたぶんを含めて 1737 行になり、Issue #128 が自身の設定を親へ返す経路とその回帰テストで +95 行し、Issue #132 が監査ログの案内とその回帰テストで +148 行し、Issue #140 が +9 行（回帰テストと道具は `page/pagetest` へ出したため増分が小さい）、Issue #141 / #142 が見出しの回帰テスト（`title_test.go`、92 行）で +109 行して当時 2098 行・残り -98 行になった。Issue #147 が一覧の行の組み立てを `page/config/itemview` へ出した現在は **1992 行・残り 8 行**）**。** ただし本節がそれまで挙げていた候補——「残った tea 非依存の部分（`items.go` の要約、`form.go` の検証）を `config/edit` へ出す」——は**もう使えなかった。**
...
2. **(2) テストの重複削減は本 Issue の範囲外である。** `ui/page` 直下のテストは 6 ファイル 824 行で、削るなら `overlay_test.go`（296 行）の重なりの検証であり、**本 Issue が触れていない部分を触ることになる**（**この 2 つは Issue #155 当時の値である**。切り出した Issue #159 がこの候補をそのまま採り、現在は **5 ファイル 776 行**・`overlay_test.go` は **284 行**である）。
3. **足す側は削った。** doc は当初 13 行だったものを 4 行に詰め、詳しい理由は、まだ余裕のある `pagetest.ScanOf`（`ui/page/pagetest`・**1935 行・残り 65 行**。本節の UI の表と同じ実測値。Issue #159 の時点では 1932 行・残り 68 行で、Issue #161 が `fixtures` へ 1 件登録して 3 行増えた）側へ置いた。**同じ Issue で `ui/page/runners` は Issue #155 の時点では 2000 行（残り 0 行・pass）に収めてあった**（現在は Issue #159 が空けた **1923 行・残り 77 行**で、下記「Issue #159 の空け方」が持つ）——回帰の検証は既存の `TestDrainStopsAfterWaiting` に足し、走査の既定は `pagetest.State` へ寄せた。
4. **ERROR 境界（2200 行）まで 197 行ある。** `make check` は通る。
5. **1 ファイル 300 行も守っている**——`page.go` は 284 行（残り 16）で、警告帯 301〜330 行の外である。

**切り出した Issue #159 が空けた。** 残りマイナスの状態は解消しており（**1955 行・残り 45 行・pass**）、上記 2. で候補に挙げたテストの重複削減がそのまま採られた。本 Issue は「判断を残して警告帯へ入る」逃げ道を 1 度使い、次の Issue がそれを畳んだ形である。詳細は下記「Issue #159 の空け方」。
...
**`ui/page` は 2003 → 1955 行（残り 45 行・pass）。** Issue #155 が「候補は上記 2. のテストの重複削減」と名指ししたとおりに進めた。
...
2. **残った `TestOverlayHelpScopeIsPerPage` を `helpmodal_test.go` へ移し、`modalstack_test.go` を削除した。** 重なりの規則を持たなくなった時点で `modalstack` という名前は責務を言い当てておらず（上記「ファイルの行数」の**分けた先の名前が責務を言い当てられないなら、それは境界を間違えている**）、ヘルプの検証は `helpmodal_test.go` に集まる。移動後も 114 行で 300 行の上限内である。
...
**`ui/page/runners` は 2000 → 1923 行（残り 77 行・pass）。** Issue が挙げた 2 案（`rowview` の先例どおりのネスト / テストの重複削減）のうち、**新しいパッケージは作らなかった。**
...
**内訳は手ごとではなくファイルごとの差し引きで示す。** 2.〜4. は削った行と足した行（表の骨組み、共有ヘルパの doc、両ケースへ広げたアサーション）が同じファイルに混ざるため、手ごとの数は差し引きにならない。**`origin/feat/#1` との実測（`wc -l`）は次のとおりで、合計 −77 行が 2000 → 1923 行に一致する**——`runners_test.go` −54（うち 1. のハンクが −47、残りは 4. で消えた `mustModel`）、`logs_test.go` −10（3.）、`ops_test.go` −9（4.）、`opsflow_test.go` −9（2. と 3.）、`helper_test.go` +5（4. の doc）、`chromeorder_test.go` / `opsdrain_test.go` ±0（4. の呼び出し側の置き換えだけで行数は動かない）。**1. の移動先は別ディレクトリなので `page/runners` の予算には効かない**——`page/runners/rowview` は 269 → 308 行（残り 1692 行）になっている。

**採らなかった手と理由。**

- **(1) ネストしたパッケージの新設は要らなかった。** `keys.go` の Setup タブへの引き渡し（`n` / `D` / `u`）は親の非公開に触れずに `page/runners/setupreq` として出せる形をしており、候補としては成立する（入力はすべて他パッケージの公開型で、出力は `tea.Cmd` と通知の文字列である）。**ただし上記のテスト側の手だけで残り 77 行に届いた**ので、パッケージを 1 つ増やす代償に見合わない。**次に `page/runners` の本番コードが増えて空ける必要が出たときの第一候補はこれである。**
- **フィクスチャ（`sampleRunner` / `unmanagedRunner` / `orphanUnits`）を `pagetest` へ出す手（3）は採らなかった。** `page/runners` から 48 行減る代わりに `ui/page/pagetest`（当時 1932 行・残り 68 行。現在は 1935 行・残り 65 行）が十数行増える。**行数を空ける Issue が別のディレクトリを逼迫させるのは本末転倒である**（上記「9 周目の空け方」と同じ判断）。
- **`TestDrainWaiterSwallowsGlobalKeys` の削除は見送った。** `TestConfirmModalSwallowsGlobalKeys` がより広いキーの集合で同じ分岐（`keys.go` の `m.overlay.Active()`）を通るため重複ではあるが、この検証は Issue #155 がドレイン停止の待機に入れた守りに隣接しており、17 行のために落とす価値が無い。

**副産物として 1 ファイル 300 行の側も緩んだ。** `runners_test.go` が 298 行（残り 2）から 244 行になり、上記「ファイルの行数」の首位から外れた（`overlay_test.go` は 296 → 284 行）。**ディレクトリの予算を先に見ること**という同節の指示に、今回は逆向き——ディレクトリを空けた結果としてファイルの逼迫も解けた形で当たっている。

**次に `ui/page` 直下・`ui/page/runners` へ足す Issue へ。** それぞれ残り 45 行・77 行である。**警告帯は出たが潤沢ではない**ので、これを超える追加が要るなら足す前に空けること。`ui/page` の候補は依然として (1) の切り出し（`StateMsg` の改名の波及と `Overlay` の戻りの辺は上記「`ui/page` を警告帯へ入れた判断」の 1. が評価済み）、`page/runners` の候補は上記の `setupreq` である。
...
2. **(1) は本番コードが無いため当たらない——ただし当時の判断であり、後に (4) が当たった。** `internal/buildconfig` は [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)が明記するとおり**実行時のコードを持たないテスト専用パッケージ**（`doc.go` 12 行のほかはすべて `_test.go`）で、「本番の一部をパッケージ境界で切り出す」対象が存在しない。**ここまでが PR #153 の 2 周目レビュー時点の判断である**——当時の本節冒頭は 3 つの手しか挙げておらず（改訂 1.48）、本番コードを持たない以上どれも当たらないという行き止まりだった。**その後 Issue #161 が (4) テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける手で解いた**（下記「`internal/buildconfig` を 2 つに分けた判断」。この手はそのとき第 4 の手として本節冒頭へ足したものである）。
...
4. **ERROR 境界（2200 行）まで 78 行ある。** `make check` は通る（**PR #153 の 2 周目レビュー時点、2122 行の値である**。現在は Issue #161 が 3 つのディレクトリへ分けており、`internal/buildconfig` は **1216 行・残り 784 行・pass** で ERROR 境界まで 984 行ある。下記「`internal/buildconfig` を 2 つに分けた判断」と上記「行数の予算」の表が現在値を持つ）。
5. **1 ファイル 300 行は全ファイルが守っている**（**以下の括弧内は PR #153 の 2 周目レビュー時点では次の値だった**——ディレクトリ全体の最大は `golangci_test.go` **247 行（残り 53）**、本 PR が新設した 4 ファイルの最大は `docs_deppkg_test.go` **196 行（残り 104）**で、いずれも警告帯 301〜330 行の外。`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行。**現在は 4 ファイルとも `internal/buildconfig/docscheck` にあり、実測は `docs_deppkg_test.go` 202 行・`docs_mermaid_test.go` 194 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 129 行、`golangci_test.go` は 249 行である**）。**最終レビューの 2 巡目で `docs_depgraph_test.go` を責務の境界で 2 つに分けた**——296 行で残りが 4 行しかなく、例外定数の生存確認と「例外は 1 本だけ」の表明を足すと確実に超えたためである。境界は「ファイルの行数」の方針どおり行数だけでなく責務でも引いてある——`docs_mermaid_test.go` は「図をどう読むか」、`docs_deppkg_test.go` は「実装の側を `go list` でどう読むか」、`docs_depgraph_test.go` は「読んだ両者をどう突き合わせるか」を持つ。**当時ここに置いた「次にここへ足す Issue へ」の指示——新設分のファイル側には 104 行の余裕があるが、ディレクトリ側は既に残りマイナスである——は Issue #161 が解消した。** ディレクトリの逼迫はもう無いので、次に足す Issue が見るべき予算は下記「`internal/buildconfig` を 2 つに分けた判断」が持つ（3 つとも余裕があり、先に空ける必要は無い）。
...
**共通ヘルパは 3 つ目のディレクトリへ出した（本節冒頭の (3)）。** `_test.go` の中のヘルパはパッケージをまたげないので、選べるのは (a) `repoRoot` を両方に写す、(b) 通常のパッケージへ出す、の 2 つである。**(a) は採らなかった**——本書が繰り返し戒めている「写しの片方だけが古くなる」形そのもので、19 行のために作る写しではない。(b) の先例は `page/pagetest` / `organism/table/tabletest` / `setup/setuptest` で、いずれも「`_test.go` に置けない事情で通常のパッケージになった道具」である。**代償も先例と同じ——本番からも import できてしまう**ので、`page/pagetest/import_test.go` の `fixtures` へ `internal/buildconfig/buildconfigtest` を登録し、`TestNoProductionCodeImportsTestFixtures` の網に入れた（「足すときはここへ登録すること」と同ファイルが指示している。登録しないと検査の網から外れる）。

**分けても合計は増える。** 分けた当時は 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・**新設した 2 つの doc コメント**（`docscheck/doc.go` 14 行と `buildconfigtest/buildconfigtest.go` の doc 部分。`internal/buildconfig/doc.go` は分割前からあるもので 12 行のまま、+74 には効いていない）と、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。現在の 3 つの合計は 1216 + 1463 + 44 = **2723 行**である（Issue #164 が行数表と散文を実測と突き合わせる検査を足したぶん、`docscheck` が伸びている）。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。
...
**1 ファイル 300 行の側はほとんど動いていない。** ビルド設定側の最大は `golangci_test.go` の 247 → **249 行**（残り 51）、ドキュメント側の最大は `docs_deppkg_test.go` の 196 → **202 行**（残り 98）である（その後 Issue #164 が足した `docs_linebudget_test.go` **227 行**が、ドキュメント側の最大を更新している）。移した `docs_nolint_test.go` は分割当時 **183 行**で、どちらの側でも首位にならなかった。
...
Setup タブは追加・削除・バージョン更新の 3 操作と、フォーム・確認・進捗・結果の 4 つのモーダルを持つ（進捗と結果報告は Issue #75 で `page/progressmodal` へ切り出し済みで、当時このディレクトリに残っていたのはフォームと確認の 2 種だった）。**当時は 2081 行で警告帯（2000〜2200）に入っており、上限 2000 を 81 行超過していた**（その 2 種も Issue #105 で `page/setupmodal` へ出し、現在は 1890 行・残り 110 行・pass である。下記「4 周目」）。
...
**逼迫しているのはディレクトリだけではない。** 本 PR で `internal/ui/app_test.go` は **当時 300 行（1 ファイル 300 行の上限に張り付き・残り 0）**になり、`internal/ui/page/` も **当時 1998 行（残り 2 行）**になった。**前者のその後はこうである**——9 周目（Issue #139）が周期の状態を `discovery.State` へ移した際に `app_test.go` は 274 行まで下がっており、いま `ui` 直下で最大のファイルは `app.go` の 284 行である（上記「ファイルの行数」）。**後者の 1998 行・残り 2 行も当時の値である**——Issue #155 が `page.StateMsg` にフィールド 1 つと doc を足し、`ui/page` は当時 **2003 行・残り -3 行・warn** になった（上記「`ui/page` を警告帯へ入れた判断（Issue #155）」）。**残りがマイナスの状態も当時のものである**——切り出した Issue #159 が `ui/page` 直下のテストの重複を削り、現在は **1955 行・残り 45 行・pass** である（上記「Issue #159 の空け方」）。**残りは 45 行しか無いので、`page` 直下へ共通の `Msg` を 1 つ足すなら、それを超える追加が要らないことを確かめること。**タブをまたぐ移動や本例外と同じく「双方から見える場所がここしか無い」ことを確かめるのは、空けたうえでの話である。
...
**この周（Issue #31）は、余裕を「重複削減」ではなく「道具を `page/pagetest` へ出す」で作った。** Issue #31 でキーの配送を検証する道具を足したとき `ui` 直下は当時 1967 行（残り 33 行）まで詰まったが、走査の道具（`ScanKey`）とその形の網羅テストを `page/pagetest` へ移して当時 1875 行（残り 125 行）に戻した。`page/pagetest` は**現在 1935 行・残り 65 行である**（Issue #140 で 2121 行の警告帯へ入り、Issue #147 が `page/pagetest/cmdtest` を切り出して 1564 行へ戻し、Issue #150 以降の追従で増えた）。**そこは元々「タブと親で共用する検証の道具」の置き場である**（`helper_test.go` 冒頭の方針）が、次に道具を出すときは `page/pagetest` 側も同時に空けなければ入らない。
...
**手 (1) を 1 回打った。** `background.go` の `startBackground()` と、親が並べて持っていた `hr` / `work` / `scopes` の 3 フィールドを `internal/ui/startup` へ出し、親は `bg startup.State` の 1 つを持つ形にした。`ui` 直下は当時 **1925 行（残り 75 行・pass）**、`ui/startup` は 176 行（本番 83 行 + テスト 93 行）である。
...
**次の Issue への予算は残り 56 行、ERROR 境界 2200 まで 256 行である。** 9 周目の 47 行ではない。**採れる手は依然 (1) だけである**（(2)(3) は 6 周目までに使い切っている。受け皿の `page/pagetest` は Issue #147 が 1564 行まで空けたので、道具を出す先としては使えるようになった。現在は 1935 行・残り 65 行である）。残る `app.go` / `chrome.go` / `discover.go` / `keys.go` の 4 ファイル（`ui` 直下の本番ファイルはこれで全部である）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**次に 56 行を超える追加が要る Issue は、まず切り出せるまとまりの設計から始めること。**
...
- `ui/page` 直下には置けない。`page` は 7 タブすべてが import する共通の土台であり、runner 固有の制御をそこへ混ぜると Disk / Logs / Doctor まで引きずる。**この理由は行数に依らない**——当時の `ui/page` は 2003 行・残り -3 行の警告帯だったが、Issue #159 が 1955 行・残り 45 行・pass へ戻した現在も置き場所の判断は変わらない。`page/runnerdetail` を分けたのと同じ判断である。
...
- **`ui/tabset`（切り出し当時 344 行。現在 633 行）** — タブのメタ情報と並び、およびそのテスト。タブを足す Issue が触るのはここであって親 Model ではない
- **`ui/chrome`（切り出し当時 283 行。現在 490 行）** — ヘッダ・タブ行・状態行・フッタの組み立て。親 Model の型も bubbletea もドメインの型も知らない純粋関数にしたので、`App` も `page` も組み立てずに表示用の値だけで検証できる。`ui` 側に残るのは `App` の値を `chrome.View` へ写す `chromeView` の 1 メソッドだけである
