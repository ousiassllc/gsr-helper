...
**散文が現在形で主張する行数・残りも同じ表と突き合わせる。** `TestLineBudgetProseMatchesTables` が本書から 2 つの行数表と改訂履歴を除いた本文を文に切り、「N 行・残り M 行」「残り M 行」という主張が表の行として実在するかを見る。**当時の値を書くときは、同じ文に `当時` か `時点では` を入れること。** これが唯一の除外である——過去の値はその Issue に紐づく記録なので実測へ寄せてはならず、逆に印の無い数は現在の主張として扱う。**ファイル側の残り（1 ファイル 300 行の予算）は `296 行（残り 4）` のように `行` を付けずに書く**——ディレクトリの予算と取り違えないためで、検査もこの形は拾わない。

**改訂履歴の過去欄には当時の値だけを残す。** 現在値は上の行数表を見ること。履歴へ現在値を書くと、同じ数の 4 つ目の写しができて同じ腐り方をする。
...
| `internal/buildconfig/docscheck` | 1334 | 666 | pass |
...
**分けても合計は増える。** 分けた当時は 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・**新設した 2 つの doc コメント**（`docscheck/doc.go` 14 行と `buildconfigtest/buildconfigtest.go` の doc 部分。`internal/buildconfig/doc.go` は分割前からあるもので 12 行のまま、+74 には効いていない）と、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。現在の 3 つの合計は 1216 + 1334 + 44 = **2594 行**である（Issue #164 が行数表と散文を実測と突き合わせる検査を足したぶん、`docscheck` が伸びている）。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。
...
**1 ファイル 300 行の側はほとんど動いていない。** ビルド設定側の最大は `golangci_test.go` の 247 → **249 行**（残り 51）、ドキュメント側の最大は `docs_deppkg_test.go` の 196 → **202 行**（残り 98）である（その後 Issue #164 が足した `docs_linebudget_test.go` **227 行**が、ドキュメント側の最大を更新している）。移した `docs_nolint_test.go` は **183 行**で、どちらの側でも首位にならない。
...
**逼迫しているのはディレクトリだけではない。** 本 PR で `internal/ui/app_test.go` は **300 行（1 ファイル 300 行の上限に張り付き・残り 0）**になり、`internal/ui/page/` も **1998 行（残り 2 行）**になった。**前者は当時の値である**——9 周目（Issue #139）が周期の状態を `discovery.State` へ移した際に `app_test.go` は 274 行まで下がっており、いま `ui` 直下で最大のファイルは `app.go` の 284 行である（上記「ファイルの行数」）。**後者の 1998 行・残り 2 行も当時の値である**——Issue #155 が `page.StateMsg` にフィールド 1 つと doc を足し、`ui/page` は **2003 行・残り -3 行・warn** になった（上記「`ui/page` を警告帯へ入れた判断（Issue #155）」）。**残りがマイナスの状態も当時のものである**——切り出した Issue #159 が `ui/page` 直下のテストの重複を削り、現在は **1955 行・残り 45 行・pass** である（上記「Issue #159 の空け方」）。**残りは 45 行しか無いので、`page` 直下へ共通の `Msg` を 1 つ足すなら、それを超える追加が要らないことを確かめること。**タブをまたぐ移動や本例外と同じく「双方から見える場所がここしか無い」ことを確かめるのは、空けたうえでの話である。
