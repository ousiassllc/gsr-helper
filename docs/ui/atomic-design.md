...
| `internal/buildconfig/docscheck` | 1964 | 36 | pass |
...
| 1.72 | 2026-08-24 | 設定の保存が `scan_roots` と `audit_log` へ届かない件（Issue #132）を反映。(1) 例外 3（`ConfigSavedMsg`）の「親が持たない設定値はこの経路では直らない」の段落を 3 つに割り、`scan_roots` を**この経路へ載せ直した**こと（`Options.Roots` の意味を `--root` の値だけへ狭め、合成は `discover.go` の `scanRoots()` が周期ごとにやり直す。`--root` は保存後も効き続ける）と、`audit_log` は**載せないと決めた**こと（親が配るのは開いたファイルハンドルであり、走行中の開き直しは取りこぼしと二重オープンを招く。代わりに `rememberSelf` が起動時の値と比べて `auditRestartNote` を結果へ添える）を明記した。(2) 「行数の予算」の 2 つの表を実測へ更新し（`ui` 2000 → **2063 行・残り -63 行・warn**、`ui/page/config` 1832 → 1980 行・残り 20 行、`internal/appconfig` 1343 → 1381 行、`cmd/gsr-helper` 989 → 991 行）、降順の並びを直した。**表と同時に散文も直した**——Config タブの節の 2 箇所（1832 行・残り 168 行 → 1980 行・残り 20 行）と、「残りが 1 桁のディレクトリ」の `ui` の括弧書き。(3) 「7 周目の判断（Issue #132）——切り出さずに警告帯へ入った」を新設し、切り出さなかった 3 つの根拠（切り出せるまとまりが無い／バグ修正にスコープ外の再構成を混ぜない／ERROR まで 137 行）と、次の Issue への指示（空けること自体を独立した Issue に立てる／候補は背景処理を `ui/discovery` へ寄せる案）を記録した。`ui` 直下の全件列挙には `hostreq.go` を含め、既に本体を出した後のアダプタであることを添えた。(4) 6 周目の現在形の指示を当時の記録へ改め、**現在の予算と指示は 7 周目が持つ**と参照を付け替えた（本文 3 箇所）。(5) 「ファイルの行数」の次点を実測へ（`app.go` 298 行・`runners_test.go` 297 行） | Issue #128 が親の `cfg` を差し替えるようにしても、設定ファイルの 4 つの編集可能項目のうち `scan_roots` と `audit_log` は `cfg` を経由しておらず、**設定ファイルには書けているのに再起動まで反映されない**ままだった。利用者から見れば #128 と同じ「書いた値が効かない」体験である。本書は 1.67 でこの 2 つを「この経路では直らない」と記していたので、片方を載せ替え片方を明示的に見送った判断を書き換えないと、記述が実装と正面から食い違う。行数表は次の Issue が予算を引く唯一の材料であり、**警告帯に入った事実と入ってよいと判断した理由**を残さないと、次の Issue が同じ逃げ道を根拠なく使う。**表だけを直して散文を取り残す欠陥は 1.69 / 1.70 / 1.71 で 3 周続けて再発しており、本項もそれを踏んだ。** 1.70 が定めた手順（現在形で行数を主張する箇所を機械的に全件抽出して実測と突き合わせる）は、**本項の時点では通していない**——実際に突き合わせたのは 2 つの表と Config タブの散文 2 箇所だけで、同じ Config タブの節の 3 箇所目（「次にこのタブへ手を入れる Issue が使えるのは 168 行」。実測の残りは 20 行で **148 行過大**）と、`internal/disk` の節末尾（「結果は 1927 行（残り 73 行）である」。実測 1980 行・残り 20 行で **53 行過大**）を取り落としていた。前者は改訂 1.73 が、後者は改訂 1.74 が訂正している。**実施していない検証を実施したと書くと、次の周が「もう全件見た」と信じて同じ取り落としを繰り返す**——本欄が当初その主張を持っていたこと自体が、取り落としの訂正に 1.73 / 1.74 の 2 周を要した原因である（訂正は改訂 1.75。Issue #132） |
...
| 1.77 | 2026-08-24 | PR #144（Issue #136）のレビュー指摘（MAJOR 4 件）を反映。(1) UI 層の行数表で `ui/chrome`（490 行）が `ui/page/configmodal`（453 行）の直後に置かれ降順が崩れていたのを、`ui/page/disk/cleanview`（491 行）と `ui/page/configmodal`（453 行）の間へ移した（表の直前の散文が「行数の多い順に並べる」と定めている）。(2) 改訂 1.76 の「回帰テスト 40 行」を実測の **42 行**へ直した（同じ文書の「行数の予算」節は 42 行と書いており、1 つの文書が 2 つの値を主張していた。実測は 2063 → 2105 の +42 行）。(3) **「`chrome_test.go` が 1 ファイル 300 行を超えたので分けた」という存在しない事実を、改訂 1.76 の欄と `internal/ui/chrome/statuswidth_test.go` の doc コメントの 2 箇所とも書き直した**——同ファイルは 206 行で本 PR は 1 行も変えていない。実際は「この検証 114 行を `chrome_test.go` へ足すと 320 行となり警告帯（301〜330 行）に入るため、初めから別ファイルに置いた」である（コメントは行数を変えないよう同じ 5 行に収めた）。(4) 7 周目（Issue #132）の根拠 3 の現在形「まだ 137 行の余裕がある」を、`ui` が 2063 行だった当時の値である旨と現在値（**95 行**）およびその出所への参照を添えた過去形へ改めた（改訂 1.75(2) が `ui/page/runners` の 2164 行に施したのと同型の処置）。あわせて構成を是正し、系列の外（UI 層の行数表の直後）にあった Issue #136 の判断の段落を **`##### 8 周目の判断（Issue #136）`** として 7 周目の節の直後へ移した（6 周目 → 7 周目 の連番の系列に載せた）。本書の作法（改訂 1.72(4)）に従い、7 周目の現在形の指示「次に `ui` 直下へ足す Issue へ。…」を当時の記録へ改め、予算の引き継ぎを指す 4 箇所の相互参照（「残りが 1 桁のディレクトリ」の段落・6 周目の末尾・Issue #8 当時の記録・Issue #77 当時の記録）を「7 周目の判断」から「8 周目の判断」へ付け替えた（1 つ目は節より前にあるため「上記」を「下記」へ直した）。同段落が現在形で主張していた `ui` の 2063 行・残り -63 行も 2105 行・残り -105 行へ直した。**全件突き合わせを実際に通した結果は次のとおり。** `go tool linterly check --format json` の実測（82 ディレクトリ・622 ファイル）と、UI 層の表 38 行・UI 層の外の表 37 行のすべての行数・残り・判定・網羅性・降順を機械的に突き合わせ、**食い違いは上記 (1) の降順 1 件のみ**で数値・判定・網羅性は全件一致した。現在形で行数を主張する散文は 14 箇所を抽出して照合し、**食い違いは上記 (4) の 137 行と 2063 行・残り -63 行の 2 箇所のみ**——`internal/disk` 1980 行・残り 20 行、`internal/setup` 1970 行・残り 30 行、`ui/page/config` 残り 20 行、`ui/page/disk` 1996 行・残り 4 行、`ui/page/logs` 残り 6 行、`page/pagetest` 1936 行・残り 64 行、`page/disk/cleanview` 491 行、`ui/page/setup` 1883 行・残り 117 行、`molecule/listrow` 残り 477 行、`ui/token` 残り 783 行、`ui/tabset` 現在 625 行、`ui/chrome` 現在 490 行は実測と一致していた。「ファイルの行数」節の 3 つの主張（`internal/ui/app_test.go` 300 行・`internal/ui/app.go` 298 行・`internal/ui/page/runners/runners_test.go` 297 行、WARN 以上のファイルは無い）も実測と一致。`docs/ui/screens.md` は行数の主張を持たず、状態行の中略に関する記述と改訂 1.33 は実装（`internal/ui/chrome` の `Status`）と一致していたため変更なし。 | 4 件はいずれも**危険側**の誤りである。(4) は残り 95 行しかないところへ 137 行の予算を提示しており、次に `ui` 直下へ足す Issue が 42 行ぶん多い余裕を前提に判断して ERROR 境界（2200 行）を踏む。(2) は同じ文書が同じ増分に 40 と 42 の 2 つの値を主張する形で、改訂 1.21 / 1.40 / 1.58 / 1.60 が 4 度是正した欠陥の再発である。(3) は**実施していない事実を実施したと書く**もので、本書が 1.74 / 1.75 で 2 度是正した欠陥そのもの——「300 行を超えたら分ける」という前例として読まれると、次の Issue は超えるまで分けないことになり、実際の判断（超える前に別ファイルへ置く）とは逆の運用を導く。(1) は表の直前の散文が自ら定めた並びを表が破っているため、どこが逼迫しているかを表から読み取れない（改訂 1.57 / 1.60 が同じ理由で 2 度直している）。構成の是正は、#136 の判断だけが 6 周目 → 7 周目 の系列の外にあったことが 7 周目の「137 行」を取り残す一因だったためで、系列に載せないと次の周も同じく前周の現在形を取り残す（PR #144 のレビュー指摘） |
| 1.78 | 2026-08-24 | `make check` の並列負荷で config の自己設定テストが散発的に落ちる件（Issue #140）を反映。(1) 行数表を実測へ更新した——`ui/page/pagetest` 1936 → **2121 行・残り -121 行・warn**（`ui` の上へ移した）。**`ui/page/config` を警告帯へ入れたのは本 Issue ではない**——#140 の 2 コミット後は 1989 行・残り 11 行の pass で、警告帯へ入れたのは同じ PR の Issue #141 である（改訂 1.79）。(2) **表と同時に散文も直した**——現在形で行数を主張する箇所を全件抽出して照合し、この 2 つのディレクトリに触れる 7 箇所すべてを更新した（「残りが 1 桁のディレクトリ」の段落、Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「使えるのは 20 行しかない」）、Disk タブの節の受け皿の余裕（「そちらも残り 64 行」）、Logs タブの節の受け皿の余裕（「1936 行・残り 64 行」）、`ui` 直下 5 周目の記録（「現在 1936 行で、残りは 64 行しかない」））。他のディレクトリの数値は本 PR が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した。(3) 「Issue #140 の判断——警告帯へ入れて回帰テストを足した」を Config タブの節へ新設し、**本節冒頭の (3)（テストの道具を別ディレクトリの通常パッケージへ出す）を実際に採ったこと**——束を辿る道具を `ui/page/config` の `findMsg` から `page/pagetest` の `FindMsg` へ回帰テストごと移し、`ui/page/config` を 2119 行から 1989 行（残り 11 行・pass）へ下げた——と、それでも `page/pagetest` が警告帯に残る理由（合計は減らないので別パッケージの新設が要るが、それはバグ修正の範囲外）、および空けること自体を Issue #147 へ切り出したことを記録した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、**`ui/page/pagetest` を警告帯へ入れた**のは本 Issue である（`ui/page/config` を入れたのは同じ PR の Issue #141。改訂 1.79）。表を据え置くと、次に `ui/page/pagetest` へ手を入れる Issue は残り 64 行のつもりで 185 行ぶん過大な予算を引く。`page/pagetest` の逼迫はとりわけ危うい——Logs / Disk の両節が「詰まったら道具をそこへ出せ」と受け皿として名指ししており、その受け皿が本 PR の移動で**最も逼迫したディレクトリになった**事実を書かないと、次の Issue はもう存在しない逃げ道を当てにして計画を立てる。表だけを直して散文を取り残す欠陥は 1.69〜1.72 で 4 周続けて再発しているため、本項でも全件突き合わせを実際に通し、その範囲と結果を上記 (2) に具体的に書いた。**なお本項は当初「`findMsg` は `doneMsg` に依存するので `page/pagetest` へ出せない」と書いていたが、これは事実に反していた**——`findMsg` の引数は `tea.Cmd` と述語だけで、`doneMsg` に依存するのは述語を渡す `doneOf` の側である。レビューでこの誤りが判明したため、記述を直すのではなく**実際に移した**（Issue #140） |
| 1.79 | 2026-08-24 | 同じ PR の Issue #141（自身の設定フォームの見出しを `firstRunUnsaved` で決める）/ #142（対象選択の見出しを runner に限定しない文言へ）を行数表と散文へ反映した——`ui/page/config` **1989 → 2098 行・残り -98 行・warn**（#141 が +66 行、#142 が +43 行）。**`ui/page/config` を警告帯へ入れたのはこの 2 件のうち #141 である**（Issue #140 の 2 コミット後は 1989 行・残り 11 行の pass だった）。降順の位置は `ui` の下のまま変わらない。散文は Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「ERROR の境界まで 145 行」→ **102 行**）と「残りが 1 桁のディレクトリ」の段落を直し、「Issue #140 / #141 / #142 の判断」の節へコミット単位の実測表を置いて、どの Issue がどちらのディレクトリを警告帯へ入れたかを分けて書いた。`title_test.go`（**92 行**。#141 が 57 行、#142 が 35 行）を新設したのは、**#141 の検証だけでも `selfconf_test.go` が 307 行となり 1 ファイル 300 行の警告帯（301〜330 行）に入るため**である。#142 の検証（35 行）も足した現在、1 ファイルに置いていれば 342 行となり ERROR 境界（330 行）を超えていた（実際は `selfconf_test.go` 266 行 + `title_test.go` 92 行に分かれており、どちらも警告帯に入っていない）。境界は行数だけでなく責務でも切ってある（`selfconf_test.go` は「保存の経路が何を書くか」、`title_test.go` は「画面が何と名乗るか」）。`ui/page/pagetest`（2121 行）と `ui`（2105 行）は #141 / #142 が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と定めている。**同じ PR の中で行数が 3 度動いたのに、当初の記述は増分を Issue ごとに正しく割り振れていなかった**——#140 に +75 行、#141 / #142 に +43 行と書いていたが、実測は #140 +9 行・#141 +66 行・#142 +43 行である。#140 は道具と回帰テストを `page/pagetest` へ出したので `ui/page/config` をほとんど増やしておらず、**警告帯へ入れた Issue を取り違えたまま残すと、次に予算を引く Issue が「バグ修正が config を埋めた」という誤った前例を読む**。`title_test.go` の「43 行」も同様の取り違えで、43 は #142 単独のディレクトリ増分であってファイルの行数ではない。ファイルの行数を誤ると、分割の前例として引かれたときに判断の根拠が崩れる——改訂 1.77(3) が「300 行を超えたので分けた」という存在しない事実を是正したのと同型の欠陥である（レビュー指摘。Issue #141 / #142） |
| 1.80 | 2026-08-24 | Issue #139（`ui` 直下の分割）を反映。(1) 行数表を実測へ更新（`ui` **2105 → 1953 行・残り 47 行・pass**、`ui/discovery` 290 → **617**、`ui/hostreq` 283 → **393**）。3 行とも降順の位置を移した（`ui` は `ui/organism/table` 1969 と `ui/page/setup` 1883 の間、`ui/discovery` は `ui/tabset` 625 と `ui/molecule/chromebar` 543 の間、`ui/hostreq` は `ui/page/configmodal` 453 と `ui/page/diskclean` 372 の間）。(2) **`##### 9 周目の空け方（Issue #139）——警告帯から出た`** を新設し、打った 2 手（検出周期の状態を `discovery.State` へ／起動時の前提チェックを `hostreq.State` へ）、`tickMsg` を公開型 `discovery.TickMsg` へ反転させた設計判断とその 2 つの理由、代替の import 検査（`TestOnlyParentDrivesDiscovery`）を**採らずに散文の記録に留めた**理由（`page/pagetest` が残り 64 行しかなく、行数を空ける Issue が別のディレクトリを逼迫させる）、`refresh_test.go`（80 行）を消しても表明が落ちていない根拠（2 本は `discovery/state_test.go` へ移送し `OnTick` が busy でも `TickMsg` を返すことまで強化、1 本は `gate_test.go` の `TestRefreshDuringDiscoveryShowsNotice` が真上位集合として包含し包含側に注記を追加）、次の Issue への予算（残り 47 行・ERROR 境界まで 247 行）と、(1) 以外の手が尽きている旨を記録した。(3) **8 周目の現在形の指示（「ERROR 境界 2200 まで 95 行である」「次に足す Issue はこの 95 行を予算として読むこと」）を当時の記録へ改め**、予算の引き継ぎを指す 5 箇所の相互参照（「残りが 1 桁のディレクトリ」の段落・6 周目の末尾・Issue #8 当時の記録・Issue #77 当時の記録・7 周目の根拠 3 と末尾）を「8 周目の判断」から「9 周目の空け方」へ付け替えた。(4) 「ファイルの行数」の散文を実測へ——`internal/ui/app_test.go` の「上限ちょうどの 300 行（残り 0）で張り付いている」は 274 行になったので落とし、最大が `internal/ui/page/runners/runners_test.go` の 297 行、`ui` 直下の最大が `app.go` の 286 行であることへ書き換えた。6 周目の「本 PR で `app_test.go` は 300 行になった」にも当時の値である旨を添えた（`page/` の 1998 行・残り 2 行は現在も一致するのでそのまま）。(5) ディレクトリ構成のツリーを実ファイルへそろえた——`app.go` の説明から「Tick」を落とし（`discovery` へ移った）、`discover.go` を「検出の駆動のうち親にしか決められないもの」へ、`discovery/` を「周期の状態（State と契機 TickMsg）」を持つ形へ、`hostreq/` を「その状態（State）」を持つ形へ書き換え、`chrome.go` に `doctorKey()` の移動を反映した（`hostreq.go` は元々ツリーに無いため削除に伴う行の除去は不要だった）。(6) 実装状況の「実装済み」の行で `discovery` / `workscan` / `ghscope` の括弧書きを、`hostreq` を含む 4 つが `State` を持つ形にそろったことへ更新。(7) Issue #77 当時の記録が挙げていた `gate_test.go` の非公開参照（`after.inflight` / `tickMsg`）に、現在の姿（`disc.Busy()` / `disc.Seq()` / `discovery.TickMsg`）を添えた。**全件突き合わせを実際に通した結果は次のとおり。** `go tool linterly check --format json` の実測（82 ディレクトリ・625 ファイル）と、UI 層の表 38 行・UI 層の外の表 37 行のすべての行数・残り・判定・網羅性・降順を機械的に突き合わせ、**食い違いは上記 (1) の 3 行のみ**で、残る 72 行は数値・判定とも一致し、両表とも降順は崩れておらず、`internal/` と `cmd/` の行を持つディレクトリで表に載っていないものは無かった。行数を主張する散文（過去形の記録を含む）は 39 箇所を抽出して照合し、**実測と食い違っていたのは 5 箇所（数値としては 7 個）だけである**——「残りが 1 桁のディレクトリ」の段落の `ui` 2105 行・残り -105 行、「ファイルの行数」の `app_test.go` 300 行と `app.go` 298 行、6 周目の `app_test.go` 300 行、7 周目の「現在の余裕は 95 行」、8 周目の「2200 まで 95 行」である。残る 34 箇所——`internal/setup/tarball` 残り 1 行、`internal/logs` 残り 2 行、`internal/gh` 残り 4 行、`internal/disk` 1980 行・残り 20 行、`internal/setup` 1970 行・残り 30 行、`ui/page` 1998 行・残り 2 行、`ui/page/disk` 1996 行・残り 4 行、`ui/organism/dialog` / `ui/page/logs` 各 残り 6 行、`ui/page/runners` 1983 行・残り 17 行、`ui/page/config` 1980 行・残り 20 行、`ui/organism/table` 1969 行・残り 31 行（本体 6 ファイル 1053 行）、`page/pagetest` 1936 行・残り 64 行、`ui/page/setup` 1883 行・残り 117 行、`molecule/listrow` 残り 477 行、`ui/token` 残り 783 行、`ui/tabset` 625 行、`ui/chrome` 490 行、`page/disk/cleanview` 491 行、`organism/table/tabletest` 243 行はいずれも実測と一致していた。**依存グラフの裏取りも通した**——`go list` の直接 import で `ui/discovery` → `runner` / `exec` のみ（`appconfig` の辺は増えていない。`discovery.Input` はプリミティブだけを持つ）、`ui/hostreq` → `doctor` のみであることを確認し、[コンポーネント設計](../components/overview.md#依存の方向)のグラフに追記の要る辺は無かった。 | **`ui` 直下は 7 周目・8 周目と 2 周続けて「判断を残して警告帯へ入る」逃げ道を使っており、Issue #139 はその借りを返す周である。** 行数表は次の Issue が予算を引く唯一の材料であり、実測とずれていると境界に当たってから気付くことになる——**今回は危険側ではなく安全側のずれ（実際には 247 行あるのに 95 行と読ませる）だが、それでも誤りである**。予算を過小に見せると、47 行で足りる追加までもが「まず空けてから」という不要な前置きを負い、逆に 7 周目・8 周目が実際にそうしたように「どうせ警告帯だから」と分割の検討を省く根拠にもなる。あわせて **Issue #139 は #77 の再発である**（#77 も `ui` 直下が超過して空けた周で、当時の結論は「次に手を入れる Issue は 1 行足す前に必ず空けること」だった）。同じディレクトリが 5 周（#77 → #128 → #132 → #136 → #139）にわたって上限へ張り付き続けているのは、**現在形の予算がどの周にあるかを読み手が辿れないと、前周の指示が古いまま参照される**ためであり、改訂 1.69 が定めた「次の Issue への現在形の指示は最新の周に一本化する」規範を本項でも守った（上記 (3)）。散文を取り残す欠陥は 1.69〜1.75 で 4 周繰り返しているため、1.70 が定めた全件抽出の手順を実際に通し、その結果を上欄に列挙した（Issue #139） |
...
| 1.86 | 2026-08-24 | Issue #150（`RunAll` / `Msgs` / `ChromeOf` / `IsQuit` が締め切り無しで `Cmd` を走らせる）を反映。(1) ディレクトリ構成の `page/pagetest/cmdtest/` の道具の列挙へ、本 PR が中心に据えた `MustMsgs` / `RunAll` / `ChromeOf` を追加した。(2) 「行数の予算」の表を実測へ更新し、降順の不変条件が崩れていた 2 行を並べ直した（`ui` **1925 → 1943**・残り 57 で `ui/page/logs` の上へ、`page/pagetest` **1564 → 1677**・残り 323 で `ui/page/doctor` の上へ。ほかに `page/config` 1990 → **1992**・残り 8、`page/runners` 1989 → **1991**・残り 9、`page/setup` 1886 → **1890**・残り 110、`cmdtest` 687 → **992**）。(3) 「残りが 1 桁のディレクトリ」を **6 つから 8 つ**へ改め、本 PR が落とした `page/config`（8）と `page/runners`（9）を列挙へ加えた。(4) 「ファイルの行数」の最大ファイルの一覧から `cmdtest/run.go`（292 行）を外し、`run.go` 193 行 / `find.go` 158 行という現在値へ差し替えた。(5) 「前例」へ本 PR の `run.go` → `find.go` の分割を追記した。(6) 現在形で古い値を主張していた散文を実測へそろえた（`page/config` の残り 10 → 8、`page/setup` の 1886 → 1890、`page/pagetest` の 1564・残り 436 → 1677・残り 323 の 2 箇所、`ui` 直下の残り 75・ERROR まで 275 → 57・257、`page/runners` の 1989・残り 11 → 1991・残り 9 の 3 箇所）。(7) `ApplyChrome` と `ScanKey` が `Msgs` を使うという記述を **`MustMsgs`** へ訂正した | 本 PR は 158 行の新規ファイル（`find.go`）と `cmdtest` へ 413 行を足しながら、配置と予算の判断の唯一の拠り所である本書を更新していなかった。行数表は 6 ディレクトリが実測とずれ、うち `page/config` と `page/runners` は**本 PR 自身が残り 1 桁の帯へ落としたのに**「この N 個に 1 行でも足す Issue は足す前に空けること」の列挙から漏れており、次の Issue が予算を読み違える状態だった。表が自ら宣言する降順の並びも 2 行で破れていた。`find.go:16` は分割の根拠として本書の「ファイルの行数」を指しているのに、その節の一覧は分割前の `run.go` 292 行を現在値として載せ、「前例」には本 PR の分割が無く、**コードが存在しない記録を指す**状態だった。`ApplyChrome` / `ScanKey` の 2 箇所は、本 PR が `Msgs` から `MustMsgs`（待ち時間切れで panic）へ替えたことで、諦めた分が黙って落ちるか否かという**壊れ方の違いを取り違えさせる**記述になっていた（PR #154 のレビュー指摘） |
| 1.87 | 2026-08-24 | PR #154 の 2 巡目のレビュー指摘（CRITICAL 1 件・MAJOR 4 件・MINOR 4 件）を反映。(1) **10 周目の予算段落の自己矛盾を直した**——見出しの「残り 57 行」に対し、同じ段落の**指示の側**が「次に 75 行を超える追加が要る Issue は…」と改訂 1.86 以前の値のままだった。予算ではなく指示を読む次の Issue が 75 行を足すと `ui` は 2018 行の警告帯に入る——節が防ごうとしているものそのものである。9 周目の同型の文（予算と指示がどちらも 47 行）に合わせて **57** へそろえた。(2) 改訂 1.86 が見落とした現在形の古い値 4 箇所を実測へそろえた（`page/config` の 1990 行・残り 10 行 → **1992 行・残り 8 行** が 2 箇所、`page/disk` の 1996 行・残り 4 行 → **1997 行・残り 3 行**、`page/runners` の 1983 行 → **1991 行**）。`page/disk` は同じ文書の別の箇所（「残り 3 行」）と食い違っており、**1 つの文書が 2 つの値を主張していた**。過去形の記録（Issue #147 当時の 1990 行）は当時の値として残した。(3) `ui` 直下の最大ファイルの現在値を **286 → 284 行**へ直した（改訂 1.86 が「ファイルの行数」節では 284 へ直しながら、9 周目の節の同じ主張を残していた）。同じ文の `app_test.go` 274 行は **当時の値として残した**——文が「前者は当時の値である」と明示しており、`git show 78a4d1c` で Issue #139 当時に実測 274 行であることを確認した（現在値は上記「ファイルの行数」を見よ）。現在値へ書き換えると「ファイルの行数」節の同じ記録と新たに食い違う。(4) 改訂 1.86 の変更理由の 2 つの数を実測へ直した（`find.go` **157 → 158 行**——同じ行の項目 (4) と実測はどちらも 158 で、1 行の中で 2 つの値を主張していた。`cmdtest` の増分は「約 253 行」の概数をやめ、origin/feat/#1 の 687 行からの純増 **406 行**とした）。(5) 「ファイルの行数」の `cmdtest` のテストの内訳を **`run_test.go` 161 行 / `cmd_test.go` 151 行 / `find_test.go` 141 行**へ更新した（本 PR が `find.go` の検証を `find_test.go` へ分けた）。(6) 行数表を実測へ更新した（`page/pagetest` 1677 → **1763**・残り 237 で `ui/page/jobs` の上へ移し降順を保った。`cmdtest` 992 → **1100**・残り 900）。あわせて 10 周目の節の `page/pagetest` の現在値も 1677・残り 323 → **1763・残り 237** へそろえた | 予算段落の (1) は**指示として読まれる文が予算と食い違う**という点で、単なる古い数値より害が大きい——次の Issue は見出しの残り行数ではなく「次に N 行を超える追加が要るなら」の N を自分の上限として読むためである。(2) の 3 件は改訂 1.86 が「現在形で行数を主張する散文を全件照合した」と宣言しながら取り残したもので、`page/disk` に至っては同一文書内で 2 つの値が並立していた。(3) は同じ型の取り残しで、**同じ commit が片方だけ直して他方を残した**ため、文書が自分自身と食い違う状態が新たに生まれていた。(4) の `find.go` 157/158 も 1 行の中での食い違いである。本 PR のコード側の修正（`assertNested` が締め切り無しで Cmd を走らせていた点、`IsQuit` の doc がコードと逆を述べていた点、`OpenTabOf` の未検証の error の腕）は本書の記述と直接には関わらないが、(5)(6) はその修正がファイルとディレクトリの行数を動かしたことへの追従である（PR #154 のレビュー指摘） |
| 1.88 | 2026-08-24 | Issue #151（依存グラフの辺を `go list` の import と突き合わせる検査の新設）を反映し、「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（1475 → **1969 行・残り 31 行**・pass）。降順の並びが変わるため、`internal/doctor/jobreq`（1495）の直前から `internal/setup`（1970）と `internal/config/edit`（1922）の間へ移した。新設したのは 3 ファイル（`docs_depgraph_test.go` 259 行・`docs_depnodes_test.go` 125 行・`docs_mermaid_test.go` 110 行）で、いずれも 1 ファイル 300 行の警告帯（301〜330 行）に入らない。**これも当時の値である。3 ファイルはその後 Issue #161 が 3 つとも `internal/buildconfig/docscheck/` へ移しており、改訂 1.90 が現在値として書いた 296 行 / 123 行 / 147 行はいずれも古い**（現在値は上記「ファイルの行数」を見よ。出所は下記「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」。この訂正は改訂 1.94）。警告帯に入らないことは現在も変わらない（余裕がどれだけ残っているかも上記「ファイルの行数」が持つ）。**パーサを別ファイルへ分けたのは行数のためだけではなく責務の境界でもある**——`docs_mermaid_test.go` は「mermaid をどう読むか」、`docs_depgraph_test.go` は「読んだ辺を実装とどう突き合わせるか」を持つ（「ファイルの行数」の方針どおり）。ディレクトリは 2000 行の警告帯には入っておらず残りは 31 行だった。**これは当時の値であり、改訂 1.89 の時点では 2025 行・残り -25 行、改訂 1.91 の時点では 2122 行・残り -122 行の warn だった。**現在は警告帯の外の pass である**（Issue #161 が `docscheck` を切り出して警告帯を出た。現在値は上記「行数の予算」の行数表を見よ。出所は下記「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」。この訂正は改訂 1.94）**（PR #153 の 2 周目が +56 行。予算と次の Issue への指示は下記「`internal/buildconfig` を警告帯へ入れた判断」が持つ）。次に `internal/buildconfig` へテストを足す Issue は先に空け方を検討すること（採れるのは**検査そのものをパッケージ境界で別ディレクトリへ分ける**手である——ビルド設定の検査とドキュメントの検査は増え方が違う。手 (1) は「**本番の一部を**パッケージ境界で切り出す」であり、`internal/buildconfig` は [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)が明記するとおり**実行時のコードを持たないテスト専用パッケージ**（`doc.go` 12 行のほかはすべて `_test.go`）なのでそのままでは当たらない。理由——増え方が違う——は同じである。この訂正は改訂 1.89） | 表は「実測値」と明記してあり、**この節が次の Issue の読む行数予算の規範である**。494 行を足したのに表を据え置くと、次に `buildconfig` へ検査を足す Issue が 494 行ぶん過大な予算を引き、警告帯を越えてから気付くことになる。表だけを直して散文や並びを取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 で繰り返し再発しているため、今回は降順の位置も同時に直し、`internal/buildconfig` の行数を現在形で主張する散文が他に無いことも確認した（改訂 1.71 の記述は当時の値の記録なので変更しない）（Issue #151） |
...
| 1.90 | 2026-08-24 | PR #153 の最終レビュー（変異注入で実測した major 4 件）を反映。(1) **改訂 1.88 が現在形で与えていた 3 ファイルの行数（`docs_depgraph_test.go` 259 行・`docs_depnodes_test.go` 125 行・`docs_mermaid_test.go` 110 行）を当時の記録へ改め、現在値（296 行・123 行・147 行。**この 3 つは 1.90 当時の値である。改訂 1.91 の分割後は `docs_deppkg_test.go` 196 行・`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行**）と出所への参照を添えた**——改訂 1.89 がディレクトリ値に施したのと同じ処置であり、同じ欄のファイル値だけが取り残されていた。(2) 「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（2025 → **2041 行・残り -41 行・warn**）。降順の先頭という位置は変わらない。表の直後の散文の数値も揃えた。(3) 「`internal/buildconfig` を警告帯へ入れた判断」に最終レビューの **+16 行**とその内訳（塞いだ 3 つの「黙って緑になる」経路）を追記し、項 4 の ERROR 境界までの残り（175 → **159 行**）と項 5 のファイル行数（最大 286 → **296 行・残り 4 行**）を実測へ更新した。あわせて**`docs_depgraph_test.go` の残り 4 行がディレクトリの残量より先に尽きる**こと、5 行以上足すなら先にこのファイルを責務の境界で分けることを指示として書いた。(4) 「1 ファイル 300 行」の次点の一覧に `docs_depgraph_test.go` 296 行を加えた。(5) 改訂 1.89 が自らの記録として並べた 5 つの根拠のうち 2 つ（ERROR 境界まで 175 行／最大 286 行）が現在形のままだったので、当時の記録である旨と現在値の出所（改訂 1.90）を添えた。(6) 改訂 1.89 の文末が指す改訂番号が `1.86` のままだったのを `1.88` へ直した（ベース取り込みで PR #154 が 1.86 / 1.87 を使ったため #151 側を採番し直した際の取り残し） | **1 つの文書が同じファイルに 2 つの行数を与えている状態だった**——改訂 1.88 は `docs_depgraph_test.go` を 259 行と現在形で主張し、同じ文書の「`internal/buildconfig` を警告帯へ入れた判断」の項 5 は 286 行と主張していた。[コンポーネント設計](../components/overview.md)の改訂 1.51 が「1 つの文書が同じ対象に 2 つの数を主張する形で、本書が繰り返し是正してきた欠陥である」と呼ぶ形そのものである。**しかも危険側のずれである**——1.88 を読んだ次の Issue は `docs_depgraph_test.go` の 300 行までの余裕を **41 行**と見積もるが、**実際は 4 行しかない**（改訂 1.85 の (7) が「予算を 3 行多く見せる危険側のずれ」として直したのと同型で、ずれ幅はより大きい）。改訂 1.89 がディレクトリ値には但し書きを足しながら**同じ欄のファイル値には足していない**ことから、意図的な当時記録ではなく取り残しであることが読み取れる。(2)(3) は表と判断の節が**次の Issue の読む行数予算の規範である**ため、16 行を足して据え置くと次の周が残量を過大に見積もり、ERROR 境界（2200 行）へ近づいてから気付く。(5) を放置すると、改訂履歴を索引に使う読み手が存在しない訂正関係を辿る（PR #153 最終レビュー） |
| 1.91 | 2026-08-25 | PR #153 の最終レビュー 2 巡目（3 観点が**変異注入で実測**した MAJOR 3 件）を反映。(1) 「行数の予算」の非 UI の表と「残りが 1 桁のディレクトリ」の段落の `internal/buildconfig` を実測へ更新した（2041 → **2122 行・残り -122 行・warn**。表の位置は最上段のままで降順は保たれている）。(2) 「`internal/buildconfig` を警告帯へ入れた判断」に **`**最終レビューの 2 巡目**` の段落を新設**し、塞いだ 3 経路（フェンスの重複ガードが CommonMark の許すインデント付き・4 個以上のバッククォートを数え落としていた／例外として名指しした `UIApp --> UIParts` の定数に生存確認が無く改名 1 手で 1 巡目の穴が復活した／散文が「例外は 1 本だけ」と宣言しながら 2 本目を描いても通った）と、フェンスを数える範囲を「## 依存関係」節の中に限ったことを記録した。既存の段落は「1 巡目で +16 行して」と当時の記録へ改めた。(3) 項 4 の ERROR 境界までの残りを 159 → **78 行**へ、項 5 を**分割後の実態**へ更新した（ディレクトリ全体の最大は `golangci_test.go` 247 行、本 PR の新設分の最大は `docs_deppkg_test.go` **196 行・残り 104 行**。`docs_mermaid_test.go` 192 行・`docs_depgraph_test.go` 136 行・`docs_depnodes_test.go` 123 行）。**`docs_depgraph_test.go` を責務の境界で 2 つに分けた判断**（296 行・残り 4 行で、例外定数の生存確認と「1 本だけ」の表明を足すと確実に超えた）と、境界を責務でも引いた根拠（図をどう読むか／実装の側を `go list` でどう読むか／読んだ両者をどう突き合わせるか）を同項に残した。あわせて項 5 が主張していた「残り 4 行はディレクトリの残量より先に尽きる」という**同じ節の中で矛盾する記述**（同節と表はディレクトリを -41 行、すなわち既に尽きていると書いていた）を、比較対象が ERROR 境界までの余裕であることを明示する形へ直した。(4) 改訂 **1.88** のディレクトリ値の停滞を訂正した——同欄は「現在は 2025 行・残り -25 行の warn である」と現在形で主張しており、同じ文書の表と判断の節（2122 行）と食い違っていた。あわせて **1.89 (4)** の「現在値（2025 行・残り -25 行）」と **1.90** のファイル行数（296/123/147）にも当時の記録である旨と現在値を添えた。(5) 判断の節の「上記 1.」が実際には 2 行下を指していたのを直した | (4) は**危険側のずれ**である。改訂履歴を索引に使う次の Issue は残量を -25 と読み、実際より 97 行多い予算を引いて ERROR 境界（2200 行）へ近づいてから気付く。**これは改訂 1.90 が同じ欄の「ファイル値」に対して直したのと同型の欠陥の鏡像**で、1.90 は「1.89 がディレクトリ値には但し書きを足しながらファイル値には足していないことから、意図的な当時記録ではなく取り残しであることが読み取れる」と自ら述べていた——今回はファイル値を更新しながらディレクトリ値の但し書きだけが取り残されていた。**表だけを直して散文や改訂欄を取り残す欠陥は 1.69 / 1.70 / 1.71 / 1.72 / 1.73 に続く再発**であり、本項では現在形で行数を主張する箇所を機械的に全件抽出して実測（`go tool linterly check --format json`）と突き合わせた。(1)〜(3) は表が「実測値」と明記しており**この節が次の Issue の読む行数予算の規範である**ため、81 行を足したまま据え置くとその分だけ過大な予算を引かせる。(2) の 3 経路はいずれも**この検査群の存在意義（黙って緑にしない）そのもの**に反する穴で、判断の記録に残さないと「1 巡目で塞いだ」という記述だけが残り、次の周が同じ変異を試さない。(3) の矛盾は本書が繰り返し是正してきた「1 つの文書が同じ対象に 2 つの数を主張する」形である（Issue #151） |
| 1.92 | 2026-08-25 | Issue #155〜#157 を反映。(1) 行数表を実測へ更新した（`ui/page` 1998 → **2003 行・残り -3・warn**、`ui/page/runners` 1991 → **2000 行・残り 0**、`ui/page/pagetest` 1763 → **1932 行・残り 68**、`ui/page/pagetest/cmdtest` 1100 → **1341 行・残り 659**、`ui/page/runnerop` 1400 → **1404 行・残り 596**、`ui` 1943 → **1944 行・残り 56**）。降順を保つため `ui/page/runners` を `ui/page/disk` の上へ、`ui/page/pagetest` を `ui/page/logs`（1927 行）の上へ移した。(2) 「残りが 1 桁のディレクトリ」の段落を 8 つ → **7 つ**へ改め、警告帯のディレクトリを `internal/buildconfig` の 1 つ → **`internal/buildconfig` と `ui/page` の 2 つ**へそろえた。(3) 新しい節「`ui/page` を警告帯へ入れた判断（Issue #155）」を足した——ドレイン停止の走査の継ぎ目（`page.StateMsg.ScanProcs`）は共有状態にしか置けず、着手時点の残りが 2 行だったため doc 付きのフィールド 1 つで警告帯に入る。空ける作業は Issue #159 へ切り出した。(4) 「ファイルの行数」の `cmdtest` の内訳を実測へ更新した（`run.go` **193 行** / `find.go` 158 → **163 行**、テストは `run_test.go` 161 → **182 行** / `pump_test.go` **204 行**（Issue #157 の新設） / `cmd_test.go` **151 行** / `find_test.go` 141 → **149 行**）。**当初この項は `find.go` / `find_test.go` の 2 つだけを直して「実測へ更新した」と書いていたが、実際には `run_test.go` が 161 行のまま残り、テストファイルの本数も Issue #157 が `pump_test.go` を足して 4 つになったのに「3 つに分かれている」のままだった**（本項の記述をその実態に合わせて書き直し、本文の数値と本数も直した）。 (5) 同節の「次点は 296 行が 3 つ」から `internal/buildconfig/docs_depgraph_test.go` を外した——改訂 1.91 が責務の境界で 2 つに分けて **136 行**になっており、296 行のままだと**そのファイルに残り 4 行しか無いと読ませる**。次点は `runners.go` / `overlay_test.go` の 2 つである。 (6) 「6 周目の空け方」の「後者の 1998 行・残り 2 行は現在も同じ」を当時の記録へ改め、現在値（**2003 行・残り -3 行・warn**）と本改訂 (3) の節への参照を添えた。 (7) 「10 周目の空け方」の現在形の予算を実測へそろえた（`ui` の残り 57 → **56 行**、ERROR 境界 2200 までの余裕 257 → **256 行**、受け皿の `page/pagetest` 1763 行・残り 237 行 → **1932 行・残り 68 行**）。 (8) 「残りが 1 桁のディレクトリ」の段落末の `ui` 直下の現在値を表とそろえた（1943 行・残り 57 行 → **1944 行・残り 56 行**）。 (9) 本改訂 (3) の節の項 3 が `ui/page/pagetest` の残りを **217 行**と書いていたのを実測（**1932 行・残り 68 行**）へ直した——同じコミットの表が別の数を書いており、1 つの文書が同じ対象に 2 つの数を主張していた。 (10) **本 PR の残る現在形の行数を全件測り直してそろえた**（`go tool linterly check` と `wc -l`）。`page/pagetest` の現在値を **1932 行・残り 68 行**へ統一した 4 箇所（`ui/page/disk` の節の「受け皿は空いた」、`ui/page/logs` の節の「受け皿ごと空けた…使える」、「Issue #31 の周」、「10 周目の空け方」）、`ui/page/runners` の現在値を **2000 行・残り 0 行**へ直した 3 箇所（Config タブの節の「現在 1991 行」、サービス制御の節の 2 箇所）、`ui/page` 直下を **2003 行・残り -3 行の警告帯**へ直した 1 箇所（サービス制御の節の「残り 2 行」）、`ui/tabset` の現在値 635 → **633 行**、`runners` のサービス制御のテスト 3 ファイルの合計 589 → **602 行**、`organism` 一覧の `organism/table` の規模 1700 → **2000 行**、本節の型 (a) の手順が挙げる本書の一意な識別子の総数 875 → **948 個**（同じ抽出コマンドの実測）。 (11) **助言として読まれる文を残量に合わせて書き直した**——`ui/page/disk` と `ui/page/logs` の 2 節は `page/pagetest` を「空いた／使える」とだけ述べており、68 行という上限を示していなかった。`ui/page/runners` の「**残り 9 行は実質ゼロである**」は残量が 0 になったので「上限まで 1 行も無い」へ改めた | (1)(4) 行数表は次の Issue が予算を引く唯一の材料であり、実測とずれていると境界に当たってから気付くことになる（改訂 1.47 / 1.86 / 1.87 が同じ理由で更新している）。(2) は本節が自ら「この N 個に 1 行でも足す Issue は、足す前に空けること」という**指示として読まれる文**を持つため、対象の集合が実態とずれると次の Issue が空けるべき場所を取り違える（改訂 1.87 の (1) と同型）。(3) は本節が定める「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」に従ったものである。**当初この節は「切り出せば循環になる」と書いていたが、これは誤りだった**——`StateMsg` が参照するのは下位パッケージだけで `page` の識別子を 1 つも使わないため、`page/state` への切り出しに循環は生じない。**コンパイラが禁じている**と書くと、実際には開いている選択肢を次の Issue が検討せずに捨てる。理由を波及の広さ（数十箇所の改名）へ改め、循環の主張は実際に跨がりのある `Overlay` に限って条件付きで残した。 (5)〜(9) は**行数を現在形で主張する散文が、同じコミットで直した表と食い違って残っていた**箇所である。表だけを直して散文を取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 で繰り返し再発しており、本項では `wc -l` と `go tool linterly check` で全件を測り直した。とりわけ (6) と (9) は**危険側のずれ**である——(6) は `ui/page` の予算を +2 行と読ませるが実際は -3 行の警告帯であり、(9) は `page/pagetest` の余裕を実際の 2 倍以上に見せる。(4)(5) は「実測へ更新した」という改訂欄の記述自体が**実際にやったことより広く**書かれていた例で、改訂履歴を索引に使う読み手が「もう直っている」と信じて元の数を読み続ける。(10)(11) は本項の (5)〜(9) と**同じ周で同じ欠陥が残っていた**もので、表と一部の散文だけを直して残りを取り残す形（改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 に続く再発）である。とりわけ `ui/page/runners` の「残り 9 行」と `ui/page` 直下の「残り 2 行」は**危険側のずれ**であり、前者は上限まで 0 行しか無いところを 9 行あると読ませ、後者は既に警告帯（-3 行）にあるところを 2 行の余地があると読ませる。本項 (1)(4)(7)(9) の数も本 PR のその後のコミット（Issue #157 の `pump_test.go` の追加検証と `pagetest.ScanOf` の doc）で動いたため、改訂欄自身を最終値へそろえた——改訂欄が表と食い違うと、索引として読む次の Issue が古い予算を引く（PR のレビュー指摘 / PR #160 のレビュー指摘） |
| 1.93 | 2026-08-25 | Issue #159 を反映。(1) 行数表を実測へ更新した（`ui/page` 2003 → **1955 行・残り 45・pass**、`ui/page/runners` 2000 → **1923 行・残り 77**、`ui/page/runners/rowview` 269 → **308 行・残り 1692**）。降順を保つため `ui/page` を `ui` の上へ、`ui/page/runners` を `ui/page/setup` の上へ、`ui/page/runners/rowview` を `ui/ghscope` の上へ移した。(2) 「残りが 1 桁のディレクトリ」を 7 つから **6 つ**へ改め、`ui/page/runners` を一覧から外した。警告帯のディレクトリを「`internal/buildconfig` と `ui/page` の 2 つ」から**`internal/buildconfig` の 1 つだけ**へ改めた。(3) 「ファイルの行数」の最大を `runners_test.go` 298 行から **`runners.go` 296 行（残り 4）**へ改めた。(4) 「`ui/page` を警告帯へ入れた判断（Issue #155）」の末尾を、切り出した Issue #159 が空けた旨へ書き換え、新節「**Issue #159 の空け方**」を追加した。 (5) **表を直しながら現在形のまま取り残していた散文を全件測り直してそろえた**——`ui/page/runners` を **1923 行・残り 77 行**へ直した 4 箇所（Config タブの節の「現在 2000 行・残り 0 行」、「`ui/page` を警告帯へ入れた判断」の項 3、サービス制御の節の段落中と段落末の 2 箇所）とその節末の指示（「残りは 0 行——上限まで 1 行も無い」→ 残り 77 行と次の第一候補 `setupreq`。同節のテスト 3 ファイルの合計 602 → **584 行**）、`ui/page` 直下を **1955 行・残り 45 行・pass** へ直した 2 箇所（「6 周目の空け方」の「残りはもうプラスではない」、サービス制御の節の置き場所の理由）、「`ui/page` を警告帯へ入れた判断」の項 2 のテストの本数と行数（6 ファイル 824 行・`overlay_test.go` 296 行を Issue #155 当時の値と明示し、現在値 **5 ファイル 776 行**・**284 行**を併記）、「ファイルの行数」の `overlay_test.go` を 277 → **284 行**。 (6) 本改訂 (4) の新節の 2 つの誤りを直した——「削除した `TestOverlayRegisteredModalFollowsStackRules` の個々のアサーションは**すべて**別のテストが持つ」は偽で、「2 枚重ねた状態で `View()` が最上位を描く」だけがどこにも残っていなかった（`TestOverlayCloseOneByOne` へ足し直し、その経緯を節に記した）。手ごとの内訳（−45 / −21 / −19 / −14）は削除行だけを数えており節自身の −81 行と合わず、ファイルごとの差し引き（合計 **−77 行**）へ置き換えた。 (7) 同節の項 3 に、`pagetest.OpenTabOf` への置き換えが孤児ユニットの行の 1 箇所だけ `err == nil` のままで `ErrNotFound` と `ErrCmdTimeout` を再び潰していたこと（本 PR のレビュー指摘で `errors.Is` へ修正）を書き足した | Issue #155 が `ui/page` を警告帯（残り -3 行）に、`ui/page/runners` を残り 0 行に残しており、どちらも次の 1 行で warn に入る状態だった。本番コードを 1 行も変えず、手 (2) のテストの重複削減だけで両方に余裕を作ったため、実測値と「次に足す Issue への予算」の記述が古くなっていた。ネストしたパッケージの新設（`setupreq`）とフィクスチャの `pagetest` への移動を**採らなかった**理由も、次の Issue が同じ検討を繰り返さないように書き残した。(5) は**表だけを直して散文を取り残す**欠陥で、改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 / 1.90 / 1.91 / 1.92 で繰り返し再発しており、1.92 が全件の測り直しを規範として書いたにもかかわらず本改訂の初版で再び残っていた。とりわけ `ui/page/runners` の「残り 0 行」は**危険側のずれ**であり、77 行の余裕があるところを「1 行も無い」と読ませて、次の Issue に不要な分割を検討させる。(6) の「すべて別の経路が持つ」は**テストを削る判断の根拠そのものが偽だった**もので、`View()` を `stack[0]` へ書き換える変異で実際に覆いが 1 つ落ちていたことを確かめた。内訳の数は節自身が主張する −81 行と足し合わせが合わず、読み手が どちらかを誤りと判断できない（本 PR のレビュー指摘）。行数は本 PR のレビュー修正（`overlay_test.go` +7 行 / `logs_test.go` +4 行）で動いたため、表・散文・本欄をすべて最終の実測へそろえた |
| 1.94 | 2026-08-25 | Issue #161 を反映。(1) 行数表を実測へ更新した（`internal/buildconfig` 2122 → **1216 行・残り 784・pass**、`ui/page/pagetest` 1932 → **1935 行・残り 65**）。降順を保つため `internal/buildconfig` を最上段から `internal/svc` の下へ移し、新設した `internal/buildconfig/docscheck`（**936 行・残り 1064**）を `cmd/gsr-helper` の下へ、`internal/buildconfig/buildconfigtest`（**44 行・残り 1956**）を表の末尾へ足した。(2) 「残りが 1 桁のディレクトリ」の段落を、**警告帯に入っているディレクトリが 1 つも無い**状態へ書き換えた。(3) 「`internal/buildconfig` を警告帯へ入れた判断」の末尾に、その節が出した「空けること自体を独立した Issue に立てること」という指示が Issue #161 として実施されたことを追記し、実施の判断を「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」の節に記録した（境界を引けた根拠の実測——共有していた識別子は `repoRoot` 1 つだけ——、共通ヘルパを写さず `buildconfigtest` へ出して `pagetest/import_test.go` の `fixtures` へ登録した判断、分けても合計は **+74 行**になること、ファイル側の実測）。(4) 「Issue #159 の空け方」の `ui/page/pagetest` の実測 2 箇所を、表と同じ 1935 行・残り 65 行へそろえた。(5) **PR #163 のレビュー指摘で `nolint` 棚卸しの検査を `docscheck` へ移し、本欄を含む実測値を全件測り直した**——`TestSetupDocNolintInventoryMatchesTree` は `docs/environment/setup.md` の表を見るドキュメントの検査でありながらビルド設定側に残っており、同じ setup.md を読む `TestSetupDocEmbedsConfigFilesVerbatim`（`docs_sync_test.go`）と 2 パッケージへ割れていた（`nolint_inventory_test.go` → `docscheck/docs_nolint_test.go`）。移動で動いた値は本欄の (1)(3) のほか、「残りが 1 桁のディレクトリ」の段落、「2 つに分けた判断」の節（合計 **2196 行**・ビルド設定側に残るのは **9 つの `_test.go`**・ファイルの最大は `golangci_test.go` **249 行**とドキュメント側の `docs_deppkg_test.go` **202 行**）である。(6) 同じレビューで「`internal/buildconfig` を警告帯へ入れた判断」の項 4・項 5 を**当時の値と明示したうえで現在値を併記する**形へ直し、項 5 末尾の次 Issue 向け指示（「ディレクトリ側は既に残りマイナスである」）を Issue #161 が解消した旨へ書き換えた。(7) 「空け方の手は 4 つある」（当時は 3 つ）の (3) が挙げるフィクスチャ用の通常パッケージに `internal/buildconfig/buildconfigtest` を、「ファイルの行数」の `docs_depgraph_test.go` の参照を移動後のパス（`internal/buildconfig/docscheck/`）へ足した。(8) **PR #163 の 2 周目レビュー指摘を反映した。** (a) **本節冒頭の列挙へ第 4 の手を足した**——「テスト側（検査そのもの）をパッケージ境界で別ディレクトリへ分ける」で、適用条件は (1) と同じく**増え方が違い、互いに依存が無い**こと、先例は `internal/buildconfig` → `internal/buildconfig/docscheck` である。あわせて第 4 の手で古くなる言い回しを全文で洗い、「3 つの手のうち」8 箇所と「3 つの手（本節冒頭）」3 箇所を「4 つの手」へ、6 周目の「上の 3 つの手」を「上の (1)〜(3)」へそろえた（**改訂 1.48 / 1.50 の欄は当時の記録なので数はそのままにし**、1.50 の理由欄からは本文と食い違う「3 つ」だけを外した）。(b) 「警告帯へ入れた判断」の項 2 を**当時の判断と明示したうえで、Issue #161 が (4) で解いた旨を添える**形へ直した。(c) 「2 つに分けた判断」の節に採った手の番号 **(4)** を他節と同じ形式で明記し、共通ヘルパの段落にも **(3)** を添えた。(d) 「増えたのは package 宣言・import・**3 つ**の doc コメント」を**新設した 2 つ**（`docscheck/doc.go` 14 行・`buildconfigtest/buildconfigtest.go`）へ直した——`internal/buildconfig/doc.go` は分割前からあり **12 行のまま**で +74 には効いていない（合計 +74 の結論は変わらない）。(e) 「次に `internal/buildconfig` へテストを足す Issue へ」の一文で**句点だけが bold で囲まれていた編集残り**を直した。(f) `internal/buildconfig/doc.go` の列挙へ**依存更新設定（`.github/dependabot.yml`）**を、`internal/buildconfig/docscheck/doc.go` の列挙へ **`//nolint` 抑制の棚卸し**を足した（`TestDependabotWatchesGitHubActions` と `TestSetupDocNolintInventoryMatchesTree` が列挙から漏れていた）。**どちらも行を折り返して行数（12 行 / 14 行）を保った**ので、上の行数表と散文の実測値は 1 つも動かない。(9) **PR #163 の 3 周目レビュー指摘を反映し、(5) の「移動で動く値を全件測り直してそろえた」という本欄の宣言を実態に合わせた。** (a) `ui/page/pagetest` を **1932 行・残り 68 行**と現在形で主張していた散文 5 箇所（「Disk タブの空け方」の 2 箇所・「`ui` 直下の空け方」の 6 周目と `tickMsg` の項と 10 周目）を実測（**1935 行・残り 65 行**）へそろえた。うち 2 箇所は次の Issue へ「**移せるのは 68 行まで**」と**受け皿の実容量より 3 行過大な予算**を指示していたので、予算そのものも **65 行**へ直した（68 行移すと 2003 行で警告帯に入る）。**当時の値であることが明記された箇所（Issue #159 の時点・`tickMsg` の項の「当時 1936 行」・改訂 1.92 の欄）は据え置いた。** (b) 改訂 1.88 の欄が現在形で主張していた `internal/buildconfig` **2122 行・残り -122 行の warn** を実測（**1216 行・残り 784・pass**）へ、`docs_depnodes_test.go` **123 行**・`docs_depgraph_test.go` **296 行**・`docs_mermaid_test.go` **147 行** を実測（**129 行 / 136 行 / 194 行**。移動後のパスは `internal/buildconfig/docscheck/`）へ直した。改訂 1.89 の欄が同じく現在形で持っていた 2122 行・残り -122 行も同様に直した。(c) 「次に `internal/buildconfig` へテストを足す Issue へ」の一文にあった**全角句点の直後の半角空白**（文書中この 1 箇所だけ）を除いた | `internal/buildconfig` が警告帯（2122 行・残り -122 行）にあり、同節自身が空けることを独立した Issue に立てるよう指示していた。ドキュメントの検査を `docscheck` へ分けて警告帯を出したため、行数表・段落・判断の節が実装と食い違う状態になった。(5)〜(7) は PR #163 のレビュー指摘である。**`nolint` 棚卸しの置き場所は「検査に使う道具」ではなく「検査の対象」で決まる**——落ちたときに直すのは setup.md の表であって `.golangci.yml` ではないので、ビルド設定側に残すと `internal/buildconfig/doc.go` の宣言（ビルド設定ファイルの不変条件だけを置く）と食い違い、同一文書の検査を探す読み手が 2 つのパッケージを見る羽目になる。**項 4・項 5 は現在形のまま偽になっていた**もので、とりわけ項 5 末尾の「ディレクトリ側は既に残りマイナスである」は同じ文書の後続節「3 つとも余裕があるので、足す前に空ける必要は無い」と正面から矛盾し、次の Issue に不要な分割を検討させる**危険側のずれ**である。**表だけを直して散文を取り残す**欠陥は改訂 1.69〜1.73 / 1.90〜1.93 で繰り返し再発しており、本改訂の初版でも再発したため、移動で動く値を全件測り直してそろえた。**(8) は 2 周目レビューの MAJOR である。** 本節は「空け方の判断はここに集約する」と宣言し、他のすべての判断節が採った手を番号で名指しする形式を守っている（「4 つの手のうち (1)」「(3) を採った」「(1) は当たらない」）のに、**Issue #161 が実際に採った手——検査そのものをパッケージ境界で分ける——は 3 つのどれにも当たらず、936 行の移動が列挙から漏れていた**。(3) の例示へ `buildconfigtest` を足したのは共有ヘルパ 44 行分の説明にしかならない。列挙に無い手は次の Issue が同じ場面で辿れず、とりわけ項 2 の「(1) は当たらない」が現在形のままだと**同じ対象について「切り出す対象は無い」と「切り出した」が並んで見える**。**手を 1 つ足す以上、文書内の数え方の食い違いを残さないことが同時に要る**——数の食い違いは本 PR が 1 周目に指摘された欠陥そのものである。**(9) は 3 周目レビューの MAJOR である。** (5) が「移動で動く値を全件測り直してそろえた」と宣言しながら、実際に直したのは行数表と散文 2 箇所だけで、**同じ文書のなかに表（1935 行）と散文（1932 行）が正面から矛盾したまま 5 箇所残っていた**。とりわけ「移せるのは 68 行まで」は次の Issue へ**受け皿の実容量より 3 行多い予算**を渡すもので、そのとおりに移すと `ui/page/pagetest` が 2003 行の警告帯に入る**危険側のずれ**である。**宣言と実態が食い違う欄は、次の周が「ここは測り直し済み」と信じて読み飛ばす**ため、表だけを直して散文を取り残す欠陥（改訂 1.69〜1.73 / 1.90〜1.93）をそのまま再生産する。改訂 1.88 の欄を直すのは、同じ欄を改訂 1.89 / 1.91 が現在値へ訂正してきた前例に従うものである（当時の記録として明示された数ではなく、**現在形で主張している数**だから直す）。 |
...
| 1.96 | 2026-08-25 | PR #168 のレビュー指摘（major 8 件）を反映。(1) 「行数表と散文は実測に合わせる」の除外を**文どまりから句どまりへ**狭めた——`TestLineBudgetProseMatchesTables` は `当時` / `時点では` を `、` で区切った句の中でだけ効かせる。規約の文言も同じくそろえた。(2) **文が backtick で名指ししたディレクトリの行とだけ突き合わせる**ようにした（新設した `docs_linebudgetdir_test.go` の `budgetRowIndex` が、UI 層の表の `internal/` 抜きの綴りと散文の `internal/…/` 付き・末尾スラッシュ付きの綴りを同じ行へ向ける。名指しの無い文だけが表全体との照合に落ちる）。規約にも明記した。(3) 次の Issue へ予算を渡す 2 つの指示（「この N 行を予算として読む」「移せるのは N 行まで」）も残りの主張として拾うようにし、`## 改訂履歴` の切り出しを**行頭の見出し行だけに効く**形へ直した。(4) 新しい検査が挙げた散文 6 箇所へ `当時` を付け直した（`ui/page/config` が警告帯へ入った経緯、`internal/ui/app_test.go` と `internal/ui/page/` の逼迫、`ui` 直下を 1875 行へ戻した記録、9 周目が次の Issue へ渡した予算の指示）。(5) **本 PR が新設した規約に本 PR 自身が違反していた箇所を直した**——「`internal/buildconfig` を 2 つに分けた判断」の `docscheck` を分割当時の値（当時 936 行・残り 1064 行）へ戻し、`internal/ui/page/` の「1998 行（残り 2 行）」へ `当時` を付け、改訂履歴の過去欄が現在形のまま行数を主張していた 4 箇所を当時の値か「上記「行数の予算」の行数表を見よ」への参照へ書き換えた。(6) 規約の小節に、`TestLineBudgetProseMatchesTables` は `## 改訂履歴` 以降を検査対象から外しているので**過去欄は規約で守るしかない**ことを明記した。(7) 行数表の `internal/buildconfig/docscheck` を実測へ更新し（本 PR が検査を足したぶん 1334 → **当時 1463 行・残り 537・pass**）、降順を保つため `internal/svc` の下から `internal/doctor/jobreq`（当時 1495 行）の直下へ移した。あわせてその値を現在形で参照していた散文——「2 つに分けた判断」の 3 つの合計——を **当時 2723 行**（1216 + 1463 + 44）へ直し、分割時点の値であることが読み取れなかった `docs_nolint_test.go` の 183 行に `分割当時` を付けた | 新設した検査と規約に、**それ自身では塞げない抜け道と、本 PR 自身の違反**が残っていた。(a) 除外が文全体に掛かっていたため、過去と現在を 1 文で対比する本書の文体では、`当時` を 1 つ書いた時点で**同じ文の現在形の主張までまるごと検査から外れる**——この検査が最も守るべき数がちょうど抜ける。(b) **ディレクトリ名と数値が結び付いていなかった**ので、表のどこかに同じ組があれば緑になった。現に `internal/ui/page/` の「1998 行（残り 2 行）」は真値が 1955 行・残り 45 行でありながら、`internal/logs` がちょうど 1998 行・残り 2 行だったため**別ディレクトリの値と偶然一致して素通りしていた**——検査を入れた当の PR が、その検査で守ったはずの数を古いまま通していた。(c) 「N 行を予算として読む」形式は未カバーで、9 周目が次の Issue へ渡した予算が現に陳腐化していた。これは Issue #164 が実害として挙げた「移せるのは 68 行まで（実際は 65）」と**まったく同型**であり、片方だけ塞いでも次の周で同じことが起きる。(d) `## 改訂履歴` を文字列として探していたため、散文がこの見出しをコード引用として書くだけで**それ以降がまるごと検査から外れる**。(e) 「改訂履歴・過去の記録には当時の値だけを残す」は本 PR が新設した規約であり、**その規約を作った PR 自身が現在形の行数を過去欄と判断の記録に書いていた**。規約と前例が食い違えば、次の周は規約ではなく前例のほうを真似る。(f) 表を実測へ合わせるのは本節が定める手順そのもので、本 PR が検査を足して `docscheck` を伸ばした以上、表と、その値を現在形で参照する散文を同じ変更で合わせなければ、新設した検査が最初の `make check` で落ちる |
| 1.97 | 2026-08-25 | Issue #164 の再設計を反映。**散文の陳腐化を「言い回しの検出」で防ぐのをやめ、散文から実測値そのものを無くした。** (1) 小節「行数表と散文は実測に合わせる」を「**行数の実測値は表だけが持つ**」へ改め、規約を「現在の行数を数値で書いてよいのは行数表だけで、散文は表を参照する」という形に置き換えた。(2) `TestLineBudgetProseMatchesTables` と、名指しされたディレクトリの行を引く `docs_linebudgetdir_test.go`（`budgetRowIndex`）を削除し、`TestLineBudgetProseHasNoMeasuredNumbers` を新設した——除外を落とした散文に `残り N` と 3 桁以上の `N 行` が 1 つも現れないことだけを見る。**言い回しは一切見ない。** (3) 除外を正規表現ではなく明示の一覧にした——2 つの行数表 / コードブロック / 行頭の `## 改訂履歴` 以降 / 過去の値を記録する節の見出しを並べた `budgetProseExcluded`（「…の空け方」「…判断」と「一覧タブを 1 枚足せる余裕」）。**行数表が 2 つ見つからない・一覧に挙げた見出しが消えた・除外を落とすと散文が空になる、のいずれでも `t.Fatal` する。** (4) 散文に数で書いてよい定数を 300 / 330 / 2000 / 2200 の 4 つに限った（`.linterly.yml` の上限と警告帯の境界であって実測値ではない）。(5) 散文から実測値を落とした——「残りが 1 桁のディレクトリが 6 つある」の段落、「UI 層の外のディレクトリ」の導入、「ファイルの行数」のファイル別実測の一覧と前例、「一覧タブを 1 枚足せる余裕」「`internal/disk` から `pathguard` を切り出した判断」「`internal/setup` のフィクスチャを `setuptest` へ出した判断」の現在値、`ui/organism/table` の節の見出しの括弧——はいずれも行数表への参照に書き換え、過去の値は当時の記録として残した。(6) 改訂履歴の過去欄が現在値を指していた 2 箇所（改訂 1.74 の `ui/molecule/listrow`、改訂 1.96 の `docscheck` の実測と 3 つの合計）を、行数表への参照と `当時` 付きの値へ直した。(7) 改訂 1.95 が定めた「ファイル側の残りは `行` を付けず `（残り 4）` と書く」書き分けは廃した——`残り N` は `行` の有無に関わらず実測値として拾うためで、書き分けで検査を避ける道を残さない | **言い回しを並べて陳腐化を検出する形は収束しなかった。** 改訂 1.95 が 2 形式、1.96 がさらに 2 形式を足してなお、PR #168 のレビューは変異注入で未カバーの書き方を挙げ続けた（1 周目 8 件・2 周目 9 件）。**日本語で行数を主張する書き方に上限は無い**ので、検出側を足す限り次の周でも同じ指摘が出る。検出をやめて**散文から数値を無くす**ほうが、規約が 1 行で言えて（現在の行数を数で持ってよいのは表だけ）、検査も「数値が不在であること」しか見ないので**言い回しの追加では破れない**。除外を正規表現ではなく見出しの一覧にしたのも同じ理由で、読み手が**何が検査されていないか**をその場で数えられる。過去の値を検査の外に置き続けるのは、空け方と判断の記録がその Issue に紐づく事実であり、現在の実測へ寄せると議論の根拠そのものが消えるからである。**外にあるということは規約で守るしかないということ**なので、その旨を規約の小節に明記した |
| 1.98 | 2026-08-25 | PR #168 のレビュー指摘（major 7 件）を反映。**改訂 1.97 が定めた方式（現在の行数を数で持ってよいのは行数表だけ）はそのままに、適用の穴を塞いだ。** (1) **除外の一覧へ逃がしてあった節から、現在形の実測値を全部落とした**——ディレクトリの値は「行数の予算」の行数表への参照へ、ファイル単位の値は `go tool linterly check` の出力への参照へ寄せ、その Issue に紐づく過去の値だけを `当時` の印を付けて残した。(2) 後続の Issue へ与える予算の指示から数値を落とし、「行数表の `残り` 列を見よ。足す前に空けること」の形へ揃えた（Issue #164 が実害として名指しした「移せるのは N 行まで」がまさにこの形の指示である）。(3) 現在形の実測値が無くなった節（「`ui/organism/table` の本体を分割しない判断」）を `budgetProseExcluded` の一覧から外し、**除外に残してよいのは過去の値だけを含む節に限る**と規約へ明記した。(4) **除外が入れ子で黙って広がっていた**のを直した——打ち切りが「同じ深さの次の見出しまで」だったため、一覧に挙げた見出しに対して実効の除外はその 1.5 倍ほどへ膨らみ、一覧に無い小節（`internal/buildconfig` の 2 つの判断が `ui/page/config` の節の配下にあった）まで検査の外にあった。打ち切りを**任意の深さの次の見出しまで**へ変えて**一覧の見出しの集合がそのまま除外範囲**になるようにし、それまで実効的に外れていた入れ子の小節は一覧へ明示した。(5) 検査の床を 3 桁から 2 桁へ下げた（`N 行` は `N` が 2 桁以上なら拾う）。あわせて `残り` の直後の助詞（`は` / `が` / `も`）を吸収し、逆に助数詞を伴う形（`残りが 1 桁` / `残り 3 件` など）は実測値ではないので拾わないようにした。(6) 本 PR が入れた数値が同じ PR の中で既に偽になっていた 2 か所（3 つのディレクトリの合計を書いた式と、ドキュメント側の最大ファイル）を参照へ寄せた。(7) **コードブロックのフェンスを閉じ忘れると、そこから本書の末尾までが黙って検査対象から消えていた**のを `t.Fatal` するようにし、本書の「落ちる条件」の列挙を実装と一致させた。あわせて 2 件——規約の説明にあった「小さい残りは `残り N` の側が桁に関わらず拾う」という**嘘**（実際には助詞が挟まると拾わなかった）を実装に合わせて直し、`internal/buildconfig/docscheck` のパッケージ doc が旧方式の言い方（「表と散文が linterly の実測とずれても」）のままだったのを新方式（「表が実測とずれても、散文が実測値を数で持っても」）へ直した | (1) が本 PR の主眼である。**方式は正しかったが適用が不完全だった**——改訂 1.97 は規約を 1 行に縮める一方で、実測値の残る節を片端から除外の一覧へ逃がしており、除外領域は本書の本文の約 3 割を占め、そこに現在形の実測値がそのまま残った。つまり**散文は古い予算を現在形で語り続けているのに検査は緑**という、Issue #164 が問題にした状態が除外の中でそっくり生き延びていた。しかも Issue #164 の実害（後続の Issue が古い残量を信じて超過を指示する）は、まさに空け方と判断を記録するこれらの節で起きる。除外を「逃がし先」に使う限り、規約は書かれただけで効かない。(2) は #164 の実害そのものの形なので、言い回しを変えるのではなく数値ごと落とすほかにない。(3) を明記しないと、次に実測値の残る節が出たとき再び一覧へ足して (1) の状態へ戻る。(4) は**一覧が嘘になる**ものである。除外を正規表現ではなく見出しの一覧にしたのは、読み手が**何が検査されていないか**をその場で数えられるようにするため（改訂 1.97）だったのに、その数え上げは実効の除外範囲の 3 分の 2 しか捉えていなかった。加えて、入れ子の小節を 1 つ足すだけで除外が黙って広がるので、**除外は誰も意図しないまま増え続ける。** (5) **3 桁の床は実害の帯をまるごと見逃していた**——予算の残りは逼迫したディレクトリほど小さく、危険な場所ほどほぼ常に 2 桁以下である。Issue #164 が名指しした原文の数も 2 桁だった。3 桁以上しか見ない検査は、**最も危険な行だけを素通りさせる**形になっていた。助詞を吸収するのも同じ理由で、`残りは` と一字挟むだけで検査から逃げられる状態だった（これは言い回しの列挙ではない——拾う形は「残り」と数の並びのままで、間に挟まる閉じた集合の助詞を吸収しているだけである）。助数詞を落とすのは逆向きで、本書が「残りが 1 桁のディレクトリ」のように性質を語る形まで実測値として拾うと、偽陽性を避けるために規約のほうを曲げることになる。(6) 同じ PR の中で偽になる数を新たに入れるのは、規約の趣旨（写しを 1 つに減らす）に真っ向から反する——**規約を入れた PR 自身が規約を破っていた。** (7) 他の 4 つの保証（行数表が 2 つある / 行頭の `## 改訂履歴` がある / 一覧の見出しが本書にある / 除外を落として散文が残る）はどれも食い違えば `t.Fatal` するのに、フェンスの閉じ忘れだけが保証の外だった。**閉じ忘れは検査対象を静かに縮めるので、緑になったことが検査されたことを意味しなくなる。** 残る 2 件のうち前者は規約の説明が実装より広い保証を約束していたもので、読んだ人が拾われると思って書いた形が実際は素通りする。後者はパッケージ doc が検査の目的を旧方式で語り続けるもので、次に検査を触る人が最初に読む場所である |
| 1.99 | 2026-08-25 | PR #168 のレビュー指摘（major 6 件）を反映。**改訂 1.97 / 1.98 が定めた方式（現在の行数を数で持ってよいのは行数表だけ）はそのままに、検査の床と保証の書き方を実態へそろえた。** (1) **`TestLineBudgetProseHasNoMeasuredNumbers` の床を 2 桁から 1 桁へ広げた**——`N 行` は `N` が 3 以上なら拾い、**見ないのは `1 行` と `2 行` だけ**である（本書がこの 2 つを「表の 1 行」「一覧の 1 行」の意味で常用するため。3 観点のレビューが独立に、検査対象の散文で行の意味に使う 1 桁はこの 2 つだけだと実測している）。あわせて `残り` の直後に吸収する助詞へ `の` を足した（`残りの N 行` が助詞の集合から漏れて第 1 式に一致していなかった）。規約小節「行数の実測値は表だけが持つ」と [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)の一覧表の説明も同じ床へそろえた（setup.md 改訂 1.36）。(2) 「9 周目の空け方」の中で**本書の一意な識別子の総数を数で書いていた**のを落とした（改訂 1.92 が 948 個と測った値。本改訂の時点で同じ抽出コマンドを実行すると 1021 個で、既に食い違っていた）。(3) 「`ui/organism/table` の本体を分割しない判断」が実体を「**6 ファイルである**」と現在形で断定していたのを、ファイル名の列挙と実測コマンド（`ls internal/ui/organism/table/*.go` と `go tool linterly check`）への参照へ寄せた。(4) 「`internal/buildconfig` を 2 つに分けた判断」末尾の「**3 つとも余裕があるので、足す前に空ける必要は無い**」を、他節と同じ安全側の書き方（行数表を見よ／行数表の残りを超える追加が要るなら足す前に空けること）へそろえた。同じ助言が「警告帯へ入れた判断」の項 5 末尾にもあったので同時に直した。(5) 改訂 1.98 の理由欄の「除外領域に現在形の実測値が**約 20 か所**そのまま残った」から**件数の断定を落とした**。(6) `budgetProseExcluded` の doc コメントの保証を**実際に検証した範囲へ狭めた**——「**この検査が拾う形（`N 行` と `残り N`）が残っていないことだけを、一覧から 1 件ずつ外して確かめた**」とし、規約小節にも**検査が構造的に見ない範囲**（改訂履歴／除外一覧の節／数値を伴わない語形の主張／行以外の助数詞で数える個数）を明記した。**その範囲にある語形の主張（`余裕がある` / `潤沢ではない` / `残りはテスト 1 本ぶんしかない`）は本改訂では書き換えていない**——判断の記録に多数あり、言い回しを並べて拾う手は 2 周かけて捨てた方式そのもので収束しないので、**規約に範囲を明記することをもって対応とした**（(4) だけは数値を伴わないが安全側を崩す助言なので例外として直した）。(7) (1) で `docs_linebudgetprose_test.go` が伸びたので、「行数の予算」の UI 層の外の表の `internal/buildconfig/docscheck` を実測（**1505 行・残り 495・pass**）へ更新した（降順の並びは変わらない） | (1) **2 桁の床は今日いちばん逼迫している帯をまるごと素通りさせていた。** 予算の残りは逼迫したディレクトリほど小さく、本改訂の時点で残りが最小の 3 つはいずれも 1 桁である。そこへ「移せるのは N 行まで」「あと N 行しか無い」と書けば、Issue #164 が実害として名指しした形が**最も危険な帯でだけ無検査**という状態になる。助詞 `の` の追加は同じ穴で、`残りの` と一字挟むだけで第 1 式から逃げられた（これは言い回しの列挙ではなく、閉じた集合である助詞の吸収である）。(2)(3) は**除外の前提そのものを崩していた**——(2) の節は当時の値だけを記録するという前提で除外の一覧に載っており、(3) の節は「現在のファイル数と行数を落とした」ことを根拠に改訂 1.98 が一覧から外したのに、どちらも現在形の実測値を残していた。**個数は検査が構造的に見ない**ので、規約でしか止まらない。(4) は他のすべての節が「行数表の残りを超える追加が要るなら足す前に空けること」と安全側に揃えているなかで、**ここだけが現在の残予算を根拠に「空けなくてよい」と指示していた**。`internal/buildconfig` 系が埋まった時点で次の Issue を誤らせる（改訂 1.94 の理由欄が同型の矛盾を**危険側のずれ**として指摘している）。(5) は**自分で数えていない件数を書く**もので、本 PR は 1 周目にも「約 20 箇所と書いてあるが実際は 42 箇所」と同型の指摘を受けている。数えた数だけを書くか、件数を書かないかのどちらかにする。(6) は**保証が循環していた**——「現在形で実測値を語る節は 1 つも残っていない」は、その確かめ方（一覧から 1 件ずつ外して検査を回す）では裏付けられない。手順が示せるのは「この検査が拾う形が残っていないこと」だけで、検査が構造的に見ない形（個数・語形の主張・床より下の桁）は最初から検出対象外である。実際 (2)(3) がその反例だった。**検査が何を見ないかを書かない限り、読み手は緑を「全部見た」と読む。** |
| 1.100 | 2026-08-25 | Issue #167（散文の行数の「合計の式」が行数の検査の網から外れている）を反映。(1) **合計の式の不在を見る検査を新設した**——`TestLineBudgetProseHasNoSumExpressions`（`internal/buildconfig/docscheck/docs_linebudgetsum_test.go`）が、除外を落とした散文に `A + B = C` の形があり、そこに 3 桁以上の数が 1 つでも含まれていれば落とす。既存の `docs_linebudgetprose_test.go` が 1 ファイル 300 行の上限に近かったのでファイルを分けた（除外を落とす `budgetProseLines` はそのまま共有する）。あわせて検出の両方向を固定する `TestProseMeasuredSumMatchesOnlyLineTotals` を置いた——行数の合計の式（右辺が bold で囲まれる形と、右辺が助数詞を持たない形を含む）は拾い、列幅の見積もり（`6 + 66 + 3 = 75 セル`）は拾わない。(2) 規約の小節「行数の実測値は表だけが持つ」に方針を明記した——**式を落として行数表への参照へ一本化する**（検査を式の評価へ広げる手もあるが、それは表と散文の一致を守るだけで写しそのものは残る）、**過去の値を記録する節では `当時` と読めるように書けば式を書いてよい**、**桁で切るのは行数の合計が 1 ディレクトリ 2000 行の桁で数えるものだから**（2 桁以下だけで閉じた式は行数ではない算術である）。(3) 「検査が構造的に見ない範囲」を 4 つから 5 つへ広げ、(e) **式にも助数詞にもならない裸の数**（`内訳は A / B / C` のように並べただけの形）を足した——数だけを見て実測値かどうかは判定できないので、ここも規約でしか守れない。あわせて拾う形の列挙を `N 行` / `残り N` / `A + B = C` の 3 つへそろえた。(4) 「`internal/buildconfig` を 2 つに分けた判断」末尾の括弧が、散文側の検査を「行数表と散文を実測と突き合わせる」という**旧方式のまま**説明していたのを、「行数表を実測と突き合わせる検査と、散文から実測値を無くす検査」へ直した。(5) (1) で `docscheck` が伸びたので、UI 層の外の行数表の `internal/buildconfig/docscheck` を実測（**1620 行・残り 380・pass**）へ更新した（降順の並びは変わらない） | **合計の式は各項が裸の数なので、既存の `N 行` の検査では右辺しか拾えない。** `A + B + C = D 行` は 1 文で 4 つの実測値を主張するのに、検査に見えているのは右辺の 1 つだけで、左辺の 3 項は助数詞を持たないため最初から検査の外にある。しかも**右辺が `行` を伴わない書き方になった瞬間に 4 項ともまるごと素通りする**——写しを 1 つに減らすという本規約の目的に照らせば、ここは 1 か所ではなく 4 か所ぶんの写しが無検査で増える場所だった。日本語の言い回しを並べて拾う方式は 2 周かけて捨てた（改訂 1.97）ので、拾うのは「数値の形」だけに保ち、除外は桁という**閉じた条件**で切った——行数の合計は 1 ディレクトリ 2000 行の桁で数える以上どうしても 3 桁以上になり、2 桁以下だけで閉じた式は行数ではない算術（本書が書く列幅の見積もり）に限られるので、**規則を足し続けることにはならない**。単体テストで両方向を固定するのは、拾えなくなれば Issue #167 の実害がそのまま戻り、逆に拾いすぎれば正当な算術を通すために除外の一覧を膨らませることになる（本書の規約は**一覧は小さいほどよい**と定めている）ためである。(3) を足したのは、式を塞いだ結果として**次に安いのが数を裸で並べる形**になるからで、そこは検査が構造的に見ない——見ない範囲を書かない限り、読み手は緑を「全部見た」と読む（改訂 1.99 の (6) と同じ理由である） |
| 1.101 | 2026-08-25 | Issue #169（`components/overview.md` の行数の実測値が PR #168 の検査の対象外で無検査のまま残っている）を反映。(1) **散文の検査の対象を 1 文書から 2 文書へ広げた**——`TestLineBudgetProseHasNoMeasuredNumbers` と `TestLineBudgetProseHasNoSumExpressions` が `docs/ui/atomic-design.md` に加えて `docs/components/overview.md` も走査する。登録は新設した `docs_linebudgetdoc_test.go` の `budgetProseDocs`（既存の `docs_linebudgetprose_test.go` が 1 ファイル 300 行の上限に近かったのでファイルを分けた）が持ち、パス・エラーメッセージに出す名前・**その文書が持つ行数表の数**・**その文書用の除外の一覧**を 1 件ずつ書く。既存の 5 つの `t.Fatal` の保証（行数表が期待した数だけある / 行頭の `## 改訂履歴` がある / 一覧の見出しが本文にある / コードブロックが閉じている / 除外を落として散文が残る）は 1 つも弱めず、どれも文書名を添えて落ちる形にした。(2) **除外の一覧は文書ごとに持つ**ことにし、規約の (4) にその旨と理由を明記した——`overview.md` の一覧（`overviewProseExcluded`）に載るのは「### `internal/disk`」の 1 つだけである。(3) `overview.md` から現在形の実測値 2 箇所を落とした（`internal/runner` 系 4 ディレクトリの使用量と残り / `internal/disk` の現在値）——どちらも本節の行数表への参照へ寄せ、Issue #101 当時の値は `当時` と読める形で残した。(4) 規約の冒頭に、この規約が掛かるのは本書だけではないこと（対象は 2 文書で、実測値を数で持ってよいのは本書の行数表だけであること）を明記した。(5) (1) で `docscheck` が伸びたので、UI 層の外の行数表の `internal/buildconfig/docscheck` を実測（**1708 行・残り 292・pass**）へ更新し、`internal/runner`（1704 行）を追い越したので降順を保つため 1 行上へ移した | **検査の corpus が 1 ファイルだったので、同じ形の写しが隣の文書で無検査のまま残っていた。** `overview.md` の `internal/runner` 系の使用量は改訂 1.19 / 1.39 で **2 度陳腐化して人手で追随した記録がある**——つまり Issue #164 が実害として名指しした「散文が古い予算を現在形で語り続ける」状態が、検査の外で現に 2 度起きていた場所である。しかも `internal/disk` の段落は末尾で「実測値の一次情報は同書の表であり、本書はここを重複して持たない」と**宣言しながら、その直前で現在形の実測値を持っていた**。規約を本書の中だけで守っても、判断材料を引く側の文書が古い数を語れば次の Issue は誤る。除外の一覧を文書ごとに分けたのは、**除外が文書の構造に結びついている**からである——本書は行数表を 2 つ持ち記録の節を 20 以上抱えるが、`overview.md` は表を持たず記録の節は 1 つだけで、混ぜると片方にしか無い見出しが消えたことを検査が知らせられなくなる（一覧に挙げた見出しが本文に無いと `t.Fatal` する仕組みが、混ぜた瞬間にもう一方の文書の見出しを毎回取り逃がす）。同じ理由で表の数も文書ごとに持つ——数が違っても「見つからなければ落ちる」保証は残したいからである |
...
| 1.103 | 2026-08-25 | PR #173 のレビュー指摘（major 6 件）を反映。**規約はそのままに、合計の式の検査を素通り経路が塞がる形へ直した。** (1) 拾う式の形を広げた——**必須なのは「2 つ以上の数が演算子で連なっていること」だけで `= D` の右辺は任意**、演算子は `+` `-` とその全角、ただし `-` は**前後の空白を必須**にする（空白なしを許すと ISO 形式の日付が式として拾われ、改訂履歴の外にある日付まで偽陽性になる）。(2) **床を `N 行` の側とそろえた**——式に含まれる数が**すべて 2 以下のときだけ**実測値でないと見なす。(3) (2) の結果として行数ではない算術（列幅の見積もり）が偽陽性になるので、**合計の式の検査にだけ効く見出しの一覧**（`budgetProseSumExcluded`）を新設し、規約の除外を 4 つから 5 つへ広げた。載せたのは「幅」の 1 つだけで、実装後に全文を走査して偽陽性を全件列挙した結果それだけだった。(4) **`overview.md` の除外を空にした**——唯一あった「### `internal/disk`」は節まるごと（API 責務表と全段落）を外していながら「載せてよいのは過去の値だけを持つ節」を満たしておらず、当時の値は上記「`internal/disk` から `pathguard` を切り出した判断（Issue #101）」が既に持っている。(5) 「見ないのは `1 行` と `2 行` だけ」を**実装どおり `0 行` / `1 行` / `2 行` の 3 つ**へ直した（式が `[3-9]` を要求する帰結で、残りがゼロになる状態は実在しうるので本規約でしか守れない）。(6) 「**本節の**行数表」という不揃いな指し方を、他の参照と同じ「上記「行数の予算」の行数表」へそろえた（2 箇所）。(7) (1)〜(3) で `docscheck` が伸びたので、UI 層の外の行数表の当該行を実測（**1992 行・残り 8・pass**）へ更新し、降順を保つため `internal/gh` の直下へ移した | **塞いだ 3 形はいずれも実測で素通りを再現している。** (a) `内訳は 44 + 30 = 74 である。`——3 桁の床が同じスイートの `N 行` 側の床（3 以上）と食い違っており、ファイル単位や小さいディレクトリの 2 桁の行数を並べた式がまるごと通っていた。(b) `内訳は 1216 + 1334 + 44 で、合わせて 2594 になる。`——`=` を要求していたので `+` の連なりだけの形が通っており、**これは Issue #167 が実害として挙げた「1 文で 4 つの実測値を主張する」形そのままである**。(c) 全角の演算子。**検査が「数値・記号の形の不在」を見る方式である以上、形の集合に穴があれば規約の側は何も守らない**——日本語の言い回しを並べる方式を 2 周かけて捨てた（改訂 1.97）代わりに引き受けたのがこの責任である。(3) で正規表現を足さず見出しの一覧にしたのは同じ理由で、式の形で除き分けようとした瞬間に捨てたはずの方式へ戻る。(4) は**除外の一覧が自分で定めた条件を満たしていなかった**もので、24 行ぶんの節が「過去の値を持つ」という理由だけで丸ごと検査の外にあった——一覧は小さいほどよいという規約に照らせば、参照へ寄せて空にできるものを載せ続けてはならない |
