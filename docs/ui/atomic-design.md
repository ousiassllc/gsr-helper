...
**残りが 1 桁のディレクトリが 3 つある**（`internal/setup/tarball` 1 行・`internal/logs` 2 行・`internal/gh` 4 行。UI 側では `ui/page` が残り 2 行、`ui/page/disk` が残り 4 行、`ui/organism/dialog` と `ui/page/logs` が各 残り 6 行である）。**この 3 つに 1 行でも足す Issue は、足す前に空けること。** どれも警告帯には入っていないので `make check` は通るが、通ることと余裕があることは違う。**`ui` 直下は残り 1 桁ではなく既に警告帯である**（2063 行・残り -63 行。判断は上記「7 周目の判断」）。**`ui/page/config` も残り 20 行まで詰まった**（Issue #132 の回帰テスト）。
...
3. **CI は落ちない。** ERROR の境界は 2200 行で、**まだ 137 行の余裕がある。**

**次に `ui` 直下へ足す Issue へ。** 警告帯に入った以上、次は「足すついでに空ける」ではなく**空けること自体を独立した Issue として立てる**こと。切り出しの候補として現実的なのは、親 Model の背景処理（`background.go` の `startBackground` と `discover.go` の `tick` / `onTick` / `discover` / `scanRoots` / `refresh`）を `internal/ui/discovery` 側へ寄せる案だが、これらは `a.inflight` / `a.seq` / `a.applied` を進めるため、状態ごと移さなければ意味が無い。**「増え方が違うまとまり」であることは言えるので、(1) の条件は満たしうる**——ただし移す状態の設計から始まるので、行数を空ける目的だけで片手間に着手しないこと。
...
| 1.76 | 2026-08-24 | 行数表の `ui` を 2063 → 2105、`ui/chrome` を 347 → 490、`ui/atom` を 1262 → 1264、`ui/molecule` を 1513 → 1515 へ実測に合わせて更新し、`ui/chrome` を降順の正しい位置へ移した。切り出しの記録（`ui/chrome`）の散文にある「現在 347 行」も直した。`ui/atom` と `ui/molecule` は doc コメントの加筆分である。あわせて `internal/ui/chrome/chrome_test.go` が 1 ファイル 300 行を超えたため、状態行を幅に収める検証を `statuswidth_test.go` へ分けた（「ファイルの行数」の方針どおり、同じディレクトリの中で責務の境界に沿って分割。`ui/chrome` は残り 1500 行超なのでディレクトリの予算は問題にならない）。あわせて、警告帯の `ui` へ Issue #136 の回帰テスト 40 行を足した判断と、その検証を `ui` 直下から動かせない理由を「行数の予算」節に残した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、`ui` は既に警告帯だった。**表の値を据え置いたまま行を足すと、次に予算を引きに来た Issue が実際より多い残量を前提に判断する**。改訂 1.58 / 1.60 が「行数表は後続 Issue が読む予算の規範」「1 つの文書が 2 つの値を主張する欠陥」として 2 度是正した箇所そのものである（PR のレビュー指摘） |
