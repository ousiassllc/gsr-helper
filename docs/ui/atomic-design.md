...
| ディレクトリ | 行数 | 残り | 判定 |
|------------|------|------|------|
| `ui/page/config` | 2119 | -119 | **warn** |
| `ui` | 2105 | -105 | **warn** |
| `ui/page/pagetest` | 2014 | -14 | **warn** |
| `ui/page` | 1998 | 2 | pass |
...
**次にこのタブへ手を入れる Issue には予算が無い。** 2119 行・残り -119 行の警告帯で、ERROR の境界（2200 行）まで 81 行しかない。採れる手は、`config.go` / `flow.go` / `results.go` の状態遷移そのものではなく、初回設定ウィザード（`selfconf.go`。編集対象が runner ではなくアプリ自身で、FR-41 / FR-42 と他の項目で要件が分かれている）を出すことである。

##### Issue #140 の判断——警告帯へ入れて回帰テストを足した

Issue #140（`make check` の並列負荷で自己設定テストが散発的に落ちる）は、`ui/page/config` を 1980 行から 2119 行へ、`ui/page/pagetest` を 1936 行から 2014 行へ押し上げ、**2 つとも警告帯に入れた**。切り出さずに入れた根拠は 3 つある。

- **出せる受け皿が無い。** 増えたのは束を辿る道具（`helper_test.go` の `findMsg`）とその回帰テストである。**道具そのものは移せる**——`findMsg` の引数は `tea.Cmd` と述語だけで、`doneMsg` に依存するのは述語を渡す `doneOf` と回帰テストの側である。移せないのは受け皿の都合で、本節冒頭の (3) が名指しする `page/pagetest` 自体が同じ Issue で警告帯に入った（2014 行・残り -14 行）。**移しても 2 つの合計は減らない**ので、警告帯を出るには別ディレクトリの通常パッケージを新設するしかなく、それはバグ修正の範囲を超える（Issue #147 に切り出した）。
- **バグ修正にスコープ外の再構成を混ぜない。** 7 周目（Issue #132）と 8 周目（Issue #136）が `ui` 直下で採ったのと同じ判断である。
...
| 1.78 | 2026-08-24 | `make check` の並列負荷で config の自己設定テストが散発的に落ちる件（Issue #140）を反映。(1) 行数表を実測へ更新した——`ui/page/config` 1980 → **2119 行・残り -119 行・warn**、`ui/page/pagetest` 1936 → **2014 行・残り -14 行・warn**。どちらも新たに警告帯へ入ったため降順の位置も動かした（`ui/page/config` は `ui` の上、`ui/page/pagetest` は `ui` の下）。(2) **表と同時に散文も直した**——現在形で行数を主張する箇所を全件抽出して照合し、この 2 つのディレクトリに触れる 7 箇所すべてを更新した（「残りが 1 桁のディレクトリ」の段落、Config タブの節の 3 箇所（警告帯へ入った経緯・Issue #104 からの推移・「使えるのは 20 行しかない」）、Disk タブの節の受け皿の余裕（「そちらも残り 64 行」）、Logs タブの節の受け皿の余裕（「1936 行・残り 64 行」）、`ui` 直下 5 周目の記録（「現在 1936 行で、残りは 64 行しかない」））。他のディレクトリの数値は本 PR が触れていないため据え置き、`go tool linterly check` の実測（0 エラー・3 警告）が表と一致することを確認した。(3) 「Issue #140 の判断——警告帯へ入れて回帰テストを足した」を Config タブの節へ新設し、切り出さずに入れた 3 つの根拠（出せる受け皿が無い／バグ修正にスコープ外の再構成を混ぜない／落とせる検証が無い）と、空けること自体を Issue #147 へ切り出したことを残した | 本節は「警告帯に入ったディレクトリへ部品を足すときは、先に分割の是非を検討し、判断と理由をこの節に残すこと」と自ら定めており、**2 つのディレクトリを同時に警告帯へ入れた**のは本 PR である。表を据え置くと、次に `ui/page/config` へ手を入れる Issue は残り 20 行のつもりで 139 行ぶん過大な予算を引き、ERROR 境界（2200 行）まで実際には 81 行しかないことに気付けない。`page/pagetest` はさらに危うい——Logs / Disk の両節が「詰まったら道具をそこへ出せ」と受け皿として名指ししており、その受け皿が満杯になった事実を書かないと、次の Issue は**もう存在しない逃げ道**を当てにして計画を立てる。表だけを直して散文を取り残す欠陥は 1.69〜1.72 で 4 周続けて再発しているため、本項でも全件突き合わせを実際に通し、その範囲と結果を上記 (2) に具体的に書いた。**なお当初この欄と (3) の根拠 1 は「`findMsg` は `doneMsg` に依存するので移せない」と書いていたが、これは事実に反する**——`findMsg` の引数は `tea.Cmd` と述語だけで、`doneMsg` に依存するのは述語を渡す `doneOf` と回帰テストの側である。移せない理由は受け皿（`page/pagetest`）が同じ Issue で埋まったことと、移しても合計が減らないことなので、そちらへ書き直した（レビュー指摘。Issue #140） |
