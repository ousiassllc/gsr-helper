...
**この規約が掛かるのは本書だけではない。** 散文の検査の対象は `docs/ui/atomic-design.md` と `docs/components/overview.md` の **2 文書**であり（`internal/buildconfig/docscheck/docs_linebudgetdoc_test.go` の `budgetProseDocs` が持つ）、**行数の実測値を数で持ってよいのは本書の行数表だけ**である。`overview.md` は行数表を持たないので、行数に触れるときは上記「行数の予算」の行数表を参照する。**対象を広げたのは corpus が 1 ファイルだったからである**——`overview.md` の `internal/runner` 系の使用量は改訂 1.19 / 1.39 で 2 度陳腐化して人手で追随した記録があるのに、PR #168 が入れた検査は本書しか読んでおらず無検査のまま残っていた（Issue #169）。

**表は実測と突き合わせる。** `TestLineBudgetTablesMatchLinterly`（`internal/buildconfig/docscheck`）が `go tool linterly check --format json` の実測と、行数・残り・判定・行数の多い順の並びを突き合わせる。ずれたときに直すのは表の側であって、上限でも実測でもない。**手で写した値を書き換えて辻褄を合わせるのではなく、実測を取り直して表を作り直すこと。**
...
| ディレクトリ | 行数 | 残り | 判定 |
|------------|------|------|------|
| `internal/buildconfig/docscheck` | 2003 | -3 | warn |
...
| `internal/buildconfig` | 1216 | 784 | pass |
...
**分けても合計は増える。** 分けた当時は 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・**新設した 2 つの doc コメント**（`docscheck/doc.go` と `buildconfigtest/buildconfigtest.go` の doc 部分。`internal/buildconfig/doc.go` は分割前からあるもので行数が動いておらず、+74 には効いていない）と、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。**現在の 3 つの行数は上記「行数の予算」の行数表を見よ**（Issue #164 が行数表を実測と突き合わせる検査と、散文から実測値を無くす検査を足したぶん、`docscheck` が伸びており、合計は分けた当時より増えたままである）。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。
...
| 1.49 | 2026-08-24 | 「`page/` は「1 ディレクトリ 1 タブ」ではない」の段落が 2 つ並んでいたのを 1 つに統合し、共有部品の列挙を `page/pagetest/import_test.go` の `shared` と一致させた（`page/ac…**詳細は下記「改訂 1.49」。** | 下記「改訂 1.49」を見よ |
