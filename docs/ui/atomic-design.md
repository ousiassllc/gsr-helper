...
| `ui/page/pagetest` | 1677 | 323 | pass |
...
| `ui/page/pagetest/cmdtest` | 992 | 1008 | pass |
...
**次の Issue への予算は残り 57 行、ERROR 境界 2200 まで 257 行である。** 9 周目の 47 行ではない。**採れる手は依然 (1) だけである**（(2)(3) は 6 周目までに使い切っている。受け皿の `page/pagetest` は Issue #147 が 1564 行まで空けたので、道具を出す先としては使えるようになった。現在は 1677 行・残り 323 行である）。残る `app.go` / `chrome.go` / `discover.go` / `keys.go` の 4 ファイル（`ui` 直下の本番ファイルはこれで全部である）はいずれも `App` の非公開な状態を読み書きするので、切り出せば内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。**次に 75 行を超える追加が要る Issue は、まず切り出せるまとまりの設計から始めること。**
