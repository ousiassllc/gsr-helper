...
**現在の行数を数値で書いてよいのは本節の行数表だけである。** 散文は数を書かず、「本節の行数表を見よ」と参照する。同じ実測値が本書の 3 つの系（2 つの行数表・散文・改訂履歴）へ手で写されていたことが陳腐化の原因であり、**写しを 1 つに減らすのが唯一の確実な手だからである。**

**この規約が掛かるのは本書だけではない。** 散文の検査の対象は `docs/ui/atomic-design.md` と `docs/components/overview.md` の **2 文書**であり（`internal/buildconfig/docscheck/docs_linebudgetdoc_test.go` の `budgetProseDocs` が持つ）、**行数の実測値を数で持ってよいのは本書の行数表だけ**である。`overview.md` は行数表を持たないので、行数に触れるときは本節の行数表を参照する。**対象を広げたのは corpus が 1 ファイルだったからである**——`overview.md` の `internal/runner` 系の使用量は改訂 1.19 / 1.39 で 2 度陳腐化して人手で追随した記録があるのに、PR #168 が入れた検査は本書しか読んでおらず無検査のまま残っていた（Issue #169）。
...
| `internal/buildconfig/docscheck` | 1906 | 94 | pass |
