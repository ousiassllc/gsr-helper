...
Issue #155 が残した 2 つ——`ui/page` 2003 行（残り -3 行・warn）と `ui/page/runners` 2000 行（残り 0 行・pass。次に 1 行でも足せば warn）——を、**本番コードを 1 行も変えずに**空けた。3 つの手のうち使ったのは (2) テストの重複削減だけである。
...
2. **3 つの手のうち (1) は当たらない。** `internal/buildconfig` は [開発環境](../environment/setup.md#設定ファイルの不変条件をテストで守る)が明記するとおり**実行時のコードを持たないテスト専用パッケージ**（`doc.go` 12 行のほかはすべて `_test.go`）で、「本番の一部をパッケージ境界で切り出す」対象が存在しない。
...
着手時点で 2122 行（残り -122 行・warn）だった。上記「警告帯へ入れた判断」が名指しした候補——**検査そのものをパッケージ境界で別ディレクトリへ分ける**——をそのまま採り、ビルド設定の検査（`internal/buildconfig` **1216 行・残り 784 行**）とドキュメントの検査（`internal/buildconfig/docscheck` **936 行・残り 1064 行**）に分けた。**検査は 1 件も削っていない**——分割の前後で同じテストがすべて走る（同節 1.「正しさを行数より優先した」を覆さないため、削る手は初めから検討していない）。
...
**共通ヘルパは 3 つ目のディレクトリへ出した。** `_test.go` の中のヘルパはパッケージをまたげないので、選べるのは (a) `repoRoot` を両方に写す、(b) 通常のパッケージへ出す、の 2 つである。**(a) は採らなかった**——本書が繰り返し戒めている「写しの片方だけが古くなる」形そのもので、19 行のために作る写しではない。(b) の先例は `page/pagetest` / `organism/table/tabletest` / `setup/setuptest` で、いずれも「`_test.go` に置けない事情で通常のパッケージになった道具」である。**代償も先例と同じ——本番からも import できてしまう**ので、`page/pagetest/import_test.go` の `fixtures` へ `internal/buildconfig/buildconfigtest` を登録し、`TestNoProductionCodeImportsTestFixtures` の網に入れた（「足すときはここへ登録すること」と同ファイルが指示している。登録しないと検査の網から外れる）。

**分けても合計は増える。** 2122 行が 1216 + 936 + 44 = **2196 行**（+74）になった。増えたのは package 宣言・import・3 つの doc コメントと、移したファイルの責務の境界を述べる見出しコメントで、**検査の中身は 1 行も足していない**。上限はディレクトリ単位に掛かるので、逼迫を解くのは合計ではなく分布である——3 つとも警告帯の外にある。
...
1. **切り出せるまとまりが見つからなかった。** 3 つの手のうち (2)（テストの重複削減）と (3)（道具を `page/pagetest` へ）は 6 周目までに使い切っている。残る (1)（本番の切り出し）は「依存の向きを強制できる、あるいは増え方が違うまとまり」があるときだけの手だが、`ui` 直下に残るのは親 Model そのもの（`app.go` / `keys.go` / `discover.go` / `background.go` / `chrome.go` / `hostreq.go`）であり、どれも `App` の非公開な状態を読み書きする。**`hostreq.go` は既に本体を `ui/hostreq` へ出した後の 26 行のアダプタ**（`startHostReq` が `hostreq.StartOnce` を呼ぶだけ）で、これ以上出せるものが無いことの実例である。切り出せば `App` の内部を export することになり、`ui/organism/table` を分割しない判断と同じ理由で採れない。
...
着手時点で **2105 行（残り -105 行・WARN）** で、8 周目が残した予算は ERROR 境界（2200 行）まで 95 行だった。7 周目・8 周目が 2 周続けて使った逃げ道（判断を残して警告帯へ入る）を返す周であり、**行数を空けること自体が唯一のスコープである**。上限値は緩めず、3 つの手のうち **(1) 本番の一部をパッケージ境界で切り出す**を 2 回打って **1953 行（残り 47 行・pass）** へ戻した。7 周目が挙げた候補（背景処理を `discovery` 側へ**状態ごと**寄せる）をそのまま採っている。
...
| 1.94 | 2026-08-25 | Issue #161 を反映。(1) 行数表を実測へ更新した（`internal/buildconfig` 2122 → **1216 行・残り 784・pass**、`ui/page/pagetest` 1932 → **1935 行・残り 65**）。降順を保つため `internal/buildconfig` を最上段から `internal/svc` の下へ移し、新設した `internal/buildconfig/docscheck`（**936 行・残り 1064**）を `cmd/gsr-helper` の下へ、`internal/buildconfig/buildconfigtest`（**44 行・残り 1956**）を表の末尾へ足した。(2) 「残りが 1 桁のディレクトリ」の段落を、**警告帯に入っているディレクトリが 1 つも無い**状態へ書き換えた。(3) 「`internal/buildconfig` を警告帯へ入れた判断」の末尾に、その節が出した「空けること自体を独立した Issue に立てること」という指示が Issue #161 として実施されたことを追記し、実施の判断を「`internal/buildconfig` を 2 つに分けた判断（Issue #161）」の節に記録した（境界を引けた根拠の実測——共有していた識別子は `repoRoot` 1 つだけ——、共通ヘルパを写さず `buildconfigtest` へ出して `pagetest/import_test.go` の `fixtures` へ登録した判断、分けても合計は **+74 行**になること、ファイル側の実測）。(4) 「Issue #159 の空け方」の `ui/page/pagetest` の実測 2 箇所を、表と同じ 1935 行・残り 65 行へそろえた。(5) **PR #163 のレビュー指摘で `nolint` 棚卸しの検査を `docscheck` へ移し、本欄を含む実測値を全件測り直した**——`TestSetupDocNolintInventoryMatchesTree` は `docs/environment/setup.md` の表を見るドキュメントの検査でありながらビルド設定側に残っており、同じ setup.md を読む `TestSetupDocEmbedsConfigFilesVerbatim`（`docs_sync_test.go`）と 2 パッケージへ割れていた（`nolint_inventory_test.go` → `docscheck/docs_nolint_test.go`）。移動で動いた値は本欄の (1)(3) のほか、「残りが 1 桁のディレクトリ」の段落、「2 つに分けた判断」の節（合計 **2196 行**・ビルド設定側に残るのは **9 つの `_test.go`**・ファイルの最大は `golangci_test.go` **249 行**とドキュメント側の `docs_deppkg_test.go` **202 行**）である。(6) 同じレビューで「`internal/buildconfig` を警告帯へ入れた判断」の項 4・項 5 を**当時の値と明示したうえで現在値を併記する**形へ直し、項 5 末尾の次 Issue 向け指示（「ディレクトリ側は既に残りマイナスである」）を Issue #161 が解消した旨へ書き換えた。(7) 「空け方の手は 3 つある」の (3) が挙げるフィクスチャ用の通常パッケージに `internal/buildconfig/buildconfigtest` を、「ファイルの行数」の `docs_depgraph_test.go` の参照を移動後のパス（`internal/buildconfig/docscheck/`）へ足した | `internal/buildconfig` が警告帯（2122 行・残り -122 行）にあり、同節自身が空けることを独立した Issue に立てるよう指示していた。ドキュメントの検査を `docscheck` へ分けて警告帯を出したため、行数表・段落・判断の節が実装と食い違う状態になった。(5)〜(7) は PR #163 のレビュー指摘である。**`nolint` 棚卸しの置き場所は「検査に使う道具」ではなく「検査の対象」で決まる**——落ちたときに直すのは setup.md の表であって `.golangci.yml` ではないので、ビルド設定側に残すと `internal/buildconfig/doc.go` の宣言（ビルド設定ファイルの不変条件だけを置く）と食い違い、同一文書の検査を探す読み手が 2 つのパッケージを見る羽目になる。**項 4・項 5 は現在形のまま偽になっていた**もので、とりわけ項 5 末尾の「ディレクトリ側は既に残りマイナスである」は同じ文書の後続節「3 つとも余裕があるので、足す前に空ける必要は無い」と正面から矛盾し、次の Issue に不要な分割を検討させる**危険側のずれ**である。**表だけを直して散文を取り残す**欠陥は改訂 1.69〜1.73 / 1.90〜1.93 で繰り返し再発しており、本改訂の初版でも再発したため、移動で動く値を全件測り直してそろえた |
