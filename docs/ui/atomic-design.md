...
**検査が構造的に見ない範囲がある。** 拾うのは `N 行` / `残り N` / `A + B = C` という**数値**の形だけなので、次の 5 つは検査の外にある——(a) 改訂履歴、(b) 上記 (4) の一覧に挙げた節、(c) **数値を伴わない語形の主張**（`余裕がある` / `潤沢ではない` / `残りはテスト 1 本ぶんしかない`）、(d) **行以外の助数詞で数える値**（`N 個` / `N ファイル` のような個数）、(e) **式にも助数詞にもならない裸の数**（`内訳は A / B / C` のように、スラッシュや読点で並べただけの形）。(e) を拾えないのは**数だけを見て実測値かどうかは判定できない**からである——本書は Issue 番号も改訂の版番号もセル数も数で書くので、裸の数を一律に落とすと本書が書けなくなる。だから**内訳を語りたくなったら数を並べず、行数表を指すこと**。(a)(b) は除外として意図したものだが、**(c)(d)(e) は意図の外で、一覧の大小に関わらず本規約でしか守れない。** (c) を言い回しの列挙で拾う手は 2 周かけて捨てた方式そのもので、書き方は無限にあり収束しない。だから**現在の状態を語りたくなったら、数値でも語形でも「上記「行数の予算」の行数表を見よ」と参照へ寄せること**。とりわけ「空ける必要は無い」のように**安全側を崩す助言**は、数値を伴わなくても次の Issue を誤らせるので書かないこと——予算の逼迫は書いた後に進む。
