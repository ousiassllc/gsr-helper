...
| ディレクトリ | 行数 | 残り | 判定 |
|------------|------|------|------|
| `internal/buildconfig` | 2041 | -41 | warn |
...
**最終レビューでさらに +16 行して 2041 行（残り -41 行・warn）になった。** 塞いだのは 3 つで、いずれも**変異注入で実測した**「黙って緑になる」経路である——mermaid フェンスに重複ガードが無く「## 依存関係」節に 2 つ目のグラフを置くと**読者が見るほうが無検査**になる、散文が唯一の例外として名指しする `UIApp --> UIParts` が除外条件に飲まれて**図から消しても緑**、`mermaidArrow` がテキスト付き点線リンク `-. text .->` を取りこぼして**どの case にも入らず黙って無視**。上記 1.（正しさを行数より優先する）と同じ理由で、行数を根拠に省かなかった。
...
4. **ERROR 境界（2200 行）まで 159 行ある。** `make check` は通る。
5. **1 ファイル 300 行は全ファイルが守っている**（最大は `docs_depgraph_test.go` **296 行・残り 4 行**で、警告帯 301〜330 行の外。`docs_mermaid_test.go` 147 行・`docs_depnodes_test.go` 123 行）。**`docs_depgraph_test.go` の残り 4 行は、ディレクトリの残量より先に尽きる。** ここへ 5 行以上足す Issue は、ディレクトリを空ける前に**このファイルを責務の境界で分ける**こと（「ファイルの行数」の方針どおり、切り出し先は同じディレクトリでよい）。
