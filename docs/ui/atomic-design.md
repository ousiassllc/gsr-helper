...
**切り出した Issue #159 が空けた。** 残りマイナスの状態は解消しており（**1948 行・残り 52 行・pass**）、上記 2. で候補に挙げたテストの重複削減がそのまま採られた。本 Issue は「判断を残して警告帯へ入る」逃げ道を 1 度使い、次の Issue がそれを畳んだ形である。詳細は下記「Issue #159 の空け方」。
...
**`ui/page` は 2003 → 1948 行（残り 52 行・pass）。** Issue #155 が「候補は上記 2. のテストの重複削減」と名指ししたとおりに進めた。

1. **`TestOverlayRegisteredModalFollowsStackRules` を削除した。** この検証が主張していた「種類を登録しても重なりの規則はそのまま効く」は**別の経路を通っていない**——共有の `newOverlay()` 自身が同じ `Register` で `kindFirst` / `kindSecond` を登録しているためである。個々のアサーションはすべて `overlay_test.go` が持つ（キーが最上位だけに届く＝`TestOverlayDeliversToTopOnly`、フッタのヒントが最上位のものになる＝`TestOverlayHintsAndTitleFollowTop`、esc が 1 枚だけ閉じて背後が描かれる＝`TestOverlayCloseOneByOne`）。
...
**`ui/page/runners` は 2000 → 1919 行（残り 81 行・pass）。** Issue が挙げた 2 案（`rowview` の先例どおりのネスト / テストの重複削減）のうち、**新しいパッケージは作らなかった。**

1. **行の注意記号と SVC 列の不明表示を見る 2 つのテストを `page/runners/rowview` へ移した**（`TestUnavailableManagedRowIsWarned` / `TestUnknownServiceStateIsDistinguished`。−45 行）。**これは行数のためだけの移動ではない。** 判定を持つのは `rowview` の `warned` / `svcUnknown` なのに、検証はタブ全体を描いて記号の文字列を探しており、しかも `rowview_test.go` はこの 2 つを 1 つも覆っていなかった（覆っていたのは `_WORK` 列だけである）。移動先では `RunnerView.Warn` / `RunnerView.SvcUnknown` を直接見るので**検証はむしろ強くなる**。`rowview` への export の追加は 0 件で、`rowview.go` は無変更である（上記「`ui/organism/table` の本体を分割しない判断」が退けた「内部を export することになる」形には当たらない）。
2. **塞がれた操作の 2 つのテストを表にまとめた**（直起動・非 root。−21 行）。本体が同じで、違うのはフィクスチャ・キーの集合・理由の定数だけだった。直起動側にしか無かった「確認ダイアログが開かない」の検査は両ケースへ広げてある。
3. **`pagetest` の再実装を 2 つ消した**（−19 行）。`logs_test.go` の `openTabMsg` は `pagetest.OpenTabOf` の再実装で、しかも `ErrNotFound` と `ErrCmdTimeout` を bool 1 つに潰しており「配線が無い」と「Cmd が戻らない」を区別できなかった。`opsflow_test.go` の `standaloneRunner` は `pagetest.StandaloneRunner` と同じもので、唯一の呼び出しが渡す名前は `SampleRunner` の既定と一致していた。
4. **`newModel` / `newOpsModel` / `mustModel` の 3 つを 1 本にした**（−14 行）。どれも `runners.New(0, st).Update(st)` に還元できた。`newModel` が `testState` を 2 回呼んで別のスナップショットを 2 つ配っていた不具合もここで直している。

**採らなかった手と理由。**

- **(1) ネストしたパッケージの新設は要らなかった。** `keys.go` の Setup タブへの引き渡し（`n` / `D` / `u`）は親の非公開に触れずに `page/runners/setupreq` として出せる形をしており、候補としては成立する（入力はすべて他パッケージの公開型で、出力は `tea.Cmd` と通知の文字列である）。**ただし上記のテスト側の手だけで残り 81 行に届いた**ので、パッケージを 1 つ増やす代償に見合わない。**次に `page/runners` の本番コードが増えて空ける必要が出たときの第一候補はこれである。**
...
**副産物として 1 ファイル 300 行の側も緩んだ。** `runners_test.go` が 298 行（残り 2 行）から 244 行になり、上記「ファイルの行数」の首位から外れた（`overlay_test.go` も 296 → 277 行）。**ディレクトリの予算を先に見ること**という同節の指示に、今回は逆向き——ディレクトリを空けた結果としてファイルの逼迫も解けた形で当たっている。

**次に `ui/page` 直下・`ui/page/runners` へ足す Issue へ。** それぞれ残り 52 行・81 行である。**警告帯は出たが潤沢ではない**ので、これを超える追加が要るなら足す前に空けること。`ui/page` の候補は依然として (1) の切り出し（`StateMsg` の改名の波及と `Overlay` の戻りの辺は上記「`ui/page` を警告帯へ入れた判断」の 1. が評価済み）、`page/runners` の候補は上記の `setupreq` である。
...
**逼迫しているのはディレクトリだけではない。** 本 PR で `internal/ui/app_test.go` は **300 行（1 ファイル 300 行の上限に張り付き・残り 0）**になり、`internal/ui/page/` も **1998 行（残り 2 行）**になった。**前者は当時の値である**——9 周目（Issue #139）が周期の状態を `discovery.State` へ移した際に `app_test.go` は 274 行まで下がっており、いま `ui` 直下で最大のファイルは `app.go` の 284 行である（上記「ファイルの行数」）。**後者の 1998 行・残り 2 行も当時の値である**——Issue #155 が `page.StateMsg` にフィールド 1 つと doc を足し、`ui/page` は **2003 行・残り -3 行・warn** になった（上記「`ui/page` を警告帯へ入れた判断（Issue #155）」）。**残りはもうプラスではないので、`page` 直下へ共通の `Msg` を 1 つ足すなら、その前に空けること**（空ける作業自体は Issue #159 へ切り出してある）。タブをまたぐ移動や本例外と同じく「双方から見える場所がここしか無い」ことを確かめるのは、空けたうえでの話である。
...
- `ui/page` 直下（2003 行・残り -3 行の警告帯）には置けない。`page` は 7 タブすべてが import する共通の土台であり、runner 固有の制御をそこへ混ぜると Disk / Logs / Doctor まで引きずる。`page/runnerdetail` を分けたのと同じ判断である。
...
`ui/page/runners` は 1187 → 2091 行になり、**警告帯に入った**。Setup タブ（Issue #8）が `n` / `D` / `u` の引き渡し（`keys.go` の `openSetup` / `setupBlocked`）を足したことで 2164 行（残り -164 行）まで伸びた（**当時の値である。現在は本段落の末尾に記す 2000 行・残り 0 行**）。増分の大半はサービス制御の検証（発行コマンド列・確認の経路・一括操作・可否の再判定・詳細画面からの起点）で、Logs タブ（Issue #9）が足した `l` の経路もここに乗る。**分割せず警告帯に入ることを選んだのは `ui/page/disk` と同じ判断である**（上記）。サービス制御の配線と Logs タブのマージを 1 つの差分で行っており、そこへパッケージ移動を混ぜるとレビューで「どちらが壊したか」を切り分けられなくなる。エラー境界（2200 行）までは 36 行しかなく、`page/runners/rowview` を切り出して 2018 行へ戻した。**その後 Issue #107 が本節の指示どおりの削減を実施し、2018 行から 1983 行（残り 17 行）まで空けた**（Issue #147 の `cmdtest` への追従で 6 行増え、Issue #150 の追従でさらに 2 行増え、Issue #155 が走査の既定を `pagetest.State` へ寄せた追従で 9 行増えた。現在は 2000 行・残り 0 行である）**。** `runners/helper_test.go` が持っていた `findChrome`（束を辿る再帰）と `collect`（Msg の平坦化）はどちらも `pagetest.ChromeOf` / `pagetest.Msgs` の写しだったので捨てた。**そのとき `pagetest.ChromeOf` の側にも欠陥が見つかった**——「1 段だけ展開すればよい（ChromeMsg が入れ子の奥から出てくることは無い）」という前提が成り立たず、絞り込みを始める `/` では page 自身の `ChromeMsg` と部品の返した束がもう 1 段深い形になる。写しの側だけが正しく辿っていたので、共有の道具を再帰へ直した（見つかった時点で打ち切るので、点滅の `Cmd` を踏まないという性質は変えていない）。

**残りは 0 行である——上限まで 1 行も無い。次にこのタブへ足す Issue は、1 行足す前に空けること。** 道具の重複はこの 1 周で使い切ったので、次に採れるのはテストの重複削減（`ops_test.go` / `opsdrain_test.go` / `opsflow_test.go` の 602 行はサービス制御の経路を 3 ファイルに分けて見ている）である。
...
| 1.93 | 2026-08-25 | Issue #159 を反映。(1) 行数表を実測へ更新した（`ui/page` 2003 → **1948 行・残り 52・pass**、`ui/page/runners` 2000 → **1919 行・残り 81**、`ui/page/runners/rowview` 269 → **308 行・残り 1692**）。降順を保つため `ui/page` を `ui` の上へ、`ui/page/runners` を `ui/page/setup` の上へ、`ui/page/runners/rowview` を `ui/ghscope` の上へ移した。(2) 「残りが 1 桁のディレクトリ」を 7 つから **6 つ**へ改め、`ui/page/runners` を一覧から外した。警告帯のディレクトリを「`internal/buildconfig` と `ui/page` の 2 つ」から**`internal/buildconfig` の 1 つだけ**へ改めた。(3) 「ファイルの行数」の最大を `runners_test.go` 298 行から **`runners.go` 296 行（残り 4）**へ改めた。(4) 「`ui/page` を警告帯へ入れた判断（Issue #155）」の末尾を、切り出した Issue #159 が空けた旨へ書き換え、新節「**Issue #159 の空け方**」を追加した | Issue #155 が `ui/page` を警告帯（残り -3 行）に、`ui/page/runners` を残り 0 行に残しており、どちらも次の 1 行で warn に入る状態だった。本番コードを 1 行も変えず、手 (2) のテストの重複削減だけで両方に余裕を作ったため、実測値と「次に足す Issue への予算」の記述が古くなっていた。ネストしたパッケージの新設（`setupreq`）とフィクスチャの `pagetest` への移動を**採らなかった**理由も、次の Issue が同じ検討を繰り返さないように書き残した |
