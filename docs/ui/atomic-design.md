...
| ディレクトリ | 行数 | 残り | 判定 |
|------------|------|------|------|
| `internal/buildconfig` | 2122 | -122 | warn |
...
- **フィクスチャ（`sampleRunner` / `unmanagedRunner` / `orphanUnits`）を `pagetest` へ出す手（3）は採らなかった。** `page/runners` から 48 行減る代わりに `ui/page/pagetest`（1932 行・残り 68 行）が十数行増える。**行数を空ける Issue が別のディレクトリを逼迫させるのは本末転倒である**（上記「9 周目の空け方」と同じ判断）。
