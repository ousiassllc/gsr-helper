...
**`?` の全キー一覧も Context と同じ範囲で組む。** Logs タブのヘルプ（`Set.LogsHelp`）は「次のタブ」の `tab` を出さない。出すと、押しても次のタブへ行かないキーをこの画面のヘルプが案内することになる。ヘルプの範囲と Context の範囲がずれると、**重複検査は通るのに `?` だけが嘘をつく**という、テストで捕まえられない食い違いが残る。
