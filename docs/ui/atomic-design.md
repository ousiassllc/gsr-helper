...
**散文は実測値が無いことだけを見る。** `TestLineBudgetProseHasNoMeasuredNumbers` が本書から下記の除外を落とした散文を走査し、`残り N` と 2 桁以上の `N 行` が 1 つも現れないことを確かめる。**この検査は言い回しを一切見ない。** 以前は「N 行・残り M 行」「移せるのは N 行まで」のように行数を主張する書き方を並べて表と突き合わせていたが、**言い回しの列挙は収束しなかった**——レビューのたびに未カバーの書き方が見つかり、そのどれもが現に古い数を素通りさせていた。数値の不在だけを見る形なら、新しい言い回しを考えなくてよい。見ないのは 1 桁の `N 行` だけで、それは本書が「表の 1 行」「一覧の 1 行」の意味で `1 行` を常用するからである（行数の主張ではない）。**2 桁の行数は検査が拾う**——予算の残りも「移せるのは N 行まで」も 2 桁の帯に入ることが多く、Issue #164 の実害となった原文「移せるのは N 行まで」の N も 2 桁だった。`残り N` の側は `残りは` `残りが` のように助詞が挟まっても拾うが、`残りが 1 桁` `残り 3 件` のように助数詞を伴う形は実測値ではないので拾わない。
...
| `internal/buildconfig/docscheck` | 1498 | 502 | pass |
...
**次に `internal/buildconfig` へテストを足す Issue へ。** 3 つとも余裕があるので、足す前に空ける必要は無い。**足す先は対象で選ぶこと**——設定ファイルの不変条件は `buildconfig`、ドキュメントの不変条件は `docscheck` である。`buildconfigtest` へ置いてよいのは**両方が使う道具だけ**で、片方しか使わないものをここへ寄せると分けた意味（増え方の違う 2 つを別々に育てる）が消え、3 つ目の逼迫する置き場になる。
...
末尾の `()` を任意にし `.` を語に含めているのが要点で、`input()` のような関数名だけでなく、`()` の付かない識別子（`tick` / `tickMsg` / `firstTick` / `inflight`）・ファイル名（`hostreq.go`）・パッケージ名を接頭辞に持つ型やメソッド（`hostreq.StartOnce` / `discovery.State.Start`）も 1 本で拾える。**本書全体では一意な識別子が 948 個挙がるので全件は当てない**——**現在形の散文が名指ししているものに絞り**、それを `grep -rn` で本番コードに当てること。
...
**本体（`Model[T]` の実装）は分割しない。** 実体は `api.go` / `keys.go` / `rows.go` / `section.go` / `state.go` / `table.go` の 6 ファイルである（**ファイル単位の実測は `go tool linterly check` の出力を見よ**）。中身は `Model[T]` という 1 つの型に対する区画・行・列・キー・状態の内訳であり、切り出せる単位はいずれも `Model[T]` の非公開な状態に触れる。サブパッケージへ出すには内部を export して `table` から切り出し先への参照を作ることになり、**「一覧の共通実装は 1 つ」（`Table` を増やさない）という規則を構造で守れなくなる**。分割の目的は行数上限の分散であって部品同士の依存を増やすことではない、という本書の方針とも衝突する。
