...
| `internal/buildconfig` | 1969 | 31 | pass |
...
| 1.86 | 2026-08-24 | Issue #151（依存グラフの辺を `go list` の import と突き合わせる検査の新設）を反映し、「行数の予算」の非 UI の表の `internal/buildconfig` を実測へ更新した（1475 → **1969 行・残り 31 行**・pass）。降順の並びが変わるため、`internal/doctor/jobreq`（1495）の直前から `internal/setup`（1970）と `internal/config/edit`（1922）の間へ移した。新設したのは 3 ファイル（`docs_depgraph_test.go` 259 行・`docs_depnodes_test.go` 125 行・`docs_mermaid_test.go` 110 行）で、いずれも 1 ファイル 300 行の警告帯（301〜330 行）に入らない。**パーサを別ファイルへ分けたのは行数のためだけではなく責務の境界でもある**——`docs_mermaid_test.go` は「mermaid をどう読むか」、`docs_depgraph_test.go` は「読んだ辺を実装とどう突き合わせるか」を持つ（「ファイルの行数」の方針どおり）。ディレクトリは 2000 行の警告帯には入っていないが**残りは 31 行しかない**ので、次に `internal/buildconfig` へテストを足す Issue は先に空け方を検討すること（本節の 3 つの手のうち (1) パッケージ境界での切り出しが採れる——ビルド設定の検査とドキュメントの検査は増え方が違う） | 表は「実測値」と明記してあり、**この節が次の Issue の読む行数予算の規範である**。494 行を足したのに表を据え置くと、次に `buildconfig` へ検査を足す Issue が 494 行ぶん過大な予算を引き、警告帯を越えてから気付くことになる。表だけを直して散文や並びを取り残す欠陥は改訂 1.69 / 1.70 / 1.71 / 1.72 / 1.73 で繰り返し再発しているため、今回は降順の位置も同時に直し、`internal/buildconfig` の行数を現在形で主張する散文が他に無いことも確認した（改訂 1.71 の記述は当時の値の記録なので変更しない）（Issue #151） |
