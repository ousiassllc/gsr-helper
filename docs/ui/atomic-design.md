...
**行数表は `go tool linterly check --format json` の実測と一致していなければならない。** `TestLineBudgetTablesMatchLinterly`（`internal/buildconfig/docscheck`）が行数・残り・判定と並び順を突き合わせる。ずれたときに直すのは表の側であって、上限でも実測でもない。**手で写した値を書き換えて辻褄を合わせるのではなく、実測を取り直して表を作り直すこと。**

**散文が現在形で主張する行数・残りも同じ表と突き合わせる。** `TestLineBudgetProseMatchesTables` が本書から 2 つの行数表と改訂履歴を除いた本文を文に切り、「N 行・残り M 行」「残り M 行」に加えて、次の Issue へ予算を渡す 2 つの指示（「この N 行を予算として読む」「移せるのは N 行まで」）も残りの主張として拾い、表の行として実在するかを見る。**文が backtick でディレクトリを名指ししていれば、その行とだけ突き合わせる**——表のどこかに同じ数があればよい形だと、別のディレクトリの値と偶然一致した古い主張が素通りするからである（名指しの無い文だけが表全体との照合に落ちる）。**当時の値を書くときは、同じ句——`、` で区切られた範囲——に `当時` か `時点では` を入れること。** これが唯一の除外である——過去の値はその Issue に紐づく記録なので実測へ寄せてはならず、逆に印の無い句の数は現在の主張として扱う。**除外を文ではなく句どまりにしてあるのは、過去と現在を 1 文で対比するのが本書の文体だからである**——文ごと外すと、この検査が最も守るべき現在形の主張がちょうど抜ける。**ファイル側の残り（1 ファイル 300 行の予算）は `296 行（残り 4）` のように `行` を付けずに書く**——ディレクトリの予算と取り違えないためで、検査もこの形は拾わない。

**改訂履歴の過去欄には当時の値だけを残す。** 現在値は上の行数表を見ること。履歴へ現在値を書くと、同じ数の 4 つ目の写しができて同じ腐り方をする。**過去欄に数を残すときは同じ文に `当時` を入れ、現在値へは「上記「行数の予算」の行数表を見よ」と参照だけを書くこと。**

**改訂履歴の過去欄だけは機械的な検査が効かない。** `TestLineBudgetProseMatchesTables` は下記「改訂履歴」の見出し以降を検査対象から除外している（見出しは行頭でだけ効く。散文が同じ文字列をコード引用として書いても、そこで打ち切りはしない）——過去欄は当時の値を持つのが正しく、実測と突き合わせるほうが誤りだからである。したがって履歴の欄が現在形で行数を主張しても `make check` は緑のままであり、**この規約で守るしかない**。過去の欠陥を引用している箇所（「同欄は『現在は 2025 行…』と現在形で主張しており」のように、直した誤りをそのまま引く記述）は引用なので現在形のままでよい。
...
| `internal/buildconfig/docscheck` | 1463 | 537 | pass |
...
**残りが 1 桁のディレクトリが 6 つある**（`internal/setup/tarball` 1 行・`internal/logs` 2 行・`internal/gh` 4 行。UI 側では `ui/page/disk` が残り 3 行、`ui/organism/dialog` が残り 6 行、`ui/page/config` が残り 8 行である）。**この 6 つに 1 行でも足す Issue は、足す前に空けること。** **`ui/page/runners` はこの一覧から外れた**——Issue #155 の時点では残り 0 行まで詰まっていた（その前に Issue #150 が `cmdtest` の締め切りへの追従で `page/config` を 1990 → **1992 行**、`page/runners` を 1989 → **1991 行**にしていた）のを、Issue #159 が **1923 行・残り 77 行**へ戻したためである（下記「Issue #159 の空け方」）。残る 6 つはどれも警告帯には入っていないので `make check` は通るが、通ることと余裕があることは違う。**警告帯に入っているディレクトリは 1 つも無い**——最後まで残っていた `internal/buildconfig` を Issue #161 が出した（着手時点では 2122 行・残り -122 行・warn だった）。現在は **1216 行・残り 784 行・pass** である。ドキュメントの検査を `internal/buildconfig/docscheck` へ分けたもので、判断は下記「`internal/buildconfig` を 2 つに分けた判断」にある。**`ui/page` は Issue #159 が警告帯から出した**（Issue #155 の時点では 2003 行・残り -3 行・warn まで押し上げられていた）。現在は **1955 行・残り 45 行・pass** である。その前に入っていた 3 つは Issue #147 が出した——`ui/page/pagetest` 2197 → **1564**、`ui/page/config` 2098 → **1990**、`ui/page/logs` 2025 → **1927**（判断は下記「Issue #147 の空け方」の節）。**`ui` 直下も警告帯の外にある**（1944 行・残り 56 行・pass。Issue #139 が 2105 行から戻し、Issue #148 がさらに下げた。予算と次の Issue への指示は下記「10 周目の空け方」）。
...
**切り出した直後は 1927 行（残り 73 行）だった**（Issue #101 当時の値）。**現在は 1980 行・残り 20 行である**（本節の非 UI の表と同じ実測値。その後の Issue が 53 行足した）。**残り 20 行は実質ゼロなので、次にこのディレクトリへ手を入れる Issue は 1 行足す前に空けること。**
...
**同じチェックが 1 ファイル 300 行にも掛かる。** ディレクトリと同じく `warning_threshold: 10` が効くので、301〜330 行が **WARN**、331 行以上が **ERROR** である（設定の詳細は[環境構築](../environment/setup.md#linterly)）。現在 WARN 以上のファイルは無く、最大は `internal/ui/page/runners/runners.go` の **296 行（残り 4）**である。**`internal/ui/page/runners/runners_test.go` は 298 行（残り 2）でこの首位に居たが、Issue #159 が行の描画の検証を `page/runners/rowview` へ移して 244 行になり、一覧から外れた**（同じ Issue で `internal/ui/page/overlay_test.go` も 296 行から 284 行になっている。**`internal/buildconfig/docscheck/docs_depgraph_test.go` もかつてここに並んでいたが、改訂 1.91 が責務の境界で 2 つに分けて 136 行になり、この一覧から外れた**——分けた当時は分割前の `internal/buildconfig` 直下にあり、Issue #161 が `docscheck` へ移した。行数は移動では動かない）。次点は `internal/setup/tarball/fetch_test.go` が 293 行、`internal/ui/tabset/tabset.go` が 292 行、`internal/ui/organism/choicelist_test.go` が 291 行、`internal/ui/page/doctor/doctor_test.go` が 290 行と続く。**`cmdtest/run.go` はこの一覧から外れた**——Issue #150 で 292 行から `find.go` を切り出し、現在は `run.go` 193 行 / `find.go` 163 行である（同じ Issue でテストも `run_test.go` / `cmd_test.go` / `find_test.go` の 3 つに分かれ、Issue #157 が `pump_test.go` を足して**現在は 4 つ**である。実測は `pump_test.go` 204 行 / `run_test.go` 182 行 / `cmd_test.go` 151 行 / `find_test.go` 149 行。`find_test.go` は `find.go` の検証を集めた新規ファイルで、PR #154 のレビュー指摘で `ChromeOf` / `HostReqOf` の未検証の腕を足した際に `run_test.go` から分けた。`pump_test.go` は `run.go` の `Pump` / `Drained` の未検証の分岐を集めた新規ファイルで、`run_test.go` を 300 行へ近づけないために分けた）。**`internal/ui` 直下は最大でも `app.go` の 284 行である**（6 周目の 300 行張り付きは `app_test.go` で、Issue #139 が周期の状態を `discovery.State` へ移したことで 274 行になった。下記「9 周目の空け方」）。
...
**Disk / Logs の 2 枚ぶんは既に消費済みである。** 残りは 1392 行から `disk_row`（122 + 170 = 292 行）と `log_row`（62 + 79 = 141 行）を引いて 959 行になった——**この 959 行は Issue #35 当時の値であって、現在の余裕ではない**（現在の `ui/molecule/listrow` の残りは上記のとおり 477 行である）。**足す必要のある一覧タブも 2 枚減り、見出しどおり Doctor の 1 枚だけになった**ので、余裕の判定は変わらない。むしろ 2 枚ぶんの実測値（292 行 / 141 行）が得られたことで、見積り 272 行が概ね妥当であること（Disk のように選択不可の理由まで持つ行は上振れすること）が確かめられた。
