...
**現在の行数を数値で書いてよいのは行数表だけである。** 散文は数を書かず、「本節の行数表を見よ」と参照する。同じ実測値が本書の 3 つの系（2 つの行数表・散文・改訂履歴）へ手で写されていたことが陳腐化の原因であり、**写しを 1 つに減らすのが唯一の確実な手だからである。**
...
**除外は正規表現ではなく明示の一覧で持つ。** 外すのは次の 4 つだけである。(1) 2 つの行数表（実測と突き合わせるのがここである）。(2) コードブロック。(3) `## 改訂履歴` 以降——**行頭の見出し行でだけ切る**（字下げや文中の引用で切ると、そこから先がまるごと検査から外れる）。(4) 過去の値を記録する節を見出しの文字列で並べた一覧（`docs_linebudgetprose_test.go` の `budgetProseExcluded`）——「…の空け方」「…判断」の節と「一覧タブを 1 枚足せる余裕」がここに入る。**(4) に載せてよいのは過去の値だけを持つ節である。** 現在の値を行数表や `go tool linterly check` の出力への参照へ寄せれば除外は要らなくなるので、そうなった節は一覧から外すこと（「`ui/organism/table` の本体を分割しない判断」は現在のファイル数と行数を落とした時点で外れた）。**一覧は小さいほどよい**——載っている節は検査の外にあり、そこだけは規約で守るしかないからである。**(4) の除外は入れ子では広がらない**——1 つの見出しが外すのは次の見出し（深さは問わない）までの本文だけで、入れ子の小節は自分が一覧に載っているときだけ外れる。**一覧に挙げた見出しの集合が、そのまま除外範囲である。** **除外が現実と食い違ったら検査は声を上げて落ちる**——行数表が 2 つ見つからないとき、行頭の `## 改訂履歴` が見つからないとき、一覧に挙げた見出しが本書から消えたとき、コードブロックの囲みが閉じ忘れられているとき、除外を落とした結果として散文が 1 行も残らないときは `t.Fatal` する。
...
| `internal/buildconfig/docscheck` | 1620 | 380 | pass |
