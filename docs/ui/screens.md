...
- 詳細画面でも各操作キー（`s` / `x` / `X` / `d` / `R` / `e` / `u` / `D` / `l`）を直接打てる。
...
**操作対象はジョブではなく runner である**（[FR-47](../requirements/functional.md)）。取り違えを防ぐため、フッタでも対象を明示する。

```
 enter:runner の詳細  d:runner をドレイン停止  X:runner を強制停止  R:再起動  l:ログ
```
