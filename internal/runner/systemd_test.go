...
// showOutput は file が指定されていれば testdata/systemd から読み、無ければ out を返す。
func showOutput(t *testing.T, file, out string) string {
	t.Helper()
	if file == "" {
		return out
	}
	return string(fixture(t, "systemd/"+file))
}
...
func TestParseShow(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "MainPID=(unknown) は 0。Id 行が無ければ引数の unit が残る", file: "show_unknown_pid.txt",
			want: SvcState{Unit: arg, Load: "loaded", Active: "activating", Sub: "start"}},
		// WorkingDirectory=-/path の先頭 - を除去する（孤児判定の照合キーがずれる）。
		{name: "WorkingDirectory の先頭 - を除去", file: "show_dash_workdir.txt", want: SvcState{Unit: u2,
			Load: "loaded", Active: "active", Sub: "running", FileState: "enabled",
			WorkingDir: "/opt/actions-runner-3", User: "runner", MainPID: 999}},
		{name: "Id 行が無い場合は引数の unit を使う", file: "show_no_id.txt", want: SvcState{Unit: arg,
			Load: "loaded", Active: "active", Sub: "running", FileState: "enabled",
		...
		{name: "CRLF 行末", out: "Id=actions.runner.crlf.service\r\nLoadState=loaded\r\nUser=runner\r\n",
			want: SvcState{Unit: "actions.runner.crlf.service", Load: "loaded", User: "runner"}},
		{name: "値に = を含む", out: "WorkingDirectory=/opt/a=b\n",
			want: SvcState{Unit: arg, WorkingDir: "/opt/a=b"}},
		{name: "空文字", want: SvcState{Unit: arg}},
	}
	for _, tt := range tests {
...
func TestSvcStateLabel(t *testing.T) {
	tests := []struct {
		name string
		st   SvcState
		want string
		...
		{"Active が空", SvcState{}, "-"},
		{"Active と Sub が異なる", SvcState{Active: "active", Sub: "running"}, "active/running"},
		{"Active と Sub が同じ", SvcState{Active: "failed", Sub: "failed"}, "failed"},
		{"Sub が空", SvcState{Active: "inactive"}, "inactive"},
		{"停止中", SvcState{Active: "inactive", Sub: "dead"}, "inactive/dead"},
	}
	for _, tt := range tests {
