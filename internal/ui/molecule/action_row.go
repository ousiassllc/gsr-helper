...
// 装飾前なら中略記号まで Muted の内側に入る。装飾済みでも ANSI 列は壊れない
// （atom.Truncate は lipgloss に切る位置を任せる）が、中略記号だけは装飾の外に
// 付いて既定色で描かれる——それを避けられるのがこの順序の利点である。色を
// 使い分ける行では採れず、chrome.Status は装飾後に切っている。
//
