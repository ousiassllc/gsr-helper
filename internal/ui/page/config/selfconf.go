...
//
// **保存済みの値があればそちらを優先する。** page.ConfigDeps.Conf は起動時の値に
// 固定されない——書き込めた設定は ConfigSavedMsg で親へ返り、親が差し替えて配り直す
// （page.ConfigDeps の doc）。ただし差し替えが届くのは親が配り直す次の StateMsg で
// あり、書き込んだ直後に開き直すフォームには間に合わない。優先しないと、保存した直後に開き直したフォームが古い値を出し、
