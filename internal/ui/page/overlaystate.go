...
//
// 閉じているモーダルへは配らない。配ると、誰も見ていないヘルプを毎周期
// 組み直す（pane.NewHelp が全行を描く）ような無駄が周期ごとに積み上がる。古い
// 状態で描かれる心配は、Open が開く直前にリプレイすることで断ってある。
//
// 領域は変化したときだけ配る。変わっていない SizeMsg は下位に再計算を強いるだけで
// あり、必要性は既に下位が認識している（organism/table.Model.SetSize の差分判定）。
