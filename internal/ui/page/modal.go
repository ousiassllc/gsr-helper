...
// （TabMsg の doc）。**受け取った側はドメイン層の呼び出しと page へ返す決定を
// Do(Tab, ...) で包むこと。** 構築関数の引数にしないのは、モーダルを足す Issue
// ごとに「tab を受け取って持ち回る」規律を書き写させないためである。
...
// 宛先を明示すると、**開いていなくても、最上位でなくても**その種類へ届く。
...
// Model へ渡るため、Overlay は個々のモーダルの API を知らない。見出しとキーヒントは
// tea.Model から取れないため、Model を引数に取る関数として登録する（Model は Update で
// 差し替わるので、値を閉じ込めると古くなる）。
type Modal struct {
