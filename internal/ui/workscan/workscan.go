...
// （親の startBackground と、r の処理）。**「1 度目か」は State が覚える**（StartOnce）。
// 呼び出し元の契機は「検出が成功した周期」であり初回とは限らないので、素の Start を
// そこから呼ぶと 3 秒ごとに再走査が始まる。手動の再読み込みは何度でも走ってよいので、
// Start と StartOnce を分けてある。
//
