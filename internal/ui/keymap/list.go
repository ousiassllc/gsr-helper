...
//
// **共有をやめてキーを分けることはしない。** enter で確定、esc で取消は端末 UI の
// 慣習であり、絞り込みだけ別のキーにすると「どの画面でも同じキーで動く」という
// 約束から外れる。安全側は「同時に有効にならない」ことを構造で示すことであり、
// TestListModesPartitionEveryKey が全フィールドをどちらか一方のモードへ必ず
// 割り当てさせることで担保する（Issue #50）。
//
