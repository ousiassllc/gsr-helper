// Package ghtoken は GitHub のトークンを取得する。
//
// internal/gh から分けたのは、トークンの取得が API クライアント（Client）に
// 一切依存しないためである。外部コマンド（gh auth token）と環境変数だけを見る
// 処理であり、Client のメソッドと同じディレクトリに置く必然性が無い。
// 1 ディレクトリ 2000 行の上限に対して internal/gh が超過したのを機に分けた
// （Issue #81）。呼び出し側は従来どおり gh.Token / gh.HasToken を使える。
package ghtoken
...
// ErrNoToken はトークンを取得できなかった場合のエラー。
