// Package audit は本ツールの破壊的操作を JSON Lines で追記する。
//
// 呼び出すのは次の 2 層だけである。
//   - 外部コマンドの実行実装（internal/exec/command）
//   - 外部コマンドを伴わない破壊的操作を行うドメイン実装（例: internal/disk の
//     ファイル削除）
//
// 当初は internal/exec の実行実装だけに限っていたが、ファイルの再帰削除は
// os.Remove を直接呼ぶだけで外部コマンドを起動しないため、exec 側の 1 箇所に
// 記録を寄せる形では拾えなかった（Issue #71）。そこで契約を「外部コマンドと、
// それに準ずる破壊的操作」へ広げ、ドメイン層にも Logger を配れるようにしている。
//
// 契約を広げても記録漏れが構造的に増えないのは、**層ごとに記録の起点を 1 関数へ
// 固定する**という規則を保っているためである。exec 側は internal/exec/command の
// Run 1 箇所、disk 側は internal/disk の removeTarget 1 箇所というように、
// 「その層で破壊的操作を行う経路が必ず通る 1 点」に Report / Write の呼び出しを
// 置く。呼び出し箇所を増やすのではなく、層が増えるたびに新しい「1 箇所」を
// 追加する形を守る限り、記録が漏れる余地は生まれない。
//
// マスクは呼び出し側の責務であり、audit は受け取った値をそのまま書く。
// マスクを audit 側にも置くと実装が 2 箇所に分かれて片方が古くなるため、
// 値一致マスクの実装は internal/exec/mask に一本化している。
package audit
