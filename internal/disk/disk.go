...
// 同じものに限られるという不変条件を 1 か所で守るためである。削除パスの検証だけは
// internal/disk/pathguard に置いてある（判定が走査・削除とは別の理由——脅威モデル——で
// 変わるため。Issue #101）。**外へ出したのは検証の実装であって、通す義務ではない**——
// PlanClean と Apply の両方から必ず pathguard.Validate を通す構造は変えていない。
// 検証を通らないパスは計画にも載らず、計画に載っていても実行直前にもう一度検証される
