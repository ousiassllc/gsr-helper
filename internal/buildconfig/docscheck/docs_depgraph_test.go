...
// モジュールの全パッケージが、依存グラフのノード対応表のいずれかのプレフィックスに一致しなければ
// ならない。最長プレフィックス一致なので、既存のどれにも当たらない新しい最上位ツリーだけが落ちる。
//
...
// 実装にある層をまたぐ直接 import は、すべて依存グラフに辺として描かれていなければならない。
// 図の UI 内部の辺は散文が例外として名指しする `UIApp --> UIParts` の 1 本だけで、`subgraph UI` に
// UI 層でないノードが混ざらないことも見る。
//
